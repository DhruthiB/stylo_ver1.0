let styloCloudBaseEndpoint = "https://stylocloud.com";
let isFCUser = false;

$(document).ready(function () {
	if (
		stylo.boot.is_fc_site &&
		!!stylo.boot.setup_complete &&
		!stylo.is_mobile() &&
		stylo.user.has_role("System Manager")
	) {
		stylo.call({
			method: "stylo.integrations.stylo_providers.stylocloud_billing.current_site_info",
			callback: (r) => {
				if (!r?.message) return;

				const response = r.message;
				const trial_end_date = new Date(response.trial_end_date);
				styloCloudBaseEndpoint = response.base_url;
...
function openStyloCloudDashboard() {
	window.open(`${styloCloudBaseEndpoint}/dashboard/sites/${stylo.boot.sitename}`, "_blank");
}
