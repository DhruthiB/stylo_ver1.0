...
function clone_field(df) {
	...
	if (cloned.custom) {
		...
		cloned.fieldname += "_" + stylo.utils.get_random(8);
	}
	return cloned;
}
...
let print_templates = computed(() => {
	...
	for (let template of templates) {
		let df;
		if (template.field) {
			df = stylo.meta.get_docfield(meta.value.name, template.field);
		} else {
			df = {
				label: template.name,
				fieldname: stylo.scrub(template.name),
			};
		}
		out.push({
...
onMounted(() => {
	let method =
		"stylo.printing.page.print_format_builder_beta.print_format_builder_beta.get_google_fonts";
...
.control-font :deep(.stylo-control[data-fieldname="font"] label) {
	display: none;
}
</style>
