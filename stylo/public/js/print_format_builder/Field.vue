...
function edit_html() {
	let d = new stylo.ui.Dialog({
		...
		primary_action: ({ html }) => {
			html = stylo.dom.remove_script_and_style(html);
			props.df["html"] = html;
			d.hide();
		},
	});
...
function get_all_columns() {
	let meta = stylo.get_meta(props.df.options);
	let more_columns = [
		{
			label: __("Sr No."),
			value: "idx",
	...
	return more_columns.concat(
		meta.fields
			.map((tf) => {
				if (stylo.model.no_value_type.includes(tf.fieldtype)) {
					return;
				}
				return {
					label: tf.label,
...
function get_column_to_add(fieldname) {
	...
	return {
		...stylo.meta.get_docfield(props.df.options, fieldname),
		width: 10,
	};
}
