...
export default {
	...
	mounted() {
		this.doc_select = stylo.ui.form.make_control({
		...
		this.preview_type = stylo.ui.form.make_control({
	...
	methods: {
		...
		get_default_docname() {
			return stylo.db.get_list(this.doctype, { limit: 1 }).then((doc) => {
	...
	computed: {
		...
		url() {
			...
			let url =
				this.type == "PDF"
					? `/api/method/stylo.utils.weasyprint.download_pdf`
					: "/printpreview";
