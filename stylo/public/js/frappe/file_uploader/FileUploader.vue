				...
				<div class="mt-3 text-center">
					...
					>
						<svg width="30" height="30">
							<image
								href="/assets/stylo/icons/social/google_drive.svg"
								width="30"
								height="30"
							/>
						</svg>
...
if (stylo.user_id !== "Guest" && props.allow_google_drive) {
	stylo.call({
		// method only available after login
		method: "stylo.integrations.doctype.google_settings.google_settings.get_file_picker_settings",
		callback: (resp) => {
			if (!resp.exc) {
...
if (props.restrictions.max_file_size == null) {
	stylo.call("stylo.core.api.file.get_max_file_size").then((res) => {
		props.restrictions.max_file_size = Number(res.message);
	});
}
if (props.restrictions.max_number_of_files == null && props.doctype) {
	props.restrictions.max_number_of_files = stylo.get_meta(props.doctype)?.max_attachments;
}
...
function upload_files(dialog) {
	...
	if (!files.value.length) {
		stylo.msgprint(__("Please select a file first."));
	...
	return stylo.run_serially(files.value.map((file, i) => () => upload_file(file, i)));
}
function upload_via_file_browser() {
	let selected_file = file_browser.value.selected_node;
	if (!selected_file.value) {
		stylo.msgprint(__("Click on a file to select it."));
...
function return_as_dataurl() {
	let promises = files.value.map((file) =>
		stylo.dom.file_to_base64(file.file_obj).then((dataurl) => {
		...
		xhr.onreadystatechange = () => {
			if (xhr.readyState == XMLHttpRequest.DONE) {
				if (xhr.status === 200) {
					...
					// regular stylo.throw() in backend
					file.failed = true;
...
function capture_image() {
	const capture = new stylo.ui.Capture({
		animate: false,
		error: true,
	});
	capture.show();
	capture.submit((data_urls) => {
		data_urls.forEach((data_url) => {
			let filename = `capture_${stylo.datetime
				.now_datetime()
				.replaceAll(/[: -]/g, "_")}.png`;
