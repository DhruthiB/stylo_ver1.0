	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="stylo.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && stylo.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: stylo.utils.icon("folder-open", "md"),
		closed: stylo.utils.icon("folder-normal", "md"),
		leaf: stylo.utils.icon("primitive-dot", "xs"),
		search: stylo.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return stylo.utils.get_form_link("File", filename);
};
