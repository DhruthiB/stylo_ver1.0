...
stylo.provide("stylo.treeview_settings");
stylo.provide("stylo.views.trees");
window.cur_tree = null;

stylo.views.TreeFactory = class TreeFactory extends stylo.views.Factory {
	make(route) {
		stylo.model.with_doctype(route[1], function () {
			var options = {
				doctype: route[1],
				meta: stylo.get_meta(route[1]),
			};

			if (
				!stylo.treeview_settings[route[1]] &&
				!stylo.meta.get_docfield(route[1], "is_group")
			) {
				stylo.msgprint(__("Tree view is not available for {0}", [route[1]]));
				return false;
			}
			$.extend(options, stylo.treeview_settings[route[1]] || {});
			stylo.views.trees[options.doctype] = new stylo.views.TreeView(options);
		});
	}
	...
	on_show() {
		...
		let treeview = stylo.views.trees[route[1]];
...
stylo.views.TreeView = class TreeView {
	constructor(opts) {
		var me = this;
		...
		this.get_tree_nodes = me.opts.get_tree_nodes || "stylo.desk.treeview.get_children";
	...
	get_permissions() {
		this.can_read = stylo.model.can_read(this.doctype);
		this.can_create =
			stylo.boot.user.can_create.indexOf(this.doctype) !== -1 ||
			stylo.boot.user.in_create.indexOf(this.doctype) !== -1;
		this.can_write = stylo.model.can_write(this.doctype);
		this.can_delete = stylo.model.can_delete(this.doctype);
	}
	make_page() {
		var me = this;
		if (!this.opts || !this.opts.do_not_make_page) {
			this.parent = stylo.container.add_page(this.page_name);
			$(this.parent).addClass("treeview");
			stylo.ui.make_app_page({ parent: this.parent, single_column: true });
			this.page = this.parent.page;
			stylo.container.change_to(this.page_name);
			stylo.breadcrumbs.add(
			...
			$(this.page[0]).addClass("stylo-card");
		}

		if (stylo.meta.has_field(me.doctype, "disabled")) {
	...
	make_filters() {
		var me = this;
		stylo.treeview_settings.filters = [];
		$.each(this.opts.filters || [], function (i, filter) {
			if (stylo.route_options && stylo.route_options[filter.fieldname]) {
				filter.default = stylo.route_options[filter.fieldname];
			}
	...
	rebuild_tree() {
		let me = this;
		stylo.call({
			method: "stylo.utils.nestedset.rebuild_tree",
			args: {
				doctype: me.doctype,
			},
			callback: function (r) {
	...
	select_node(node) {
		...
		if (this.opts.view_template) {
			...
			$(
				stylo.render_template(me.opts.view_template, {
					data: node.data,
					doctype: me.doctype,
				})
		...
		var toolbar = [
			{
				...
				click: function (node) {
					stylo.set_route("Form", me.doctype, node.label);
				},
			},
			{
				label: __("Add Child"),
			...
			},
			{
				...
				condition: function (node) {
					...
					if (me.doctype && stylo.get_meta(me.doctype)) {
						if (!stylo.get_meta(me.doctype).allow_rename) allow_rename = false;
					}
					return !node.is_root && me.can_write && allow_rename;
				},
				click: function (node) {
					stylo.model.rename_doc(me.doctype, node.label, function (new_name) {
			...
			},
			{
				...
				click: function (node) {
					stylo.model.delete_doc(me.doctype, node.label, function () {
						node.parent.remove();
					});
				},
				btnClass: "hidden-xs",
	...
	new_node() {
		...
		if (!(node && node.expandable)) {
			stylo.msgprint(__("Select a group {0} first.", [__(me.doctype)]));
			return;
		}
		...
		d.set_primary_action(__("Create New"), function () {
			...
			d.hide();
			stylo.dom.freeze(__("Creating {0}", [me.doctype]));

			$.extend(args, v);
			return stylo.call({
				method: me.opts.add_tree_node || "stylo.desk.treeview.add_node",
				args: args,
				callback: function (r) {
					if (!r.exc) {
	...
	print_tree() {
		if (!stylo.model.can_print(this.doctype)) {
			stylo.msgprint(__("You are not allowed to print this report"));
			return false;
		}
		var tree = $(".tree:visible").html();
		var me = this;
		stylo.ui.get_print_settings(false, function (print_settings) {
			var title = __(me.docname || me.doctype);
			stylo.render_tree({ title: title, tree: tree, print_settings: print_settings });
			stylo.call({
				method: "stylo.core.doctype.access_log.access_log.make_access_log",
				args: {
					doctype: me.doctype,
	...
	set_menu_item() {
		...
		this.menu_items = [
			{
				label: __("View List"),
				action: function () {
					stylo.set_route(["List", me.doctype, "List"]);
				},
			},
			{
				label: __("Print"),
		...
		if (
			...
			stylo.meta.has_field(me.doctype, "lft") &&
			stylo.meta.has_field(me.doctype, "rgt")
		) {
