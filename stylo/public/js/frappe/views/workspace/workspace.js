...
stylo.views.Workspace = class Workspace {
	constructor(wrapper) {
		...
		this.sidebar = stylo.app.sidebar;
		this.app_switcher_menu = stylo.app.app_switcher_menu;
		this.sidebar.setup_pages();
		this.cached_pages = $.extend(true, {}, stylo.boot.sidebar_pages);
		this.has_access = stylo.boot.sidebar_pages.has_access;
		this.has_create_access = stylo.boot.sidebar_pages.has_create_access;
		this.show();
	...
	`).appendTo(this.body);
		...
		this.body.find(".btn-edit-workspace").on("click", async () => {
			...
			this.editor.isReady.then(() => {
				...
				stylo.app.sidebar.toggle_sorting();
			});
		});
	}

	get_pages() {
		return stylo.xcall("stylo.desk.desktop.get_workspace_sidebar_items", null, "GET");
	}
	...
	show() {
		...
		if (!stylo.router.current_route[0]) {
			...
			stylo.set_route(stylo.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return stylo
			.call({
				method: "stylo.desk.desktop.get_desktop_page",
				args: {
	...
	get_page_to_show() {
		...
		if (stylo.boot.user.default_workspace) {
			default_page = {
				name: stylo.boot.user.default_workspace.name,
				public: stylo.boot.user.default_workspace.public,
			};
		} else if (
		...
		if (this.sidebar.all_pages.length) {
			...
			if (!this._page.public) {
				...
				if (!app && this._page.module) {
					app = stylo.boot.module_app[stylo.router.slug(this._page.module)];
				}
				if (!app) app = "stylo";
			}
			...
			stylo.app.sidebar.apps_switcher.set_current_app(app);
			this.wrapper.find(".workspace-title").html(__(this._page.title));
			this.wrapper
				.find(".workspace-icon")
				.html(stylo.utils.icon(this._page.icon || "folder-normal", "md"));
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			await this.editor.readOnly.toggle();
			this.is_read_only = true;
			stylo.boot.sidebar_pages = this.cached_pages;
			this.reload();
			...
			stylo.call({
				...
				callback: (r) => {
					if (r.message) {
						...
						stylo.boot.sidebar_pages = r.message;
						this.sidebar.setup_pages();

						if (!stylo.boot.app_data_map["private"] && new_page.public === 0) {
