/**
 * stylo.views.InboxView
 */

stylo.provide("stylo.views");

stylo.views.InboxView = class InboxView extends stylo.views.ListView {
	static load_last_view() {
		const route = stylo.get_route();
		if (!route[3] && stylo.boot.email_accounts.length) {
			let email_account;
			if (stylo.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = stylo.boot.email_accounts[0].email_account;
			}
			stylo.set_route("List", "Communication", "Inbox", email_account);
			return true;
		...
		function is_valid(email_account) {
			return stylo.boot.email_accounts.find((d) => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = stylo.get_route()[3];
	...
	get_seen_class(doc) {
		return Boolean(doc.seen) || JSON.parse(doc._seen || "[]").includes(stylo.session.user)
			? ""
			: "bold";
	}
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${stylo.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", stylo.boot.all_accounts, true],
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = stylo.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !stylo.boot.email_accounts.length) {
		...
		const html = stylo.model.can_create(args.doctype)
			? `<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !stylo.boot.email_accounts.length) {
			stylo.route_options = {
				email_id: stylo.session.user_email,
			};
			stylo.new_doc("Email Account");
		} else {
