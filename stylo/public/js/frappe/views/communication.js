...
stylo.last_edited_communication = {};
const separator_element = "<div>---</div>";

stylo.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	make() {
		...
		this.dialog = new stylo.ui.Dialog({
	...
	get_fields() {
		...
		const fields = [
			...
			{
				fieldtype: "Button",
				label: stylo.utils.icon("down", "xs"),
			...
			},
			{
				...
				fieldname: "content",
				onchange: stylo.utils.debounce(this.save_as_draft.bind(this), 300),
			},
			{
				fieldtype: "Button",
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: stylo.boot.user.send_me_a_copy,
			},
			{
		...
		const email_accounts = stylo.boot.email_accounts.filter((account) => {
			return (
		...
		if (email_accounts.length) {
			...
			} else if (this.user_email_accounts.includes(stylo.session.user_email)) {
				this["sender"] = stylo.session.user_email;
			}
		}
	...
	guess_language() {
		...
		if (print_format != "Standard") {
			print_format_lang = stylo.get_doc(
				"Print Format",
				print_format
			)?.default_print_language;
		}

		let lang = document_lang || print_format_lang || stylo.boot.lang;
	...
	toggle_more_options(show_options) {
		...
		const label = stylo.utils.icon(show_options ? "up-line" : "down", "xs");
	...
	setup_multiselect_queries() {
		["recipients", "cc", "bcc"].forEach((field) => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				stylo.call({
					method: "stylo.email.get_contact_list",
					args: args,
					callback: (r) => {
	...
	setup_email_template() {
		...
		function add_template() {
			...
			stylo.call({
				method: "stylo.email.doctype.email_template.email_template.get_email_template",
				args: {
		...
		stylo.utils.add_select_group_button(clear_and_add_template, email_template_actions);
	}
	...
	get_last_edited_communication(clear) {
		if (!stylo.last_edited_communication[this.doctype]) {
			stylo.last_edited_communication[this.doctype] = {};
		}

		if (clear || !stylo.last_edited_communication[this.doctype][this.key]) {
			stylo.last_edited_communication[this.doctype][this.key] = {};
		}

		return stylo.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = this.subject ? stylo.utils.html2text(this.subject) : "";
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = stylo.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		attach
			...
			.on("click", () => new stylo.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = stylo.urllib.get_full_url(f.file_url);
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on("click", () => {
			...
			stylo.db.set_value("User", stylo.session.user, "send_me_a_copy", val);
			stylo.boot.user.send_me_a_copy = val;
		});
	}
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			stylo.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !stylo.model.can_email(this.doc.doctype, this.frm)) {
			stylo.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}

		return stylo.call({
			method: "stylo.core.doctype.communication.email.make",
			args: {
				...
				sender_full_name: form_values.sender ? stylo.user.full_name() : undefined,
			...
			callback(r) {
				if (!r.exc) {
					stylo.utils.play_sound("email");

					if (r.message["emails_not_sent_to"]) {
						stylo.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								stylo.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					}
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find(".form-print-wrapper").is(":visible")) {
			...
			return (
				stylo.model.get_doc(":Print Settings", "Print Settings") || { with_letterhead: 1 }
			).with_letterhead
				? 1
				: 0;
		}
	...
	async get_signature(sender_email) {
		let signature = stylo.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await stylo.db.get_list("Email Account", {
				filters: filters,
		...
		if (!stylo.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = stylo.datetime.global_date_format(
	...
	html2text(html) {
		...
		const text = stylo.utils.html2text(html);
		return text.replace(/\n{3,}/g, "\n\n");
	}
};
