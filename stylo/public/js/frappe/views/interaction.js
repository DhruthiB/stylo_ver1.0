...
stylo.provide("stylo.interaction_settings");

stylo.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		stylo.model.with_doctype("Event", () => {
			let categories = stylo.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new stylo.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = stylo.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = stylo.session.user;
		}
		return stylo.call({
	...
	add_attachments(doc, attachments) {
		stylo.call({
			method: "stylo.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
