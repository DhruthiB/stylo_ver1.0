...
stylo.views.FileView = class FileView extends stylo.views.ListView {
	static load_last_view() {
		const route = stylo.get_route();
		if (route.length === 2) {
			const view_user_settings = stylo.get_user_settings("File", "File");
			stylo.set_route(
				"List",
				"File",
				view_user_settings.last_folder || stylo.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			stylo.views.FileView.grid_view = !stylo.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (stylo.views.FileView.grid_view) {
			Object.assign(args, {
		...
		return [
			{
				label: __("Home"),
				action: () => {
					stylo.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					stylo.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								stylo.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							stylo.call({
								method: "stylo.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new stylo.ui.FileUploader({
						...
						on_success: (file) => {
							stylo.show_alert(__("Unzipping files..."));
							stylo
								.call("stylo.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										stylo.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				stylo.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => stylo.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/stylo.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => stylo.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (stylo.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
		...
		title = stylo.utils.escape_html(title);
	...
	before_render() {
		super.before_render();
		stylo.model.user_settings.save("File", "grid_view", stylo.views.FileView.grid_view);
	...
	render_grid_view() {
		...
		let html = this.data
			.map((d) => {
				...
				const file_url = stylo.utils.escape_html(d.file_url);
	...
	get_header_html() {
		...
		let header_selector_html = !stylo.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !stylo.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === stylo.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = stylo.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? stylo.form.formatters.FileSize(file.file_size) : "";
		...
		return `
			<div class="list-row-col ellipsis list-subject level">
				...
				<span class="level-item  ellipsis" title="${stylo.utils.escape_html(file.file_name)}">
					<a class="ellipsis" href="${route_url}" title="${stylo.utils.escape_html(file.file_name)}">
						${file.subject_html}
					</a>
				</span>
			</div>
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					stylo.file_manager.paste(folder_name);
					stylo.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	setup_copy_event() {
		this.$result.on("click", ".copy-file-url", (e) => {
			stylo.utils.copy_to_clipboard(e.currentTarget.getAttribute("data-file-url"));
			e.preventDefault();
			e.stopPropagation();
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!stylo.file_manager.can_paste ||
			stylo.file_manager.old_folder === this.current_folder;
...
stylo.views.FileView.grid_view = stylo.get_user_settings("File").grid_view || false;
