...
stylo.views.KanbanView = class KanbanView extends stylo.views.ListView {
	...
	static load_last_view() {
		...
		if (route.length === 3) {
			...
			const user_settings = stylo.get_user_settings(doctype)["Kanban"] || {};
			if (!user_settings.last_kanban_board) {
				return new stylo.views.KanbanView({ doctype: doctype });
			}
	...
	show() {
		stylo.views.KanbanView.get_kanbans(this.doctype).then((kanbans) => {
			if (!kanbans.length) {
				return stylo.views.KanbanView.show_kanban_dialog(this.doctype, true);
			} else if (kanbans.length && stylo.get_route().length !== 4) {
				// Try to use the last board the user used, else default to the first available board
				const last_board = stylo.get_user_settings(this.doctype)["Kanban"]
					?.last_kanban_board;
				if (last_board && kanbans.includes(last_board)) {
					stylo.set_route("List", this.doctype, "Kanban", last_board);
					return;
				} else {
					const first_board = kanbans[0];
					stylo.set_route("List", this.doctype, "Kanban", first_board.name);
					return;
				}
			} else {
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.board_name = stylo.get_route()[3] || get_board_name() || null;
	...
	set_board_perms_and_push_menu_items() {
		...
		return stylo.call({
			method: "stylo.client.get_doc_permissions",
			args: {
	...
	push_menu_items() {
		...
		if (this.board_perms.delete) {
			this.menu_items.push({
				...
				action: () => {
					stylo.confirm(__("Are you sure you want to proceed?"), () => {
						stylo.db.delete_doc("Kanban Board", this.board_name).then(() => {
							stylo.show_alert(`Kanban Board ${this.board_name} deleted.`);
							stylo.set_route("List", this.doctype, "List");
						});
					});
				},
			});
	...
	get_board() {
		return stylo.db.get_doc("Kanban Board", this.board_name).then((board) => {
			this.board = board;
	...
	save_kanban_board_filters() {
		...
		stylo.db.set_value("Kanban Board", this.board_name, "filters", filters).then((r) => {
			if (r.exc) {
				stylo.show_alert({
					indicator: "red",
	...
	render() {
		...
		if (!this.kanban) {
			this.kanban = new stylo.views.KanbanBoard({
	...
	get_card_meta() {
		var meta = stylo.get_meta(this.doctype);
		// preserve route options erased by new doc
		let route_options = { ...stylo.route_options };
		var doc = stylo.model.get_new_doc(this.doctype);
		stylo.route_options = route_options;
		var title_field = null;
		var quick_entry = false;

		if (this.meta.title_field) {
			title_field = stylo.meta.get_field(this.doctype, this.meta.title_field);
		}
		...
		if (
			mandatory.some((df) => stylo.model.table_fields.includes(df.fieldtype)) ||
			mandatory.length > 1
		) {
			quick_entry = true;
		}
		...
		if (!title_field) {
			title_field = stylo.meta.get_field(this.doctype, "name");
		}
...
stylo.views.KanbanView.get_kanbans = function (doctype) {
	let kanbans = [];

	return get_kanban_boards().then((kanban_boards) => {
		if (kanban_boards) {
			kanban_boards.forEach((board) => {
				let route = `/app/${stylo.router.slug(board.reference_doctype)}/view/kanban/${
					board.name
				}`;
	...
	function get_kanban_boards() {
		return stylo
			.call("stylo.desk.doctype.kanban_board.kanban_board.get_kanban_boards", { doctype })
			.then((r) => r.message);
	}
};

stylo.views.KanbanView.show_kanban_dialog = function (doctype) {
	let dialog = new_kanban_dialog();
	dialog.show();

	function make_kanban_board(board_name, field_name, project) {
		return stylo.call({
			method: "stylo.desk.doctype.kanban_board.kanban_board.quick_kanban_board",
			args: {
				doctype,
				board_name,
				field_name,
			...
			callback: function (r) {
				var kb = r.message;
				if (kb.filters) {
					stylo.provide("stylo.kanban_filters");
					stylo.kanban_filters[kb.kanban_board_name] = kb.filters;
				}
				stylo.set_route("List", doctype, "Kanban", kb.kanban_board_name);
			},
		});
	}
	...
	function new_kanban_dialog() {
		...
		const select_fields = stylo.get_meta(doctype).fields.filter((df) => {
		...
		let primary_action = () => {
			if (to_save) {
				...
				make_kanban_board(values.board_name, values.field_name, values.project).then(
					() => dialog.hide(),
					(err) => stylo.msgprint(err)
				);
			} else {
				stylo.set_route("Form", "Customize Form", { doc_type: doctype });
			}
		};

		return new stylo.ui.Dialog({
			title: dialog_title,
