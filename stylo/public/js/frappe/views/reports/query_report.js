...
stylo.views.QueryReport = class QueryReport extends stylo.views.BaseList {
	...
	get_no_result_message() {
		return `<div class="msg-box no-border">
			<div>
				<img src="/assets/stylo/images/ui-states/list-empty-state.svg" alt="Generic Empty State" class="null-state">
			</div>
	...
	get_query_params() {
		...
		const query_params = stylo.utils.get_query_params(query_string);
		return query_params;
	}
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				stylo.call({
					method: "stylo.desk.query_report.background_enqueue_run",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
	...
	get_menu_items() {
		let items = [
			...
			},
			{
				...
				condition: () => stylo.model.can_set_user_permissions("Report"),
				standard: true,
			},
		];
...
Object.defineProperty(stylo, "query_report_filters_by_name", {
	get() {
		console.warn(
			"[Query Report] stylo.query_report_filters_by_name is deprecated. Please use the new api: stylo.query_report.get_filter_value(fieldname) and stylo.query_report.set_filter_value(fieldname, value)"
		);
		return null;
	},
});
