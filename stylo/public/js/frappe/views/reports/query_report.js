...
stylo.provide("stylo.widget.utils");
stylo.provide("stylo.views");
stylo.provide("stylo.query_reports");

stylo.standard_pages["query-report"] = function () {
	var wrapper = stylo.container.add_page("query-report");
	...
	$(wrapper).bind("show", function () {
		stylo.query_report.show();
	});
};

stylo.views.QueryReport = class QueryReport extends stylo.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = stylo.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = stylo.utils.throttle(this.refresh, 300);
	...
	update_url_with_filters() {
		if (stylo.get_route_str() == this.page_name) {
	...
	setup_events() {
		stylo.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					stylo.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (stylo.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, stylo.route_options);

		if (this.report_name !== stylo.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (stylo.has_route_options()) {
	...
	add_card_button_to_toolbar() {
		if (!stylo.model.can_create("Number Card")) return;
	...
	add_chart_buttons_to_toolbar(show) {
		if (!stylo.model.can_create("Dashboard Chart")) return;
		if (show) {
	...
	add_card_to_dashboard() {
		let field_options = stylo.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = stylo.dashboard_utils.get_dashboard_link_field();
		const set_standard = stylo.boot.developer_mode;

		const dialog = new stylo.ui.Dialog({
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			stylo.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"stylo.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || stylo.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"stylo.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		stylo.xcall(method, { args: args }).then(() => {
			let message;
			if (dashboard_name) {
			...
			stylo.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	get_report_doc() {
		return stylo.model
			...
			.then(() => stylo.model.with_doctype(this.report_doc?.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (stylo.query_reports[this.report_name]) {
				this.report_settings = stylo.query_reports[this.report_name];
				resolve();
			} else {
				stylo
					.xcall("stylo.desk.query_report.get_script", {
						report_name: this.report_name,
					})
					.then((settings) => {
						stylo.dom.eval(settings.script || "");
						stylo.after_ajax(() => {
							...
							stylo.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings(custom_report_name) {
		let report_script_name =
			this.report_doc.report_type === "Custom Report"
				...
				: this.report_name;
		return stylo.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			stylo.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = stylo.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					stylo.throw(
	...
	add_collapse_button() {
		...
		if (this.filters[filter_no]) {
			this.$collapse_button = $(`<div>${stylo.utils.icon("chevron-down", "md")}</div>`);
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = stylo.route_options;
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			const js_filters = (stylo.query_reports[this.report_name]?.filters || [])
				.filter(
					(filter) => filter.fieldtype === "Link" && filters[filter.fieldname] !== ""
				)
				.map(({ fieldname, fieldtype, options }) => ({ fieldname, fieldtype, options }));

			this.last_ajax = stylo.call({
				method: "stylo.desk.query_report.run",
				type: "GET",
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = stylo.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			stylo.utils.build_summary_item(summary).appendTo(this.$summary);
		});
	...
	get_query_params() {
		const query_string = stylo.utils.get_query_string(stylo.get_route_str());
		return stylo.utils.get_query_params(query_string);
	}
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
			this.page.add_inner_button(
				...
				function () {
					window.open(
						stylo.urllib.get_full_url(
							"/api/method/stylo.core.doctype.prepared_report.prepared_report.download_attachment?" +
								"dn=" +
			...
			let pretty_diff = stylo.datetime.comment_when(doc.report_end_time);
			const days_old = stylo.datetime.get_day_diff(
				stylo.datetime.now_datetime(),
				doc.report_end_time
			);
			if (days_old > 1) {
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					stylo.set_route(stylo.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		stylo
			.xcall(
				"stylo.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = stylo.warn(
					...
					stylo.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		stylo
			.xcall("stylo.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				stylo.call({
					method: "stylo.core.doctype.prepared_report.prepared_report.make_prepared_report",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
				...
				let alert_message =
					...
					`</a>`;
				stylo.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (data.length > (cint(stylo.boot.sysdefaults.max_report_rows) || 100000)) {
			let msg = __(
			...
			this.toggle_message(true, `${stylo.utils.icon("solid-warning")} ${msg}`);
			return;
		}
		...
		if (
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: stylo.boot.lang,
				translations: stylo.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: "fixed",
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: stylo.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: stylo.utils.report_column_total,
				},
			};
	...
	update_masked_fields_in_columns(columns) {
		const masked_fields = stylo.get_meta(this.report_doc?.ref_doctype).masked_fields;
	...
	get_chart_options(data) {
		...
		options.axisOptions = {
			...
			numberFormatter: stylo.utils.format_chart_axis_number,
		};
		options.height = 280;
		return options;
	}
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new stylo.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = stylo.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new stylo.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		...
		return columns.map((column) => {
			column = stylo.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = stylo.datetime.user_to_obj(keyword);
					const cellValue = stylo.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = stylo.get_meta(this.report_doc.ref_doctype);
		stylo.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		stylo.call("stylo.core.doctype.access_log.access_log.make_access_log", {
	...
	async pdf_report(print_settings) {
		const base_url = stylo.urllib.get_base_url();
		const print_css = stylo.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = stylo.render_template(template, {
			title: __(this.report_name),
			subtitle: print_settings?.include_filters ? filters_html : null,
		...
		const html = stylo.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: stylo.boot.lang,
			layout_direction: stylo.utils.is_rtl() ? "rtl" : "ltr",
			can_use_smaller_font: this.report_doc.is_standard === "Yes" && custom_format ? 0 : 1,
		});
		...
		stylo.render_pdf(html, print_settings);
	}
	...
	async get_report_print_format(report_name) {
		...
		const r = await stylo.db.get_value("Print Format", filters, ["html", "css"]);
		if (r && r.message && r.message.html) {
			const css = r.message.css || "";
			const html = r.message.html || "";
			return `<style>${css}</style>${html}`;
		} else {
			stylo.msgprint(__("Print Format not found"));
			return null;
		}
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				const docfield = stylo.query_report.get_filter(fieldname).df;
				const value = applied_filters[fieldname];

				if (stylo.utils.is_empty(value) || docfield.hidden_due_to_dependency) {
					return null;
				}

				return `<div class="filter-row">
					<b>${__(docfield.label, null, docfield.parent)}:</b> ${stylo.format(value, docfield)}
				</div>`;
			})
			.join("");
	}
	...
	export_report() {
		...
		this.export_dialog = stylo.report_utils.get_export_dialog(
			__(this.report_name),
			extra_fields,
			({
				file_format,
				...
				for (const [key, value] of Object.entries(filters)) {
					const df = stylo.query_report.get_filter(key).df;
				...
				const args = {
					cmd: "stylo.desk.query_report.export_query",
					report_name: this.report_name,
				...
				if (export_in_background) {
					...
					open_url_post(stylo.request.url, args);
				}
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = stylo.utils.get_formatted_duration(cell.content);
				}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __("Edit"),
				action: () => stylo.set_route("Form", "Report", this.report_name),
				condition: () => stylo.user.is_report_manager(),
				standard: true,
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = stylo.ui.get_print_settings(
						false,
				...
				condition: () => stylo.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => stylo.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("Setup Auto Email"),
				action: () =>
					stylo.set_route("List", "Auto Email Report", { report: this.report_name }),
				standard: true,
			},
			{
				label: __("Add Column"),
				action: () => {
					let d = new stylo.ui.Dialog({
						...
						fields: [
							{
								...
								options: this.linked_doctypes?.map((df) => ({
									label: df.doctype + " (" + stylo.unscrub(df.fieldname) + ")",
								...
								change: () => {
									...
									stylo.model.with_doctype(doctype, () => {
										let options = stylo.meta
											.get_docfields(doctype)
											.filter(stylo.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = stylo.meta.get_docfield(values.doctype, values.field);
							...
							custom_columns.push({
								fieldname: this.columns
									...
									? df.fieldname + "-" + stylo.scrub(values.doctype)
									: df.fieldname,
							...
							stylo.call({
								method: "stylo.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
									names: Array.from(
			...
			},
			{
				...
				condition: () => stylo.user.has_role("System Manager"),
				standard: true,
			},
		];

		if (stylo.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new stylo.ui.Dialog({
						...
						primary_action: (values) => {
							stylo.call({
								method: "stylo.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									stylo.set_route("query-report", r.message);
								},
							});
						},
					});
	...
	add_custom_column(custom_column, custom_data, new_column_data, insert_after_index) {
		...
		this.data.forEach((row) => {
			if (column[0].fieldname.includes("-")) {
				row[column_field + "-" + stylo.scrub(new_column_data.doctype)] =
