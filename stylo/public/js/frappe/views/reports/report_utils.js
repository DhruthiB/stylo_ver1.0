stylo.provide("stylo.report_utils");
...
stylo.report_utils = {
	make_chart_options: function (
		...
		function get_translated_column_label(fieldname) {
			...
			return column?.label ?? __(stylo.model.unscrub(fieldname));
		}
	},
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = stylo.report_utils.prepare_field_from_column(field);
					return { label: field.label, value: field.fieldname };
				}
			});
		}
	...
	get_report_filters: function (report_name) {
		if (stylo.query_reports[report_name]) {
			let filters = stylo.query_reports[report_name].filters;
		...
		return stylo
			...
			.then((r) => {
				stylo.dom.eval(r.script || "");
				return stylo.after_ajax(() => {
					if (
						stylo.query_reports[report_name] &&
						!stylo.query_reports[report_name].filters &&
						r.filters
					) {
						return (stylo.query_reports[report_name].filters = r.filters);
					}
					return (
						stylo.query_reports[report_name] &&
						stylo.query_reports[report_name].filters
	...
	get_export_dialog(report_name, extra_fields, callback) {
		...
		function update_csv_preview(dialog) {
			const is_query_report = stylo.get_route()[0] === "query-report";
			const report = is_query_report ? stylo.query_report : cur_list;
			...
			dialog.set_value(
				"csv_preview",
				stylo.report_utils.get_csv_preview(
					PREVIEW_DATA,
	...
	get_csv_preview(data, quoting, delimiter, decimal_sep) {
		...
		if (delimiter.length > 1) {
			stylo.throw(__("Delimiter must be a single character"));
		}

		if (decimal_sep.length > 1) {
			stylo.throw(__("Decimal Separator must be a single character"));
		}

		if (0 > quoting || quoting > 3) {
			stylo.throw(__("Quoting must be between 0 and 3"));
		}

		if (decimal_sep !== "." && quoting === QUOTING.NonNumeric) {
			stylo.throw(__("Decimal Separator must be '.' when Quoting is set to Non-numeric"));
		}
