...
stylo.provide("stylo.views.calendar");
stylo.provide("stylo.views.calendars");

stylo.views.CalendarView = class CalendarView extends stylo.views.ListView {
	static load_last_view() {
		const route = stylo.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = stylo.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = stylo.views.calendar[this.doctype] || {};
			this.calendar_name = stylo.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new stylo.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, stylo.views.calendar[this.doctype]);
				resolve(options);
			} else {
				stylo.model.with_doc("Calendar View", calendar_name, () => {
					const doc = stylo.get_doc("Calendar View", calendar_name);
					if (!doc) {
						stylo.show_alert(
						...
						stylo.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
	...
	get required_libs() {
		let assets = [
			"assets/stylo/js/lib/fullcalendar/fullcalendar.min.css",
			"assets/stylo/js/lib/fullcalendar/fullcalendar.min.js",
		];
		let user_language = stylo.boot.user.language;
		if (user_language && user_language !== "en") {
			assets.push("assets/stylo/js/lib/fullcalendar/locale-all.js");
		}
		return assets;
	}
};

stylo.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(stylo.boot.calendars, function (i, doctype) {
			if (stylo.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					stylo.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = stylo.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(stylo.utils.icon("left"));
		this.$wrapper
			.find(".fc-next-button span")
			.attr("class", "")
			.html(stylo.utils.icon("right"));

		this.$wrapper.find(".fc-today-button").prepend(stylo.utils.icon("today"));
	...
	get_system_datetime(date) {
		date._offset = moment(date).tz(stylo.sys_defaults.time_zone)._offset;
		return stylo.datetime.convert_to_system_tz(moment(date).locale("en"));
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			locale: stylo.boot.user.language || "en",
			header: {
			...
			events: function (start, end, timezone, callback) {
				return stylo.call({
					method: me.get_events_method || "stylo.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (event) {
				...
				if (stylo.model.can_read(doctype)) {
					stylo.set_route("Form", doctype, event.name);
				}
			},
			...
			select: function (startDate, endDate, jsEvent, view) {
				...
				var event = stylo.model.get_new_doc(me.doctype);
				...
				stylo.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = stylo.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = stylo.datetime.convert_to_user_tz(d.start);
				d.end = stylo.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!stylo.datetime.validate(d.start) && d.end) {
				d.start = stylo.datetime.add_days(d.end, -1);
			}

			if (d.start && !stylo.datetime.validate(d.end)) {
				d.end = stylo.datetime.add_days(d.start, 1);
			}

			me.fix_end_date_for_event_render(d);
			me.prepare_colors(d);

			d.title = stylo.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = stylo.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = stylo.ui.color.get(color_name, "extra-light");
			d.textColor = stylo.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!stylo.ui.color.validate_hex(color) || !color) {
				color = stylo.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = stylo.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		stylo.model.remove_from_locals(me.doctype, event.name);
		return stylo.call({
			method: me.update_event_method || "stylo.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					stylo.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {
