...
stylo.views.Calendar = class Calendar {
	...
	make() {
		...
		this.fullCalendar = new stylo.FullCalendar(this.$cal[0], this.cal_options);
	...
	get_system_datetime(date) {
		return stylo.datetime.convert_to_system_tz(date, true);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			plugins: stylo.FullCalendar.Plugins,
			initialView: defaults.initialView || "dayGridMonth",
			locale: stylo.boot.lang,
			firstDay: 1,
			headerToolbar: {
			...
			eventClick: function (info) {
				...
				if (stylo.model.can_read(doctype)) {
					stylo.set_route("Form", doctype, info.event.id);
				}
			},
	...
	update_event(event, revertFunc) {
		var me = this;
		stylo.model.remove_from_locals(me.doctype, event.id);
		return stylo.call({
