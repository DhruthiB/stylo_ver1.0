...
stylo.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[stylo.breadcrumbs.current_page()] = obj;
		this.update();
		stylo.app.sidebar.set_active_workspace_item();
	},

	current_page() {
		return stylo.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[stylo.breadcrumbs.current_page()];
		...
		if (
			...
			stylo.workspace_map[breadcrumbs.workspace]?.app &&
			stylo.workspace_map[breadcrumbs.workspace]?.app != stylo.current_app
		) {
			let app = stylo.workspace_map[breadcrumbs.workspace].app;
	...
	get last_route() {
		return stylo.route_history.slice(-2)[0];
	},
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (
			...
			(breadcrumbs.module_info.blocked ||
				!stylo.visible_modules.includes(breadcrumbs.module_info.module))
		) {
			return;
		}

		this.append_breadcrumb_element(
			`/app/${stylo.router.slug(breadcrumbs.workspace)}`,
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			if (
				breadcrumbs.module &&
				stylo.boot.module_wise_workspaces[breadcrumbs.module]?.includes(last_workspace)
			) {
				breadcrumbs.workspace = last_workspace;
			}
		} else {
			// choose from __workspaces
			const doctype_meta = stylo.get_meta(breadcrumbs.doctype);
			...
			if (breadcrumbs.module) {
				...
				breadcrumbs.module_info = stylo.get_module(breadcrumbs.module);

				// set workspace
				if (
					breadcrumbs.module_info &&
					stylo.boot.module_wise_workspaces[breadcrumbs.module]
				) {
					breadcrumbs.workspace =
						stylo.boot.module_wise_workspaces[breadcrumbs.module][0];
				}
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = stylo.get_meta(doctype);
		if (
			(doctype === "User" && !stylo.user.has_role("System Manager")) ||
			doctype_meta?.issingle
		) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = stylo.router.slug(stylo.router.doctype_layout || doctype);
			if (doctype_meta?.is_tree) {
				let view = stylo.model.user_settings[doctype].last_view || "Tree";
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		let docname = stylo.get_route().slice(2).join("/");
		let doc = stylo.get_doc(doctype, docname);

		if (doc.__islocal) return; // new doc, no breadcrumb required

		let title = stylo.model.get_doc_title(doc);

		if (title == doc.name) return; // title and name are same, don't add breadcrumb

		let form_route = `/app/${stylo.router.slug(doctype)}/${encodeURIComponent(docname)}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				stylo.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	},
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = stylo.get_route()[1];
		let dashboard_route = `/app/${stylo.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`).appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!stylo.visible_modules) {
			stylo.visible_modules = $.map(stylo.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete stylo.breadcrumbs.all[old_route_str];
		this.update();
	},
