/**
 * stylo.views.ImageView
 */
stylo.provide("stylo.views");

stylo.views.ImageView = class ImageView extends stylo.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = stylo.utils.escape_html(title);
	...
	get_attached_images() {
		return stylo
			.call({
				method: "stylo.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new stylo.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
	...
	get required_libs() {
		return [
			"assets/stylo/node_modules/photoswipe/src/photoswipe.css",
			"photoswipe.bundle.js",
		];
	}
};

stylo.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
		me.prepare();
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = stylo.render_template("photoswipe_dom");
	...
	_show(docname) {
		...
		// init
		this.pswp = new stylo.PhotoSwipe(options);
		this.pswp.init();
	}
};
