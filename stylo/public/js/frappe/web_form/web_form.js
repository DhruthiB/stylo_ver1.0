...
stylo.provide("stylo.web_form");
import EventEmitterMixin from "../../stylo/event_emitter";

export default class WebForm extends stylo.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		stylo.web_form = this;
		stylo.web_form.events = {};
		Object.assign(stylo.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		stylo.init_client_script && stylo.init_client_script();
		this.setup_listeners();
		stylo.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = stylo.utils.get_query_params();
		delete values.new;
	...
	delete_form() {
		...
		stylo.confirm(__("Are you sure you want to delete this record?"), () => {
			stylo.call({
				method: "stylo.website.doctype.web_form.web_form.delete",
				args: {
					web_form_name: this.name,
					docname: this.doc.name,
				},
				callback: () => {
					stylo.msgprint(__("Deleted!"));
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${stylo.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".stylo-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		stylo.form_dirty = false;

		stylo.call({
			type: "POST",
			method: "stylo.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					stylo.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						stylo.call({
							type: "POST",
							method: "stylo.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			stylo.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
