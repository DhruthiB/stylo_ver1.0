...
stylo.ready(function () {
	let web_form_doc = stylo.web_form_doc;
	let reference_doc = stylo.reference_doc;
	...
	function show_login_prompt() {
		if (stylo.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new stylo.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(stylo.form.formatters.FileSize($(this).text()));
		});
	}
