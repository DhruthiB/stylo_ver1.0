...
export default {
	...
	methods: {
		...
		refresh: function () {
			stylo.call("stylo.recorder.get").then((r) => (this.requests = r.message));
		},
		update: function (message) {
			this.requests.push(JSON.parse(message));
		},
		clear: function () {
			stylo.call("stylo.recorder.delete").then((r) => this.refresh());
		},
		start: function () {
			stylo.call("stylo.recorder.start").then((r) => this.fetch_status());
		},
		stop: function () {
			stylo.call("stylo.recorder.stop").then((r) => this.fetch_status());
		},
		fetch_status: function () {
			stylo.call("stylo.recorder.status").then((r) => this.update_status(r.message));
		},
		update_status: function (result) {
			...
			if (this.status.status == "Active") {
				stylo.realtime.on("recorder-dump-event", this.update);
			} else {
				stylo.realtime.off("recorder-dump-event", this.update);
			}
		...
		export_data: function () {
			...
			stylo.call("stylo.recorder.export_data").then((r) => {
