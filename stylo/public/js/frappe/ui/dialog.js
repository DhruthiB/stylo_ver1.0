...
stylo.ui.open_dialogs = [];

stylo.ui.Dialog = class Dialog extends stylo.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				stylo.ui.form.get_open_grid_form?.()?.hide_form();

				if (stylo.ui.open_dialogs[stylo.ui.open_dialogs.length - 1] === me) {
					stylo.ui.open_dialogs.pop();
					if (stylo.ui.open_dialogs.length) {
						window.cur_dialog =
							stylo.ui.open_dialogs[stylo.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				stylo.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("stylo.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(stylo.utils.icon(icon));
