...
stylo.ui.FieldGroup = class FieldGroup extends stylo.ui.form.Layout {
	constructor(opts) {
		super(opts);
		this.dirty = false;
		this.fetch_dict = {};
	...
	make() {
		...
		if (this.fields) {
			...
			$.each(this.fields_list, function (i, field) {
				...
				if (
					def_value == null ||
					(!def_value && !stylo.model.is_numeric_field(field.df.fieldtype))
				)
					return;

				if (def_value == "Today" && field.df["fieldtype"] == "Date") {
					def_value = stylo.datetime.get_today();
				}
