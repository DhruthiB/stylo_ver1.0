...
stylo.tags.utils = {
	get_tags: function (txt) {
		...
		if (!stylo.tags.tags) {
			stylo.tags.utils.fetch_tags();
			return [];
		}

		for (let i in stylo.tags.tags) {
			let tag = stylo.tags.tags[i];
			let level = stylo.search.utils.fuzzy_search(txt, tag);
			if (level) {
				out.push({
					type: "Tag",
					label: __("#{0}", [stylo.search.utils.bolden_match_part(__(tag), txt)]),
					value: __("#{0}", [__(tag)]),
					index: 1 + level,
					match: tag,
					onclick() {
						// Use Global Search Dialog for tag search too.
						stylo.searchdialog.search.init_search("#".concat(tag), "tags");
					},
				});
			}
		}
	...
	fetch_tags() {
		stylo.call({
			method: "stylo.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function (r) {
				if (r && r.message) {
					stylo.tags.tags = $.extend([], r.message);
				}
			},
		});
	},
	...
	get_tag_results: function (tag) {
		...
		return new Promise(function (resolve) {
			stylo.call({
				method: "stylo.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag,
				},
				callback: function (r) {
