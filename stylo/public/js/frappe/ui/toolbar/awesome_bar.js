...
stylo.provide("stylo.tags");

stylo.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
		...
		var awesomplete = new Awesomplete(input, {
			...
			item: function (item, term) {
				...
				let target = "#";
				if (d.route) {
					target = stylo.router.make_url(
						stylo.router.convert_from_standard_route(
							stylo.router.get_route_from_arguments(
			...
			stylo.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(stylo.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(stylo.search.utils.get_frequent_links());
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				stylo.route_options = item.route_options;
			}
		...
		stylo.search.utils.setup_recent();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function () {
				...
				stylo.msgprint(txt, __("Search Help"));
			},
		});
	}
	...
	build_options(txt) {
		var options = stylo.search.utils
			.get_creatables(txt)
			.concat(
				stylo.search.utils.get_search_in_list(txt),
				stylo.search.utils.get_doctypes(txt),
				stylo.search.utils.get_reports(txt),
				stylo.search.utils.get_pages(txt),
				stylo.search.utils.get_workspaces(txt),
				stylo.search.utils.get_dashboards(txt),
				stylo.search.utils.get_recent_pages(txt || ""),
				stylo.search.utils.get_executables(txt),
				stylo.search.utils.get_marketplace_apps(txt)
			);
		if (txt.charAt(0) === "#") {
			options = stylo.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${stylo.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	stylo.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [stylo.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
			value: __("Search for {0}", [stylo.utils.xss_sanitise(txt)]),
			match: txt,
			index: 100,
			default: "Search",
			onclick: function () {
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			const doctype = stylo.container.page?.list_view?.doctype;
			if (!doctype) return;
			...
			this.options.push({
				label: __("Find {0} in {1}", [
					stylo.utils.xss_sanitise(txt).bold(),
					__(route[1]).bold(),
				]),
				value: __("Find {0} in {1}", [stylo.utils.xss_sanitise(txt), __(route[1])]),
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				var formatted_value = __("{0} = {1}", [
					stylo.utils.xss_sanitise(txt),
					(rounded_val + "").bold(),
				]);
				this.options.push({
					label: formatted_value,
					value: __("{0} = {1}", [stylo.utils.xss_sanitise(txt), rounded_val]),
					match: rounded_val,
					index: 80,
					default: "Calculator",
					onclick: function () {
						stylo.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: stylo.utils.get_random(16),
				onclick: function () {
					stylo.msgprint(stylo.utils.get_random(16), __("Result"));
				},
			});
		}
	}
