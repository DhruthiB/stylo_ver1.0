stylo.ui.FilterGroup = class {
	...
	add_filters(filters) {
		...
		return stylo.run_serially(promises).then(() => this.update_filters());
	}
	...
	_push_new_filter(doctype, fieldname, condition, value, hidden = false) {
		...
		let filter = new stylo.ui.Filter(args);
	...
	filter_exists(filter_value) {
		...
		return this.filters
			...
			.some((f) => {
				...
				return stylo.utils.arrays_equal(f_value.slice(0, 4), filter_value.slice(0, 4));
			});
	}
