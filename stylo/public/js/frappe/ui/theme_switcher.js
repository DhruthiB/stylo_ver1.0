...
stylo.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = stylo.ui.keys.get_key(e);
			let increment_by;
	...
	toggle_theme(theme) {
		...
		stylo.show_alert(__("Theme Changed"), 3);

		stylo.xcall("stylo.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
stylo.ui.add_system_theme_switch_listener = () => {
	stylo.ui.dark_theme_media_query.addEventListener("change", () => {
		stylo.ui.set_theme();
	});
};

stylo.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

stylo.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = stylo.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
