export class DropdownConsole {
	constructor() {
		this.dialog = new stylo.ui.Dialog({
			...
			fields: [
				{
					description: `
					${stylo.utils.icon("solid-warning", "xs")}
	...
	async load_contextual_boilerplate() {
		...
		if (cur_frm && !cur_frm.is_new()) {
			default_code = `doc = stylo.get_doc("${cur_frm.doc.doctype}", "${cur_frm.doc.name}")\n`;
		} else if (cur_list && stylo.get_route()[0] == "List") {
			let args = cur_list.get_args();
			default_code = `docs = stylo.get_all("${args.doctype}",
				fields="*",
	...
	async execute_code() {
		...
		const start = stylo.datetime.now_datetime(true);
		let { output } = await stylo.xcall(
			"stylo.desk.doctype.system_console.system_console.execute_code",
			{
				doc: {
		...
		const end = stylo.datetime.now_datetime(true);
		...
		output_field.set_description(`Executed in ${time_taken} milliseconds.
			<a target="_blank" href="/app/console-log?owner=${stylo.session.user}" >View Logs</a>`);
	}

	async load_completions() {
		let me = this;
		let items = await stylo.xcall(
			"stylo.core.doctype.server_script.server_script.get_autocompletion_items",
			null,
			"GET",
			{ cache: true }
		);
