// stylo.ui.Capture
// Author - Achilles Rasquinha <achilles@stylo.io>
 ...
 *
 * @example
 * stylo._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
stylo._.get_data_uri = (element) => {
...
/**
 * @description Stylo's Capture object.
 *
 * @example
 * const capture = stylo.ui.Capture()
 * capture.show()
 *
 ...
 */
stylo.ui.Capture = class {
	constructor(options = {}) {
		this.options = stylo.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...stylo.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(stylo.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (stylo.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(stylo.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					stylo.show_alert(stylo.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = stylo._.get_data_uri(me.video);
...
stylo.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
stylo.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
stylo.ui.Capture.TEMPLATE = `
<div class="stylo-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${stylo.ui.Capture.ERR_MESSAGE}</video>
	</div>
