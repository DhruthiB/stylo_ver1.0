 ...
 *
 * @returns {stylo.ui.Page}
 */

/**
 * @typedef {Object} stylo.ui.Page
 */

stylo.ui.make_app_page = function (opts) {
	opts.parent.page = new stylo.ui.Page(opts);
	return opts.parent.page;
};

stylo.ui.pages = {};

stylo.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		this.make();
		stylo.ui.pages[stylo.get_route_str()] = this;
	}
	...
	load_lib(callback) {
		stylo.require(this.required_libs, callback);
	}

	add_main_section() {
		$(stylo.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_page() {
		...
		this.card_layout && this.main.addClass("stylo-card");
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (stylo.utils.is_xs() || stylo.utils.is_sm()) {
	...
	update_sidebar_icon() {
		...
		sidebar_toggle_icon.html(
			stylo.utils.icon(is_sidebar_visible ? "sidebar-collapse" : "sidebar-expand", "md")
		);
	}
	...
	add_action_icon(icon, click, css_class = "", tooltip_label) {
		...
		button.click(click);
		button
			.attr("title", __(tooltip_label || stylo.unscrub(icon)))
	...
	get_icon_label(icon, label) {
		...
		return `${icon ? stylo.utils.icon(icon_name, size) : ""} <span class="hidden-xs"> ${__(
			label
		)} </span>`;
	}
	...
	set_action(btn, opts) {
		...
		stylo.ui.keys.get_shortcut_group(this).add(btn, text_span.length ? text_span : btn);
	}
	...
	add_dropdown_item({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${stylo.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			stylo.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
	...
	set_title(title, icon = null, strip = true, tab_title = "") {
		...
		this.title = title;
		stylo.utils.set_title(tab_title || title);
		if (icon) {
			title = `${stylo.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || "btn-default"} ${opts.btn_size || "btn-sm"} ellipsis">
				${opts.icon ? stylo.utils.icon(opts.icon) : ""}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		stylo.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = stylo.ui.form.make_control({
			df: df,
