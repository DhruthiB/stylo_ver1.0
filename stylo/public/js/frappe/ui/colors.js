...
stylo.ui.color = {
	get: function (color_name, shade) {
		...
		return stylo.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(stylo.ui.color_map);
		if (color_names.includes(color_name)) {
			return stylo.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(stylo.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(stylo.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in stylo.ui.color_map) {
			const colors = stylo.ui.color_map[key];
...
stylo.ui.color_map = stylo.ui.color.get_color_map();
