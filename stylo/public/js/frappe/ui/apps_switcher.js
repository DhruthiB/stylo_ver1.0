stylo.ui.AppsSwitcher = class AppsSwitcher {
	constructor(sidebar) {
	...
	make() {
		this.wrapper = $(
			stylo.render_template("apps_switcher", {
				app_logo_url: stylo.boot.app_data[0].app_logo_url,
				app_title: __(stylo.boot.app_data[0].app_title),
			})
	...
	create_app_data_map() {
		stylo.boot.app_data_map = {};
		for (var app of stylo.boot.app_data) {
			stylo.boot.app_data_map[app.app_name] = app;
	...
	add_private_app() {
		...
		stylo.boot.app_data_map["private"] = app;
	...
	setup_select_app() {
		this.app_switcher_menu.find(".app-item").on("click", (e) => {
			...
			if (item.attr("data-app-name") == "settings") {
				stylo.quick_edit("Workspace Settings");
				return;
			}
			if (route.startsWith("/app/private")) {
				this.set_current_app("private");
				let ws = Object.values(stylo.workspace_map).find((ws) => ws.public === 0);
				route += "/" + stylo.router.slug(ws.title);
		...
		this.add_app_item(
			{
				...
				app_route: "/",
				app_logo_url: "/assets/stylo/images/web.svg",
			},
	...
	add_settings_select() {
		...
		this.add_app_item({
			...
			app_logo_url: "/assets/stylo/images/settings-gear.svg",
		});
	...
	set_current_app(app) {
		...
		let app_data = stylo.boot.app_data_map[app] || stylo.boot.app_data_map["stylo"];
		...
		stylo.stylo_toolbar.set_app_logo(app_data.app_logo_url);

		if (stylo.current_app === app) return;
		stylo.current_app = app;

		// re-render the sidebar
		stylo.app.sidebar.make_sidebar();
	}
