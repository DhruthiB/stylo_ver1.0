...
stylo.ui.Notifications = class Notifications {
	constructor() {
		this.tabs = {};
		this.notification_settings = stylo.boot.notification_settings;
		this.make();
	}
	...
	setup_headers() {
		...
		$(`<span class="notification-settings pull-right" data-action="go_to_settings">
			${stylo.utils.icon("setting-gear")}
		</span>`)
			.on("click", (e) => {
				e.stopImmediatePropagation();
				this.dropdown.dropdown("hide");
				stylo.set_route("Form", "Notification Settings", stylo.session.user);
			})
	...
	mark_all_as_read(e) {
		...
		stylo.call("stylo.desk.doctype.notification_log.notification_log.mark_all_as_read");
	}
...
stylo.ui.notifications = {
	get_notification_config() {
		return stylo.xcall("stylo.desk.notifications.get_notification_info").then((r) => {
			stylo.ui.notifications.config = r;
			return r;
		});
	},

	show_open_count_list(doctype) {
		if (!stylo.ui.notifications.config) {
	...
	route_to_list_with_filters(doctype) {
		let filters = stylo.ui.notifications.config["conditions"][doctype];
		if (filters && $.isPlainObject(filters)) {
			if (!stylo.route_options) {
				stylo.route_options = {};
			}
			$.extend(stylo.route_options, filters);
		}
		stylo.set_route("List", doctype);
	},
};
...
class NotificationsView extends BaseNotificationsView {
	make() {
		...
		this.get_notifications_list(this.max_length).then((r) => {
			...
			stylo.update_user_info(r.message.user_info);
	...
	mark_as_read(docname, $el) {
		stylo
			.call("stylo.desk.doctype.notification_log.notification_log.mark_as_read", {
				docname: docname,
			})
			.then(() => {
	...
	get_dropdown_item_html(notification_log) {
		...
		message = title
			? message.replace(title[1], stylo.ellipsis(strip_html(title[1]), 100))
			: message;

		let timestamp = stylo.datetime.comment_when(notification_log.creation);
		...
		let user_avatar = stylo.avatar(user, "avatar-medium user-avatar");
			...
			if (this.dropdown_items.length) {
				...
				this.container.append(
					...
					<div class="text-center">
						<img src="/assets/stylo/images/ui-states/notification-empty-state.svg" alt="Generic Empty State" class="null-state">
	...
	get_notifications_list(limit) {
		return stylo.call({
			method: "stylo.desk.doctype.notification_log.notification_log.get_notification_logs",
			args: { limit: limit },
			type: "GET",
			cache: true,
		});
	...
	get_item_link(notification_doc) {
		...
		return stylo.utils.get_form_link(link_doctype, link_docname);
	}
	...
	toggle_seen(flag) {
		stylo.call(
			"stylo.desk.doctype.notification_settings.notification_settings.set_seen_value",
			{
				value: cint(flag),
	...
	setup_notification_listeners() {
		stylo.realtime.on("notification", () => {
			this.toggle_notification_icon(false);
			this.update_dropdown();
		});

		stylo.realtime.on("indicator_hide", () => {
		...
		this.parent.on("show.bs.dropdown", () => {
			...
			if (this.notifications_icon.find(".notifications-unseen").is(":visible")) {
				this.toggle_notification_icon(true);
				stylo.call(
					"stylo.desk.doctype.notification_log.notification_log.trigger_indicator_hide"
				);
			}
		});
	}
...
class EventsView extends BaseNotificationsView {
	make() {
		let today = stylo.datetime.get_today();
		stylo
			.xcall(
				"stylo.desk.doctype.event.event.get_events",
				{
					start: today,
					end: today,
				},
	...
	render_events_html(event_list) {
		...
		if (event_list.length) {
			let get_event_html = (event) => {
				let time = __("All Day");
				if (!event.all_day) {
					let start_time = stylo.datetime.get_time(event.starts_on);
					let days_diff = stylo.datetime.get_day_diff(event.ends_on, event.starts_on);
					let end_time = stylo.datetime.get_time(event.ends_on);
					if (days_diff > 1) {
				...
				if (event.particpants) {
					particpants = stylo.avatar_group(event.particpants, 3);
				}
			...
			html = `
				<div class="notification-null-state">
					<div class="text-center">
					<img src="/assets/stylo/images/ui-states/event-empty-state.svg" alt="Generic Empty State" class="null-state">
...
class ChangelogFeedView extends BaseNotificationsView {
	make() {
		this.render_changelog_feed_html(stylo.boot.changelog_feed || []);
	}
	...
	render_changelog_feed_html(changelog_feed) {
		...
		if (changelog_feed.length) {
			...
			const get_changelog_feed_html = (changelog_feed_item) => {
				const timestamp = stylo.datetime.prettyDate(
