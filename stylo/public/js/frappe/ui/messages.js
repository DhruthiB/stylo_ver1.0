...
stylo.msgprint = function (msg, title, is_minimizable, re_route) {
	if (!msg) return;
	let data;
	...
	if (stylo.msg_dialog && data.re_route) {
		stylo.msg_dialog.custom_onhide = function () {
			stylo.route_flags.replace_route = true;
			let prev_route = stylo.get_prev_route();
			if (prev_route.length == 0) stylo.set_route("");
			stylo.set_route(prev_route);
		};
	}
	if (!stylo.msg_dialog) {
	...
	if (data.secondary_action) {
		...
		stylo.msg_dialog.set_secondary_action_label(
...
stylo.show_progress = (title, count, total = 100, description, hide_on_completion = false) => {
	...
	stylo.cur_progress.$wrapper.css("z-index", 2000);
	return dialog;
};
