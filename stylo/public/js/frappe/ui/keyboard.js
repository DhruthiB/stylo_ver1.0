...
stylo.provide("stylo.ui.keys.handlers");

stylo.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = stylo.ui.keys.get_key(e);
		if (stylo.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = stylo.ui.keys.handlers[key].length; i < l; i++) {
				var handler = stylo.ui.keys.handlers[key][i];
...
stylo.ui.keys.standard_shortcuts = standard_shortcuts;
stylo.ui.keys.add_shortcut = ({
	...
	stylo.ui.keys.off(shortcut, page);
	// attach new handler
	stylo.ui.keys.on(shortcut, handler);
...
stylo.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (stylo.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(stylo.utils.to_title_case)
					.join("+");
				if (stylo.utils.is_mac()) {
	...
	let dialog = new stylo.ui.Dialog({
		...
		on_hide() {
			stylo.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	stylo.ui.keys.is_dialog_shown = true;
};

stylo.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = stylo.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
stylo.ui.keys.on = function (key, handler) {
	if (!stylo.ui.keys.handlers[key]) {
		stylo.ui.keys.handlers[key] = [];
	}
	stylo.ui.keys.handlers[key].push(handler);
};

stylo.ui.keys.off = function (key, page) {
	let handlers = stylo.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	stylo.ui.keys.handlers[key] = handlers.filter((h) => {
...
stylo.ui.keys.add_shortcut({
	...
	action: function (e) {
		...
		stylo.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
stylo.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

stylo.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

stylo.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

stylo.ui.keys.on("ctrl+down", function (e) {
	const grid_row = stylo.ui.form.get_open_grid_form();
...
stylo.ui.keys.on("ctrl+up", function (e) {
	const grid_row = stylo.ui.form.get_open_grid_form();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	stylo.ui.keys.key_map[65 + i] = letter;
});
...
stylo.ui.keys.add_shortcut({
	shortcut: "shift+t",
	action: function (e) {
		if (!stylo.model.can_write("System Console")) {
			return;
		}
