stylo.provide("stylo.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
stylo.ui.keys.shortcut_groups = shortcut_groups;

stylo.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new stylo.ui.keys.AltShortcutGroup());
	}
...
stylo.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (stylo.ui.keys.key_map[e.which] || "").toLowerCase();
...
stylo.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = stylo.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
