stylo.ui.Sidebar = class Sidebar {
	constructor() {
		this.items = {};
		this.parent_items = [];
	...
	make_dom() {
		...
		this.wrapper = $(stylo.render_template("sidebar")).prependTo("body");
		...
		this.apps_switcher = new stylo.ui.AppsSwitcher(this);
	...
	set_all_pages() {
		this.sidebar_pages = stylo.boot.sidebar_pages;
	...
	set_default_app() {
		...
		stylo.boot.app_data.sort((a, b) => (a.workspaces.length < b.workspaces.length ? 1 : -1));
		stylo.current_app = stylo.boot.app_data[0].app_name;
		stylo.stylo_toolbar.set_app_logo(stylo.boot.app_data[0].app_logo_url);
	}

	set_active_workspace_item() {
		if (!stylo.get_route()) return;
		let current_route = stylo.get_route();
		let current_route_str = stylo.get_route_str();
		let current_item;
		if (current_route[0] == "Workspaces") {
			current_item = current_route[1];
		} else if (stylo.breadcrumbs) {
			if (Object.keys(stylo.breadcrumbs.all).length == 0) return;
			if (stylo.breadcrumbs.all[current_route_str]) {
				current_item =
					stylo.breadcrumbs.all[current_route_str].workspace ||
					stylo.breadcrumbs.all[current_route_str].module;
			}
		}
		...
		if (this.all_pages) {
			...
			for (let page of this.all_pages) {
				...
				if (!page.app && page.module) {
					page.app = stylo.boot.module_app[stylo.slug(page.module)];
				}
				stylo.workspace_map[page.name] = page;
				stylo.workspace_list.push(page);
			}
			this.make_sidebar();
		}
		this.set_hover();
	...
	make_sidebar() {
		...
		let app_workspaces = stylo.boot.app_data_map[stylo.current_app || "stylo"].workspaces;

		let parent_pages = this.all_pages.filter((p) => !p.parent_page).uniqBy((p) => p.name);
		if (stylo.current_app === "private") {
		...
		this.wrapper.find(".selected").length &&
			!stylo.dom.is_element_in_viewport(this.wrapper.find(".selected")) &&
	...
	sidebar_item_container(item) {
		...
		if (item.type === "Link") {
			if (item.link_type === "Report") {
				path = stylo.utils.generate_route({
					type: item.link_type,
					name: item.link_to,
				...
				path = stylo.utils.generate_route({ type: item.link_type, name: item.link_to });
			}
		} else if (item.type === "URL") {
			path = item.external_link;
		} else {
			if (item.public) {
				path = "/app/" + stylo.router.slug(item.name);
			} else {
				path = "/app/private/" + stylo.router.slug(item.name.split("-")[0]);
			}
		}
						...
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>
							${
								item.public || item.icon
									? stylo.utils.icon(item.icon || "folder-normal", "md")
	...
	add_toggle_children(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(`<button class="btn-reset drop-icon hidden">`)
			.html(stylo.utils.icon(drop_icon, "sm"))
		...
		for (let container of this.$sidebar.find(".nested-container")) {
			this.sorting_items[this.sorting_items.length] = Sortable.create(container, {
				...
				onEnd: () => {
					...
					stylo.xcall(
						"stylo.desk.doctype.workspace_settings.workspace_settings.set_sequence",
						{
	...
	close_sidebar() {
		...
		if (stylo.is_mobile()) stylo.app.sidebar.prevent_scroll();
	}
	open_sidebar() {
		this.sidebar_expanded = true;
		this.expand_sidebar();
	...
	reload() {
		return stylo.workspace.get_pages().then((r) => {
