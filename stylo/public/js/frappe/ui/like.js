...
stylo.ui.is_liked = function (doc) {
	return stylo.ui.get_liked_by(doc).includes(stylo.session.user);
};

stylo.ui.get_liked_by = function (doc) {
	return doc._liked_by ? JSON.parse(doc._liked_by) : [];
};

stylo.ui.toggle_like = function ($btn, doctype, name, callback) {
	const add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css("pointer-events", "none");

	stylo.call({
		method: "stylo.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
		...
		callback: function (r) {
			...
			if (doc) {
				let liked_by = stylo.ui.get_liked_by(doc);

				if (add === "Yes" && !liked_by.includes(stylo.session.user)) {
					liked_by.push(stylo.session.user);
				}

				if (add === "No" && liked_by.includes(stylo.session.user)) {
					liked_by = liked_by.filter((user) => user !== stylo.session.user);
				}
...
stylo.ui.click_toggle_like = function () {
	console.warn("`stylo.ui.click_toggle_like` is deprecated and has no effect.");
};

stylo.ui.setup_like_popover = ($parent, selector) => {
	if (stylo.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach((user) => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${stylo.avatar(user, "avatar-xs")}
							<span>${stylo.user.full_name(user)}</span>
						</li>
					`);
				});
				...
				liked_by_list.children("li").click((ev) => {
					...
					stylo.set_route(link_base + user);
				});
