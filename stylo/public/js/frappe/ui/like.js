...
stylo.ui.is_liked = function (doc) {
	var liked = stylo.ui.get_liked_by(doc);
	return liked.indexOf(stylo.session.user) === -1 ? false : true;
};
			...
			if (!r.exc) {
				...
				if (doc) {
					var liked_by = JSON.parse(doc._liked_by || "[]"),
						idx = liked_by.indexOf(stylo.session.user);
					if (add === "Yes") {
						if (idx === -1) liked_by.push(stylo.session.user);
					} else {
						if (idx !== -1) {
...
stylo.ui.click_toggle_like = function () {
	...
	stylo.ui.toggle_like($btn, doctype, name, function () {
		if (not_liked) {
...
stylo.ui.setup_like_popover = ($parent, selector, check_not_liked = true) => {
