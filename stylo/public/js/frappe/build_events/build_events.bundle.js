...
stylo.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						stylo.assets.executed_ = stylo.assets.executed_.filter(
		...
		// update assets json
		stylo.call("stylo.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				stylo.boot.assets_json = r.message;

				if (stylo.hot_update) {
					stylo.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
