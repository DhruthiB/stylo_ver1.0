...
stylo.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + stylo.dom.id_count;
		stylo.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + stylo.dom.id_count;
		$ele.attr("id", id);
		stylo.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = stylo.dom.by_id(parent);
	...
	freeze: function (msg, css_class) {
		...
		stylo.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!stylo.dom.freeze_count) return; // anything open?
		stylo.dom.freeze_count--;
		if (!stylo.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				stylo.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
stylo.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
stylo.run_serially = function (tasks) {
...
stylo.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
stylo.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

stylo.scrub = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

stylo.unscrub = function (txt) {
	return stylo.model.unscrub(txt);
};

stylo.get_data_pill = (label, target_id = null, remove_action = null, image = null) => {
...
stylo.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${stylo.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${stylo.utils.icon("close-alt", "sm", "close-alt")}
						</button>
					</div>
				</div>
...
stylo.is_online = function () {
	if (stylo.boot.developer_mode == 1) {
...
stylo.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + stylo.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = stylo.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = stylo.assets.bundled_asset("desk.bundle.css");
