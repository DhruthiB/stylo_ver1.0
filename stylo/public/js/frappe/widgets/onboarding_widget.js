	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					...
					<div class="step-index step-skipped">${stylo.utils.icon("tick", "xs")}</div>
					<div class="step-index step-complete">${stylo.utils.icon("tick", "xs")}</div>
	...
	show_step(step) {
		...
		const set_description = () => {
			let content = step.description
				? stylo.markdown(step.description)
				: `<h1>${__(step.title)}</h1>`;
