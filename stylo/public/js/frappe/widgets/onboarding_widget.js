...
export default class OnboardingWidget extends Widget {
	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					<div class="step-index step-pending">${stylo.utils.icon(
						"es-line-success",
						"md",
						"",
						"",
					...
					<div class="step-index step-skipped">${stylo.utils.icon(
					...
					<div class="step-index step-complete">${stylo.utils.icon(
						"es-solid-success",
						"md",
						"",
						"",
	...
	show_step(step) {
		...
		let actions = {
			...
			"Create Entry": (step) => {
				if (step.is_complete) {
					stylo.set_route(`/app/List/${step.reference_document}`);
				} else {
		...
		const set_description = () => {
			let content = __(step.description)
				? stylo.markdown(__(step.description))
				: `<h1>${__(step.title)}</h1>`;

			if (step.action === "Create Entry") {
				// add a secondary action to view list
				content += `<p>
					<a href='/app/${stylo.router.slug(step.reference_document)}'>
		...
		const toggle_video = () => {
			...
			let plyr = new stylo.Plyr(video[0], {
				hideControls: true,
				resetOnEnd: true,
			});
	...
	go_to_page(step) {
		...
		stylo.set_route(step.path).then(() => {
			let message =
	...
	open_report(step) {
		let route = stylo.utils.generate_route({
		...
		let current_route = stylo.get_route();

		stylo.set_route(route).then(() => {
			let msg_dialog = stylo.msgprint({
				message: __(step.report_description),
				title: __(step.reference_report),
				primary_action: {
					action: () => {
						stylo.set_route(current_route).then(() => {
							this.mark_complete(step);
						});
						msg_dialog.hide();
					},
			...
			stylo.msg_dialog.custom_onhide = () => this.mark_complete(step);
		});
	}
	...
	show_form_tour(step) {
		let route;
		if (step.is_single) {
			route = stylo.router.slug(step.reference_document);
		} else {
			route = `${stylo.router.slug(step.reference_document)}/new`;
		}

		let current_route = stylo.get_route();

		stylo.route_hooks = {};
		stylo.route_hooks.after_load = (frm) => {
			const on_finish = () => {
				let msg_dialog = stylo.msgprint({
	...
	update_settings(step) {
		...
		stylo.route_hooks.after_save = (frm) => {
			let success = false;
			let args = {};
			...
			if (success) {
				...
				args.secondary_action = {
					action: () => stylo.set_route(current_route),
					label: __("Go Back"),
				};
			...
			stylo.msgprint(args);
			stylo.msg_dialog.custom_onhide = () => custom_onhide();
		};

		stylo.set_route("Form", step.reference_document);
	}
	...
	async create_entry(step) {
		...
		if (step.is_submittable) {
			stylo.route_hooks.after_save = () => {
				stylo.msgprint({
					message: __("Submit this document to complete this step."),
					title: __("Document Saved"),
				});
			};
			stylo.route_hooks.after_submit = callback;
		} else {
			stylo.route_hooks.after_save = callback;
		}

		stylo.set_route("Form", step.reference_document, docname);
	}

	show_quick_entry(step) {
		let current_route = stylo.get_route_str();
		stylo.ui.form.make_quick_entry(
			step.reference_document,
			() => {
				if (stylo.get_route_str() != current_route) {
					let success_dialog = stylo.msgprint({
						message: __("Let's take you back to onboarding"),
						title: __("Document Saved"),
						primary_action: {
							action: () => {
	...
	show_video(step) {
		stylo.help.show_video(step.video_url, step.title);
		this.mark_complete(step);
	}
	...
	update_step_status(step, status, value, callback) {
		...
		stylo
			.call("stylo.desk.desktop.update_onboarding_step", {
				name: step.name,
				field: status,
				value: value,
			})
	...
	show_success() {
		...
		let success_state_image =
			this.success_state_image || "/assets/stylo/images/ui-states/success-color.png";
		let documentation = "";
		if (this.docs_url) {
			documentation = __(
	...
	is_dismissed() {
		...
		if (Object.keys(dismissed).includes(this.title)) {
			...
			let today = new Date();
			let diff = stylo.datetime.get_hour_diff(today, last_hidden);
			return diff < 24;
		}
		return false;
	}
	...
	set_actions() {
		...
		dismiss.on("click", () => {
			let dismissed = JSON.parse(localStorage.getItem("dismissed-onboarding") || "{}");
			dismissed[this.title] = stylo.datetime.now_datetime();

			localStorage.setItem("dismissed-onboarding", JSON.stringify(dismissed));
			this.delete(true, true);
			this.widget.closest(".ce-block").hide();
			stylo.telemetry.capture("dismissed_" + stylo.scrub(this.title), "stylo_onboarding");
		});
		...
		return stylo.model
			...
			.then((onboarding_doc) => {
				if (onboarding_doc) {
					...
					const method =
						"stylo.desk.doctype.onboarding_step.onboarding_step.get_onboarding_steps";
					return stylo
	...
	async get_first_document(doctype) {
		...
		if (message.first_document) {
			await stylo.db.get_list(doctype, { order_by: "creation" }).then((res) => {
