		...
		this.dialog = new stylo.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${stylo.utils.get_random(20)}`;
				}
	...
	get_title() {
		if (this.editing) {
			switch (this.type) {
				...
				default:
					return __("Edit {0}", [__(stylo.model.unscrub(this.type))]);
			}
		}
		...
		switch (this.type) {
			...
			default:
				return __("New {0}", [__(stylo.model.unscrub(this.type))]);
		}
	}
...
class QuickListDialog extends WidgetDialog {
	...
	generate_filter_from_json() {
		if (this.values && this.values.quick_list_filter) {
			this.filters = stylo.utils.get_filter_from_json(
...
class ShortcutDialog extends WidgetDialog {
			...
			},
			{
				...
				onchange: () => {
					...
					if (doctype && this.dialog.get_value("type") == "DocType") {
						stylo.model.with_doctype(doctype, async () => {
							let meta = stylo.get_meta(doctype);

							if (doctype && stylo.boot.single_types.includes(doctype)) {
								this.hide_filters();
							} else if (doctype) {
							...
							if (stylo.boot.calendars.includes(doctype)) views.push("Calendar");

							const response = await stylo.db.get_value(
								"Kanban Board",
			...
			},
			{
				...
				depends_on: (state) => {
					if (this.dialog) {
						...
						let is_single = stylo.boot.single_types.includes(doctype);
	...
	process_data(data) {
		...
		data.label = data.label ? data.label : stylo.model.unscrub(data.link_to);

		if (data.url) {
			let _url = data.url;
			if (data.url.startsWith("/")) {
				_url = stylo.urllib.get_base_url() + data.url;
			}
			!validate_url(_url) &&
				stylo.throw({
...
class NumberCardDialog extends WidgetDialog {
			...
			return [
				{
					...
					get_query: () => {
						return {
							query: "stylo.desk.doctype.number_card.number_card.get_cards_for_user",
							filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type && stylo.get_meta(this.document_type)) {
			stylo.get_meta(this.document_type).fields.map((df) => {
		...
		return [
			{
				...
				get_query: () => {
					return {
						query: "stylo.desk.doctype.custom_html_block.custom_html_block.get_custom_blocks_for_user",
					};
				},
			},
		];
