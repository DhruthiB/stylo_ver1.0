...
export default class ShortcutWidget extends Widget {
	...
	setup_events() {
		this.widget.click((e) => {
			...
			let filters = stylo.utils.get_filter_from_json(this.stats_filter);
	...
	set_actions() {
		if (this.in_customize_mode) return;
		let icon_to_append = stylo.utils.icon("es-line-arrow-up-right", "xs", "", "", "ml-2");
		if (stylo.utils.is_rtl(stylo.boot.lang)) {
			icon_to_append = stylo.utils.icon("es-line-arrow-up-left", "xs", "", "", "ml-2");
		}
		...
		let filters = stylo.utils.process_filter_expression(this.stats_filter);

		if (
			this.type == "DocType" &&
			this.doc_view != "New" &&
			!stylo.boot.single_types.includes(this.link_to)
		) {
			stylo.db
				.count(this.link_to, {
					filters: filters || [],
	...
	set_count(count) {
		...
		$(stylo.utils.icon("es-line-arrow-up-right", "xs", "", "", "ml-2")).appendTo(
			this.action_area
		);
	}
}
