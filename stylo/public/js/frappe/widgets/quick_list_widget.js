...
export default class QuickListWidget extends Widget {
	...
	set_actions() {
		...
		if (stylo.model.can_create(this.document_type)) {
	...
	setup_refresh_list_button() {
		this.refresh_list = $(
			`<div class="refresh-list btn btn-xs pull-right" title="${__("Refresh List")}">
				${stylo.utils.icon("es-line-reload", "sm")}
			</div>`
		);
	...
	setup_filter(doctype) {
		...
		this.filters = stylo.utils.process_filter_expression(this.quick_list_filter);
	...
	setup_quick_list_item(doc) {
		const indicator = stylo.get_indicator(doc, this.document_type);
			...
			<div class="quick-list-item">
				<div class="ellipsis left">
					...
					<div class="timestamp text-muted">
						${stylo.datetime.prettyDate(doc.modified)}
					</div>
				</div>
			</div>
		`);
		...
		let icon_to_append = `<div class="right-arrow">${stylo.utils.icon("right", "xs")}</div>`;
		if (stylo.utils.is_rtl(stylo.boot.lang)) {
			icon_to_append = `<div class="left-arrow">${stylo.utils.icon("left", "xs")}</div>`;
		}
		...
		$quick_list_item.click((e) => {
			...
			stylo.set_route(`${stylo.utils.get_form_link(this.document_type, doc.name)}`);
		});
	...
	set_body() {
		...
		stylo.model.with_doctype(this.document_type, () => {
			let fields = ["name"];

			// get name of title field
			if (!this.title_field_name) {
				let meta = stylo.get_meta(this.document_type);
			...
			this.has_status_field = stylo.meta.has_field(this.document_type, "status");
			...
			let workflow_fieldname = stylo.workflow.get_state_fieldname(this.document_type);
			workflow_fieldname && fields.push(workflow_fieldname);
			fields.push("modified");

			let add_fields = stylo.listview_settings?.[this.document_type]?.add_fields;
			if (Array.isArray(add_fields)) {
				for (const fieldname of add_fields) {
					// Only keep fields that exist and are permitted
					if (stylo.meta.has_field(this.document_type, fieldname)) {
			...
			let quick_list_filter = stylo.utils.process_filter_expression(this.quick_list_filter);

			let args = {
				method: "stylo.desk.reportview.get",
				args: {
	...
	set_footer() {
		...
		let filters = stylo.utils.get_filter_from_json(this.quick_list_filter);
		let route = stylo.utils.generate_route({ type: "doctype", name: this.document_type });
		this.see_all_button = $(`
