...
class FormTimeline extends BaseTimeline {
	...
	add_web_page_view_count() {
		if (this.frm.doc.route && cint(stylo.boot.website_tracking_enabled)) {
			stylo.utils.get_page_view_count(this.frm.doc.route).then((res) => {
	...
	get_milestone_timeline_contents() {
		...
		(this.doc_info.milestones || []).forEach((milestone_log) => {
			const field = stylo.meta.get_label(this.frm.doctype, milestone_log.track_field);
		...
		(this.doc_info.additional_timeline_content || []).forEach((custom_item) => {
			if (custom_item.timeline_badge) {
				custom_timeline_contents.push({
					...
					content: stylo.utils.eval(custom_item.method, {
	...
	add_dropdown_item(timeline_box, menu_items) {
		...
		menu_items.forEach((m) => {
			...
			m.empty();
			m.append(
				__("{0}", [stylo.utils.to_title_case(formatted_action_name.replace("-", " "))])
			);
			m.removeClass();
	...
	setup_comment_actions(comment_wrapper, doc) {
		...
		if (stylo.session.user == doc.owner || stylo.user.has_role("System Manager")) {
			if (stylo.model.can_delete("Comment")) {
				const delete_option = $(`
	...
	update_comment_publicity(comment_name, publish) {
		...
		stylo.confirm(message, () => {
			return stylo
				.xcall("stylo.desk.form.utils.update_comment_publicity", {
					name: comment_name,
					publish,
				})
				.then(() => {
