...
class FormTimeline extends BaseTimeline {
	...
	setup_timeline_actions() {
		if (stylo.model.can_email(null, this.frm)) {
		...
		if (this.frm.meta.allow_events_in_timeline) {
			let create_event = () => {
				const args = {
					...
					txt: stylo.markdown(this.frm.comment_box.get_value()),
				};
				return new stylo.views.InteractionComposer(args);
			};
	...
	setup_document_email_link() {
		...
		if (doc_info.document_email) {
			...
			this.document_email_link_wrapper.find(".document-email-link").on("click", (e) => {
				...
				stylo.utils.copy_to_clipboard(text);
			});
		}
	}
	...
	render_timeline_items() {
		...
		stylo.utils.bind_actions_with_object(this.timeline_items_wrapper, this);
	}

	set_document_info() {
		// TODO: handle creation via automation
		const creation = comment_when(this.frm.doc.creation);
		let creation_message = stylo.utils.is_current_user(this.frm.doc.owner)
		...
		let modified_message = stylo.utils.is_current_user(this.frm.doc.modified_by)
		...
		if (this.frm.doc.route && cint(stylo.boot.website_tracking_enabled)) {
			let route = this.frm.doc.route;
			stylo.utils.get_page_view_count(route).then((res) => {
	...
	get_user_link(user) {
		const user_display_text = (stylo.user_info(user).fullname || "").bold();
		return stylo.utils.get_form_link("User", user, true, user_display_text);
	}
	...
	get_view_timeline_contents() {
		...
		(this.doc_info.views || []).forEach((view) => {
			...
			let view_message = stylo.utils.is_current_user(view.owner)
	...
	async get_more_communication_timeline_contents() {
		...
		let response = await stylo.call({
			method: "stylo.desk.form.load.get_communications",
			args: {
	...
	get_communication_timeline_content(doc, allow_reply = true) {
		doc._url = stylo.utils.get_form_link("Communication", doc.name);
		...
		doc.content = stylo.dom.remove_script_and_style(doc.content);
		let communication_content = $(stylo.render_template("timeline_message_box", { doc }));
		if (allow_reply) {
	...
	get_comment_timeline_content(doc) {
		doc.content = stylo.dom.remove_script_and_style(doc.content);
		const comment_content = $(stylo.render_template("timeline_message_box", { doc }));
		...
		(this.doc_info.milestones || []).forEach((milestone_log) => {
			milestone_timeline_contents.push({
				...
				content: __("{0} changed {1} to {2}", [
					...
					stylo.meta.get_label(this.frm.doctype, milestone_log.track_field),
	...
	get_custom_timeline_contents() {
		...
		(this.doc_info.additional_timeline_content || []).forEach((custom_item) => {
			custom_timeline_contents.push({
				...
				content:
					custom_item.content ||
					stylo.render_template(custom_item.template, custom_item.template_data),
			});
		});
	...
	get_energy_point_timeline_contents() {
		...
		(this.doc_info.energy_point_logs || []).forEach((log) => {
			...
			energy_point_timeline_contents.push({
				...
				content: stylo.energy_points.format_form_log(log),
			});
		});
	...
	setup_reply(communication_box, communication_doc) {
		...
		let reply = $(`<a class="action-btn reply">${stylo.utils.icon("reply", "md")}</a>`).click(
			() => {
				this.compose_mail(communication_doc);
			}
		);
		let reply_all = $(
			`<a class="action-btn reply-all">${stylo.utils.icon("reply-all", "md")}</a>`
		).click(() => {
	...
	compose_mail(communication_doc = null, reply_all = false) {
		const args = {
			...
			frm: this.frm,
			recipients:
				communication_doc && communication_doc.sender != stylo.session.user_email
		...
		const email_accounts = stylo.boot.email_accounts
			.filter((account) => {
				return (
		...
		if (communication_doc && args.is_a_reply) {
			args.cc = "";
			if (
				email_accounts.includes(stylo.session.user_email) &&
				communication_doc.sender != stylo.session.user_email
			) {
				...
				args.cc =
					recipients.filter((r) => r != stylo.session.user_email).join(", ") + ", ";
			}
			if (reply_all) {
		...
		if (this.frm.doctype === "Communication") {
			...
			const comment_value = stylo.markdown(this.frm.comment_box.get_value());
			args.message = strip_html(comment_value) ? comment_value : "";
		}

		new stylo.views.CommunicationComposer(args);
	}
	...
	setup_comment_actions(comment_wrapper, doc) {
		...
		if (
			stylo.model.can_delete("Comment") &&
			(stylo.session.user == doc.owner || stylo.user.has_role("System Manager"))
		) {
			const delete_option = $(`
				<li>
		...
		let edit_button = $();
		let current_user = stylo.session.user;
	...
	make_editable(container) {
		return stylo.ui.form.make_control({
			parent: container,
			df: {
				fieldtype: "Comment",
				fieldname: "comment",
	...
	update_comment(name, content) {
		return stylo
			.xcall("stylo.desk.form.utils.update_comment", { name, content })
			.then(() => {
	...
	delete_comment(comment_name) {
		stylo.confirm(__("Delete comment?"), () => {
			return stylo
				.xcall("stylo.client.delete", {
					doctype: "Comment",
					name: comment_name,
				})
				.then(() => {
	...
	copy_link(ev) {
		let doc_link = stylo.urllib.get_full_url(
			stylo.utils.get_form_link(this.frm.doctype, this.frm.docname)
		);
		let element_id = $(ev.currentTarget).closest(".timeline-content").attr("id");
		stylo.utils.copy_to_clipboard(`${doc_link}#${element_id}`);
	}
}
