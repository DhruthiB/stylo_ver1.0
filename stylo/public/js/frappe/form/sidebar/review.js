...
stylo.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = stylo.boot.points;
	...
	update_points() {
		return stylo
			.xcall("stylo.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: stylo.session.user,
			})
			.then((data) => {
				stylo.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new stylo.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return stylo.msgprint(__("You do not have enough points"));
				}
				stylo
					.xcall("stylo.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = stylo.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${stylo.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${stylo.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${stylo.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${stylo.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
