...
stylo.ui.form.Attachments = class Attachments {
	constructor(opts) {
		$.extend(this, opts);
	...
	add_attachment(attachment) {
		...
		let file_label = `
			<a href="${file_url}" target="_blank" title="${stylo.utils.escape_html(file_name)}"
				class="ellipsis attachment-file-label"
			>
				<span>${stylo.utils.xss_sanitise(file_name)}</span>
			</a>`;

		let remove_action = null;
		if (this.can_delete_attachment()) {
			remove_action = function (target_id) {
				stylo.confirm(__("Are you sure you want to delete the attachment?"), function () {
		...
		const icon = `<a href="/app/file/${fileid}" class="attachment-icon">
				${stylo.utils.icon(attachment.is_private ? "es-line-lock" : "es-line-unlock", "sm ml-0")}
			</a>`;

		$(`<div class="attachment-row"></div>`)
			.append(stylo.get_data_pill(file_label, fileid, remove_action, icon))
	...
	remove_attachment(fileid, callback) {
		...
		var me = this;
		return stylo.call({
			method: "stylo.desk.form.utils.remove_attach",
			type: "DELETE",
			args: {
				fid: fileid,
				dt: me.frm.doctype,
			...
			callback: function (r, rt) {
				if (r.exc) {
					if (!r._server_messages) stylo.msgprint(__("There were errors"));
					return;
				}
