...
stylo.ui.form.Sidebar = class {
	...
	make() {
		var sidebar_content = stylo.render_template("form_sidebar", {
			...
			frm: this.frm,
			can_write: stylo.model.can_write(this.frm.doctype, this.frm.docname),
		});
	...
	refresh_creation_modified() {
		...
		let avatar_group = stylo.avatar_group(user_list, 5, {
			align: "left",
			overlap: true,
		});
		...
		let creation_message =
			get_user_message(
				...
				" · " +
				cint(stylo.boot.user.show_absolute_datetime_in_timeline) ||
			cint(stylo.boot.sysdefaults.show_absolute_datetime_in_timeline)
				? stylo.datetime.str_to_user(this.frm.doc.creation)
		...
		let modified_message =
			get_user_message(
				...
				cint(stylo.boot.user.show_absolute_datetime_in_timeline) ||
			cint(stylo.boot.sysdefaults.show_absolute_datetime_in_timeline)
				? stylo.datetime.str_to_user(this.frm.doc.modified)
