...
stylo.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make() {
		var sidebar_content = stylo.render_template("form_sidebar", {
		...
		stylo.ui.form.setup_user_image_event(this.frm);
	...
	bind_events() {
		...
		this.comments.on("click", function () {
			stylo.utils.scroll_to(me.frm.footer.wrapper.find(".comment-box"), true);
		});

		this.like_icon.on("click", function () {
			stylo.ui.toggle_like(me.like_wrapper, me.frm.doctype, me.frm.doc.name, function () {
				me.refresh_like();
			});
		});
	}
	...
	setup_keyboard_shortcuts() {
		...
		stylo.ui.keys.get_shortcut_group(this.page).add(assignment_link);
	}
	...
	refresh() {
		if (this.frm.doc.__islocal) {
			...
			this.sidebar
				...
				.html(
					__(
						"{0} edited this {1}",
						[
							stylo.user.full_name(this.frm.doc.modified_by).bold(),
			...
			this.sidebar
				...
				.html(
					__(
						"{0} created this {1}",
						[
							stylo.user.full_name(this.frm.doc.owner).bold(),
			...
			stylo.ui.form.set_user_image(this.frm);
		}
	}
			...
			stylo.call({
				...
				callback: function (res) {
					...
					me.sidebar.find(".auto-repeat-status").on("click", function () {
						stylo.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				},
			});
		}
	...
	make_tags() {
		...
		this.frm.tags = new stylo.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new stylo.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new stylo.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new stylo.ui.form.Share({
			frm: this.frm,
	...
	make_like() {
		...
		stylo.ui.setup_like_popover(this.sidebar.find(".form-stats-likes"), ".like-icon");
	}
	...
	refresh_like() {
		...
		const liked = stylo.ui.is_liked(this.frm.doc);
		this.like_wrapper
	...
	make_review() {
		...
		if (stylo.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new stylo.ui.form.Review({
	...
	reload_docinfo(callback) {
		stylo.call({
			method: "stylo.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,
