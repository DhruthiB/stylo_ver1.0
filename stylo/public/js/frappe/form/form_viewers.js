stylo.ui.form.FormViewers = class FormViewers {
	...
	refresh() {
		...
		let avatar_group = stylo.avatar_group(this.active_users, 5, {
			align: "left",
			overlap: true,
		});
	...
	setup_events() {
		let me = this;
		stylo.realtime.off("doc_viewers");
	...
	async update_users({ doctype, docname, users = [] }) {
		users = users.filter((u) => u != stylo.session.user);
	...
	async fetch_user_info(users) {
		...
		const data = await stylo.xcall("stylo.desk.form.load.get_user_info_for_viewers", {
			users: unknown_users,
		});
