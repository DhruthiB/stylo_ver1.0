stylo.ui.form.FormViewers = class FormViewers {
	...
	refresh() {
		...
		let currently_viewing = users.current.filter((user) => user != stylo.session.user);
		let avatar_group = stylo.avatar_group(currently_viewing, 5, {
			align: "left",
			overlap: true,
		});
...
stylo.ui.form.FormViewers.set_users = function (data, type) {
	const doctype = data.doctype;
	const docname = data.docname;
	const docinfo = stylo.model.get_docinfo(doctype, docname);
	...
	const set_and_refresh = () => {
		...
		stylo.model.set_docinfo(doctype, docname, type, info);
	...
	for (let user of users) {
		if (!stylo.boot.user_info[user]) unknown_users.push(user);
	}
	...
	if (unknown_users.length === 0) {
		...
		stylo
			.xcall("stylo.desk.form.load.get_user_info_for_viewers", { users: unknown_users })
			.then((data) => {
				Object.assign(stylo.boot.user_info, data);
				set_and_refresh();
			});
	}
};
