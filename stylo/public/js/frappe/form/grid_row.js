...
export default class GridRow {
	...
	set_docfields(update = false) {
		if (this.doc && this.parent_df.options) {
			stylo.meta.make_docfield_copy_for(
				this.parent_df.options,
				this.doc.name,
				this.docfields
			);
			const docfields = stylo.meta.get_docfields(this.parent_df.options, this.doc.name);
			if (update) {
			...
			if (this.frm) {
				...
				stylo
					.run_serially([
						...
						() => {
							stylo.model.clear_doc(this.doc.doctype, this.doc.name);
		...
		stylo.prompt(
			...
			function (values) {
				if (me.doc._sortable === false) {
					stylo.msgprint(__("Cannot move row"));
					return;
				}
	...
	render_template() {
		...
		this.row_display = $('<div class="row-data sortable-handle template-row"></div>')
			.appendTo(this.row)
			.html(
				stylo.render(this.grid.template, {
					doc: this.doc ? stylo.get_format_helper(this.doc) : null,
					frm: this.frm,
					row: this,
				})
			);
	...
	render_row(refresh) {
		...
		if (!this.row_index && !this.show_search) {
			...
			stylo.utils.only_allow_num_decimal(this.row_index.find("input"));
		}
		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col d-flex justify-content-center" style="cursor: pointer;">
					<a>${stylo.utils.icon("setting-gear", "sm", "", "filter: opacity(0.5)")}</a>
				</div>
			`)
				.appendTo(this.row)
				.on("click", () => {
	...
	configure_dialog_for_columns_selector() {
		this.grid_settings_dialog = new stylo.ui.Dialog({
		...
		d.set_primary_action(__("Add"), () => {
			...
			if (selected_fields) {
				selected_fields.forEach((selected_column) => {
					let docfield = stylo.meta.get_docfield(this.grid.doctype, selected_column);
	...
	prepare_columns_for_dialog(selected_fields) {
		...
		const blocked_fields = stylo.model.no_value_type;
	...
	render_selected_columns() {
		...
		if (this.selected_columns_for_grid) {
			this.selected_columns_for_grid.forEach((d) => {
				let docfield = stylo.meta.get_docfield(this.grid.doctype, d.fieldname);
				...
				fields += `
					<div class='control-input flex align-center form-control fields_order sortable-handle sortable'
						...
						<div class='row'>
							<div class='col-1'>
								<a style='cursor: grabbing;'>${stylo.utils.icon("drag", "xs")}</a>
							</div>
							<div class='col-6'>
		...
		$(this.fields_html_wrapper)
			...
			.change((event) => {
				if (cint(event.target.value) === 0) {
					...
					stylo.throw(__("Column width cannot be zero."));
				}
	...
	validate_columns_width() {
		...
		if (total_column_width && total_column_width > 10) {
			stylo.throw(__("The total column width cannot be more than 10."));
		}
	}
		...
		$(this.fields_html_wrapper)
			...
			.click((event) => {
				...
				if (selected_columns_for_grid && selected_columns_for_grid.length === 0) {
					stylo.throw(__("At least one column is required to show in the grid."));
				}
	...
	update_user_settings_for_grid() {
		...
		stylo.model.user_settings.save(this.frm.doctype, "GridView", value).then((r) => {
			stylo.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}

	reset_user_settings_for_grid() {
		stylo.model.user_settings.save(this.frm.doctype, "GridView", null).then((r) => {
			stylo.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}
	...
	setup_columns() {
		...
		this.grid.visible_columns.forEach((col, ci) => {
			...
			let txt = this.doc
				? stylo.format(this.doc[df.fieldname], df, null, this.doc)
				: __(df.label);
	...
	evaluate_depends_on_value(expression) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = stylo.utils.eval(expression.substr(5), { doc, parent });
	...
	make_search_column(df, colsize) {
		...
		["Currency", "Float", "Int", "Percent", "Rating"].includes(df.fieldtype) &&
			stylo.utils.only_allow_num_decimal($search_input);
		...
		var $col = $(
			...
			.on("touchmove", function (event) {
				...
				if (horizontal) {
					...
					grid.style.left = `${stylo.utils.is_rtl() ? "" : "-"}${grid_start}px`;
				}
			})
			...
			.on("click", function (event) {
				if (stylo.ui.form.editable_row !== me) {
	...
	toggle_editable_row(show) {
		...
		if (
			...
			if (stylo.ui.form.editable_row && stylo.ui.form.editable_row !== this) {
			...
			stylo.ui.form.editable_row = this;
			return false;
		} else {
			...
			stylo.ui.form.editable_row = null;
		}
	}
	...
	make_control(column) {
		...
		var field = stylo.ui.form.make_control({
			df: df,
			parent: parent,
			only_input: true,
			with_link_btn: true,
	...
	set_arrow_keys(field) {
		...
		if (field.$input) {
			field.$input.on("keydown", function (e) {
				var { TAB, UP: UP_ARROW, DOWN: DOWN_ARROW } = stylo.ui.keyCode;
	...
	show_form() {
		...
		stylo.dom.freeze("", "dark grid-form");
		if (cur_frm) cur_frm.cur_grid = this;
		this.wrapper.addClass("grid-row-open");
		if (
			!stylo.dom.is_element_in_viewport(this.wrapper) &&
			!stylo.dom.is_element_in_modal(this.wrapper)
		) {
			// -15 offset to make form look visually centered
			stylo.utils.scroll_to(this.wrapper, true, -15);
		}
	...
	hide_form() {
		...
		if (!stylo.dom.is_element_in_modal(this.row)) {
			stylo.utils.scroll_to(this.row, true, 15);
		}
		this.refresh();
	...
	refresh_field(fieldname, txt) {
		...
		if (df && this.doc) {
			txt = stylo.format(this.doc[fieldname], df, null, this.doc);
		}

		if (!txt && this.frm) {
			txt = stylo.format(this.doc[fieldname], df, null, this.frm.doc);
		}
	...
	get_visible_columns(blacklist = []) {
		...
		var visible_columns = $.map(this.docfields, function (df) {
			var visible =
				...
				!in_list(stylo.model.layout_fields, df.fieldtype) &&
