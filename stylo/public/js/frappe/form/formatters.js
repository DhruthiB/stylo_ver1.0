...
stylo.provide("stylo.form.formatters");

stylo.form.link_formatters = {};

stylo.form.formatters = {
	...
	_apply_custom_formatter: function (value, df) {
		...
		example:
			stylo.meta.docfield_map[df.parent][df.fieldname].formatter = (value) => {
		...
		*/
		...
		if (df) {
			const std_df =
				stylo.meta.docfield_map[df.parent] &&
				stylo.meta.docfield_map[df.parent][df.fieldname];
	...
	Data: function (value, df) {
		...
		if (df && df.options == "IBAN") {
			if (!value) return;
			return stylo.utils.get_formatted_iban(value);
		}
		value = value == null ? "" : value;

		return stylo.form.formatters._apply_custom_formatter(value, df);
	},
	Autocomplete: function (value, df) {
		return __(stylo.form.formatters["Data"](value, df));
	},
	...
	Float: function (value, docfield, options, doc) {
		...
		var precision =
			docfield.precision ||
			cint(stylo.boot.sysdefaults && stylo.boot.sysdefaults.float_precision) ||
			null;
		if (docfield.options && docfield.options.trim()) {
			...
			return stylo.form.formatters.Currency(value, docfield, options, doc);
		} else {
			// show 1.000000 as 1
			...
			return stylo.form.formatters._right(format_number(value, null, precision), options);
		}
	},
	Int: function (value, docfield, options) {
		...
		if (cstr(docfield.options).trim() === "File Size") {
			return stylo.form.formatters.FileSize(value);
		}
		return stylo.form.formatters._right(value == null ? "" : cint(value), options);
	},
	...
	Currency: function (value, docfield, options, doc) {
		...
		var currency = stylo.meta.get_field_currency(docfield, doc);
		...
		if (typeof docfield.precision == "number") {
			...
			precision = cint(
				docfield.precision || stylo.boot.sysdefaults.currency_precision || 2
			);
		}
		...
		if (precision > 2) {
			...
			if (decimals.length < 3 || decimals.length < precision) {
				const fraction =
					stylo.model.get_value(":Currency", currency, "fraction_units") || 100; // if not set, minimum 2.
		...
		if (options && options.only_value) {
			...
			return stylo.form.formatters._right(value, options);
		}
	},
	...
	Link: function (value, docfield, options, doc) {
		...
		let link_title = stylo.utils.get_link_title(doctype, value);
		...
		if (stylo.form.link_formatters[doctype]) {
			// don't apply formatters in case of composite (parent field of same type)
			if (doc && doctype !== doc.doctype) {
				value = stylo.form.link_formatters[doctype](value, doc, docfield);
			}
		}
		...
		if (docfield && docfield.link_onclick) {
			...
			if (stylo.model.can_read(doctype)) {
				const a = document.createElement("a");
				a.href = `/app/${encodeURIComponent(
					stylo.router.slug(doctype)
	...
	Date: function (value) {
		if (!stylo.datetime.str_to_user) {
			return value;
		}
		if (value) {
			value = stylo.datetime.str_to_user(value, false, true);
			// handle invalid date
	...
	DateRange: function (value) {
		if (Array.isArray(value)) {
			return __("{0} to {1}", [
				stylo.datetime.str_to_user(value[0]),
				stylo.datetime.str_to_user(value[1]),
			]);
		} else {
			return value || "";
		}
	...
	Datetime: function (value) {
		if (value) {
			return moment(stylo.datetime.convert_to_user_tz(value)).format(
				stylo.boot.sysdefaults.date_format.toUpperCase() +
					" " +
					(stylo.boot.sysdefaults.time_format || "HH:mm:ss")
			);
		} else {
			return "";
		}
	...
	Text: function (value, df) {
		if (value) {
			...
			if (!match) {
				value = stylo.utils.replace_newlines(value);
			}
		}

		return stylo.form.formatters.Data(value, df);
	},
	Time: function (value) {
		if (value) {
			value = stylo.datetime.str_to_user(value, true);
		}
	...
	Duration: function (value, docfield) {
		if (value) {
			let duration_options = stylo.utils.get_duration_options(docfield);
			value = stylo.utils.get_formatted_duration(value, duration_options);
		}
	...
	SmallText: function (value) {
		return stylo.form.formatters.Text(value);
	},
	TextEditor: function (value) {
		let formatted_value = stylo.form.formatters.Text(value);
	...
	WorkflowState: function (value) {
		var workflow_state = stylo.get_doc("Workflow State", value);
		if (workflow_state) {
			return repl(
	...
	TableMultiSelect: function (rows, df, options) {
		...
		const formatted_values = rows.map((row) => {
			...
			return `<span class="text-nowrap">
				${stylo.format(value, link_field, options, row)}
			</span>`;
		});
	...
	Icon: (value) => {
		return value
			? `<div>
			<div class="selected-icon">${stylo.utils.icon(value, "md")}</div>
...
stylo.form.get_formatter = function (fieldtype) {
	if (!fieldtype) fieldtype = "Data";
	return stylo.form.formatters[fieldtype.replace(/ /g, "")] || stylo.form.formatters.Data;
};

stylo.format = function (value, df, options, doc) {
	let mask_readonly = false;
	if (df.parent) {
		const mask_fields = stylo.get_meta(df.parent)?.masked_fields;
	...
	var formatter = df.formatter || stylo.form.get_formatter(fieldtype);

	var formatted = formatter(value, df, options, doc);

	if (typeof formatted == "string") formatted = stylo.dom.remove_script_and_style(formatted);

	return formatted;
};

stylo.get_format_helper = function (doc) {
	var helper = {
		get_formatted: function (fieldname) {
			var df = stylo.meta.get_docfield(doc.doctype, fieldname);
			if (!df) {
				console.log("fieldname not found: " + fieldname);
			}
			return stylo.format(doc[fieldname], df, { inline: 1 }, doc);
		},
	};
	$.extend(helper, doc);
	return helper;
...
stylo.form.link_formatters["User"] = function (value, doc, docfield) {
