...
stylo.ui.form.Layout = class Layout {
	constructor(opts) {
		this.views = {};
		this.pages = [];
		this.tabs = [];
	...
	get_doctype_fields() {
		...
		if (this.doctype_layout) {
			...
			fields = fields.concat(
				stylo.meta.sort_docfields(stylo.meta.docfield_map[this.doctype])
			);
		}
	...
	get_fields_from_layout() {
		...
		for (let f of this.doctype_layout.fields) {
			const docfield = copy_dict(stylo.meta.docfield_map[this.doctype][f.fieldname]);
	...
	show_message(html, color, permanent = false) {
		...
		const close_message = $(`<div class="close-message">${stylo.utils.icon("close")}</div>`);
		if (!permanent) {
	...
	init_field(df, parent, render = false) {
		...
		const fieldobj = stylo.ui.form.make_control({
			df: df,
	...
	make_page(df) {
		...
		this.fold_btn = head.find(".btn-fold").on("click", function () {
			...
			if (page.hasClass("hide")) {
				...
				stylo.utils.scroll_to($(this), true, 30);
				me.folded = false;
			} else {
	...
	is_numeric_field_active() {
		const control = $(document.activeElement).closest(".stylo-control");
	...
	refresh_sections() {
		...
		this.wrapper.find(".form-section:not(.hide-control)").each(function () {
			...
			if (section.find(".stylo-control:not(.hide-control)").length) {
	...
	attach_doc_and_docfields(refresh) {
		...
		for (let i = 0, l = this.fields_list.length; i < l; i++) {
			...
			if (me.doc) {
				...
				fieldobj.df =
					stylo.meta.get_docfield(me.doc.doctype, fieldobj.df.fieldname, me.doc.name) ||
					fieldobj.df;
			}
		...
		for (let i = start_idx + 1, len = fields.length; i < len; i++) {
			...
			if (this.is_visible(field)) {
				if (field.df.fieldtype === "Table") {
					...
					!stylo.model.no_value_type.includes(field.df.fieldtype)
				) {
