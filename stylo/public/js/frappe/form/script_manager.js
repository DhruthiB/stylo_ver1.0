...
stylo.provide("stylo.ui.form.handlers");
...
stylo.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!stylo.ui.form.handlers[doctype]) {
		stylo.ui.form.handlers[doctype] = {};
	}
	if (!stylo.ui.form.handlers[doctype][fieldname]) {
		stylo.ui.form.handlers[doctype][fieldname] = [];
	}
	return stylo.ui.form.handlers[doctype][fieldname];
};

stylo.ui.form.on = stylo.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = stylo.ui.form.get_event_handler_list(doctype, fieldname);
...
stylo.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = stylo.ui.form.get_event_handler_list(doctype, fieldname);
...
stylo.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

stylo.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = stylo.get_doc(doctype, name);
	...
	has_handler(event_name) {
		...
		return (
			stylo.ui.form.handlers[this.frm.doctype] &&
			stylo.ui.form.handlers[this.frm.doctype][event_name]
		);
	}
	...
	get_handlers(event_name, doctype) {
		...
		if (stylo.ui.form.handlers[doctype] && stylo.ui.form.handlers[doctype][event_name]) {
			$.each(stylo.ui.form.handlers[doctype][event_name], function (i, fn) {
				handlers.new_style.push(fn);
			});
		}
		if (stylo.ui.form.handlers["*"] && stylo.ui.form.handlers["*"][event_name]) {
			$.each(stylo.ui.form.handlers["*"][event_name], function (i, fn) {
				handlers.new_style.push(fn);
			});
		}
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (stylo.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					stylo.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && stylo.dom.set_style(doctype.__css);

		this.trigger("setup");
	}

	log_error(caller, e) {
		stylo.show_alert({ message: __("Error in Client Script."), indicator: "error" });
