...
stylo.quick_edit = function (doctype, name) {
	if (!name) name = doctype; // single
	stylo.db.get_doc(doctype, name).then((doc) => {
		stylo.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

stylo.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (stylo.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	stylo.quick_entry = new stylo.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return stylo.quick_entry.setup();
};

stylo.ui.form.QuickEntryForm = class QuickEntryForm extends stylo.ui.Dialog {
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = stylo.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.onhide = () => (stylo.quick_entry = null);
		this.show();
			...
			if (data) {
				...
				me.script_manager.trigger("validate").then(() => {
					me.insert().then(() => {
						...
						me.dialog.hide();
						setTimeout(function () {
							stylo.show_alert({ message: messagetxt, indicator: "green" }, 3);
						}, 500);
					});
				});
			}
			...
			stylo.call({
				...
				callback: function (r) {
					if (
						...
						stylo.model.can_submit(me.doctype) &&
						!stylo.model.has_workflow(me.doctype)
					) {
						stylo.run_serially([
							() => (me.dialog.working = true),
							() => {
	...
	process_after_insert(r) {
		...
		} else if (stylo._from_link) {
			stylo.ui.form.update_calling_link(this.doc);
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			stylo.run_serially([() => stylo.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			stylo.route_options = stylo.route_options || {};
			stylo.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		this.doc.__run_link_triggers = false;
		stylo.set_route("Form", this.doctype, this.doc.name);
	}
