stylo.ui.get_print_settings = function (
	pdf,
	callback,
	letter_head,
	pick_columns,
	...
	var company = stylo.defaults.get_default("company");
		...
		},
		{
			...
			get_query: () => ({
				filters: {
					...
					report: stylo.query_report ? stylo.query_report.report_name : "",
					disabled: 0,
				},
			}),
		},
	...
	return stylo.prompt(
		...
		function (data) {
			...
			if (data.letter_head) {
				data.letter_head = stylo.boot.letter_heads[print_settings.letter_head];
			}
			callback(data);
		},
		__("Print Settings")
...
stylo.ui.form.qz_connect = function () {
	return new Promise(function (resolve, reject) {
		stylo.ui.form.qz_init().then(() => {
			if (qz.websocket.isActive()) {
				// if already active, resolve immediately
				// stylo.show_alert({message: __('QZ Tray Connection Active!'), indicator: 'green'});
				resolve();
			} else {
				// try to connect once before firing the mimetype launcher
				stylo.show_alert({
...
stylo.ui.form.qz_init = function () {
	...
	return new Promise((resolve) => {
		if (typeof qz === "object" && typeof qz.version === "string") {
			...
			let qz_required_assets = [
				"/assets/stylo/node_modules/js-sha256/build/sha256.min.js",
				"/assets/stylo/node_modules/qz-tray/qz-tray.js",
			];
			stylo.require(qz_required_assets, () => {
			...
			// note 'stylo.require' does not have callback on fail. Hence, any failure cannot be communicated to the user.
		}
	});
};

stylo.ui.form.qz_get_printer_list = function () {
	// returns the list of printers that are available to the QZ Tray
	return stylo.ui.form
		.qz_connect()
		.then(function () {
...
stylo.ui.form.qz_success = function () {
...
stylo.ui.form.qz_fail = function (e) {
	// notify qz errors
	stylo.show_alert(
		{
