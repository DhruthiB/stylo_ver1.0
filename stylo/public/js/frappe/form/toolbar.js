...
stylo.ui.form.Toolbar = class Toolbar {
			...
			return stylo
				...
				.then((new_docname) => {
					...
					if (input_name && new_docname == docname) {
						stylo.socketio.doc_subscribe(doctype, input_name);
						stylo.realtime.on("doc_update", (data) => {
	...
	make_viewers() {
		...
		this.frm.viewers = new stylo.ui.form.FormViewers({
			frm: this.frm,
	...
	make_menu_items() {
		...
		const is_submittable = stylo.model.is_submittable(this.frm.doc.doctype);
		...
		if (
			...
			if (stylo.model.can_print(null, me.frm) && !this.frm.meta.issingle) {
		...
		// email
		if (stylo.model.can_email(null, me.frm) && me.frm.doc.docstatus < 2) {
			this.page.add_menu_item(
				__("Email"),
				function () {
					me.frm.email_doc();
		...
		// duplicate
		if (in_list(stylo.boot.user.can_create, me.frm.doctype) && !me.frm.meta.allow_copy) {
			this.page.add_menu_item(
				__("Duplicate"),
				function () {
					me.frm.copy_doc();
		...
		this.page.add_menu_item(
			...
			function () {
				stylo.utils.copy_to_clipboard(JSON.stringify(me.frm.doc));
			},
			true
		);
		...
		if (
			...
			stylo.model.can_delete(me.frm.doctype)
		) {
			this.page.add_menu_item(
				__("Delete"),
				function () {
		...
		if (this.can_repeat()) {
			this.page.add_menu_item(
				__("Repeat"),
				function () {
					stylo.utils.new_auto_repeat_prompt(me.frm);
				},
				true
			);
		}
		...
		if (p[CREATE] && !this.frm.meta.issingle && !this.frm.meta.in_create) {
			this.page.add_menu_item(
				...
				function () {
					stylo.new_doc(me.frm.doctype, true);
				},
				true,
				{
					shortcut: "Ctrl+B",
	...
	make_customize_buttons() {
		...
		if (stylo.model.can_create("DocType")) {
			if (stylo.boot.developer_mode === 1 && !is_doctype_form) {
				// edit doctype
				this.page.add_menu_item(
					__("Edit DocType"),
					() => {
	...
	has_workflow() {
		if (this._has_workflow === undefined)
			this._has_workflow = stylo.get_list("Workflow", {
