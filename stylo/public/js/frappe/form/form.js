...
stylo.provide("stylo.model.docinfo");
...
stylo.ui.form.Controller = class FormController {
	constructor(opts) {
		$.extend(this, opts);
	}
};

stylo.ui.form.Form = class StyloForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.sections = [];
		this.grids = [];
		this.cscript = new stylo.ui.form.Controller({ frm: this });
		this.events = {};
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = stylo.get_meta(doctype_layout_name);
		this.undo_manager = new UndoManager({ frm: this });
		this.setup_meta(doctype);
		this.debounced_reload_doc = stylo.utils.debounce(this.reload_doc.bind(this), 1000);
	...
	setup_meta() {
		this.meta = stylo.get_meta(this.doctype);

		if (this.meta.istable) {
			this.meta.in_dialog = 1;
		}

		this.perm = stylo.perm.get_perm(this.doctype); // for create
	...
	setup() {
		this.fields = [];
		this.fields_dict = {};
		this.state_fieldname = stylo.workflow.get_state_fieldname(this.doctype);
		...
		this.toolbar = new stylo.ui.form.Toolbar({
			frm: this,
			page: this.page,
		});

		this.viewers = new stylo.ui.form.FormViewers({
			frm: this,
		...
		this.script_manager = new stylo.ui.form.ScriptManager({
			frm: this,
		});
		this.script_manager.setup();
		this.watch_model_updates();

		if (!this.meta.hide_toolbar && stylo.boot.desk_settings.timeline) {
			// this.footer_tab = new stylo.ui.form.Tab(this.layout, {
			// 	label: __("Activity"),
			// 	fieldname: 'timeline'
			// });

			this.footer = new stylo.ui.form.Footer({
				frm: this,
	...
	add_form_keyboard_shortcuts() {
		...
		stylo.ui.keys.add_shortcut({
			...
			condition: () => stylo.model.can_print(this.doctype, this) && !this.meta.issingle,
		});
	...
	setup_std_layout() {
		...
		// layout
		this.layout = new stylo.ui.form.Layout({
			parent: this.body,
		...
		this.dashboard = new stylo.ui.form.Dashboard(dashboard_parent, this);

		this.tour = new stylo.ui.form.FormTour({
			frm: this,
		});

		// workflow state
		this.states = new stylo.ui.form.States({
			frm: this,
		});
	}
	...
	watch_model_updates() {
		...
		// on table fields
		var table_fields = stylo.get_children("DocType", me.doctype, "fields", {
	...
	setup_notify_on_rename() {
		...
		stylo.realtime.on("doc_rename", (data) => {
			...
			if (data.doctype == this.doctype && data.old == this.docname) {
				...
				stylo.set_route("Form", this.doctype, data.new);
			}
		});
	}
	...
	setup_file_drop() {
		...
		this.$wrapper.on("dragenter dragover", false).on("drop", function (e) {
			...
			if (me.doc.__islocal) {
				stylo.msgprint(__("Please save before attaching."));
				throw "attach error";
			}

			new stylo.ui.FileUploader({
				doctype: me.doctype,
				docname: me.docname,
				frm: me,
		...
		this.fields.map((field) => {
			if (field.df.fieldtype === "Markdown Editor") {
				this.set_df_property(field.df.fieldname, "autocompletions", () => {
					...
					return attachments
						.filter((file) => stylo.utils.is_image_file(file.file_url))
						.map((file) => {
							return {
	...
	refresh(docname) {
		...
		if (this.docname) {
			...
			// set the doc
			this.doc = stylo.get_doc(this.doctype, this.docname);

			// check permissions
			this.fetch_permissions();
			if (!this.has_read_permission()) {
				stylo.show_not_permitted(__(this.doctype) + " " + __(cstr(this.docname)));
				return;
			}
			...
			// read only (workflow)
			this.read_only = stylo.workflow.is_read_only(this.doctype, this.docname);
			if (this.read_only) {
				this.set_read_only();
			}
	...
	setup_doctype_actions() {
		if (this.meta.actions) {
			for (let action of this.meta.actions) {
				stylo.ui.form.on(this.doctype, "refresh", () => {
					if (!this.is_new()) {
						if (!action.hidden) {
	...
	execute_action(action) {
		if (typeof action === "string") {
			...
			if (typeof action === "string") {
				stylo.throw(`Action ${action} not found`);
			}
		}
		if (action.action_type === "Server Action") {
			return stylo.xcall(action.action, { doc: this.doc }).then((doc) => {
				if (doc.doctype) {
					// document is returned by the method,
					// apply the changes locally and refresh
					stylo.model.sync(doc);
					this.refresh();
				}
			...
			return stylo.set_route(action.action);
		}
	}
	...
	initialize_new_doc() {
		...
		this.script_manager.trigger("before_load", this.doctype, this.docname).then(() => {
			...
			stylo.after_ajax(function () {
				me.trigger_link_fields();
			});

			stylo.breadcrumbs.add(me.meta.module, me.doctype);
		});
	...
	render_form(switched) {
		if (!this.meta.istable) {
			...
			if (stylo.boot.desk_settings.form_sidebar) {
				this.sidebar = new stylo.ui.form.Sidebar({
					frm: this,
					page: this.page,
				});
				this.sidebar.make();
	...
	run_after_load_hook() {
		if (stylo.route_hooks.after_load) {
			let route_callback = stylo.route_hooks.after_load;
			delete stylo.route_hooks.after_load;
	...
	refresh_header(switched) {
		...
		if (!this.meta.in_dialog || this.in_form) {
			stylo.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
		}
		...
		var after_save = function (r) {
			...
			if (!r.exc) {
				if (["Save", "Update", "Amend"].indexOf(save_action) !== -1) {
					stylo.utils.play_sound("click");
				}

				me.script_manager.trigger("after_save");

				if (stylo.route_hooks.after_save) {
					let route_callback = stylo.route_hooks.after_save;
					delete stylo.route_hooks.after_save;
			...
			stylo
				.run_serially([
					...
					() => {
						...
						stylo.ui.form.save(me, save_action, after_save, btn);
					},
				])
				.catch(fail);
		} else {
	...
	discard(btn, callback, on_error) {
		...
		return new Promise((resolve) => {
			stylo.confirm(__("Discard {0}", [this.docname]), function () {
						...
						me.save(
							...
							function (r) {
								if (r.exc) {
									...
									stylo.utils.play_sound("submit");
									callback && callback();
									me.script_manager
										.trigger("on_submit")
										.then(() => resolve(me))
										.then(() => {
											if (stylo.route_hooks.after_submit) {
												let route_callback =
													stylo.route_hooks.after_submit;
												delete stylo.route_hooks.after_submit;
												route_callback(me);
											}
										});
								}
	...
	savecancel(btn, callback, on_error) {
		...
		stylo
			.call({
				method: "stylo.desk.form.linked_with.get_submitted_linked_docs",
				args: {
	...
	_cancel_all(r, btn, callback, on_error) {
		...
		for (let doctype of doctypes) {
			if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
				let docnames = links
					...
					.map((link) => stylo.utils.get_form_link(link.doctype, link.name, true))
					.join(", ");
		...
		let can_cancel = links.every((link) => stylo.model.can_cancel(link.doctype));
		if (can_cancel) {
		...
		let d = new stylo.ui.Dialog(
			{
				...
				fields: [
					{
						fieldtype: "HTML",
						options: `<p class="stylo-confirm-message">${confirm_message}</p>`,
					},
				],
			},
			() => me.handle_save_fail(btn, on_error)
		...
		if (can_cancel) {
			d.set_primary_action(__("Cancel All"), () => {
				d.hide();
				stylo.call({
					method: "stylo.desk.form.linked_with.cancel_all_linked_docs",
					args: {
						docs: links,
						ignore_doctypes_on_cancel_all: me.ignore_doctypes_on_cancel_all || [],
					},
			...
			me.script_manager.trigger("before_cancel").then(() => {
				...
				var after_cancel = function (r) {
					if (r.exc) {
						...
						stylo.utils.play_sound("cancel");
						me.refresh();
						callback && callback();
						me.script_manager.trigger("after_cancel");
					}
				};
				stylo.ui.form.save(me, "cancel", after_cancel, btn);
			});
		};
	...
	_discard(btn, on_error, skip_confirm) {
		...
		const discard_doc = () => {
			...
			me.script_manager.trigger("before_discard").then(() => {
				...
				//stylo.ui.form.discard(me, after_discard, btn);
				stylo.call({
					freeze: true,
					method: "stylo.desk.form.save.discard",
					args: {
						doctype: me.doc.doctype,
						name: me.doc.name,
					},
	...
	savetrash() {
		...
		stylo.model.delete_doc(this.doctype, this.docname, function () {
			window.history.back();
		});
	}
	...
	amend_doc() {
		if (!this.fields_dict["amended_from"]) {
			stylo.msgprint(__('"amended_from" field must be present to do an amendment.'));
			return;
		}

		stylo
			.xcall("stylo.client.is_document_amended", {
			...
			.then((is_amended) => {
				...
				var fn = function (newdoc) {
					...
					if (me.fields_dict && me.fields_dict["amendment_date"])
						newdoc.amendment_date = stylo.datetime.obj_to_str(new Date());
				};
				this.copy_doc(fn, 1);
				stylo.utils.play_sound("click");
			});
	...
	validate_form_action(action, resolve) {
		...
		var perms = stylo.perm.get_perm(this.doc.doctype)[0];
		...
		if (
			(stylo.workflow.is_read_only(this.doctype, this.docname) &&
	...
	show_submit_message() {
		if (
			...
			!this.is_dirty() &&
			!this.is_new() &&
			!stylo.model.has_workflow(this.doctype) && // show only if no workflow
	...
	fetch_permissions() {
		...
		this.perm = stylo.perm.get_perm(dt, this.doc);
	}
	...
	check_doctype_conflict(docname) {
		if (this.doctype == "DocType" && docname == "DocType") {
			stylo.msgprint(__("Allowing DocType, DocType. Be careful!"));
		} else if (this.doctype == "DocType") {
			if (stylo.views.formview[docname] || stylo.pages["List/" + docname]) {
				window.location.reload();
			}
		} else {
			if (
				stylo.views.formview.DocType &&
				stylo.views.formview.DocType.frm.opendocs[this.doctype]
			) {
				window.location.reload();
			}
		}
	...
	rename_notify(dt, old, name) {
		...
		// cleanup
		if (this && this.opendocs[old] && stylo.meta.docfield_copy[dt]) {
			// delete docfield copy
			stylo.meta.docfield_copy[dt][name] = stylo.meta.docfield_copy[dt][old];
			delete stylo.meta.docfield_copy[dt][old];
		}
		...
		stylo.re_route[stylo.router.get_sub_path()] = `${encodeURIComponent(
			stylo.router.slug(this.doctype)
		)}/${encodeURIComponent(name)}`;
		!stylo._from_link && stylo.set_route("Form", this.doctype, name);
	}
	...
	print_doc() {
		...
		stylo.set_route("print", this.doctype, this.doc.name);
	}

	navigate_records(prev) {
		let filters, sort_field, sort_order;
		let list_view = stylo.get_list_view(this.doctype);
		if (list_view) {
			filters = list_view.get_filters_for_args();
			sort_field = list_view.sort_by;
			sort_order = list_view.sort_order;
		} else {
			let list_settings = stylo.get_user_settings(this.doctype)["List"];
			if (list_settings) {
		...
		stylo
			.call({ method: "stylo.desk.form.utils.get_next", args, freeze: true })
			.then((r) => {
				if (r.message) {
					stylo.set_route("Form", this.doctype, r.message);
	...
	rename_doc() {
		stylo.model.rename_doc(this.doctype, this.docname, () => this.refresh_header());
	}
	...
	email_doc(message) {
		new stylo.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	copy_doc(onload, from_amend) {
		...
		var newdoc = stylo.model.copy_doc(this.doc, from_amend);
	...
	reload_doc() {
		...
		if (!this.doc.__islocal) {
			stylo.model.remove_from_locals(this.doctype, this.docname);
			return stylo.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	has_perm(ptype) {
		return stylo.perm.has_perm(this.doctype, 0, ptype, this.doc);
	}
	...
	get_docinfo() {
		return stylo.model.docinfo[this.doctype][this.docname];
	}
	...
	set_footnote(txt) {
		this.footnote_area = stylo.utils.set_footnote(this.footnote_area, this.body, txt);
	}
	...
	scroll_to_element() {
		if (stylo.route_options && stylo.route_options.scroll_to) {
			var scroll_to = stylo.route_options.scroll_to;
			delete stylo.route_options.scroll_to;
			...
			if (selector.length) {
				stylo.utils.scroll_to(selector);
			}
		} else if (window.location.hash) {
			if ($(window.location.hash).length) {
				stylo.utils.scroll_to(window.location.hash, true, 200, null, null, true);
			} else {
	...
	show_success_action() {
		...
		const success_action = new stylo.ui.form.SuccessAction(this);
		success_action.show();
	}
	...
	set_currency_labels(fields_list, currency, parentfield) {
		...
		$.each(fields_list, function (i, fname) {
			var docfield = stylo.meta.docfield_map[doctype][fname];
			if (docfield) {
	...
	field_map(fnames, fn) {
		...
		for (var i = 0, l = fnames.length; i < l; i++) {
			...
			var field = stylo.meta.get_docfield(this.doctype, fieldname, this.docname);
			if (field) {
				fn(field);
	...
	get_docfield(fieldname1, fieldname2) {
		if (fieldname2) {
			...
			return stylo.meta.get_docfield(doctype, fieldname2, this.docname);
		} else {
			// for parent
			return stylo.meta.get_docfield(this.doctype, fieldname1, this.docname);
		}
	}
	...
	set_df_property(fieldname, property, value, docname, table_field, table_row_name = null) {
		...
		if (!docname || !table_field) {
			...
			const filtered_fields = stylo.utils.filter_dict(grid.docfields, {
				fieldname: table_field,
			});
			if (filtered_fields.length) {
				df = stylo.meta.get_docfield(
	...
	get_files() {
		return this.attachments
			? stylo.utils.sort(this.attachments.get_attachments(), "file_name", "string")
			: [];
	}
	...
	clear_table(fieldname) {
		stylo.model.clear_table(this.doc, fieldname);
	}

	add_child(fieldname, values) {
		var doc = stylo.model.add_child(
			this.doc,
			stylo.meta.get_docfield(this.doctype, fieldname).options,
			fieldname
		);
		if (values) {
	...
	set_value(field, value, if_missing, skip_dirty_trigger = false) {
		...
		var _set = function (f, v) {
			...
			if (fieldobj) {
				if (!if_missing || !stylo.model.has_value(me.doctype, me.doc.name, f)) {
					if (
						stylo.model.table_fields.includes(fieldobj.df.fieldtype) &&
						$.isArray(v)
					) {
						// set entire child table from specified array as value
						stylo.model.clear_table(me.doc, fieldobj.df.fieldname);

						const standard_fields = [
							...stylo.model.std_fields_list,
							...stylo.model.child_table_field_list,
						];
						v.forEach((d, idx) => {
							let child = stylo.model.add_child(
								me.doc,
								fieldobj.df.options,
				...
				stylo.msgprint(__("Field {0} not found.", [f]));
		...
		if (typeof field == "string") {
			...
			return stylo.run_serially(tasks);
		}
	}
	...
	call(opts, args, callback) {
		...
		if (!opts.doc) {
			if (opts.method.indexOf(".") === -1)
				opts.method = stylo.model.get_server_module_name(me.doctype) + "." + opts.method;
					...
					if (opts.child) {
						...
						if (opts.child) {
							var std_field_list = ["doctype"]
								.concat(stylo.model.std_fields_list)
								.concat(stylo.model.child_table_field_list);
	...
	set_read_only() {
		const docperms = stylo.perm.get_perm(this.doc.doctype);
	...
	get_formatted(fieldname) {
		return stylo.format(
			this.doc[fieldname],
			stylo.meta.get_docfield(this.doctype, fieldname, this.docname),
			{ no_icon: true },
			this.doc
		);
	}
	...
	open_grid_row() {
		return stylo.ui.form.get_open_grid_form();
	}

	get_title() {
		return stylo.model.get_doc_title(this.doc);
	}
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		stylo.meta.get_table_fields(this.doctype, true).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		var doctype;
		if (stylo.meta.docfield_map[this.doctype][fieldname]) {
			doctype = this.doctype;
		} else {
			stylo.meta.get_table_fields(this.doctype, true).every(function (df) {
				if (stylo.meta.docfield_map[df.options][fieldname]) {
					doctype = df.options;
					return false;
				} else {
					return true;
		...
		stylo.meta.docfield_map[doctype][fieldname].formatter = function (
			value,
			df,
			options,
			doc
			...
			if (value) {
				...
				if (get_text) {
					label = get_text(doc);
				} else if (stylo.form.link_formatters[df.options]) {
					label = stylo.form.link_formatters[df.options](value, doc, df);
				} else {
					label = value;
				}
				...
				return `
						<a class="indicator ${get_color(doc || {})}"
							href="/app/${stylo.router.slug(df.options)}/${escaped_name}"
							data-doctype="${df.options}"
							data-name="${stylo.utils.escape_html(value)}">
							${label}
						</a>
					`;
			} else {
	...
	can_create(doctype) {
		...
		if (!stylo.model.can_create(doctype)) {
			return false;
		}
	...
	make_new(doctype) {
		...
		if (this.make_methods && this.make_methods[doctype]) {
			...
			stylo.model.with_doctype(doctype, function () {
				let new_doc = stylo.model.get_new_doc(doctype, null, null, true);

				// set link fields (if found)
				me.set_link_field(doctype, new_doc);

				stylo.ui.form.make_quick_entry(doctype, null, null, new_doc);
				// stylo.set_route('Form', doctype, new_doc.name);
			});
		}
	}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		stylo.get_meta(doctype).fields.forEach(function (df) {
	...
	update_in_all_rows(table_fieldname, fieldname, value) {
		...
		stylo.model
			...
			.filter((child) => !stylo.model.has_value(child.doctype, child.name, fieldname))
			.forEach((child) =>
				stylo.model.set_value(child.doctype, child.name, fieldname, value)
			);
	}
	...
	scroll_to_field(fieldname, focus = true) {
		...
		// scroll to input
		stylo.utils.scroll_to($el, true, 15);
		...
		let control_element = $el.closest(".stylo-control");
	...
	setup_docinfo_change_listener() {
		...
		if (this.doc && !this.is_new()) {
			stylo.realtime.doc_subscribe(doctype, docname);
		}
		stylo.realtime.off("docinfo_update");
		stylo.realtime.on("docinfo_update", ({ doc, key, action = "update" }) => {
			if (
			...
			let doc_list = stylo.model.docinfo[doctype][docname][key] || [];
			let docindex = doc_list.findIndex((old_doc) => {
				return old_doc.name === doc.name;
			});

			if (action === "add") {
				stylo.model.docinfo[doctype][docname][key].push(doc);
			}
			if (docindex > -1) {
				if (action === "update") {
					stylo.model.docinfo[doctype][docname][key].splice(docindex, 1, doc);
				}
				if (action === "delete") {
					stylo.model.docinfo[doctype][docname][key].splice(docindex, 1);
				}
			}
	...
	set_fields_as_options(
		...
		return new Promise((resolve) => {
			stylo.model.with_doctype(reference_doctype, () => {
				stylo.get_meta(reference_doctype).fields.map((df) => {
					filter_function(df) &&
	...
	get_involved_users() {
		...
		return involved_users
			.uniqBy((u) => u)
			.filter((user) => !["Administrator", stylo.session.user].includes(user))
			.filter(Boolean);
	}
	...
	show_submission_queue_banner() {
		...
		stylo
			.call({
				method: "stylo.core.doctype.submission_queue.submission_queue.get_latest_submissions",
	...
	show_workflow_read_only_banner() {
		...
		if (this.dashboard) {
			...
			stylo.after_ajax(_show_read_only_banner);
		}
	}
};
