...
stylo.ui.form.Form = class StyloForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = stylo.get_doc("DocType Layout", doctype_layout_name);
	...
	setup_meta() {
		this.meta = stylo.get_doc("DocType", this.doctype);
	...
	watch_model_updates() {
		...
		$.each(table_fields, function (i, df) {
			stylo.model.on(df.options, "*", function (fieldname, value, doc) {
	...
	refresh(docname) {
		...
		if (this.docname) {
			...
			if (this.read_only) {
				...
				stylo.show_alert(__("This form is not editable due to a Workflow."));
			}
	...
	check_doctype_conflict(docname) {
		if (this.doctype == "DocType" && docname == "DocType") {
			...
			if (stylo.views.formview[docname] || stylo.pages["List/" + docname]) {
				...
				//	stylo.msgprint(__("Cannot open {0} when its instance is open", ['DocType']))
			...
			if (
				...
				window.location.reload();
				//	stylo.msgprint(__("Cannot open instance when its {0} is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		}
	}
	...
	navigate_records(prev) {
		...
		stylo.call("stylo.desk.form.utils.get_next", args).then((r) => {
			if (r.message) {
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		stylo.meta.get_table_fields(this.doctype).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		if (stylo.meta.docfield_map[this.doctype][fieldname]) {
			...
			stylo.meta.get_table_fields(this.doctype).every(function (df) {
	...
	setup_docinfo_change_listener() {
		...
		if (this.doc && !this.is_new()) {
			stylo.socketio.doc_subscribe(doctype, docname);
		}
...
stylo.validated = 0;
// Proxy for stylo.validated
Object.defineProperty(window, "validated", {
	get: function () {
		console.warn(
			"Please use `stylo.validated` instead of `validated`. It will be deprecated soon."
		); // eslint-disable-line
		return stylo.validated;
	},
	set: function (value) {
		console.warn(
			"Please use `stylo.validated` instead of `validated`. It will be deprecated soon."
		); // eslint-disable-line
		stylo.validated = value;
		return stylo.validated;
	},
});
