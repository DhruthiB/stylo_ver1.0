stylo.ui.form.MultiSelectDialog = class MultiSelectDialog {
	constructor(opts) {
		...
		if (!this.for_select) {
			stylo.model.with_doctype(this.doctype, () => this.init());
		} else {
			this.init();
		}
	}
	...
	make_new_document(e) {
		...
		if (e) {
			...
			stylo.new_doc(this.doctype, true);
		}
	}
	...
	setup_child_datatable() {
		...
		this.child_datatable = new stylo.DataTable(this.$child_wrapper.get(0), {
	...
	get_primary_filters() {
		...
		if ($.isArray(this.setters)) {
			...
			Object.keys(this.setters).forEach((setter, index) => {
				let df_prop = stylo.meta.docfield_map[this.doctype][setter];
		...
		if (this.allow_child_item_selection) {
			this.child_doctype = stylo.meta.get_docfield(
				this.doctype,
				...
				${
					head
						? `<span class="ellipsis text-muted" title="${__(
								stylo.model.unscrub(column)
						  )}">${__(stylo.model.unscrub(column))}</span>`
						: column !== "name"
						? `<span class="ellipsis result-row" title="${__(
								result[column] || ""
						  )}">${__(result[column] || "")}</span>`
						: `<a href="${
								"/app/" + stylo.router.slug(me.doctype) + "/" + result[column] ||
								""
	...
	render_result_list(results, more = 0, empty = true) {
		...
		if (!stylo.flags.auto_scroll && empty) {
			this.empty_list();
		}
		more_btn.hide();
	...
	async perform_search(args) {
		const res = await stylo.call({
			type: "GET",
			method: "stylo.desk.search.search_widget",
			no_spinner: true,
			args: args,
		});
