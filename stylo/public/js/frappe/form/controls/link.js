...
stylo.ui.form.ControlLink = class ControlLink extends stylo.ui.form.ControlData {
	...
	is_translatable() {
		return in_list(stylo.boot?.translated_doctypes || [], this.get_options());
	}
	is_title_link() {
		return in_list(stylo.boot?.link_title_doctypes || [], this.get_options());
	}
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				return $("<li></li>")
					...
					.html(`<a><p title="${stylo.utils.escape_html(_label)}">${html}</p></a>`)
					.get(0);
			},
			sort: function () {
				return 0;
	...
	validate_link_and_fetch(value) {
		...
		if (value) {
			return stylo
				.xcall("stylo.client.validate_link", {
					doctype: options,
					docname: value,
