...
stylo.ui.form.recent_link_validations = {};

stylo.ui.form.ControlLink = class ControlLink extends stylo.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-open" tabIndex='-1' style="display: inline-block;" title="${__("Open Link")}">
					${stylo.utils.icon("arrow-right", "xs")}
				</a>
			</span>
	...
	show_link_and_clear_buttons() {
		if (this.$input.val() && this.get_options()) {
			...
			this.$link_open.attr("href", stylo.utils.get_form_link(doctype, name));
		}
	}
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return stylo.get_route && stylo.get_route()[0] === "List"
				? stylo.get_route()[1]
				: null;
		}
	}
	...
	is_translatable() {
		return (stylo.boot?.translated_doctypes || []).includes(this.get_options());
	}
	is_title_link() {
		return (stylo.boot?.link_title_doctypes || []).includes(this.get_options());
	}
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			stylo.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			stylo.route_options = df.get_route_options_for_new_doc(this);
		} else {
			stylo.route_options = {};
		}

		// partially entered name field
		stylo.route_options.name_field = this.get_label_value();

		// reference to calling link
		stylo._from_link = stylo.utils.deep_clone(this);
		stylo._from_link_scrollY = $(document).scrollTop();

		stylo.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				let _label = stylo.utils.escape_html(me.get_translated(d.label));
				...
				if (
					...
					html +=
						'<br><span class="small">' +
						__(stylo.utils.escape_html(d.description)) +
						"</span>";
				}
				return $(`<div role="option">`)
					...
					.html(`<p title="${stylo.utils.escape_html(_label)}">${html}</p>`)
					.get(0);
			},
			sort: function () {
				return 0;
		...
		this.$input.on(
			"input",
			stylo.utils.debounce(function (e) {
				...
				var args = {
					...
					page_length: cint(stylo.boot.sysdefaults?.link_field_results_limit) || 10,
				};
					...
					callback: function (r) {
						...
						if (!me.df.only_select) {
							...
							let custom__link_options =
								stylo.ui.form.ControlLink.link_options &&
								stylo.ui.form.ControlLink.link_options(me);
						...
						r.message.forEach((item) => {
							stylo.utils.add_link_title(doctype, item.value, item.label);
						});
					},
				});
			}, 500)
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				stylo.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (stylo.model.can_select(doctype) && !stylo.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = stylo.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : stylo.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = stylo.meta.get_docfield(this.frm.doctype, key);
	...
	parse_filters(link_filters) {
		...
		link_filters.forEach((filter) => {
			...
			if (value?.startsWith?.("eval:")) {
				...
				value = stylo.utils.eval(value, context);
			}
