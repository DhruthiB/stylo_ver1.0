...
import ImageResize from "stylo-quill-image-resize";
...
stylo.ui.form.ControlTextEditor = class ControlTextEditor extends stylo.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			stylo.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = stylo.ui.keys && stylo.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: stylo.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "stylo.desk.search.get_names_for_mentions";
				let values = await stylo.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? stylo.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return stylo.dom.remove_script_and_style(value);
	}
