stylo.ui.form.ControlButton = class ControlButton extends stylo.ui.form.ControlData {
	can_write() {
	...
	make_input() {
		...
		this.$input = $(
			`<button
				class="btn ${stylo.utils.escape_html(btn_size)} ${stylo.utils.escape_html(btn_type)}"
				title="${stylo.utils.escape_html(this.df.label)}"
			>`
		)
	...
	set_label(label) {
		...
		label =
			(this.df.icon ? stylo.utils.icon(this.df.icon) : "") +
