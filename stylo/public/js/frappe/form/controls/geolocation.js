...
stylo.ui.form.ControlGeolocation = class ControlGeolocation extends stylo.ui.form.ControlData {
	...
	set_disp_area(value) {
		...
		if (this.frm) {
			...
			$(document).on("stylo.ui.Dialog:shown", () => {
				this.make_map();
				if (value) {
					this.bind_leaflet_data(value);
				}
	...
	bind_leaflet_map() {
		...
		this.map.setView(stylo.utils.map_defaults.center, stylo.utils.map_defaults.zoom);

		this.streetLayer = L.tileLayer(
			stylo.utils.map_defaults.tiles.default_tile.url,
			stylo.utils.map_defaults.tiles.default_tile.options
		);
		this.satelliteLayer = L.tileLayer(
			stylo.utils.map_defaults.tiles.satellite_tile.url,
			stylo.utils.map_defaults.tiles.satellite_tile.options
		);
		this.labelsLayer = L.tileLayer(
			stylo.utils.map_defaults.tiles.labels_tail.url,
			stylo.utils.map_defaults.tiles.labels_tail.options
		);
		this.terrainLayer = L.tileLayer(
			stylo.utils.map_defaults.tiles.terrain_lines_tail.url,
			stylo.utils.map_defaults.tiles.terrain_lines_tail.options
		);
		...
		return new L.Control.Draw({
			...
			draw: {
				...
				polygon: {
					...
					drawError: {
						color: stylo.ui.color.get("orange"), // Color the shape will turn when intersects
			...
			if (e.name === "Satellite") {
				...
				Object.values(this.map._layers).forEach((layer) => {
					if (
						...
						(layer._url === stylo.utils.map_defaults.tiles.labels_tail.url ||
							layer._url === stylo.utils.map_defaults.tiles.terrain_lines_tail.url)
					) {
