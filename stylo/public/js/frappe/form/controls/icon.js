...
stylo.ui.form.ControlIcon = class ControlIcon extends stylo.ui.form.ControlData {
	make_input() {
	...
	get_all_icons() {
		stylo.symbols = [];
		$("#stylo-symbols > symbol[id]").each(function () {
			this.id.includes("icon-") && stylo.symbols.push(this.id.replace("icon-", ""));
		});
	}
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(
				`<div class="selected-icon">${stylo.utils.icon("folder-normal", "md")}</div>`
			);
