stylo.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='stylo-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = stylo.perm.get_field_display_status(
			this.df,
			stylo.model.get_doc(this.doctype, this.docname),
		...
		let value = stylo.model.get_value(this.doctype, this.docname, this.df.fieldname);
		...
		if (
			...
			!this.only_input &&
			is_null(value) &&
			cint(stylo.boot.sysdefaults.hide_empty_read_only_fields) &&
	...
	show_translatable_button(value) {
		...
		if (
			...
			!stylo.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!stylo.views || !stylo.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new stylo.views.TranslationManager({
						df: this.df,
