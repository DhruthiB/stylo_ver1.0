stylo.ui.form.ControlPassword = class ControlPassword extends stylo.ui.form.ControlData {
	...
	make_input() {
		...
		this.toggle_password.on("click", () => {
			if (this.$input.attr("type") === "password") {
				...
				this.toggle_password.html(stylo.utils.icon("hide", "sm"));
			} else {
				this.$input.attr("type", "password");
				this.toggle_password.html(stylo.utils.icon("unhide", "sm"));
			}
		});
	...
	get_password_strength(value) {
		...
		stylo.call({
			type: "POST",
			method: "stylo.core.doctype.user.user.test_password_strength",
			args: {
				new_password: value || "",
			},
			callback: function (r) {
