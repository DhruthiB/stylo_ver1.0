stylo.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	stylo.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${stylo.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
