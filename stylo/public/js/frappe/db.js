...
stylo.db = {
	...
	exists: function (doctype, nameOrFilters) {
		return new Promise((resolve) => {
			...
			if (typeof nameOrFilters === "string") {
				...
				stylo.db.get_value(doctype, { name: nameOrFilters }, "name").then((r) => {
					r.message && r.message.name ? resolve(true) : resolve(false);
				});
			} else if (typeof nameOrFilters === "object") {
				stylo.db.count(doctype, { filters: nameOrFilters, limit: 1 }).then((count) => {
					resolve(count > 0);
				});
			}
		});
	...
	count: function (doctype, args = {}, cache = false) {
		...
		return stylo.xcall(
			"stylo.desk.reportview.get_count",
			{
				doctype,
				filters,
				fields,
