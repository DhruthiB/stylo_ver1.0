...
Object.assign(stylo.model, {
	docinfo: {},
	sync: function (r) {
		/* docs:
			extract docs, docinfo (attachments, comments, assignments)
			from incoming request and set in `locals` and `stylo.model.docinfo`
		*/
		var isPlain;
		...
		if (r.docs) {
			for (var i = 0, l = r.docs.length; i < l; i++) {
				...
				if (locals[d.doctype] && locals[d.doctype][d.name]) {
					// update values
					stylo.model.update_in_locals(d);
				} else {
					stylo.model.add_to_locals(d);
				}
				...
				if (d.localname) {
					stylo.model.rename_after_save(d, i);
				}
			}
		}

		stylo.model.sync_docinfo(r);
		return r.docs;
	},

	rename_after_save: (d, i) => {
		stylo.model.new_names[d.localname] = d.name;
		$(document).trigger("rename", [d.doctype, d.localname, d.name]);
		delete locals[d.doctype][d.localname];

		// update docinfo to new dict keys
		if (i === 0) {
			stylo.model.docinfo[d.doctype][d.name] = stylo.model.docinfo[d.doctype][d.localname];
			stylo.model.docinfo[d.doctype][d.localname] = undefined;
		}
	},
	...
	sync_docinfo: (r) => {
		...
		if (r.docinfo) {
			...
			if (!stylo.model.docinfo[doctype]) {
				stylo.model.docinfo[doctype] = {};
			}
			stylo.model.docinfo[doctype][name] = r.docinfo;

			// copy values to stylo.boot.user_info
			Object.assign(stylo.boot.user_info, r.docinfo.user_info);
		}
	...
	add_to_locals: function (doc) {
		...
		if (!doc.name && doc.__islocal) {
			...
			if (!doc.parentfield) stylo.model.clear_doc(doc);

			doc.name = stylo.model.get_new_name(doc.doctype);

			if (!doc.parentfield)
				stylo.provide("stylo.model.docinfo." + doc.doctype + "." + doc.name);
		}
	...
	update_in_locals: function (doc) {
		...
		for (let fieldname in doc) {
			let df = stylo.meta.get_field(doc.doctype, fieldname);
			if (df && stylo.model.table_fields.includes(df.fieldtype)) {
				// table
				...
				for (let i = 0; i < doc[fieldname].length; i++) {
					...
					if (local_d) {
						...
						if (!d.name) {
							...
							d.name = stylo.model.get_new_name(doc.doctype);
						}
