stylo.provide("stylo.model.user_settings");

$.extend(stylo.model.user_settings, {
	get: function (doctype) {
		return stylo
			.call("stylo.model.utils.user_settings.get", { doctype })
			.then((r) => JSON.parse(r.message || "{}"));
	},
	save: function (doctype, key, value) {
		if (stylo.session.user === "Guest") return Promise.resolve();

		const old_user_settings = stylo.model.user_settings[doctype] || {};
	...
	remove: function (doctype, key) {
		var user_settings = stylo.model.user_settings[doctype] || {};
	...
	update: function (doctype, user_settings) {
		if (stylo.session.user === "Guest") return Promise.resolve();
		return stylo.call({
			method: "stylo.model.utils.user_settings.save",
			args: {
				doctype: doctype,
				user_settings: user_settings,
			},
			callback: function (r) {
				stylo.model.user_settings[doctype] = r.message;
			},
		});
	},
});

stylo.get_user_settings = function (doctype, key) {
	var settings = stylo.model.user_settings[doctype] || {};
	if (key) {
