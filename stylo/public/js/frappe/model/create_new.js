...
$.extend(stylo.model, {
	...
	set_default_values: function (doc, parent_doc) {
		...
		var docfields = stylo.meta.get_docfields(doctype);
		var updated = [];
		for (var fid = 0; fid < docfields.length; fid++) {
			var f = docfields[fid];
			if (!in_list(stylo.model.no_value_type, f.fieldtype) && doc[f.fieldname] == null) {
				if (f.no_default) continue;
				var v = stylo.model.get_default_value(f, doc, parent_doc);
				if (v) {
