...
$.extend(stylo.model, {
	...
	get_new_doc: function (doctype, parent_doc, parentfield, with_mandatory_children) {
		...
		var doc = {
			docstatus: 0,
			doctype: doctype,
			name: stylo.model.get_new_name(doctype),
			__islocal: 1,
			__unsaved: 1,
			owner: stylo.session.user,
		};
		stylo.model.set_default_values(doc, parent_doc);
		...
		if (parent_doc) {
			...
			stylo.provide("stylo.model.docinfo." + doctype + "." + doc.name);
		}

		stylo.model.add_to_locals(doc);

		if (with_mandatory_children) {
			stylo.model.create_mandatory_children(doc);
		}
		...
		if (stylo.route_options && stylo.route_options.name_field) {
			var meta = stylo.get_meta(doctype);
			// set title field / name as name
			if (meta.autoname && meta.autoname.indexOf("field:") !== -1) {
				doc[meta.autoname.substr(6)] = stylo.route_options.name_field;
			} else if (meta.autoname && meta.autoname === "prompt") {
				doc.__newname = stylo.route_options.name_field;
			} else if (meta.title_field) {
				doc[meta.title_field] = stylo.route_options.name_field;
			}

			delete stylo.route_options.name_field;
		}

		// set route options
		if (stylo.route_options && !doc.parent) {
			$.each(stylo.route_options, function (fieldname, value) {
				var df = stylo.meta.has_field(doctype, fieldname);
				if (df && !df.no_copy) {
					doc[fieldname] = value;
				}
			});
	...
	make_new_doc_and_get_name: function (doctype, with_mandatory_children) {
		return stylo.model.get_new_doc(doctype, null, null, with_mandatory_children).name;
	},

	get_new_name: function (doctype) {
		// random hash is added to idenity mislinked files when doc is not saved and file is uploaded.
		return stylo.router.slug(`new-${doctype}-${stylo.utils.get_random(10)}`);
	},

	set_default_values: function (doc, parent_doc) {
		let doctype = doc.doctype;
		let docfields = stylo.meta.get_docfields(doctype);
		let updated = [];

		// Table types should be initialized
		let fieldtypes_without_default = stylo.model.no_value_type.filter(
			(fieldtype) => !stylo.model.table_fields.includes(fieldtype)
		);
		docfields.forEach((f) => {
			if (
			...
			let v = stylo.model.get_default_value(f, doc, parent_doc);
			if (v) {
	...
	create_mandatory_children: function (doc) {
		var meta = stylo.get_meta(doc.doctype);
		if (meta && meta.istable) return;

		// create empty rows for mandatory table fields
		stylo.meta.get_docfields(doc.doctype).forEach(function (df) {
			if (df.fieldtype === "Table" && df.reqd) {
				stylo.model.add_child(doc, df.fieldname);
			}
		});
	},
	...
	get_default_value: function (df, doc, parent_doc) {
		var user_default = "";
		var user_permissions = stylo.defaults.get_user_permissions();
		let allowed_records = [];
		let default_doc = null;
		let value = null;
		if (user_permissions) {
			({ allowed_records, default_doc } = stylo.perm.filter_allowed_docs_for_doctype(
		...
		if (df.fieldtype === "Link" && df.options !== "User") {
			...
			if (has_user_permissions && default_doc) {
				...
				if (!df.ignore_user_permissions) {
					var user_defaults = stylo.defaults.get_user_defaults(df.options);
				...
				if (!user_default) {
					user_default = stylo.defaults.get_user_default(df.fieldname);
				}
				if (
					!user_default &&
					...
					user_default = stylo.boot.user.last_selected_values[df.options];
				}
		...
		if (!value || df["default"]) {
			...
			if (default_val == "__user" || default_val.toLowerCase() == "user") {
				value = stylo.session.user;
			} else if (default_val == "user_fullname") {
				value = stylo.session.user_fullname;
			} else if (default_val == "Today") {
				value = stylo.datetime.get_today();
			} else if ((default_val || "").toLowerCase() === "now") {
				if (df.fieldtype == "Time") {
					value = stylo.datetime.now_time();
				} else {
					// datetime fields are stored in system tz
					value = stylo.datetime.system_datetime();
				}
			} else if (default_val[0] === ":") {
				var boot_doc = stylo.model.get_default_from_boot_docs(df, doc, parent_doc);
	...
	get_default_from_boot_docs: function (df, doc, parent_doc) {
		...
		if (stylo.get_list(df["default"]).length > 0) {
			var ref_fieldname = df["default"].slice(1).toLowerCase().replace(" ", "_");
			var ref_value = parent_doc
				? parent_doc[ref_fieldname]
				: stylo.defaults.get_user_default(ref_fieldname);
			var ref_doc = ref_value ? stylo.get_doc(df["default"], ref_value) : null;
	...
	add_child: function (parent_doc, doctype, parentfield, idx) {
		...
		if (arguments.length === 2) {
			parentfield = doctype;
			doctype = stylo.meta.get_field(parent_doc.doctype, parentfield).options;
		}

		// create row doc
		idx = idx ? idx - 0.1 : (parent_doc[parentfield] || []).length + 1;

		var child = stylo.model.get_new_doc(doctype, parent_doc, parentfield);
		child.idx = idx;
	...
	copy_doc: function (doc, from_amend, parent_doc, parentfield) {
		...
		let newdoc = stylo.model.get_new_doc(doc.doctype, parent_doc, parentfield);

		for (let key in doc) {
			// don't copy name and blank fields
			let df = stylo.meta.get_docfield(doc.doctype, key);
			...
			if (df && !is_internal_field && !is_blocked_field && !is_no_copy && !is_password) {
				...
				if (stylo.model.table_fields.includes(df.fieldtype)) {
					for (let i = 0, j = value.length; i < j; i++) {
						let d = value[i];
						stylo.model.copy_doc(d, from_amend, newdoc, df.fieldname);
					}
				} else {
					newdoc[key] = doc[key];
				}
	...
	open_mapped_doc: function (opts) {
		...
		return stylo.call({
			type: "POST",
			method: "stylo.model.mapper.make_mapped_doc",
			args: {
				method: opts.method,
			...
			callback: function (r) {
				if (!r.exc) {
					...
					stylo.set_route("Form", r.message.doctype, r.message.name);
				}
			},
		});
	},
...
stylo.create_routes = {};
stylo.new_doc = function (doctype, opts, init_callback) {
	if (doctype === "File") {
	...
	return new Promise((resolve) => {
		if (opts && $.isPlainObject(opts)) {
			stylo.route_options = opts;
		}
		stylo.model.with_doctype(doctype, function () {
			if (stylo.create_routes[doctype]) {
				stylo.set_route(stylo.create_routes[doctype]).then(() => resolve());
			} else {
				stylo.ui.form
