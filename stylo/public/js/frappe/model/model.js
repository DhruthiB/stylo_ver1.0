...
$.extend(stylo.model, {
	...
	init_doctype: function (meta) {
		if (meta.name === "DocType") {
			...
			stylo.meta.__doctype_meta = JSON.parse(JSON.stringify(meta));
		}
	...
	get_children: function (doctype, parent, parentfield, filters) {
		...
		if ($.isPlainObject(doctype)) {
			...
			doc = stylo.get_doc(doctype, parent);
		}
	...
	get_doc_title(doc) {
		...
		let meta = stylo.get_meta(doc.doctype);
		if (meta.title_field) {
	...
	get_no_copy_list: function (doctype) {
		...
		var docfields = stylo.get_meta(doctype).fields || [];
