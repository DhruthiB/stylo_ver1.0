...
$.extend(stylo.model, {
	...
	init: function () {
		...
		stylo.realtime.on("doc_update", function (data) {
			var doc = locals[data.doctype] && locals[data.doctype][data.name];

			if (doc) {
				// current document is dirty, show message if its not me
				if (
					stylo.get_route()[0] === "Form" &&
					cur_frm.doc.doctype === doc.doctype &&
					cur_frm.doc.name === doc.name
				) {
					if (data.modified !== cur_frm.doc.modified && !stylo.ui.form.is_saving) {
						if (!cur_frm.is_dirty()) {
					...
					if (!doc.__unsaved) {
						...
						stylo.model.remove_from_locals(doc.doctype, doc.name);
					} else {
	...
	is_value_type: function (fieldtype) {
		...
		return stylo.model.no_value_type.indexOf(fieldtype) === -1;
	},

	is_non_std_field: function (fieldname) {
		return ![...stylo.model.std_fields_list, ...stylo.model.child_table_field_list].includes(
			fieldname
		);
	},
	...
	get_std_field: function (fieldname, ignore = false) {
		var docfield = $.map(
			[].concat(stylo.model.std_fields).concat(stylo.model.std_fields_table),
			function (d) {
		...
		if (!docfield.length) {
			...
			if (ignore) {
				...
				stylo.msgprint(__("Unknown Column: {0}", [fieldname]));
			}
		}
		return docfield[0];
	},
	...
	set_in_localstorage: function (doctype, docs) {
		try {
			...
			stylo.model.clear_local_storage();
			localStorage["_doctype:" + doctype] = JSON.stringify(docs);
		}
	},
	...
	with_doctype: function (doctype, callback, async) {
		if (locals.DocType[doctype]) {
			...
			let cached_docs = stylo.model.get_from_localstorage(doctype);
			...
			return stylo.call({
				method: "stylo.desk.form.load.getdoctype",
				type: "GET",
				args: {
					doctype: doctype,
					with_parent: 1,
				...
				callback: function (r) {
					if (r.exc) {
						stylo.msgprint(__("Unable to load: {0}", [__(doctype)]));
						throw "No doctype";
					}
					if (r.message == "use_cache") {
						stylo.model.sync(cached_doc);
					} else {
						stylo.model.set_in_localstorage(doctype, r.docs);
					}
					stylo.model.init_doctype(doctype);

					if (r.user_settings) {
						// remember filters and other settings from last view
						stylo.model.user_settings[doctype] = JSON.parse(r.user_settings);
						stylo.model.user_settings[doctype].updated_on = moment().toString();
					}
					callback && callback(r);
				},
			});
	...
	init_doctype: function (doctype) {
		...
		if (meta.__templates) {
			$.extend(stylo.templates, meta.__templates);
		}
	},
	...
	with_doc: function (doctype, name, callback) {
		return new Promise((resolve) => {
			...
			if (
				...
				resolve(stylo.get_doc(doctype, name));
			} else {
				return stylo.call({
					method: "stylo.desk.form.load.getdoc",
					type: "GET",
					args: {
						doctype: doctype,
						name: name,
	...
	get_docinfo: function (doctype, name) {
		return (stylo.model.docinfo[doctype] && stylo.model.docinfo[doctype][name]) || null;
	},

	set_docinfo: function (doctype, name, key, value) {
		if (stylo.model.docinfo[doctype] && stylo.model.docinfo[doctype][name]) {
			stylo.model.docinfo[doctype][name][key] = value;
		}
	},

	get_shared: function (doctype, name) {
		return stylo.model.get_docinfo(doctype, name).shared;
	},

	get_server_module_name: function (doctype) {
		var dt = stylo.model.scrub(doctype);
		var module = stylo.model.scrub(locals.DocType[doctype].module);
		var app = stylo.boot.module_app[module];
	...
	can_create: function (doctype) {
		return stylo.boot.user.can_create.indexOf(doctype) !== -1;
	},

	can_select: function (doctype) {
		if (stylo.boot.user) {
			return stylo.boot.user.can_select.indexOf(doctype) !== -1;
		}
	},

	can_read: function (doctype) {
		if (stylo.boot.user) {
			return stylo.boot.user.can_read.indexOf(doctype) !== -1;
		}
	},

	can_write: function (doctype) {
		return stylo.boot.user.can_write.indexOf(doctype) !== -1;
	},

	can_get_report: function (doctype) {
		return stylo.boot.user.can_get_report.indexOf(doctype) !== -1;
	},

	can_delete: function (doctype) {
		if (!doctype) return false;
		return stylo.boot.user.can_delete.indexOf(doctype) !== -1;
	},

	can_submit: function (doctype) {
		if (!doctype) return false;
		return stylo.boot.user.can_submit.indexOf(doctype) !== -1;
	},

	can_cancel: function (doctype) {
		if (!doctype) return false;
		return stylo.boot.user.can_cancel.indexOf(doctype) !== -1;
	},

	has_workflow: function (doctype) {
		return stylo.get_list("Workflow", { document_type: doctype, is_active: 1 }).length;
	},
	...
	is_single: function (doctype) {
		if (!doctype) return false;
		return stylo.boot.single_types.indexOf(doctype) != -1;
	},
	...
	can_import: function (doctype, frm, meta = null) {
		...
		if (stylo.user_roles.includes("System Manager")) return true;

		if (frm) return frm.perm[0].import === 1;
		return stylo.boot.user.can_import.indexOf(doctype) !== -1;
	},
	...
	can_export: function (doctype, frm) {
		...
		return stylo.boot.user.can_export.indexOf(doctype) !== -1;
	},

	can_print: function (doctype, frm) {
		if (frm) return frm.perm[0].print === 1;
		return stylo.boot.user.can_print.indexOf(doctype) !== -1;
	},

	can_email: function (doctype, frm) {
		if (frm) return frm.perm[0].email === 1;
		return stylo.boot.user.can_email.indexOf(doctype) !== -1;
	},

	can_share: function (doctype, frm) {
		let disable_sharing = cint(stylo.sys_defaults.disable_document_sharing);

		if (disable_sharing && stylo.session.user !== "Administrator") {
			return false;
		}
		...
		return stylo.boot.user.can_share.indexOf(doctype) !== -1;
	},
	...
	can_set_user_permissions: function (doctype, frm) {
		...
		return stylo.boot.user.can_set_user_permissions.indexOf(doctype) !== -1;
	},

	has_value: function (dt, dn, fn) {
		// return true if property has value
		var val = locals[dt] && locals[dt][dn] && locals[dt][dn][fn];
		var df = stylo.meta.get_docfield(dt, fn, dn);

		if (stylo.model.table_fields.includes(df.fieldtype)) {
			var ret = false;
	...
	get_list: function (doctype, filters) {
		...
		return stylo.utils.filter_dict(docsdict, filters);
	},
	...
	get_value: function (doctype, filters, fieldname, callback) {
		if (callback) {
			...
			if (
				...
				var l = stylo.get_list(doctype, filters);
	...
	set_value: function (
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => stylo.model.trigger(key, value, doc, skip_dirty_trigger));
			} else {
	...
	on: function (doctype, fieldname, fn) {
		...
		*/
		/* example: stylo.model.on("Customer", "age", function(fieldname, value, doc) {
		  if(doc.age < 16) {
		   	stylo.msgprint("Warning, Customer must atleast be 16 years old.");
		    raise "CustomerAgeError";
		  }
		}) */
		stylo.provide("stylo.model.events." + doctype);
		if (!stylo.model.events[doctype][fieldname]) {
			stylo.model.events[doctype][fieldname] = [];
		}
		stylo.model.events[doctype][fieldname].push(fn);
	},
		...
		function enqueue_events(events) {
			...
			for (const fn of events) {
				...
				tasks.push(() => {
					...
					// or use the default promise stylo.after_ajax
					if (return_value && return_value.then) {
						return return_value;
					} else {
						return stylo.after_server_call();
					}
				});
			}
		}

		if (stylo.model.events[doc.doctype]) {
			enqueue_events(stylo.model.events[doc.doctype][fieldname]);
			enqueue_events(stylo.model.events[doc.doctype]["*"]);
		}
	...
	get_doc: function (doctype, name) {
		...
		if ($.isPlainObject(name)) {
			var doc = stylo.get_list(doctype, name);
	...
	get_children: function (doctype, parent, parentfield, filters) {
		if ($.isPlainObject(doctype)) {
			...
			var doc = stylo.get_doc(doctype, parent);
		}

		var children = doc[parentfield] || [];
		if (filters) {
			return stylo.utils.filter_dict(children, filters);
		} else {
			return children;
		}
	},
	...
	remove_from_locals: function (doctype, name) {
		...
		if (stylo.views.formview[doctype]) {
			delete stylo.views.formview[doctype].frm.opendocs[name];
		}
	},
	...
	get_no_copy_list: function (doctype) {
		...
		var docfields = stylo.get_doc("DocType", doctype).fields || [];
	...
	delete_doc: function (doctype, docname, callback) {
		let title = docname;
		const title_field = stylo.get_meta(doctype).title_field;
		if (stylo.get_meta(doctype).autoname == "hash" && title_field) {
			const value = stylo.model.get_value(doctype, docname, title_field);
			if (value) {
				title = `${value} (${docname})`;
			}
		}
		stylo.confirm(__("Permanently delete {0}?", [title.bold()]), function () {
			return stylo.call({
				method: "stylo.client.delete",
				args: {
					doctype: doctype,
					name: docname,
				},
				...
				callback: function (r, rt) {
					if (!r.exc) {
						stylo.utils.play_sound("delete");
						stylo.model.clear_doc(doctype, docname);
	...
	round_floats_in: function (doc, fieldnames) {
		...
		if (!fieldnames) {
			fieldnames = stylo.meta.get_fieldnames(doc.doctype, doc.parent, {
	...
	validate_missing: function (doc, fieldname) {
		if (!doc[fieldname]) {
			stylo.throw(
				__("Please specify") +
					": " +
					__(stylo.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name))
			);
		}
	},
	...
	is_numeric_field: function (fieldtype) {
		...
		return stylo.model.numeric_fieldtypes.includes(fieldtype);
	},
	...
	set_default_views_for_doctype(doctype, frm) {
		stylo.model.with_doctype(doctype, () => {
			...
			if (meta.is_calendar_and_gantt && stylo.views.calendar[doctype]) {
			...
			if (doctype === "Communication" && stylo.boot.email_accounts.length) {
...
// legacy
stylo.get_doc = stylo.model.get_doc;
stylo.get_children = stylo.model.get_children;
stylo.get_list = stylo.model.get_list;
