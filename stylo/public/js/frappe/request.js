...
stylo.xcall = function (method, params, type, opts = {}) {
...
stylo.call = function (opts) {
	...
	var callback = function (data, response_text) {
		if (data.task_id) {
			...
			stylo.realtime.subscribe(data.task_id, opts);
...
stylo.request.report_error = function (xhr, request_opts) {
	...
	const copy_markdown_to_clipboard = () => {
		...
		request_data.request_id = xhr.getResponseHeader("X-Stylo-Request-Id");
	...
	if (exc) {
		...
		if (error_report_email) {
			stylo.error_dialog.set_primary_action(__("Report"), () => {
				show_communication();
				stylo.error_dialog.hide();
			});
		} else {
			stylo.error_dialog.set_primary_action(__("Copy error to clipboard"), () => {
		...
		stylo.error_dialog.$body.html(dialog_html);
...
stylo.request.cleanup_request_opts = function (request_opts) {
	...
	if (doc) {
		doc = JSON.parse(doc);
		stylo.utils.mask_passwords(doc);
		request_opts.args.doc = JSON.stringify(doc);
	}

	if (request_opts.args) {
		stylo.utils.mask_passwords(request_opts.args);
	}
