...
stylo.views.ListView = class ListView extends stylo.views.BaseList {
	static load_last_view() {
		const route = stylo.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = stylo.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			stylo.set_route(
				"list",
				stylo.router.doctype_layout || doctype,
				stylo.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		const meta = stylo.get_meta(this.doctype);
		this.is_large_table = meta?.is_large_table;

		this.debounced_refresh = stylo.utils.debounce(
	...
	has_permissions() {
		return stylo.perm.has_perm(this.doctype, 0, "read");
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			stylo.set_route("");
			stylo.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(stylo.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = stylo.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${stylo.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		stylo.msgprint(
			stylo.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? stylo.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						stylo.boot.link_title_doctypes.includes(df.options)
					) {
						stylo.model.with_doctype(df.options, () => {
							const meta = stylo.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = stylo.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = stylo.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				stylo.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !stylo.boot.read_only) {
			const doctype_name = __(stylo.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (allowed_filter_types.includes(f[2]) && stylo.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		stylo.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = stylo.meta.get_docfield.bind(null, this.doctype);
		...
		if (stylo.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (stylo.has_indicator(this.doctype) && df.fieldname === "status") {
						return false;
					}
	...
	before_refresh() {
		if (stylo.route_options && this.filter_area) {
	...
	before_render() {
		...
		stylo.model.user_settings.save(this.doctype, "last_view", this.view_name);
		...
		let $columns = this.columns
			.map((col) => {
				let classes = [
					...
					stylo.model.is_numeric_field(col.df) ? "text-right" : "",
					col.df?.fieldname,
				].join(" ");
	...
	get_left_html(doc) {
		...
		for (let i = 0; i < this.columns.length; i++) {
			...
			if (stylo.is_mobile() && col.type == "Field" && [3, 4].includes(i)) {
	...
	get_column_html(col, doc, show_in_mobile) {
		...
		let translated_doctypes = stylo.boot?.translated_doctypes || [];
		...
		const format = () => {
			if (df.fieldtype === "Percent") {
				...
				return stylo.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? stylo.utils.escape_html(value_display)
						: value_display;
			}
			...
			let masked_fields = stylo.get_meta(this.doctype).masked_fields || [];
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${stylo.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="${filterable} indicator-pill ${stylo.utils.guess_colour(
					_value
				)} ellipsis"
			...
			} else if (stylo.model.html_fieldtypes.includes(df.fieldtype)) {
				html = `<span class="ellipsis">
					${_value}
				</span>`;
			} else {
				html = `<a class="${filterable} ellipsis"
					data-filter="${fieldname},=,${stylo.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${stylo.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		let css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			stylo.model.is_numeric_field(df) ? "text-right" : "",
			fieldname,
		].join(" ");
		...
		if (stylo.is_mobile() && col.type == "Subject") {
			css_class += " bold";
		}
	...
	apply_column_widths() {
		...
		Object.entries(this.column_max_widths).forEach(([fieldname, width]) => {
			$(`.list-view .stylo-list .result .level-left .list-row-col.${fieldname}`).css({
				width: width,
				flex: `1 0 ${width}px`,
			});
		});
	...
	update_listview_classes(has_assignto, assign_to_count) {
		...
		// disable scrolling
		if (this.list_view_settings?.disable_scrolling && !stylo.is_mobile()) {
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments d-flex align-items-center">
					${
						stylo.avatar_group(assigned_users, this.max_number_of_avatars - 1, {
							filterable: true,
						})[0].outerHTML
					}
				</div>`;
		...
		if (this.list_view_settings && !this.list_view_settings.disable_comment_count) {
			comment_count = `<span class="comment-count d-flex align-items-center">
				${stylo.utils.icon("es-line-chat-alt")}
	...
	get_form_link(doc) {
		...
		return `/app/${encodeURIComponent(
			stylo.router.slug(stylo.router.doctype_layout || this.doctype)
		)}/${encodeURIComponent(cstr(doc.name))}`;
	}

	get_seen_class(doc) {
		const seen_by = doc._seen ? JSON.parse(doc._seen) : [];
		return seen_by.includes(stylo.session.user) ? "" : "bold";
	}

	get_like_html(doc) {
		const liked_by = doc._liked_by ? JSON.parse(doc._liked_by) : [];
		const is_liked = liked_by.includes(stylo.session.user);
		const title = liked_by.map((u) => stylo.user_info(u).fullname).join(", ");
	...
	get_subject_text(doc, title) {
		...
		if (stylo.model.html_fieldtypes.includes(subject_field.fieldtype)) {
			// NOTE: this is very slow, so only do it for HTML fields
			return stylo.utils.html2text(value);
		} else {
			return value;
		}
	}
	...
	get_indicator_html(doc, show_workflow_state) {
		const indicator = stylo.get_indicator(doc, this.doctype, show_workflow_state);
	...
	get_indicator_dot(doc) {
		const indicator = stylo.get_indicator(doc, this.doctype);
		if (!indicator) return "";
	...
	get_image_url(doc) {
		...
		if (window.cordova && !stylo.utils.is_url(url)) {
			url = stylo.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = stylo.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = stylo.session.user;
				}
	...
	setup_like() {
		this.$result.on("click", ".like-action", (e) => {
			...
			stylo.ui.toggle_like($this, doctype, name);
	...
	setup_realtime_updates() {
		...
		stylo.realtime.doctype_subscribe(this.doctype);
		stylo.realtime.off("list_update");
		stylo.realtime.on("list_update", (data) => {
	...
	disable_realtime_updates() {
		stylo.realtime.doctype_unsubscribe(this.doctype);
	...
	process_document_refreshes() {
		...
		stylo.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = stylo.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return stylo.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (stylo.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (stylo.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					stylo.set_route("list", "data-import", {
		...
		if (stylo.user_roles.includes("System Manager")) {
			items.push({
				...
				action: () =>
					stylo.set_route("permission-manager", {
						doctype,
					}),
				standard: true,
			});
		...
		if (
			stylo.model.can_create("Custom Field") &&
			stylo.model.can_create("Property Setter")
		) {
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						stylo.set_route("form", "doctype", doctype);
					} else if (!this.meta.custom) {
						stylo.set_route("form", "customize-form", {
							doc_type: doctype,
						});
					}
				},
		...
		if (stylo.user.has_role("System Manager") && stylo.boot.developer_mode) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => stylo.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		stylo.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: stylo.get_meta(this.doctype),
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (stylo.model.has_workflow(this.doctype)) {
			const actions = stylo.workflow.get_all_transition_actions(this.doctype);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						...
						stylo
							.xcall("stylo.model.workflow.bulk_workflow_approval", {
	...
	toggle_workflow_actions() {
		if (!stylo.model.has_workflow(this.doctype)) return;
		...
		stylo
			.xcall("stylo.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				stylo.model.is_value_type(field_doc) &&
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					stylo.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk printing
		if (stylo.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			stylo.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!stylo.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (stylo.model.can_cancel(doctype) && !stylo.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (stylo.model.can_delete(doctype) && is_bulk_edit_allowed(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		if (!params.toString() && stylo.route_options) {
			params = new Map(Object.entries(stylo.route_options));
		}
		...
		params.forEach((value, field) => {
			...
			if (!doctype) {
				doctype = stylo.meta.get_doctype_for_field(this.doctype, field);
			}
...
stylo.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return stylo.views.list_view[route];
};
