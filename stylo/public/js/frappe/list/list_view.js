...
stylo.views.ListView = class ListView extends stylo.views.BaseList {
	...
	has_permissions() {
		const can_read = stylo.perm.has_perm(this.doctype, 0, "read");
		return can_read;
	}
	...
	get_no_result_message() {
		...
		let empty_state_image =
			...
			"/assets/stylo/images/ui-states/list-empty-state.svg";
	...
	get_header_html() {
		...
		let subject_html = `
			...
			<span class="level-item list-liked-by-me hidden-xs">
				<span title="${__("Likes")}">${stylo.utils.icon("heart", "sm", "like-icon")}</span>
			</span>
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments">
					${stylo.avatar_group(assigned_users, 3, { filterable: true })[0].outerHTML}
				</div>`;
		}

		const comment_count = `<span class="comment-count">
				${stylo.utils.icon("small-message")}
	...
	get_seen_class(doc) {
		return JSON.parse(doc._seen || "[]").includes(stylo.session.user) ? "" : "bold";
	}

	get_like_html(doc) {
		const liked_by = JSON.parse(doc._liked_by || "[]");
		const heart_class = liked_by.includes(stylo.session.user)
			? "liked-by liked"
			: "not-liked";
		...
		div.innerHTML = `
			<span class="like-action ${heart_class}">
				${stylo.utils.icon("heart", "sm", "like-icon")}
			</span>
	...
	get_subject_html(doc, title) {
		...
		let textValue = stylo.utils.html2text(value);
	...
	setup_like() {
		this.$result.on("click", ".like-action", stylo.ui.click_toggle_like);
	...
	setup_realtime_updates() {
		...
		stylo.socketio.doctype_subscribe(this.doctype);
	...
	disable_realtime_updates() {
		stylo.socketio.doctype_unsubscribe(this.doctype);
	...
	get_menu_items() {
		...
		if (stylo.model.can_set_user_permissions(doctype)) {
			items.push({
		...
		if (stylo.user.has_role("System Manager") && stylo.boot.developer_mode === 1) {
			// edit doctype
			items.push({
	...
	get_actions_menu_items() {
		...
		// bulk delete
		if (stylo.model.can_delete(doctype) && !stylo.model.has_workflow(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		for (let field in stylo.route_options) {
			let doctype = null;
			let value = stylo.route_options[field];
