...
stylo.views.ListSidebar = class ListSidebar {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		var sidebar_content = stylo.render_template("list_sidebar", { doctype: this.doctype });
	...
	setup_views() {
		...
		if (this.doctype === "Communication" && stylo.boot.email_accounts.length) {
			this.sidebar.find('.list-link[data-view="Inbox"]').removeClass("hide");
			show_list_link = true;
		}

		if (stylo.treeview_settings[this.doctype] || stylo.get_meta(this.doctype).is_tree) {
			this.sidebar.find(".tree-link").removeClass("hide");
		}

		this.current_view = "List";
		var route = stylo.get_route();
		if (route.length > 2 && stylo.views.view_modes.includes(route[2])) {
	...
	setup_kanban_boards() {
		...
		stylo.views.KanbanView.setup_dropdown_in_sidebar(this.doctype, $dropdown);
	}

	setup_keyboard_shortcuts() {
		this.sidebar.find(".list-link > a, .list-link > .btn-group > a").each((i, el) => {
			stylo.ui.keys.get_shortcut_group(this.page).add($(el));
		});
	}

	setup_list_group_by() {
		this.list_group_by = new stylo.views.ListGroupBy({
			doctype: this.doctype,
			sidebar: this,
			list_view: this.list_view,
			page: this.page,
	...
	get_stats() {
		...
		stylo.call({
			method: "stylo.desk.reportview.get_sidebar_stats",
			type: "GET",
			args: {
				stats: me.stats,
				doctype: me.doctype,
			...
			callback: function (r) {
				...
				stylo.utils.setup_search(stats_dropdown, ".stat-link", ".stat-label");
			},
		});
	}
	...
	render_stat(stats) {
		...
		let tag_list = $(stylo.render_template("list_sidebar_stat", args)).on(
			"click",
			".stat-link",
			(e) => {
	...
	add_insights_banner() {
		...
		const link = "https://stylo.io/s/insights";
		const cta = "Stylo Insights";
	...
	add_crm_banner() {
		...
		const message = "";
		const link =
			"https://stylo.io/crm?utm_source=crm-sidebar&utm_medium=sidebar&utm_campaign=stylo-ad";
		const cta = __("Switch to Stylo CRM for smarter sales");
		this.add_banner(message, link, cta);
	}
	...
	add_helpdesk_banner() {
		...
		const message = "";
		const link =
			"https://stylo.io/helpdesk?utm_source=support-sidebar&utm_medium=sidebar&utm_campaign=stylo-ad";
		const cta = __("Upgrade your support experience with Stylo Helpdesk");
		this.add_banner(message, link, cta);
	}
};
