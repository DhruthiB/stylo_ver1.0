...
stylo.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = stylo.utils.to_title_case(route[2] || "");
		if (route.length > 2 && stylo.views.view_modes.includes(view_name)) {
	...
	set_route(view, calendar_name) {
		...
		if (search_params) {
			stylo.route_options = Object.fromEntries(search_params);
		}
	...
	setup_views() {
		const views = {
			...
			Calendar: {
				condition: stylo.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && stylo.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(stylo.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => stylo.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					stylo.treeview_settings[this.doctype] ||
					stylo.get_meta(this.doctype).is_tree,
		...
		stylo.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => stylo.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return stylo.utils.to_title_case(stylo.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(stylo.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			stylo.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						stylo.set_route("list", doctype, "kanban", board.name);
					} else {
						stylo.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			stylo.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			stylo.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					stylo.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return stylo.db
			...
			.then((result) => {
				...
				if (stylo.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = stylo.boot.email_accounts;
	...
	slug() {
		return stylo.router.slug(stylo.router.doctype_layout || this.doctype);
	}
};
