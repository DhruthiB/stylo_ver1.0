...
stylo.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || stylo.router.doctype_layout || __(this.doctype);
		this.meta = stylo.get_meta(this.doctype);
		this.settings = stylo.listview_settings[this.doctype] || {};
		this.user_settings = stylo.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = stylo.is_large_screen() ? 100 : 20;
		this.selected_page_count = this.page_length;
		this.data = [];
		this.method = "stylo.desk.reportview.get";

		this.can_create = stylo.model.can_create(this.doctype);
		this.can_write = stylo.model.can_write(this.doctype);
	...
	get_list_view_settings() {
		return stylo
			.call("stylo.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(stylo.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			stylo.model.std_fields_list.includes(fieldname) ||
			stylo.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = stylo.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!stylo.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return stylo.model.with_doctype(this.doctype);
	}
	...
	setup_view_menu() {
		if (stylo.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			const icon_map = {
				Image: "image-view",
				List: "list",
				Report: "small-file",
			...
			this.views_list = new stylo.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		stylo.breadcrumbs.add(this.meta.module, this.doctype);
	}
	...
	setup_side_bar() {
		...
		this.list_sidebar = new stylo.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$stylo_list = $('<div class="stylo-list">').appendTo(this.page.main);
	}
	 ...
	 * to the `stylo_list` container. This container is used to create a scrollable area for the result content.
	 */
	setup_result_container_area() {
		if (this.view == "List") {
			this.$stylo_list.append($(`<div class="result-container">`));
		}
	}

	setup_result_area() {
		this.$result = $(`<div class="result">`);
		let stylo_list = this.$stylo_list;
		if (this.view == "List") {
			stylo_list = this.$stylo_list.find(".result-container");
		}
		stylo_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$stylo_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$stylo_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$stylo_list.append(this.$paging_area);
	...
	set_result_height() {
		...
		this.$result.parent(".result-container").css({
			height: resultContainerHeight - (stylo.is_mobile() ? 100 : 0) + "px",
		});
	...
	get_fields() {
		...
		return this.fields.map((f) => stylo.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return stylo.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	prepare_data(r) {
		...
		Object.assign(stylo.boot.user_info, data.user_info);
		delete data.user_info;

		data = !Array.isArray(data) ? stylo.utils.dict(data.keys, data.values) : data;
...
class FilterArea {
	constructor(list_view) {
		...
		this.debounced_refresh_list_view = stylo.utils.debounce(
			this.refresh_list_view.bind(this),
			300
		);
		this.setup();
		if (stylo.is_mobile()) this.setup_mobile(list_view);
	}
	...
	clear(refresh = true) {
		...
		return stylo.run_serially(promises).then(() => {
			this.trigger_refresh = true;
				...
				.filter(
					(df) =>
						(df.fieldname === title_field ||
							(df.in_standard_filter && stylo.model.is_value_type(df.fieldtype))) &&
						stylo.perm.has_perm(this.list_view.doctype, df.permlevel)
				)
				.map((df) => {
					...
					if (
						df.fieldtype == "Link" &&
						df.options &&
						stylo.boot.treeviews.includes(df.options)
					) {
						condition = "descendants of (inclusive)";
					}
	...
	make_filter_list() {
		$(`<div class="filter-selector">
			<div class="btn-group">
				<button class="btn btn-default btn-sm filter-button">
					<span class="filter-icon button-icon">
						${stylo.utils.icon("es-line-filter")}
					</span>
				...
				<button class="btn btn-default btn-sm filter-x-button" title="${__("Clear all filters")}">
					<span class="filter-icon button-icon">
						${stylo.utils.icon("es-small-close")}
					</span>
				</button>
			</div>
		...
		this.filter_list = new stylo.ui.FilterGroup({
...
stylo.views.is_valid = (view_mode) => stylo.views.view_modes.includes(view_mode);
