...
$(window).on("hashchange", function (e) {
	...
	if (window.location.hash && !stylo.router.is_app_route(e.currentTarget.pathname)) {
		let sub_path = stylo.router.get_sub_path(window.location.hash);
		stylo.router.push_state(sub_path);
		return false;
	}
});
...
stylo.router = {
	...
	async convert_to_standard_route(route) {
		...
		if (stylo.workspaces[route[0]]) {
			// public workspace
			route = ["Workspaces", stylo.workspaces[route[0]].title];
		} else if (route[0] == "private") {
			// private workspace
			if (!stylo.workspaces[private_workspace] && localStorage.new_workspace) {
				let new_workspace = JSON.parse(localStorage.new_workspace);
				if (stylo.router.slug(new_workspace.title) === route[1]) {
					stylo.workspaces[private_workspace] = new_workspace;
				}
			}
			...
			route = ["Workspaces", "private", stylo.workspaces[private_workspace].title];
	...
	make_url(params) {
		...
		let private_home = stylo.workspaces[`home-${stylo.user.name.toLowerCase()}`];

		let workspace_name = private_home || stylo.workspaces["home"] ? "home" : "";
		let is_private = !!private_home;
		let first_workspace = Object.keys(stylo.workspaces)[0];

		if (!workspace_name && first_workspace) {
			workspace_name = stylo.workspaces[first_workspace].title;
			is_private = !stylo.workspaces[first_workspace].public;
		}

		let default_page = (is_private ? "private/" : "") + stylo.router.slug(workspace_name);
