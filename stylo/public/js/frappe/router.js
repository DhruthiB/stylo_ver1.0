...
stylo.re_route = { "#login": "" };
stylo.route_titles = {};
stylo.route_flags = {};
stylo.route_history = [];
stylo.view_factory = {};
stylo.view_factories = [];
stylo.route_options = null;
stylo.open_in_new_tab = false;
stylo.route_hooks = {};
...
$("body").on("click", "a", function (e) {
	...
	if (stylo.router.show_external_link_warning_if_needed(target_element)) {
		e.preventDefault();
	...
	if (stylo.router.is_app_route(target_element.pathname)) {
		// target has "/app, this is a v2 style route.
		if (target_element.search) {
			stylo.route_options = {};
			let params = new URLSearchParams(target_element.search);
			for (const [key, value] of params) {
				stylo.route_options[key] = value;
			}
		}
		if (target_element.hash) {
			stylo.route_hash = target_element.hash;
		}
...
stylo.router = {
	...
	setup() {
		...
		for (let doctype of stylo.boot.user.can_read) {
			this.routes[this.slug(doctype)] = { doctype: doctype };
		}
		if (stylo.boot.doctype_layouts) {
			for (let doctype_layout of stylo.boot.doctype_layouts) {
	...
	async route() {
		...
		if (stylo.boot.setup_complete) {
			!stylo.re_route["setup-wizard"] && (stylo.re_route["setup-wizard"] = "app");
		} else if (!sub_path.startsWith("setup-wizard")) {
			stylo.re_route["setup-wizard"] && delete stylo.re_route["setup-wizard"];
			stylo.set_route(["setup-wizard"]);
		}
	...
	async convert_to_standard_route(route) {
		...
		if (stylo.workspaces[route[0]]) {
			// public workspace
			route = ["Workspaces", stylo.workspaces[route[0]].name];
		} else if (route[0] == "private") {
			// private workspace
			let private_workspace = route[1] && `${route[1]}-${stylo.user.name.toLowerCase()}`;
			if (!stylo.workspaces[private_workspace]) {
				stylo.msgprint(
					__("Workspace <b>{0}</b> does not exist", [
						stylo.utils.xss_sanitise(route[1]),
					])
				);
				return ["Workspaces"];
			}
			route = ["Workspaces", "private", stylo.workspaces[private_workspace].name];
	...
	set_doctype_route(route) {
		...
		return stylo.model.with_doctype(doctype_route.doctype).then(() => {
			// doctype route
			let meta = stylo.get_meta(doctype_route.doctype);
			...
			} else if (stylo.model.is_single(doctype_route.doctype)) {
	...
	set_history() {
		stylo.route_history.push(this.current_route);
		stylo.ui.hide_open_dialog();
	},

	async set_active_sidebar_item() {
		stylo.app.sidebar.set_active_workspace_item();
	},
	...
	render_page() {
		...
		const factory = stylo.utils.to_title_case(route[0]);

		if (route[1] && stylo.views[factory + "Factory"]) {
			route[0] = factory;
			// has a view generator, generate!
			if (!stylo.view_factory[factory]) {
				stylo.view_factory[factory] = new stylo.views[factory + "Factory"]();
			}

			stylo.view_factory[factory].show();
		} else {
			// show page
			const route_name = stylo.utils.xss_sanitise(route[0]);
			if (stylo.views.pageview) {
				stylo.views.pageview.show(route_name);
			}
		}
	},

	re_route(sub_path) {
		if (stylo.re_route[sub_path] !== undefined) {
			...
			const re_route_val = this.get_sub_path(stylo.re_route[sub_path]);
			if (re_route_val === this.current_sub_path) {
				window.history.back();
			} else {
				stylo.set_route(re_route_val);
			}
	...
	set_title(sub_path) {
		if (stylo.route_titles[sub_path]) {
			stylo.utils.set_title(stylo.route_titles[sub_path]);
		}
	},

	set_route() {
		// set the route (push state) with given arguments
		// example 1: stylo.set_route('a', 'b', 'c');
		// example 2: stylo.set_route(['a', 'b', 'c']);
		// example 3: stylo.set_route('a/b/c');
		...
		return new Promise((resolve) => {
			...
			sub_path += stylo.route_hash || "";
			stylo.route_hash = null;
			if (stylo.open_in_new_tab) {
				localStorage["route_options"] = JSON.stringify(stylo.route_options);
				window.open(sub_path, "_blank");
				stylo.open_in_new_tab = false;
			} else {
				try {
					const route_options = stylo.route_options || {};
	...
	make_url(params) {
		...
		let private_home = `home-${stylo.user.name.toLowerCase()}`;
		let default_workspace = stylo.router.slug(stylo.boot.user.default_workspace?.name || "");

		let workspace =
			stylo.workspaces[default_workspace] ||
			stylo.workspaces[private_home] ||
			stylo.workspaces["home"] ||
			Object.values(stylo.workspace_map).find((w) => w.app === stylo.current_app) ||
			Object.values(stylo.workspaces)[0];

		if (workspace) {
			return (
				"/app/" + (workspace.public ? "" : "private/") + stylo.router.slug(workspace.name)
			);
		}
	...
	push_state(path, query_params = "") {
		if (window.location.pathname !== path || window.location.search !== query_params) {
			...
			const method = stylo.route_flags.replace_route ? "replaceState" : "pushState";
	...
	set_route_options_from_url() {
		// set query parameters as stylo.route_options
		...
		if (localStorage.getItem("route_options")) {
			stylo.route_options = JSON.parse(localStorage.getItem("route_options"));
	...
	show_external_link_warning_if_needed(/** @type {HTMLAnchorElement} */ aElement) {
		try {
			...
			const showWarningWhen = stylo.boot.show_external_link_warning || "Never";
				...
				fields: [
					...
					},
					{
						...
						label: __("Do not warn me again about {0}", [
							stylo.utils.escape_html(hostname).bold(),
						]),
						default: 0,
			...
			const skipToken = stylo.utils.get_random(16);
...
stylo.get_route = () => stylo.router.current_route;
stylo.get_route_str = () => stylo.router.current_route.join("/");
stylo.set_route = function () {
	return stylo.router.set_route.apply(stylo.router, arguments);
};

stylo.get_prev_route = function () {
	if (stylo.route_history && stylo.route_history.length > 1) {
		return stylo.route_history[stylo.route_history.length - 2];
	} else {
		return [];
	}
};

stylo.set_re_route = function () {
	var tmp = stylo.router.get_sub_path();
	stylo.set_route.apply(null, arguments);
	stylo.re_route[tmp] = stylo.router.get_sub_path();
};

stylo.has_route_options = function () {
	return Boolean(Object.keys(stylo.route_options || {}).length);
};

stylo.utils.make_event_emitter(stylo.router);
