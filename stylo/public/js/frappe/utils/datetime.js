...
stylo.provide("stylo.datetime");

stylo.defaultDateFormat = "YYYY-MM-DD";
stylo.defaultTimeFormat = "HH:mm:ss";
stylo.defaultDatetimeFormat = stylo.defaultDateFormat + " " + stylo.defaultTimeFormat;
moment.defaultFormat = stylo.defaultDateFormat;

stylo.provide("stylo.datetime");

$.extend(stylo.datetime, {
	convert_to_user_tz: function (date, format) {
		...
		let date_obj = null;
		if (stylo.boot.time_zone && stylo.boot.time_zone.system && stylo.boot.time_zone.user) {
			date_obj = moment
				.tz(date, stylo.boot.time_zone.system)
				.clone()
				.tz(stylo.boot.time_zone.user);
		} else {
			date_obj = moment(date);
		}

		return format === false ? date_obj : date_obj.format(stylo.defaultDatetimeFormat);
	},
	...
	convert_to_system_tz: function (date, format) {
		...
		if (stylo.boot.time_zone && stylo.boot.time_zone.system && stylo.boot.time_zone.user) {
			date_obj = moment
				.tz(date, stylo.boot.time_zone.user)
				.clone()
				.tz(stylo.boot.time_zone.system);
		} else {
			date_obj = moment(date);
		}
	...
	is_system_time_zone: function () {
		if (stylo.boot.time_zone && stylo.boot.time_zone.system && stylo.boot.time_zone.user) {
			return (
				moment().tz(stylo.boot.time_zone.system).utcOffset() ===
				moment().tz(stylo.boot.time_zone.user).utcOffset()
			);
		}
	...
	is_timezone_same: function () {
		return stylo.datetime.is_system_time_zone();
	},

	str_to_obj: function (d) {
		return moment(d, stylo.defaultDatetimeFormat)._d;
	},
	...
	obj_to_user: function (d) {
		return moment(d).format(stylo.datetime.get_user_date_fmt().toUpperCase());
	},
	...
	get_user_time_fmt: function () {
		return (stylo.sys_defaults && stylo.sys_defaults.time_format) || "HH:mm:ss";
	},

	get_user_date_fmt: function () {
		return (stylo.sys_defaults && stylo.sys_defaults.date_format) || "yyyy-mm-dd";
	},
	...
	str_to_user: function (val, only_time = false, only_date = false) {
		...
		const user_date_fmt = stylo.datetime.get_user_date_fmt().toUpperCase();
		const user_time_fmt = stylo.datetime.get_user_time_fmt();

		if (only_time) {
			let date_obj = moment(val, stylo.defaultTimeFormat);
			return date_obj.format(user_time_fmt);
		} else if (only_date || (typeof val === "string" && val.indexOf(" ") === -1)) {
			let date_obj = moment(val, stylo.defaultDateFormat);
			return date_obj.format(user_date_fmt);
		} else {
			const system_datetime = moment.tz(
				val,
				stylo.defaultDatetimeFormat,
				stylo.boot.time_zone.system
			);
			const user_datetime = system_datetime.clone().tz(stylo.boot.time_zone.user);
	...
	user_to_str: function (val, only_time = false) {
		var user_time_fmt = stylo.datetime.get_user_time_fmt();
		if (only_time) {
			return moment(val, user_time_fmt).format(stylo.defaultTimeFormat);
		}

		var user_fmt = stylo.datetime.get_user_date_fmt().toUpperCase();
	...
	user_to_obj: function (d) {
		return stylo.datetime.str_to_obj(stylo.datetime.user_to_str(d));
	},
	...
	now_date: function (as_obj = false) {
		return stylo.datetime._date(stylo.defaultDateFormat, as_obj);
	},

	now_time: function (as_obj = false) {
		return stylo.datetime._date(stylo.defaultTimeFormat, as_obj);
	},

	now_datetime: function (as_obj = false) {
		return stylo.datetime._date(stylo.defaultDatetimeFormat, as_obj);
	},

	system_datetime: function (as_obj = false) {
		return stylo.datetime._date(stylo.defaultDatetimeFormat, as_obj, true);
	},

	_date: function (format, as_obj = false, system_time = false) {
		let time_zone = stylo.boot.time_zone?.system || stylo.sys_defaults.time_zone;
		...
		if (!system_time) {
			time_zone = stylo.boot.time_zone?.user || time_zone;
		}
		let date = moment.tz(time_zone);

		return as_obj ? stylo.datetime.moment_to_date_obj(date) : date.format(format);
	},
	...
	nowdate: function () {
		return stylo.datetime.now_date();
	},
	...
	validate: function (d) {
		return moment(
			d,
			[stylo.defaultDateFormat, stylo.defaultDatetimeFormat, stylo.defaultTimeFormat],
			true
		).isValid();
	},

	get_first_day_of_the_week_index() {
		const first_day_of_the_week = stylo.sys_defaults.first_day_of_the_week || "Sunday";
...
Object.defineProperties(window, {
	dateutil: {
		get: function () {
			console.warn(
				"Please use `stylo.datetime` instead of `dateutil`. It will be deprecated soon."
			);
			return stylo.datetime;
		},
		configurable: true,
	},
	date: {
		get: function () {
			console.warn(
				"Please use `stylo.datetime` instead of `date`. It will be deprecated soon."
			);
			return stylo.datetime;
		},
		configurable: true,
	...
	get_today: {
		get: function () {
			console.warn(
				"Please use `stylo.datetime.get_today` instead of `get_today`. It will be deprecated soon."
			);
			return stylo.datetime.get_today;
		},
		configurable: true,
	},
});
