...
stylo.provide("stylo.tools");

stylo.tools.downloadify = function (data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		stylo.msgprint(
			__("Export not allowed. You need {0} role to export.", [stylo.utils.comma_or(roles)])
		);
		return;
	}

	var filename = title + ".csv";
	var csv_data = stylo.tools.to_csv(data);
...
stylo.markdown = function (txt) {
	if (!stylo.md2html) {
		stylo.md2html = new showdown.Converter({ tables: true });
	}
	...
	return stylo.md2html.makeHtml(txt);
};

stylo.tools.to_csv = function (data) {
	var res = [];
