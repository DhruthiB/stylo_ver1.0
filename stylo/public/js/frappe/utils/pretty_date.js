function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = stylo.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff = (new Date(stylo.datetime.now_datetime()).getTime() - date.getTime()) / 1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = stylo.datetime.str_to_user ? stylo.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="stylo-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
stylo.datetime.comment_when = comment_when;
stylo.datetime.prettyDate = prettyDate;

stylo.datetime.refresh_when = function () {
	if (jQuery) {
		$(".stylo-timestamp").each(function () {
