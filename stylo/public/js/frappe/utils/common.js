...
stylo.avatar = function (
	...
	if (user) {
		// desk
		user_info = stylo.user_info(user);
	} else {
		// website
		let full_name = title || stylo.get_cookie("full_name");
		user_info = {
			image: image_url === null ? stylo.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: stylo.get_abbr(full_name),
			color: stylo.get_palette(full_name),
		};
	}
...
stylo.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let abbr = stylo.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = stylo.get_palette(title);
...
stylo.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			stylo.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => stylo.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${stylo.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
stylo.ui.scroll = function (element, animate, additional_offset) {
...
stylo.get_palette = function (txt) {
	if (!txt) return stylo.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return stylo.palette[idx % 8];
};

stylo.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
stylo.gravatars = {};
stylo.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!stylo.gravatars[email_id]) {
		// TODO: check if gravatar exists
		stylo.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return stylo.gravatars[email_id];
};
...
stylo.get_cookie = function getCookie(name) {
	return stylo.get_cookies()[name];
};

stylo.get_cookies = function getCookies() {
...
stylo.is_mobile = function () {
	return $(document).width() < 768;
};

stylo.is_large_screen = function () {
	return $(document).height() > 1180;
};

stylo.utils.xss_sanitise = function (string, options) {
...
stylo.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = stylo.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(stylo.utils.xss_sanitise(url, { strategies: ["js"] }));
};

stylo.utils.strip_url = (url) => {
...
stylo.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
		...
		},
		{
			...
			label: __("Start Date"),
			reqd: 1,
			default: stylo.datetime.nowdate(),
		},
		{
			fieldname: "end_date",
			fieldtype: "Date",
	...
	stylo.prompt(
		...
		function (values) {
			stylo.call({
				method: "stylo.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
stylo.utils.get_page_view_count = function (route) {
	return stylo.call("stylo.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};
