...
window.format_number = function (v, format, decimals) {
	if (!format) {
		...
		if (decimals == null) decimals = cint(stylo.defaults.get_default("float_precision")) || 3;
	}
...
function format_currency(v, currency, decimals) {
	...
	const show_symbol_on_right =
		stylo.model.get_value(":Currency", currency, "symbol_on_right") ?? false;

	if (decimals === undefined) {
		decimals = stylo.boot.sysdefaults.currency_precision || null;
	}
...
function get_currency_symbol(currency) {
	if (stylo.boot) {
		if (stylo.boot.sysdefaults && stylo.boot.sysdefaults.hide_currency_symbol == "Yes")
			return null;

		if (!currency) currency = stylo.boot.sysdefaults.currency;

		return stylo.model.get_value(":Currency", currency, "symbol") || currency;
	} else {
		// load in template
		return stylo.currency_symbols[currency];
	}
}

function get_number_format(currency) {
	let sysdefaults = stylo?.boot?.sysdefaults;
	return (
		(cint(sysdefaults?.use_number_format_from_currency) &&
			currency &&
			stylo.model.get_value(":Currency", currency, "number_format")) ||
...
function get_number_format_info(format) {
	var info = stylo.number_format_info[format];
...
function _round(num, precision, rounding_method) {
	rounding_method =
		rounding_method || stylo.boot.sysdefaults.rounding_method || "Banker's Rounding (legacy)";
...
function precision(fieldname, doc) {
	if (cur_frm) {
		...
		var df = stylo.meta.get_docfield(doc.doctype, fieldname, doc.parent || doc.name);
		if (!df) console.log(fieldname + ": could not find docfield in method precision()");
		return stylo.meta.get_field_precision(df, doc);
	} else {
		return stylo.boot.sysdefaults.float_precision;
	}
}
...
function round_based_on_smallest_currency_fraction(value, currency, precision) {
	var smallest_currency_fraction_value = flt(
		stylo.model.get_value(":Currency", currency, "smallest_currency_fraction_value")
	);
