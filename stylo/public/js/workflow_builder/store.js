...
export const useStore = defineStore("workflow-builder-store", () => {
	...
	async function fetch() {
		await stylo.model.clear_doc("Workflow", workflow_name.value);
		await stylo.model.with_doc("Workflow", workflow_name.value);

		workflow_doc.value = stylo.get_doc("Workflow", workflow_name.value);
		await stylo.model.with_doctype(workflow_doc.value.document_type);

		if (!workflowfields.value.length) {
			await stylo.model.with_doctype("Workflow");
			workflowfields.value = stylo.get_meta("Workflow").fields;
		}

		if (!statefields.value.length) {
			await stylo.model.with_doctype("Workflow Document State");
			statefields.value = stylo.get_meta("Workflow Document State").fields;
		}

		if (!transitionfields.value.length) {
			await stylo.model.with_doctype("Workflow Transition");
			transitionfields.value = stylo.get_meta("Workflow Transition").fields;
		}

		if (!workflow_doc_fields.value.length) {
			let doc_type = workflow_doc.value.document_type;
			await stylo.model.with_doctype(doc_type);
			workflow_doc_fields.value = stylo.meta
	...
	async function save_changes() {
		...
		try {
			...
			await stylo.call("stylo.client.save", { doc });
			stylo.toast(__("Workflow updated successfully"));
			fetch();
		} catch (e) {
			console.error(e);
		} finally {
	...
	function setup_breadcrumbs() {
		...
		`;
		stylo.breadcrumbs.clear();
		stylo.breadcrumbs.$breadcrumbs.append(breadcrumbs);
	}
