stylo.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new stylo.Driver({
			className: "stylo-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				stylo.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!stylo.boot.user.onboarding_status[this.tour.name] &&
								(stylo.boot.user.onboarding_status[this.tour.name] = {});
							stylo.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								stylo.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							stylo.call({
								method: "stylo.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(stylo.boot.user.onboarding_status),
				...
				if ($input) stylo.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await stylo.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: stylo.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
stylo.ui.init_onboarding_tour = () => {
	...
	typeof stylo.boot.onboarding_tours == "undefined" && stylo.boot.onboarding_tours == [];
	typeof stylo.boot.user.onboarding_status == "undefined" &&
		stylo.boot.user.onboarding_status == {};
	let route = stylo.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		stylo.boot.onboarding_tours &&
			stylo.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (stylo.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (stylo.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(stylo.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == stylo.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == stylo.ui.next_form_tour);
	if (current_tour) {
		...
		stylo.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (stylo.ui.currentTourInstance?.driver) {
		stylo.ui.currentTourInstance.driver_steps = [];
		stylo.ui.currentTourInstance.driver.reset(true);
		stylo.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (stylo.ui.currentTourInstance = new stylo.ui.OnboardingTour());
