...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = stylo.boot.time_zone
		? stylo.boot.time_zone.user
		: stylo.sys_defaults.time_zone;
...
<template>
	<div class="control stylo-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="stylo.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="stylo.utils.icon('down', 'sm')"
		/>
	</div>
</template>
