"""
Stylo Testing Module

This module provides a comprehensive framework for running tests in Stylo applications.
...
- TestRunner: Main class for running test suites with additional Stylo-specific functionality
- TestResult: Custom test result class for improved output formatting and logging
- discover_all_tests: Function to discover all tests in specified Stylo apps
...
- Integration with Stylo's hooks and test record creation system

Usage:
This module is typically used by Stylo's CLI commands for running tests, but can also
be used programmatically for custom test execution scenarios.

Example:
    from stylo.testing import TestConfig, TestRunner, discover_all_tests
...
from stylo.utils.logger import create_handler as createStyloFileHandler
...
LOGGING_CONFIG = {
	...
	"loggers": {
		f"{__name__}": {
			"handlers": [],  # only log to the stylo handler
			"propagate": False,
		},
	},
}
...
handlers = createStyloFileHandler(__name__)
