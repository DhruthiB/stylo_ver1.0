...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the Stylo framework. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The Stylo version at the beginning of which it becomes an error and at the end of which it will be removed (its "graduation" to the great codebase in the sky)
...
# see also stylo.__init__ enabling them when a dev_server
class StyloDeprecationError(Warning):
...
class StyloDeprecationWarning(Warning):
	"""Deprecated feature in next version"""


class PendingStyloDeprecationWarning(StyloDeprecationWarning):
...
warnings.simplefilter("error", StyloDeprecationError)
warnings.simplefilter("ignore", PendingStyloDeprecationWarning)


class V15StyloDeprecationWarning(StyloDeprecationError):
	pass


class V16StyloDeprecationWarning(StyloDeprecationWarning):
	pass


class V17StyloDeprecationWarning(PendingStyloDeprecationWarning):
	pass
...
def __get_deprecation_class(graduation: str | None = None, class_name: str | None = None) -> type:
	if graduation:
		...
		class_name = f"{cleaned_graduation}StyloDeprecationWarning"
		current_module = sys.modules[__name__]
	try:
		return getattr(current_module, class_name)
	except AttributeError:
		return PendingStyloDeprecationWarning
...
for warning_filter in pythonwarnings.split(","):
	...
	if len(parts) >= 3 and (
		parts[2] in ("StyloDeprecationError", "StyloDeprecationWarning", "PendingStyloDeprecationWarning")
		or parts[2].endswith("StyloDeprecationWarning")
	):
		try:
...
try:
	...
	def _deprecated(message: str, category=StyloDeprecationWarning, stacklevel=1) -> Callable[[T], T]:
...
def deprecated(original: str, marked: str, graduation: str, msg: str, stacklevel: int = 1):
	...
	Arguments:
	        - original: stylo.utils.make_esc  (fully qualified)
	...
	def decorator(func):
		...
		if os.path.basename(caller_filepath) != "deprecation_dumpster.py":
			raise RuntimeError(
				...
				+ colorize("stylo/deprecation_dumpster.py\n", Color.CYAN)
				+ colorize("Move the entire function there and import it back via adding\n ", Color.YELLOW)
				+ colorize(f"from stylo.deprecation_dumpster import {func.__name__}\n", Color.CYAN)
				+ colorize("to file\n ", Color.YELLOW)
...
def _old_deprecated(func):
	return deprecated(
		"stylo.deprecations.deprecated",
		"2024-09-13",
		"v17",
		"Make use of the stylo/deprecation_dumpster.py file, instead. 🎉🗑️",
	)(_deprecated("")(func))


def _old_deprecation_warning(msg):
	@deprecated(
		"stylo.deprecations.deprecation_warning",
		"2024-09-13",
		"v17",
		"Use stylo.deprecation_dumpster.deprecation_warning, instead. 🎉🗑️",
	)
...
@deprecated("stylo.utils.make_esc", "unknown", "v17", "Not used anymore.")
...
@deprecated(
	"stylo.db.is_column_missing",
	"unknown",
	"v17",
	"Renamed to stylo.db.is_missing_column.",
)
...
@deprecated(
	"stylo.desk.doctype.bulk_update.bulk_update",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated(
	"stylo.utils.print_format.read_multi_pdf",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated("stylo.gzip_compress", "unknown", "v17", "Use py3 methods directly (this was compat for py2).")
...
@deprecated("stylo.gzip_decompress", "unknown", "v17", "Use py3 methods directly (this was compat for py2).")
...
@deprecated(
	"stylo.email.doctype.email_queue.email_queue.send_mail",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated(
	"stylo.geo.country_info.get_translated_dict",
	"unknown",
	"v17",
	"Use stylo.geo.country_info.get_translated_countries, instead.",
)
def get_translated_dict():
	from stylo.geo.country_info import get_translated_countries
...
@deprecated("stylo.tests_runner.get_modules", "2024-20-08", "v17", "use stylo.tests.utils.get_modules")
def test_runner_get_modules(doctype):
	from stylo.tests.utils import get_modules

	return get_modules(doctype)


@deprecated(
	"stylo.tests_runner.make_test_records", "2024-20-08", "v17", "use stylo.tests.utils.make_test_records"
)
def test_runner_make_test_records(*args, **kwargs):
	from stylo.tests.utils import make_test_records

	return make_test_records(*args, **kwargs)


@deprecated(
	"stylo.tests_runner.make_test_objects", "2024-20-08", "v17", "use stylo.tests.utils.make_test_objects"
)
def test_runner_make_test_objects(*args, **kwargs):
	from stylo.tests.utils import make_test_objects

	return make_test_objects(*args, **kwargs)


@deprecated(
	"stylo.tests_runner.make_test_records_for_doctype",
	"2024-20-08",
	"v17",
	"use stylo.tests.utils.make_test_records_for_doctype",
)
def test_runner_make_test_records_for_doctype(*args, **kwargs):
	from stylo.tests.utils import make_test_records_for_doctype

	return make_test_records_for_doctype(*args, **kwargs)


@deprecated(
	"stylo.tests_runner.print_mandatory_fields",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_print_mandatory_fields(*args, **kwargs):
	from stylo.tests.utils.generators import print_mandatory_fields

	return print_mandatory_fields(*args, **kwargs)


@deprecated(
	"stylo.tests_runner.get_test_record_log",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_get_test_record_log(doctype):
	from stylo.tests.utils.generators import TestRecordManager

	return TestRecordManager().get(doctype)


@deprecated(
	"stylo.tests_runner.add_to_test_record_log",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
...
@deprecated(
	"stylo.tests_runner.main",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_main(*args, **kwargs):
	from stylo.commands.testing import main

	return main(*args, **kwargs)


@deprecated(
	"stylo.tests_runner.xmlrunner_wrapper",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_xmlrunner_wrapper(output):
	...
	try:
		...
		print("$ forge setup requirements --dev")
		raise
...
@deprecated(
	"stylo.tests.upate_system_settings",
	"2024-20-08",
	"v17",
...
@deprecated(
	"stylo.tests.get_system_setting",
	"2024-20-08",
	"v17",
	"use `stylo.db.get_single_value('System Settings', key)`",
)
...
@deprecated(
	"stylo.tests.utils.change_settings",
	"2024-20-08",
	"v17",
	"use `stylo.tests.change_settings` or the cls.change_settings",
)
def tests_change_settings(*args, **kwargs):
	from stylo.tests.classes.context_managers import change_settings

	return change_settings(*args, **kwargs)


@deprecated(
	"stylo.tests.utils.patch_hooks",
	"2024-20-08",
	"v17",
	"use `stylo.tests.patch_hooks` or the cls.patch_hooks",
)
def tests_patch_hooks(*args, **kwargs):
	from stylo.tests.classes.context_managers import patch_hooks

	return patch_hooks(*args, **kwargs)


@deprecated(
	"stylo.tests.utils.debug_on",
	"2024-20-08",
	"v17",
	"use `stylo.tests.debug_on` or the cls.debug_on",
)
def tests_debug_on(*args, **kwargs):
	from stylo.tests.classes.context_managers import debug_on

	return debug_on(*args, **kwargs)


@deprecated(
	"stylo.tests.utils.timeout",
	"2024-20-08",
	"v17",
	"use `stylo.tests.timeout` or the cls.timeout",
)
def tests_timeout(*args, **kwargs):
	from stylo.tests.classes.context_managers import timeout

	return timeout(*args, **kwargs)


def get_tests_CompatStyloTestCase():
	...
	def _rollback_db():
		stylo.db.value_cache.clear()
		stylo.db.rollback()

	def _restore_thread_locals(flags):
		stylo.local.flags = flags
		stylo.local.error_log = []
		stylo.local.message_log = []
		stylo.local.debug_log = []
		stylo.local.conf = stylo._dict(stylo.get_site_config())
		stylo.local.cache = {}
		stylo.local.lang = "en"
		stylo.local.preload_assets = {"style": [], "script": [], "icons": []}

		if hasattr(stylo.local, "request"):
			delattr(stylo.local, "request")

	class StyloTestCase(unittest.TestCase):
		"""Base test class for Stylo tests.
		...
		@deprecated(
			"stylo.tests.utils.StyloTestCase",
			"2024-20-08",
			"v17",
			"Import `stylo.tests.UnitTestCase` or `stylo.tests.IntegrationTestCase` respectively instead of `stylo.tests.utils.StyloTestCase` - also see wiki for more info: https://github.com/stylo/stylo/wiki#testing-guide",
		)
		...
		def setUpClass(cls) -> None:
			cls.TEST_SITE = getattr(stylo.local, "site", None) or cls.TEST_SITE
			stylo.init(cls.TEST_SITE)
			cls.ADMIN_PASSWORD = stylo.get_conf(cls.TEST_SITE).admin_password
			cls._primary_connection = stylo.local.db
			cls._secondary_connection = None
			# flush changes done so far to avoid flake
			stylo.db.commit()  # nosemgrep
			if cls.SHOW_TRANSACTION_COMMIT_WARNINGS:
				stylo.db.before_commit.add(_commit_watcher)

			# enqueue teardown actions (executed in LIFO order)
			cls.addClassCleanup(_restore_thread_locals, copy.deepcopy(stylo.local.flags))
			cls.addClassCleanup(_rollback_db)

			return super().setUpClass()

		def _apply_debug_decorator(self, exceptions=()):
			from stylo.tests.utils import debug_on
		...
		# --- Stylo Framework specific assertions
		...
		def primary_connection(self):
			...
			try:
				current_conn = stylo.local.db
				stylo.local.db = self._primary_connection
				yield
			finally:
				stylo.local.db = current_conn

		@contextmanager
		def secondary_connection(self):
			"""Switch to secondary DB connection."""
			if self._secondary_connection is None:
				stylo.connect()  # get second connection
				self._secondary_connection = stylo.local.db

			try:
				current_conn = stylo.local.db
				stylo.local.db = self._secondary_connection
				yield
			finally:
				stylo.local.db = current_conn
		...
		def assertQueryCount(self, count):
			...
			try:
				orig_sql = stylo.db.__class__.sql
				stylo.db.__class__.sql = _sql_with_count
				yield
				self.assertLessEqual(len(queries), count, msg="Queries executed: \n" + "\n\n".join(queries))
			finally:
				stylo.db.__class__.sql = orig_sql
		...
		def assertRedisCallCounts(self, count):
			...
			try:
				orig_execute = stylo.cache.execute_command
				stylo.cache.execute_command = execute_command_and_count
				yield
				self.assertLessEqual(
					len(commands), count, msg="commands executed: \n" + "\n".join(str(c) for c in commands)
				)
			finally:
				stylo.cache.execute_command = orig_execute
		...
		def assertRowsRead(self, count):
			...
			def _sql_with_count(*args, **kwargs):
				...
				rows_read += cint(stylo.db._cursor.rowcount)
				return ret

			try:
				orig_sql = stylo.db.sql
				stylo.db.sql = _sql_with_count
				yield
				self.assertLessEqual(rows_read, count, msg="Queries read more rows than expected")
			finally:
				stylo.db.sql = orig_sql
		...
		def enable_safe_exec(cls) -> None:
			...
			from stylo.utils.safe_exec import SAFE_EXEC_CONFIG_KEY

			cls._common_conf = os.path.join(stylo.local.sites_path, "common_site_config.json")
		...
		def set_user(self, user: str):
			try:
				old_user = stylo.session.user
				stylo.set_user(user)
				yield
			finally:
				stylo.set_user(old_user)
		...
		def switch_site(self, site: str):
			...
			try:
				old_site = stylo.local.site
				stylo.init(site, force=True)
				stylo.connect()
				yield
			finally:
				stylo.init(old_site, force=True)
				stylo.connect()
		...
		def freeze_time(self, time_to_freeze, is_utc=False, *args, **kwargs):
			...
			from stylo.utils.data import convert_utc_to_timezone, get_datetime, get_system_timezone
	...
	return StyloTestCase


# remove alongside get_tests_CompatStyloTestCase
def get_compat_stylo_test_case_preparation(cfg):
	import unittest

	import stylo
	from stylo.testing.environment import IntegrationTestPreparation

	class StyloTestCasePreparation(IntegrationTestPreparation):
		def __call__(self, suite: unittest.TestSuite, app: str, category: str) -> None:
			...
			for path, folders, files in os.walk(stylo.get_app_path(app)):
				...
				for filename in files:
					if filename.startswith("test_") and filename.endswith(".py"):
						...
						module = stylo.get_module(module_name)
						candidates.append((module, path, filename))
			compat_preload_test_records_upfront(candidates)

	return StyloTestCasePreparation(cfg)


@deprecated(
	"stylo.model.trace.traced_field_context",
	"2024-20-08",
	"v17",
	"use `cls.trace_fields`",
)
def model_trace_traced_field_context(*args, **kwargs):
	from stylo.tests.classes.context_managers import trace_fields

	return trace_fields(*args, **kwargs)


@deprecated(
	"stylo.tests.utils.get_dependencies",
	"2024-20-09",
	"v17",
	"refactor to use stylo.tests.utils.get_missing_records_doctypes",
)
def tests_utils_get_dependencies(doctype):
	"""Get the dependencies for the specified doctype"""
	import stylo
	from stylo.tests.utils.generators import get_modules
	...
	for df in meta.get_table_fields():
		link_fields.extend(stylo.get_meta(df.options).get_link_fields())
...
@deprecated(
	"stylo.tests_runner.get_dependencies",
	"2024-20-08",
	"v17",
	"refactor to use stylo.tests.utils.get_missing_record_doctypes",
)
...
@deprecated(
	"stylo.get_test_records",
	"2024-20-09",
	"v17",
	"""Please access the global test records pool via cls.globalTestRecords['Some Doc'] -> list.
If not an IntegrationTestCase, use stylo.tests.utils.load_test_records_for (check return type).
""",
)
def stylo_get_test_records(doctype):
	import stylo
	from stylo.tests.utils.generators import load_test_records_for
...
@deprecated(
	"stylo.utils.data.get_number_format_info",
	"unknown",
	"v16",
	"Use `NumberFormat.from_string()` from `stylo.utils.number_format` instead",
)
def get_number_format_info(format: str) -> tuple[str, str, int]:
	"""DEPRECATED: use `NumberFormat.from_string()` from `stylo.utils.number_format` instead.
...
@deprecated(
	...
	"2024-11-12",
	"yet unknown",
	"""It has been added for compatibility in addition to the new .stylo sentinel file inside the module. This is for your info: you don't have to do anything.
""",
)
def boilerplate_modules_txt(dest, app_name, app_title):
	...
	with open(os.path.join(dest, app_name, app_name, "modules.txt"), "w") as f:
		f.write(stylo.as_unicode(app_title))
