stylo.pages["workflow-builder"].on_page_load = function (wrapper) {
	...
	if (stylo.boot.developer_mode) {
		...
		stylo.hot_update.push(() => load_workflow_builder(wrapper));
	}
};

stylo.pages["workflow-builder"].on_page_show = function (wrapper) {
...
function load_workflow_builder(wrapper) {
	...
	if (route.length > 1) {
		stylo.require("workflow_builder.bundle.js").then(() => {
			stylo.workflow_builder = new stylo.ui.WorkflowBuilder({
				wrapper: $parent,
				page: wrapper.page,
				workflow: route[1],
			});
		...
		let d = new stylo.ui.Dialog({
			...
			primary_action({ action, doctype, workflow, workflow_name }) {
				if (action === "Edit") {
					stylo.set_route("workflow-builder", workflow);
					...
					stylo.db
						...
						.then((doc) => {
							stylo.set_route("workflow-builder", doc.name);
						})
						.finally(() => {
