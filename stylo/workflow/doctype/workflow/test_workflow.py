...
class TestWorkflow(StyloTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		...
		workflow_actions = stylo.get_all("Workflow Action", fields=["status"])
		self.assertEqual(len(workflow_actions), 1)
	...
	def test_if_workflow_actions_were_processed_using_user(self):
		...
		stylo.qb.update(WorkflowAction).set(WorkflowAction.user, "test2@example.com").run()
		stylo.qb.update(WorkflowActionPermittedRole).set(WorkflowActionPermittedRole.role, "").run()
...
def create_todo_workflow():
	...
	if not stylo.db.exists("Role", TEST_ROLE):
		stylo.get_doc(dict(doctype="Role", role_name=TEST_ROLE)).insert(ignore_if_duplicate=True)
...
def create_new_todo():
	return stylo.get_doc(dict(doctype="ToDo", description="workflow " + random_string(10))).insert()
