...
class TestWorkflow(IntegrationTestCase):
	...
	def setUp(self):
		self.patcher = patch("stylo.attach_print", return_value={})
		self.patcher.start()
		stylo.db.delete("Workflow Action")
	...
	def tearDown(self):
		...
		stylo.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		...
		workflow_actions = stylo.get_all("Workflow Action", fields=["*"])
	...
	def test_sync_tasks(self, doc=None):
		...
		with self.patch_hooks(
			{
				"workflow_methods": [
					{
						"name": "Create Note",
						"method": "stylo.workflow.doctype.workflow.test_workflow.create_new_note",
					}
				]
			}
		):
		...
		self.assertTrue(
			stylo.db.exists("Note", {"title": "workflow - " + domain.name, "content": "workflow test"})
		)
		self.assertTrue(stylo.db.exists("Domain", {"name": "workflow - " + domain.name}))
		self.assertTrue(stylo.db.exists("Webhook Request Log", {"url": "https://workflowtasks.org/post"}))
...
def create_todo_workflow():
	from stylo.tests.ui_test_helpers import UI_TEST_USER

	if stylo.db.exists("Workflow", "Test ToDo"):
		stylo.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not stylo.db.exists("Role", TEST_ROLE):
		stylo.get_doc(doctype="Role", role_name=TEST_ROLE).insert(ignore_if_duplicate=True)
		if stylo.db.exists("User", UI_TEST_USER):
			stylo.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = stylo.new_doc("Workflow")
...
def create_domain_workflow():
	...
	if stylo.db.exists("Workflow", "Test Domain"):
		stylo.delete_doc("Workflow", "Test Domain")
	...
	pending_to_approved_transition = stylo.new_doc("Workflow Transition Tasks")
...
def create_new_todo():
	return stylo.get_doc(doctype="ToDo", description="workflow " + random_string(10)).insert()
...
def create_new_server_script():
	server_script = stylo.new_doc("Server Script")
