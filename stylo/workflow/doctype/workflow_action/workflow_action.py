...
import stylo
from stylo import _
from stylo.desk.form.utils import get_pdf_link
from stylo.desk.notifications import clear_doctype_notifications
from stylo.email.doctype.email_template.email_template import get_email_template
from stylo.model.document import Document
from stylo.model.workflow import (
	apply_workflow,
	get_workflow_name,
...
from stylo.utils import get_datetime, get_url, now_datetime
from stylo.utils.background_jobs import enqueue
from stylo.utils.data import get_link_to_form
from stylo.utils.user import get_users_with_role
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from stylo.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	stylo.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(stylo.get_roles(user))
...
def process_workflow_actions(doc, state):
	...
	if send_email_alert(workflow) and stylo.db.get_value(
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = stylo.session.user
...
def return_success_page(doc):
	stylo.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), stylo.bold(doc.get("name")), stylo.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	stylo.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			stylo.bold(doc.get("name")),
			stylo.bold(doc_workflow_state),
			stylo.bold(stylo.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else stylo.session.user

	allowed_roles = stylo.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(stylo.get_roles(user))
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		stylo.qb.update(WorkflowAction)
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = stylo._dict(
					{
						"possible_actions": [],
						"email": stylo.db.get_value("User", user, "email"),
					}
				)
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = "/api/method/stylo.workflow.doctype.workflow_action.workflow_action.apply_action"
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/stylo.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	lang = doc.get("language") or (
		stylo.get_cached_value("Print Format", print_format, "default_print_language")
		if print_format
		else None
	)
...
def get_email_template_from_workflow(doc):
	...
	template_name = stylo.db.get_value(
		"Workflow Document State",
