...
from stylo.utils import get_datetime, get_url
...
def get_permission_query_conditions(user):
	...
	return f"""(`tabWorkflow Action`.`name` in ({permitted_workflow_actions})
		or `tabWorkflow Action`.`user`={stylo.db.escape(user)})
...
def clear_old_workflow_actions_using_user(doc, user=None):
	...
	if stylo.db.has_column("Workflow Action", "user"):
		stylo.db.delete(
			"Workflow Action",
			{
...
def get_common_email_args(doc):
	...
	common_args = {
		...
		"attachments": [stylo.attach_print(doctype, docname, file_name=docname, doc=doc)],
		"subject": subject,
		"message": response,
	}
	return common_args
