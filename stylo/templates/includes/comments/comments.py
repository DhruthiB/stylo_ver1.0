...
def get_limit():
	method = stylo.get_hooks("comment_rate_limit")
	if not method:
		return 5
	else:
		limit = stylo.call(method[0])
		return limit
...
def add_comment(comment, comment_email, comment_by, reference_doctype, reference_name, route):
	if stylo.session.user == "Guest":
		...
		comments_permission_config = stylo.get_hooks("has_comment_permission")
		guest_allowed = False
		if len(comments_permission_config):
			if comments_permission_config["doctype"]:
				...
				guest_allowed = stylo.call(check_permission_method[0], ref_doctype=reference_doctype)
