...
from stylo.website.doctype.blog_settings.blog_settings import get_comment_limit
...
def add_comment(comment, comment_email, comment_by, reference_doctype, reference_name, route):
	if stylo.session.user == "Guest":
		...
		if reference_doctype == "Blog Post" and not stylo.db.get_single_value(
			"Blog Settings", "allow_guest_to_comment"
		):
			return

		if stylo.db.exists("User", comment_email):
			stylo.throw(_("Please login to post a comment."))

	if not comment.strip():
		stylo.msgprint(_("The comment cannot be empty"))
		return False

	if URLS_COMMENT_PATTERN.search(comment) or EMAIL_PATTERN.search(comment):
		stylo.msgprint(_("Comments cannot have links or email addresses"))
		return False

	doc = stylo.get_doc(reference_doctype, reference_name)
	...
	if doc.get("route"):
		url = f"{stylo.utils.get_request_site_address()}/{doc.route}#{comment.name}"
	else:
		url = f"{stylo.utils.get_request_site_address()}/app/{scrub(doc.doctype)}/{doc.name}#comment-{comment.name}"
	...
	if doc.doctype != "Blog Post" or doc.enable_email_notification:
		# notify creator
		creator_email = stylo.db.get_value("User", doc.owner, "email") or doc.owner
	...
	template = stylo.get_template("templates/includes/comments/comment.html")
