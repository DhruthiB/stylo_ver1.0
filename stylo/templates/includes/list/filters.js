function setup_list_filters() {
	...
	stylo.call('stylo.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = stylo.utils.get_query_params()[df.fieldname];
			const f = stylo.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(stylo.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = stylo.utils.make_query_string(query_params);
...
stylo.ready(setup_list_filters);
