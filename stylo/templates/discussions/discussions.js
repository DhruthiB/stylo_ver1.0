...
const setup_socket_io = () => {
	stylo.socketio.init(window.socketio_port || "9000");
	stylo.socketio.socket.on("publish_message", (data) => {
		publish_message(data);
	});
	stylo.socketio.socket.on("update_message", (data) => {
		update_message(data);
	});
	stylo.socketio.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const hide_actions_on_conditions = (template, owner) => {
	...
	stylo.session.user != owner && $template.find(".dropdown").addClass("hide");
	return $template.prop("outerHTML");
};
