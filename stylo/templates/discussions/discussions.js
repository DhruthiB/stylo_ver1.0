stylo.ready(() => {
	...
	stylo.require("controls.bundle.js", () => {
...
const setup_socket_io = () => {
	stylo.realtime.init(window.socketio_port || "9000");

	stylo.realtime.on("publish_message", (data) => {
		publish_message(data);
	});

	stylo.realtime.on("update_message", (data) => {
		update_message(data);
	});

	stylo.realtime.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const insert_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == stylo.session.user) {
	...
	} else if (topic.owner == stylo.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (strip_html(reply).trim() != "" || reply.includes("img")) {
		...
		stylo.call({
			method: "stylo.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = stylo.get_palette(element.attr("title"));
...
const delete_reply = (e) => {
	stylo.call({
		method: "stylo.website.doctype.discussion_reply.discussion_reply.delete_message",
		args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	stylo.session.user != owner && $template.find(".dropdown").remove();
	return $template.prop("outerHTML");
};
...
const make_comment_editor = (element) => {
	this.comment_editor = new stylo.ui.FieldGroup({
		fields: [
			{
	...
	element.find(".stylo-control").removeClass("hide-control");
