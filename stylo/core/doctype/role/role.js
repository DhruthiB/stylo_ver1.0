...
stylo.ui.form.on("Role", {
	refresh: function (frm) {
		...
		frm.set_df_property("is_custom", "read_only", stylo.session.user !== "Administrator");

		frm.add_custom_button("Role Permissions Manager", function () {
			stylo.route_options = { role: frm.doc.name };
			stylo.set_route("permission-manager");
		});
		frm.add_custom_button("Show Users", function () {
			stylo.route_options = { role: frm.doc.name };
			stylo.set_route("List", "User", "Report");
		});
	},
});
