...
from stylo.core.doctype.user_permission.user_permission import (
	add_user_permissions,
	remove_applicable,
)
from stylo.permissions import add_permission, has_user_permission
from stylo.tests import IntegrationTestCase
from stylo.tests.test_helpers import setup_for_tests
...
class TestUserPermission(IntegrationTestCase):
	def setUp(self):
		...
		stylo.db.delete("User Permission", {"user": ("in", test_users)})
		stylo.delete_doc_if_exists("DocType", "Person")
		stylo.db.sql_ddl("DROP TABLE IF EXISTS `tabPerson`")
		stylo.delete_doc_if_exists("DocType", "Doc A")
		stylo.db.sql_ddl("DROP TABLE IF EXISTS `tabDoc A`")
		setup_for_tests()
	...
	def test_default_user_permission_validation(self):
		...
		self.assertRaises(stylo.ValidationError, add_user_permissions, param)
	...
	def test_default_user_permission_corectness(self):
		...
		test_blog = stylo.get_doc("Test Blog Post", "_Test Blog Post 1")
		param = get_params(perm_user, "Test Blog Post", test_blog.name, is_default=1, hide_descendants=1)
		add_user_permissions(param)
		stylo.db.delete("User Permission", filters={"for_value": test_blog.name})
		stylo.delete_doc("Test Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not stylo.db.exists("Test Blog Category", category):
				stylo.get_doc({"doctype": "Test Blog Category", "title": category}).insert()
		...
		stylo.set_user("test_user_perm1@example.com")
		doc = stylo.new_doc("Test Blog Post")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = stylo.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = stylo.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		is_created_applicable_second = stylo.db.exists(
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = stylo.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = stylo.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		removed_applicable_second = stylo.db.exists(
			"User Permission", get_exists_param(user, applicable="Contact")
		)
	...
	def test_user_perm_for_nested_doctype(self):
		...
		if not stylo.db.exists("DocType", "Person"):
			doc = new_doctype(
				"Person",
		...
		parent_record = stylo.get_doc({"doctype": "Person", "person_name": "Parent", "is_group": 1}).insert()

		child_record = stylo.get_doc(
			{
				"doctype": "Person",
				"person_name": "Child",
				"is_group": 0,
		...
		self.assertTrue(has_user_permission(stylo.get_doc("Person", parent_record.name), user.name))
		self.assertTrue(has_user_permission(stylo.get_doc("Person", child_record.name), user.name))

		#  give access of Parent DocType to Blogger role
		add_permission("Person", "Blogger")
		stylo.set_user(user.name)
		visible_names = stylo.get_list(
			doctype="Person",
			pluck="person_name",
		)

		user_permission = stylo.get_doc(
		...
		# hides child records
		self.assertTrue(has_user_permission(stylo.get_doc("Person", parent_record.name), user.name))
		self.assertFalse(has_user_permission(stylo.get_doc("Person", child_record.name), user.name))

		visible_names_after_hide_descendants = stylo.get_list(
			"Person",
			pluck="person_name",
		)
	...
	def test_user_perm_on_new_doc_with_field_default(self):
		...
		stylo.set_user("new_doc_test@example.com")

		new_doc = stylo.new_doc("Doc A")
	...
	def test_user_perm_on_new_doc_with_user_default(self):
		"""Test User Perm impact on stylo.new_doc. with *user* default value"""
		...
		if not stylo.db.exists("Session Default", {"ref_doctype": "DocType"}):
		...
		stylo.set_user("user_default_test@example.com")
...
def create_user(email, *roles):
	...
	if stylo.db.exists("User", email):
		return stylo.get_doc("User", email)
