...
def get_system_settings(key: str):
	...
	if not (system_settings := getattr(stylo.local, "system_settings", None)):
		try:
			system_settings = stylo.client_cache.get_doc("System Settings")
			stylo.local.system_settings = system_settings
		except stylo.DoesNotExistError:  # possible during new install
...
def clear_system_settings_cache():
	stylo.client_cache.delete_value(stylo.get_document_cache_key("System Settings", "System Settings"))
	stylo.cache.delete_value("system_settings")
	stylo.cache.delete_value("time_zone")


def sync_system_settings():
	if stylo.db.get_single_value("System Settings", "currency") is None:
		stylo.db.set_single_value("System Settings", "currency", stylo.defaults.get_defaults()["currency"])
