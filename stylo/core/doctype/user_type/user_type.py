...
from stylo.permissions import add_permission, add_user_permission
from stylo.utils import get_link_to_form
from stylo.utils.modules import get_modules_from_app
...
class UserType(Document):
	...
	if TYPE_CHECKING:
		from stylo.core.doctype.user_document_type.user_document_type import UserDocumentType
		from stylo.core.doctype.user_select_document_type.user_select_document_type import (
			UserSelectDocumentType,
		)
		from stylo.core.doctype.user_type_module.user_type_module import UserTypeModule
	...
	def clear_cache(self):
		...
		if not self.is_standard:
			stylo.cache.delete_value("non_standard_user_types")
	...
	def on_trash(self):
		if self.is_standard:
			stylo.throw(_("Standard user type {0} can not be deleted.").format(stylo.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = stylo.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		limit = stylo.conf.get("user_type_doctype_limit", {}).get(stylo.scrub(self.name))

		if not limit and stylo.session.user != "Administrator":
			stylo.throw(
				_("User does not have permission to create the new {0}").format(stylo.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			stylo.throw(_("The field {0} is mandatory").format(stylo.bold(_("Role"))))

		if not stylo.db.get_value("Role", self.role, "is_custom"):
			stylo.throw(
				_("The role {0} should be a custom role.").format(
					stylo.bold(get_link_to_form("Role", self.role))
				)
			)

	def update_users(self):
		for row in stylo.get_all("User", filters={"user_type": self.name}):
			user = stylo.get_cached_doc("User", row.name)
	...
	def add_select_perm_doctypes(self):
		if stylo.flags.ignore_select_perm:
			return
	...
	def remove_permission_for_deleted_doctypes(self):
		...
		for perm in stylo.get_all(
			...
			stylo.delete_doc("Custom DocPerm", perm.name)


def add_role_permissions(doctype, role):
	name = stylo.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	modules = [d.get("module_name") for d in get_modules_from_app("stylo")]
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		stylo.throw(_("User Id Field is mandatory in the user type {0}").format(stylo.bold(doc.name)))

	if stylo.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		label = stylo.get_meta(doc.apply_user_permission_on).get_field(doc.user_id_field).label
		...
		stylo.msgprint(
			_(
				...
				stylo.bold(label),
				stylo.bold(user),
				stylo.bold(doc.apply_user_permission_on),
			)
		)
...
def apply_permissions_for_non_standard_user_type(doc, method=None):
	...
	if not stylo.db.table_exists("User Type") or stylo.flags.in_migrate:
		return

	user_types = stylo.cache.get_value(
	...
	for user_type, data in user_types.items():
		...
		if doc.get(data[1]) and (
			...
			if not perm_data:
				...
				user_perm = stylo.get_doc("User Permission", perm_data[0])
