...
import stylo
from stylo import _
from stylo.config import get_modules_from_app
...
class UserType(Document):
	...
	def add_role_permissions_for_user_doctypes(self):
		...
		for row in self.user_doctypes:
			...
			stylo.db.set_value("Custom DocPerm", docperm, values)
	...
	def add_role_permissions_for_file(self):
		...
		stylo.db.set_value("Custom DocPerm", docperm, {"read": 1, "create": 1, "write": 1})
	...
	for user_type, data in user_types.items():
		...
		if doc.get(data[1]) and (
			...
			if not perm_data:
				...
				stylo.db.set_value("User Permission", perm_data[0], "user", doc.get(data[1]))
