...
import stylo
from stylo.cache_manager import clear_doctype_cache
from stylo.core.doctype.doctype.doctype import (
	CannotIndexedError,
	DocType,
	DoctypeLinkError,
...
from stylo.core.doctype.rq_job.test_rq_job import wait_for_completion
from stylo.custom.doctype.custom_field.custom_field import create_custom_fields
from stylo.desk.form.load import getdoc
from stylo.model.delete_doc import delete_controllers
from stylo.model.sync import remove_orphan_doctypes
...
class TestDocType(IntegrationTestCase):
	...
	def test_validate_name(self):
		self.assertRaises(stylo.NameError, new_doctype("_Some DocType").insert)
		self.assertRaises(stylo.NameError, new_doctype("8Some DocType").insert)
		self.assertRaises(stylo.NameError, new_doctype("Some (DocType)").insert)
		self.assertRaises(
			stylo.NameError,
	...
	def test_making_sequence_on_change(self):
		stylo.delete_doc_if_exists("DocType", self._testMethodName)
		...
		self.assertEqual(
			...
			"varchar" if stylo.db.db_type == "mariadb" else "character varying",
		)

	def test_doctype_unique_constraint_dropped(self):
		if stylo.db.exists("DocType", "With_Unique"):
			stylo.delete_doc("DocType", "With_Unique")

		dt = new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = stylo.new_doc("With_Unique")
		doc2 = stylo.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(stylo.UniqueValidationError, doc2.insert)
		stylo.db.rollback()
	...
	def test_validate_search_fields(self):
		...
		self.assertRaises(stylo.ValidationError, doc.save)
	...
	def test_sync_field_order(self):
		...
		stylo.delete_doc_if_exists("DocType", "Test Field Order DocType")
		if os.path.isfile(path):
			os.remove(path)

		try:
			stylo.flags.allow_doctype_export = 1
			test_doctype.save()

			# assert that field_order list is being created with the default order
			test_doctype_json = stylo.get_file_json(path)
			...
			with open(path, "w+") as txtfile:
				txtfile.write(stylo.as_json(test_doctype_json))
			...
			stylo.reload_doctype(test_doctype.name, force=True)
			test_doctype.reload()
			...
			stylo.flags.allow_doctype_export = 0
	...
	def test_cancel_link_doctype(self):
		...
		from stylo.desk.form.linked_with import cancel_all_linked_docs, get_submitted_linked_docs
		...
		# create doctype data
		data_link_doc = stylo.new_doc("Test Linked Doctype")
		data_link_doc.some_fieldname = "Data1"
		data_link_doc.insert()
		data_link_doc.save()
		data_link_doc.submit()

		data_doc = stylo.new_doc("Test Doctype")
	...
	def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
		...
		data_doc = stylo.new_doc("Test Doctype 2")
		data_doc.some_fieldname = "Data1"
		...
		self.assertRaises(stylo.LinkExistsError, data_link_doc_1.cancel)
	...
	def test_create_virtual_doctype(self):
		...
		virtual_doc.reload()
		doc = stylo.get_doc("DocType", "Test Virtual Doctype")

		self.assertDictEqual(doc.as_dict(), virtual_doc.as_dict())
		self.assertEqual(doc.is_virtual, 1)
		self.assertFalse(stylo.db.table_exists("Test Virtual Doctype"))

	def test_create_virtual_doctype_as_child_table(self):
		"""Test virtual DocType as Child Table below a normal DocType."""
		stylo.delete_doc_if_exists("DocType", "Test Parent Virtual DocType", force=1)
		stylo.delete_doc_if_exists("DocType", "Test Virtual DocType as Child Table", force=1)
		...
		doc = stylo.get_doc("DocType", "Test Virtual DocType as Child Table")

		self.assertEqual(doc.is_virtual, 1)
		self.assertEqual(doc.istable, 1)
		self.assertFalse(stylo.db.table_exists("Test Virtual DocType as Child Table"))
		...
		self.assertRaises(stylo.exceptions.ValidationError, parent_doc.insert)
		parent_doc.is_virtual = 1
		parent_doc.insert(ignore_permissions=True)
		self.assertFalse(stylo.db.table_exists("Test Parent Virtual DocType"))
	...
	def test_autoincremented_doctype_transition(self):
		stylo.delete_doc_if_exists("DocType", "testy_autoinc_dt")
		dt = new_doctype("testy_autoinc_dt", autoname="autoincrement").insert(ignore_permissions=True)
		dt.autoname = "hash"

		dt.save(ignore_permissions=True)

		dt_data = stylo.get_doc({"doctype": dt.name, "some_fieldname": "test data"}).insert(
		...
		try:
			dt.save(ignore_permissions=True)
		except stylo.ValidationError as e:
			self.assertEqual(
				e.args[0],
	...
	def test_json_field(self):
		...
		json_doc.insert()
		json_doc.save()
		doc = stylo.get_doc("DocType", "Test Json Doctype")
		...
		test_json = stylo.get_doc("Test Json Doctype", doc.name)
	...
	def test_no_delete_doc(self):
		self.assertRaises(stylo.ValidationError, stylo.delete_doc, "DocType", "Address")
	...
	def test_export_types(self):
		...
		from stylo.types.exporter import TypeExporter
	...
	def test_custom_field_deletion(self):
		...
		stylo.delete_doc("DocType", child)
		self.assertFalse(stylo.get_meta(doctype).get_field(field))
	...
	def test_delete_doctype_with_customization(self):
		...
		# Create 1 record
		original_doc = stylo.get_doc(doctype=doctype, custom_field_name="wat").insert()
		...
		restored_doc = stylo.get_last_doc(doctype)
		...
		self.assertEqual(stylo.get_meta(doctype).get_field(field).default, "DELETETHIS")
		stylo.delete_doc("DocType", doctype)
	...
	def test_delete_orphaned_doctypes(self):
		...
		job = stylo.enqueue(remove_orphan_doctypes)
		wait_for_completion(job)

		stylo.db.rollback()
		self.assertFalse(stylo.db.exists("DocType", doctype.name))
	...
	def test_not_in_list_view_for_not_allowed_mandatory_field(self):
		...
		stylo.delete_doc("DocType", doctype.name)
	...
	def test_no_recursive_fetch(self):
		...
		self.assertRaises(stylo.ValidationError, recursive_dt.insert)
	...
	def test_meta_serialization(self):
		...
		doc = stylo.new_doc(doctype.name, some_fieldname="something").insert()
		doc.save()
		doc.submit()
		stylo.get_meta(doctype.name).as_dict()
	...
	def test_decimal_field_configuration(self):
		...
		decimal_field_type = stylo.db.get_column_type(doctype.name, "decimal_field")
	...
	def test_decimal_field_precision_exceeds_length(self):
		...
		self.assertRaises(stylo.ValidationError, doctype.insert)

	def test_delete_doc_clears_cache(self):
		dt = new_doctype(
			fields=[{"fieldname": "test_fdname", "fieldtype": "Data", "label": "Test Field"}],
		).insert()
		stylo.get_meta(dt.name)
		stylo.delete_doc("DocType", dt.name, force=1, delete_permanently=False)
		stylo.db.commit()
		with self.assertRaises(stylo.DoesNotExistError):
			stylo.get_meta(dt.name)
