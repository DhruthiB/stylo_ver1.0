stylo.listview_settings["DocType"] = {
	...
	new_doctype_dialog(args) {
		...
		let non_developer = stylo.session.user !== "Administrator" || !stylo.boot.developer_mode;
		let fields = [
			{
				label: __("Name"),
				fieldname: "name",
		...
		let new_d = new stylo.ui.Dialog({
			...
			primary_action(values) {
				...
				stylo.db
					...
					.then((doc) => {
						stylo.set_route("Form", "DocType", doc.name);
					});
			},
