...
from stylo.model.base_document import RESERVED_KEYWORDS, get_controller
...
from stylo.utils import cint, flt, get_datetime, is_a_property, random_string
...
class DocType(Document):
	...
	if TYPE_CHECKING:
		...
		from stylo.core.doctype.docperm.docperm import DocPerm
	...
	def check_indexing_for_dashboard_links(self):
		...
		for d in self.fields:
			if d.fieldtype == "Link" and not d.unique and not d.search_index:
				referred_as_link = stylo.db.exists(
					"DocType Link",
	...
	def check_developer_mode(self):
		...
		if stylo.flags.in_patch or stylo.in_test:
			return
		...
		if stylo.conf.developer_mode and not self.owner:
	...
	def validate_virtual_doctype_methods(self):
		...
		from stylo.model.virtual_doctype import validate_controller
	...
	def change_modified_of_parent(self):
		...
		for p in parent_list:
			stylo.db.set_value("DocType", p.parent, {})
	...
	def setup_autoincrement_and_sequence(self):
		...
		if self.autoname == "autoincrement":
			name_type = "bigint"
			stylo.db.create_sequence(self.name, check_not_exists=True)
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not stylo.request) or stylo.in_test or stylo.flags.in_install
	...
	def export_types_to_controller(self):
		...
		if any(stylo.get_hooks("export_python_type_annotations", app_name=app)):
	...
	def check_pending_migration(self) -> bool:
		...
		file = Path(get_file_path(stylo.scrub(self.module), self.doctype, self.name))
		content = json.loads(file.read_text())
		if content.get("modified") and get_datetime(self.modified) < get_datetime(content.get("modified")):
			stylo.msgprint(
				_(
					"This doctype has pending migrations, run 'forge migrate' before modifying the doctype to avoid losing changes."
				),
				alert=True,
				indicator="yellow",
			)
...
def validate_empty_name(dt, autoname):
	...
	if not autoname and not (dt.issingle or dt.istable):
		...
		if not controller or (not hasattr(controller, "autoname")):
			stylo.toast(_("Warning: Naming is not set"), indicator="yellow")
...
def validate_fields(meta: Meta):
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (docfield.oldfieldtype and docfield.oldfieldtype != "Data"):
			if docfield.options and (docfield.options not in data_field_options):
				df_str = stylo.bold(_(docfield.label, context=docfield.parent))
				text_str = (
	...
	def check_child_table_option(docfield):
		...
		doctype = docfield.options
		child_doctype_meta = stylo.get_meta(doctype)
	...
	for d in fields:
		...
		if not stylo.flags.in_migrate or in_ci:
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			stylo.get_doc(doctype="Domain", domain=doc.restrict_to_domain).insert()
...
def get_row_size_utilization(doctype: str) -> float:
	...
	try:
		return flt(stylo.db.get_row_size(doctype) / stylo.db.MAX_ROW_SIZE_LIMIT * 100, 2)
	except Exception:
		return 0.0
