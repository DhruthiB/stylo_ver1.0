...
from stylo.utils.caching import http_cache
from stylo.utils.safe_exec import (
	StyloTransformer,
...
class ServerScript(Document):
	...
	def on_trash(self):
		...
		if self.script_type == "Scheduler Event":
			for job in self.scheduled_jobs:
				scheduled_job_type: ScheduledJobType = stylo.get_doc("Scheduled Job Type", job.name)
	...
	def sync_scheduled_job_type(self):
		...
		def get_scheduled_job() -> "ScheduledJobType":
			if scheduled_script := stylo.db.get_value("Scheduled Job Type", {"server_script": self.name}):
				return stylo.get_doc("Scheduled Job Type", scheduled_script)
			else:
				return stylo.get_doc({"doctype": "Scheduled Job Type", "server_script": self.name})
		...
		get_scheduled_job().update(
			{
				"method": stylo.scrub(f"{self.name}-{self.event_frequency}"),
		...
		stylo.msgprint(_("Scheduled execution for script {0} has updated").format(self.name), alert=True)
	...
	def check_if_compilable_in_restricted_context(self):
		...
		try:
			compile_restricted(self.script, policy=StyloTransformer)
	...
	def execute_method(self) -> dict:
		...
		Return:
		        dict: Evaluate self.script with stylo.utils.safe_exec.safe_exec and return the flags set in its safe globals.
		"""
...
def get_autocompletion_items():
	...
	e.g., ["stylo.utils.cint", "stylo.get_all", ...]
	"""

	return stylo.cache.get_value(
...
def execute_api_server_script(script: ServerScript, *args, **kwargs):
	...
	if stylo.session.user == "Guest" and not script.allow_guest:
...
def enabled() -> bool | None:
	if stylo.has_permission("Server Script"):
