...
stylo.ui.form.on("Server Script", {
	...
	refresh: function (frm) {
		...
		setTimeout(() => {
			stylo
				.call("stylo.core.doctype.server_script.server_script.get_autocompletion_items")
				.then((r) => r.message)
				.then((items) => {
	...
	check_safe_exec(frm) {
		stylo.xcall("stylo.core.doctype.server_script.server_script.enabled").then((enabled) => {
			if (enabled === false) {
				let docs_link =
					"https://styloframework.com/docs/user/en/desk/scripting/server-script";
...
<p>Payment processing events have a special state. See the <a href="https://github.com/stylo/payments/blob/develop/payments/controllers/payment_controller.py">PaymentController in Stylo Payments</a> for details.</p>
<pre><code class="language-python">
...
<pre><code class="language-python">
tenant_id = stylo.db.get_value(...) # -> 2
