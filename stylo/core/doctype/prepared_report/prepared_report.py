...
from stylo.utils.background_jobs import enqueue, get_redis_conn
...
class PreparedReport(Document):
	...
	def clear_old_logs(days=30):
		prepared_reports_to_delete = stylo.get_all(
			"Prepared Report",
			filters={"creation": ["<", stylo.utils.add_days(stylo.utils.now(), -days)]},
		)
	...
	def on_trash(self):
		...
		with suppress(Exception):
			job = stylo.get_doc("RQ Job", self.job_id)
			job.stop_job() if self.status == "Started" else job.delete()

	def after_insert(self):
		timeout = stylo.get_value("Report", self.report_name, "timeout")
		enqueue(
			generate_report,
			queue="long",
...
def generate_report(prepared_report):
	...
	instance: PreparedReport = stylo.get_doc("Prepared Report", prepared_report)
...
def stop_prepared_report(report_name: str):
	...
	prepared_report = stylo.get_doc("Prepared Report", report_name)
	prepared_report.check_permission("write")

	job_id = prepared_report.job_id
	if not job_id.startswith(stylo.local.site):
		stylo.throw(f"Invalid job_id: must start with {stylo.local.site}")
	...
	try:
		...
		stylo.msgprint(_("Job stopped successfully"), alert=True, indicator="green")
...
def expire_stalled_report():
	stylo.db.set_value(
		...
		{
			"status": "Failed",
			"error_message": stylo._("Report timed out."),
		},
...
def create_json_gz_file(data, dt, dn, report_name):
	...
	json_filename = "{}_{}.json.gz".format(
		stylo.scrub(report_name), stylo.utils.data.format_datetime(stylo.utils.now(), "Y-m-d-H-M")
	)
...
def convert_json_to_csv(prepared_report_name):
	...
	doc = stylo.get_doc("Prepared Report", prepared_report_name)
	json_content, file_name = doc.get_prepared_data(with_file_name=True)

	if not json_content:
		stylo.log_error(f"No JSON content found for {prepared_report_name}", "CSV Conversion")
		return
	...
	if not columns or not result:
		stylo.log_error("Columns or result is empty", "CSV Conversion")
		return
