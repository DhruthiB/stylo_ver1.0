...
import stylo
from stylo import _
from stylo.defaults import clear_default, set_default
...
class Language(Document):
	...
	def on_update(self):
		...
		stylo.client_cache.delete_value("languages")
	...
	def update_user_defaults(self):
		...
		should be updated.
		"""
		users = stylo.get_all("User", filters={"language": self.name}, pluck="name")
...
def sync_languages():
	"""Create Language records from stylo/geo/languages.csv"""
	from csv import DictReader

	with open(stylo.get_app_path("stylo", "geo", "languages.csv")) as f:
		reader = DictReader(f)
		for row in reader:
			if not stylo.db.exists("Language", row["language_code"]):
				doc = stylo.new_doc("Language")
				doc.update(row)
				doc.insert()
