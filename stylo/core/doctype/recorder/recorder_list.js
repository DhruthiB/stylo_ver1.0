stylo.listview_settings["Recorder"] = {
	...
	onload(listview) {
		...
		listview.page.add_button(__("Clear"), () => {
			stylo.xcall("stylo.recorder.delete").then(listview.refresh);
		});
		...
		listview.page.add_menu_item(__("Import"), () => {
			new stylo.ui.FileUploader({
				...
				on_success: (file) => {
					...
					stylo.call({
						method: "stylo.recorder.import_data",
						args: {
							file: file.file_url,
						},
						callback: function () {
		...
		listview.page.add_menu_item(__("Export"), () => {
			stylo.call({
				method: "stylo.recorder.export_data",
				callback: function (r) {
	...
	fetch_recorder_status(listview) {
		return stylo.xcall("stylo.recorder.status").then((status) => {
	...
	stop_recorder(listview) {
		let me = this;
		stylo.xcall("stylo.recorder.stop", {}).then(() => {
			listview.refresh();
	...
	start_recorder(listview) {
		...
		stylo.prompt(
			...
			(values) => {
				stylo.xcall("stylo.recorder.start", values).then(() => {
					listview.refresh();
