...
class ScheduledJobType(Document):
	...
	def get_next_execution(self):
		...
		maintenance_offset = int(hashlib.sha1(stylo.local.site.encode()).hexdigest(), 16) % 60
		...
		CRON_MAP = {
			...
			"All": f"*/{(stylo.get_conf().scheduler_interval or 240) // 60} * * * *",
		}
	...
	def execute(self):
		if stylo.job:
			stylo.job.frequency = self.frequency
			stylo.job.cron_format = self.cron_format
	...
	def update_scheduler_log(self, status):
		...
		if status == "Failed":
			self.scheduler_log.db_set("details", stylo.get_traceback(with_context=True))
		if status == "Start":
			self.db_set("last_execution", now_datetime(), update_modified=False)
		stylo.db.commit()
...
def skip_next_execution(doc: str):
	stylo.only_for("System Manager")
	doc = json.loads(doc)
	doc: ScheduledJobType = stylo.get_doc("Scheduled Job Type", doc.get("name"))
...
def run_scheduled_job(scheduled_job_type: str, job_type: str | None = None):
	...
	try:
		stylo.get_doc("Scheduled Job Type", scheduled_job_type).execute()
	except Exception:
...
def insert_single_event(frequency: str, event: str, cron_format: str | None = ""):
	...
	if job_name := stylo.db.exists("Scheduled Job Type", {"method": event}):
		doc = stylo.get_doc("Scheduled Job Type", job_name)
