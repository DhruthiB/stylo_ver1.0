...
class TestScheduledJobType(IntegrationTestCase):
	...
	def test_throws_on_duplicate_job(self):
		job_config = dict(
			...
			method="stylo.desk.notifications.clear_notifications",
			frequency="Weekly",
		)
		stylo.get_doc(job_config).insert()

		duplicate_job = stylo.get_doc(job_config)
	...
	def test_sync_jobs(self):
		...
		daily_job = stylo.get_doc(
			"Scheduled Job Type", dict(method="stylo.desk.notifications.clear_notifications")
		)
		self.assertEqual(daily_job.frequency, "Daily Maintenance")

		# check if cron jobs are synced
		cron_job = stylo.get_doc("Scheduled Job Type", dict(method="stylo.deferred_insert.save_to_db"))
	...
	def test_weekly_job(self):
		job = stylo.get_doc(
			"Scheduled Job Type",
			dict(method="stylo.desk.form.document_follow.send_weekly_updates"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_cron_job(self):
		# runs every 10 mins
		job = stylo.get_doc(
			"Scheduled Job Type", dict(method="stylo.email.doctype.email_account.email_account.pull")
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
