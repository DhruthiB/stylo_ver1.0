...
from stylo.permissions import get_roles
...
class UserInvitation(Document):
	...
	if TYPE_CHECKING:
		from stylo.core.doctype.user_role.user_role import UserRole
	...
	def before_insert(self):
		...
		self.invited_by = stylo.session.user
		self.status = "Pending"
	...
	def expire(self):
		...
		self.save()
		email_title = self._get_email_title()
		invited_by_user = stylo.get_doc("User", self.invited_by)
		stylo.sendmail(
	...
	def _validate_invite(self):
		...
		if stylo.db.get_value(
			...
			stylo.throw(title=_("Error"), msg=_("Invitation already accepted"))
		if stylo.db.get_value(
			"User Invitation", filters={"email": self.email, "status": "Pending", "app_name": self.app_name}
		):
			stylo.throw(title=_("Error"), msg=_("Invitation already exists"))
		user_enabled = stylo.db.get_value("User", self.email, "enabled")
		if user_enabled is not None and user_enabled == 0:
			stylo.throw(title=_("Error"), msg=_("User is disabled"))

	def _after_insert(self):
		key = stylo.generate_hash()
		self.db_set("key", stylo.utils.sha256_hash(key))
		invite_link = stylo.utils.get_url(
			f"/api/method/stylo.core.api.user_invitation.accept_invitation?key={key}"
		)
		email_title = self._get_email_title()
		stylo.sendmail(
			recipients=self.email,
		...
		self.db_set("email_sent_at", stylo.utils.now())
		return key
	...
	def _accept(self):
		...
		if self.status == "Expired":
			stylo.throw(title=_("Error"), msg=_("Invitation is expired"))
		if self.status == "Cancelled":
			stylo.throw(title=_("Error"), msg=_("Invitation is cancelled"))
		self.status = "Accepted"
		self.accepted_at = stylo.utils.now()
	...
	def _upsert_user(self, ignore_permissions: bool = False):
		...
		user_inserted = False
		if stylo.db.exists("User", self.user):
			user = stylo.get_doc("User", self.user)
		else:
	...
	def _run_after_accept_hooks(self, user: Document, user_inserted: bool):
		user_invitation_hook = stylo.get_hooks("user_invitation", app_name=self.app_name)
		if not isinstance(user_invitation_hook, dict):
			return
		for dot_path in user_invitation_hook.get("after_accept") or []:
			stylo.call(dot_path, invitation=self, user=user, user_inserted=user_inserted)

	def _get_email_title(self):
		return stylo.get_hooks("app_title", app_name=self.app_name)[0]
	...
	def _get_allowed_roles(self):
		...
		for role in only_for & set(stylo.get_roles()):
			res.update(allowed_roles_mp[role])
		return list(res)

	def _validate_roles(self):
		if self.app_name == "stylo":
			return
	...
	def validate_app_name(app_name: str):
		if app_name not in stylo.get_installed_apps():
			stylo.throw(title=_("Invalid app"), msg=_("Application is not installed"))

	@staticmethod
	def validate_role(app_name: str) -> None:
		UserInvitation.validate_app_name(app_name)
		user_invitation_hook = stylo.get_hooks("user_invitation", app_name=app_name)
		only_for: list[str] = []
		if isinstance(user_invitation_hook, dict):
			only_for = list((user_invitation_hook.get("allowed_roles") or dict()).keys())
		stylo.only_for(only_for)


def mark_expired_invitations() -> None:
	days = 3
	invitations_to_expire = stylo.db.get_all(
		"User Invitation",
		filters={"status": "Pending", "creation": ["<", stylo.utils.add_days(stylo.utils.now(), -days)]},
	)
	for invitation in invitations_to_expire:
		invitation = stylo.get_doc("User Invitation", invitation.name)
		invitation.expire()
...
def get_allowed_apps(user: Document | None) -> list[str]:
	...
	for app in stylo.get_installed_apps():
		user_invitation_hooks = stylo.get_hooks("user_invitation", app_name=app)
...
def get_user(user: Document | None) -> Document:
	return user or stylo.session.user
