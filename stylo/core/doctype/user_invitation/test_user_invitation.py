...
import stylo
import stylo.utils
from stylo.core.api.user_invitation import (
	_accept_invitation,
	cancel_invitation,
	get_pending_invitations,
	invite_by_email,
)
from stylo.core.doctype.user_invitation.user_invitation import mark_expired_invitations
...
class IntegrationTestUserInvitation(IntegrationTestCase):
	...
	def tearDownClass(cls):
		...
		for user_email in emails:
			if stylo.db.exists("User", user_email):
				stylo.delete_doc("User", user_email)
	...
	def delete_all_user_roles(cls):
		...
		stylo.db.sql(cls.normalize_sql(query))
	...
	def delete_invitation(cls, name: str):
		...
		stylo.db.sql(cls.normalize_sql(query), name)
	...
	def test_insert_invitation(self):
		...
		invitation.insert()
		self.assertEqual(invitation.invited_by, stylo.session.user)
	...
	def test_mark_expired_invitations(self):
		...
		invitation.db_set("creation", stylo.utils.add_days(stylo.utils.now(), -4))
	...
	def test_invite_by_email_api(self):
		...
		stylo.delete_doc("User", user.name)
	...
	def test_invite_by_email_api_disabled_user(self):
		...
		stylo.delete_doc("User", user.email)
	...
	def test_accept_invitation_api_pass_redirect(self):
		...
		self.assertEqual(len(stylo.get_all("User", filters={"email": invitation.email}, pluck="name")), 0)
		self.assertEqual(len(self.get_email_names(False)), 1)
		key = invitation._after_insert()
		self.assertEqual(len(self.get_email_names(False)), 2)
		_accept_invitation(key, True)
		res = stylo.local.response
		self.assertEqual(res.type, "redirect")
		pattern = f"^{re.escape(stylo.utils.get_url(''))}/update-password\\?key=.+&redirect_to=/abc$"
	...
	def test_accept_invitation_api_direct_redirect(self):
		...
		self.assertEqual(len(stylo.get_all("User", filters={"email": invitation.email}, pluck="name")), 0)
		original_disable_user_pass_login = stylo.get_system_settings("disable_user_pass_login")
		stylo.db.set_single_value("System Settings", "disable_user_pass_login", 1)
		self.assertEqual(len(self.get_email_names(False)), 1)
		key = invitation._after_insert()
		self.assertEqual(len(self.get_email_names(False)), 2)
		_accept_invitation(key, True)
		...
		res = stylo.local.response
		self.assertEqual(res.type, "redirect")
		pattern = f"^{re.escape(stylo.utils.get_url(''))}/abc$"
		self.assertRegex(res.location, pattern)
		user = stylo.get_doc("User", invitation.email)
		IntegrationTestUserInvitation.delete_invitation(invitation.name)
		stylo.delete_doc("User", user.name)
	...
	def test_get_pending_invitations_api(self):
		...
		invitation.insert()
		invitation.reload()
		pending_invitations = get_pending_invitations("stylo")
	...
	def test_cancel_invitation_api(self):
		...
		self.assertEqual(invitation.status, "Pending")
		self.assertEqual(len(self.get_email_names()), 1)
		res = cancel_invitation(invitation.name, "stylo")
	...
	def get_email_names(self, sent_only=True):
		...
		return stylo.db.get_all("Email Queue", filters=filters, fields=["name"])

	def get_email_messages(self, sent_only=True):
		filters = {"status": "Sent"} if sent_only else None
		return stylo.db.get_all("Email Queue", filters=filters, fields=["message"])
