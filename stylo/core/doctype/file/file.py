...
from stylo.permissions import SYSTEM_USER_ROLE, get_doctypes_with_read
...
from stylo.utils.pdf import pdf_contains_js
...
class File(Document):
	...
	def before_insert(self):
		...
		if self.is_remote_file:
			...
			stylo.db.after_rollback.add(self.on_rollback)
	...
	def handle_is_private_changed(self):
		...
		if stylo.get_meta(self.attached_to_doctype).issingle:
	...
	def validate_file_extension(self):
		...
		if not self.file_type or not stylo.request:
			return

		allowed_extensions = stylo.get_system_settings("allowed_file_extensions")
	...
	def check_content(self):
		if self.file_type == "PDF" and self._content and pdf_contains_js(self._content):
			stylo.throw(_("This PDF cannot be uploaded as it contains unsafe content."))
	...
	def validate_protected_file(self):
		...
		try:
			ref_doc = stylo.get_doc(self.attached_to_doctype, self.attached_to_name)
...
def get_permission_query_conditions(user: str | None = None) -> str:
	...
	if SYSTEM_USER_ROLE not in stylo.get_roles(user):
