...
from stylo.core.doctype.file.exceptions import FileTypeNotAllowed
from stylo.core.doctype.file.utils import get_corrupted_image_msg, get_extension
...
from stylo.utils import get_files_path, set_request
...
class TestFSRollbacks(IntegrationTestCase):
	def test_rollback_from_file_system(self):
		file_name = content = stylo.generate_hash()
		file = stylo.new_doc("File", file_name=file_name, content=content).insert()
...
class TestExtensionValidations(IntegrationTestCase):
	...
	def test_allowed_extension(self):
		...
		file_name = content = stylo.generate_hash()
		bad_file = stylo.new_doc("File", file_name=f"{file_name}.png", content=content)
		self.assertRaises(FileTypeNotAllowed, bad_file.insert)

		bad_file = stylo.new_doc("File", file_name=f"{file_name}.csv", content=content).insert()
		stylo.db.rollback()
...
class TestBase64File(IntegrationTestCase):
	def setUp(self):
		...
		_file: stylo.Document = stylo.get_doc(
			{
				"doctype": "File",
	...
	def test_saved_content(self):
		_file: stylo.Document = stylo.get_doc("File", {"file_url": self.saved_file_url})
		content = _file.get_content()
...
class TestSameContent(IntegrationTestCase):
	...
	def test_utf8_bom_content_decoding(self):
		...
		_file.save()
		saved_file = stylo.get_doc("File", _file.name)
...
class TestFile(IntegrationTestCase):
	...
	def test_make_thumbnail(self):
		...
		self.assertEqual(
			stylo.message_log[0].get("message"),
...
class TestGuestFileAndAttachments(IntegrationTestCase):
	...
	def test_attach_unattached_guest_file(self):
		...
		d = stylo.new_doc("Test For Attachment")
