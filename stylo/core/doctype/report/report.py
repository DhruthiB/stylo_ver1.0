...
class Report(Document):
	...
	if TYPE_CHECKING:
		...
		from stylo.core.doctype.report_column.report_column import ReportColumn
		from stylo.core.doctype.report_filter.report_filter import ReportFilter
	...
	def on_trash(self):
		if (
			...
			and not stylo.flags.in_migrate
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and stylo.conf.developer_mode:
	...
	def execute_script_report(self, filters):
		...
		if not self.prepared_report:
			prepared_report_watcher = threading.Timer(
				...
				kwargs={"report": self.name, "site": stylo.local.site},
			)
...
def is_prepared_report_enabled(report):
	return cint(stylo.db.get_value("Report", report, "prepared_report"))
