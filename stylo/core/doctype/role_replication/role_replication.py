...
import stylo
from stylo.core.page.permission_manager.permission_manager import get_permissions
...
class RoleReplication(Document):
	...
	def replicate_role(self):
		...
		new_role = stylo.db.get_value("Role", self.new_role, "name")
		if not new_role:
			new_role = stylo.get_doc({"doctype": "Role", "role_name": self.new_role}).insert().name
		...
		for perm in perms:
			...
			stylo.get_doc({"doctype": "Custom DocPerm", **perm}).insert()
