...
from stylo.model import CORE_DOCTYPES
from stylo.model.document import Document
from stylo.modules.import_file import import_file_by_path
from stylo.utils.background_jobs import enqueue, get_redis_conn, is_job_enqueued
...
class DataImport(Document):
	...
	def set_delimiters_flag(self):
		if self.import_file:
			stylo.flags.delimiter_options = self.delimiter_options or ","

	def validate_doctype(self):
		if self.reference_doctype in BLOCKED_DOCTYPES:
			stylo.throw(_("Importing {0} is not allowed.").format(self.reference_doctype))
	...
	def start_import(self):
		...
		run_now = stylo.in_test or stylo.conf.developer_mode
...
def stop_data_import(doc_name: str):
	...
	data_import = stylo.get_doc("Data Import", doc_name)
	data_import.check_permission("write")

	rq_job_id = f"{stylo.local.site}||data_import||{doc_name}"
...
def export_json(doctype, path, filters=None, or_filters=None, name=None, order_by="creation asc"):
	...
	with open(path, "w") as outfile:
		outfile.write(stylo.as_json(out, ensure_ascii=False))
