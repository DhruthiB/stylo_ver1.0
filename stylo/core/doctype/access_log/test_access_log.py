...
from stylo.utils import cstr, get_site_url


class TestAccessLog(StyloTestCase):
	def setUp(self):
		# generate keys for current user to send requests for the following tests
		generate_keys(stylo.session.user)
		stylo.db.commit()
		generated_secret = stylo.utils.password.get_decrypted_password(
			"User", stylo.session.user, fieldname="api_secret"
		)
		...
		self.file_name = stylo.utils.random_string(10) + ".txt"
		self.test_content = stylo.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = stylo.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = stylo.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(stylo.local.site) + new_private_file.file_url
