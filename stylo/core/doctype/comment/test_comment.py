...
class TestComment(IntegrationTestCase):
	...
	def test_comment_creation(self):
		test_doc = stylo.get_doc(doctype="ToDo", description="test")
		test_doc.insert()
		...
		# Check comment count
		counts = stylo.get_all("ToDo", {"name": test_doc.name}, ["*"], with_comment_count=True)
	...
	def test_public_comment(self):
		...
		stylo.db.delete("Comment", {"reference_doctype": "Test Blog Post"})
		add_comment_args = {
	...
	def test_user_not_logged_in(self):
		some_system_user = stylo.db.get_value("User", {"name": ("not in", stylo.STANDARD_USERS)})

		test_blog = stylo.get_doc("Web Page", "test-web-page-1")
		with set_user("Guest"):
			self.assertRaises(
