...
class NavbarSettings(Document):
	...
	if TYPE_CHECKING:
		from stylo.core.doctype.navbar_item.navbar_item import NavbarItem
...
def get_app_logo():
	app_logo = stylo.get_website_settings("app_logo") or stylo.get_cached_value(
		"Navbar Settings",
		"Navbar Settings",
		"app_logo",
	)
...
def get_navbar_settings():
	return stylo.get_single("Navbar Settings")
...
def sync_table(key, hook):
	...
	for item in stylo.get_hooks(hook):
