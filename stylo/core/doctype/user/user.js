stylo.ui.form.on("User", {
	setup: function (frm) {
		frm.set_query("default_workspace", () => {
			return {
				filters: {
					for_user: ["in", [null, stylo.session.user]],
	...
	before_load: function (frm) {
		let update_tz_options = function () {
			frm.fields_dict.time_zone.set_data(stylo.all_timezones);
		};
		...
		if (
			...
			if (!frm.roles_editor) {
				...
				if (frm.doc.user_type == "System User") {
					...
					frm.module_editor = new stylo.ModuleEditor(
						frm,
						module_area,
	...
	refresh: function (frm) {
		...
		stylo.xcall("stylo.apps.get_apps").then((r) => {
		...
		if (!frm.is_new()) {
			if (has_access_to_edit_user()) {
				...
				frm.add_custom_button(
					...
					() =>
						stylo.set_route("query-report", "User Doctype Permissions", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
		...
		if (frm.doc.user_emails && stylo.model.can_create("Email Account")) {
			...
			if (!found) {
				frm.add_custom_button(__("Create User Email"), function () {
					if (!frm.doc.email) {
						stylo.msgprint(__("Email is mandatory to create User Email"));
						return;
					}
			...
			callback: function (r) {
				if (!Array.isArray(r.message) || !r.message.length) {
					...
					stylo.model.with_doctype("Email Account", function (doc) {
						doc = stylo.model.get_new_doc("Email Account");
	...
	after_save: function (frm) {
		...
		if (doc.name === stylo.session.user && attr_tuples.some(has_effectively_changed)) {
		...
		if (
			...
			frm.add_custom_button(__("Impersonate"), () => {
				...
				stylo.prompt(
					...
					(values) => {
						stylo
							.xcall("stylo.core.doctype.user.user.impersonate", {
								user: frm.doc.name,
...
stylo.ui.form.on("User Role Profile", {
...
function show_api_key_dialog(api_key, api_secret) {
	const dialog = new stylo.ui.Dialog({
		...
		secondary_action: () => {
			...
			stylo.utils.copy_to_clipboard(token);
			dialog.hide();
		},
	});
...
stylo.ui.form.on("User Session Display", {
	sign_out(frm, doctype, name) {
		stylo
			.xcall("stylo.core.doctype.user.user.clear_session", { sid_hash: name })
			.then(() => frm.reload_doc());
	},
});
