...
class TestUser(IntegrationTestCase):
	...
	def test_delete(self):
		...
		user = stylo.copy_doc(self.globalTestRecords["User"][1])
		...
		stylo.copy_doc(self.globalTestRecords["Role"][1]).insert()
	...
	def test_password_strength(self):
		...
		with change_settings("System Settings", enable_password_policy=1, minimum_password_score=1):
			...
			self.assertRaises(
				stylo.exceptions.ValidationError, handle_password_test_fail, result["feedback"]
			)
			self.assertRaises(
...
class TestImpersonation(StyloAPITestCase):
	def test_impersonation(self):
		with test_user(roles=["System Manager"], commit=True) as user:
			self.post(
				self.method("stylo.core.doctype.user.user.impersonate"),
				{"user": user.name, "reason": "test", "sid": self.sid},
			)
			resp = self.get(self.method("stylo.auth.get_logged_user"))
