...
from stylo.website.utils import is_signup_disabled
...
class User(Document):
	...
	def populate_role_profile_roles(self):
		if self.role_profile_name:
			role_profile = stylo.get_doc("Role Profile", self.role_profile_name)
			self.set("roles", [])
	...
	def on_update(self):
		...
		now = stylo.flags.in_test or stylo.flags.in_install
	...
	def send_login_mail(self, subject, template, add_args, now=None):
		...
		sender = (
			stylo.session.user not in STANDARD_USERS and get_formatted_email(stylo.session.user) or None
		)
	...
	def on_trash(self):
		...
		# Delete EPS data
		stylo.db.delete("Energy Point Log", {"user": self.name})
	...
	def after_rename(self, old_name, new_name, merge=False):
		...
		# set email
		stylo.db.update("User", new_name, "email", new_name)
		stylo.cache().delete_key("users_for_mentions")
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(
			stylo.db.get_single_value("System Settings", "allow_login_using_mobile_number")
		)
		login_with_username = cint(
			stylo.db.get_single_value("System Settings", "allow_login_using_user_name")
		)
...
def get_roles(arg=None):
	...
	return stylo.get_roles(stylo.form_dict["uid"])
...
def update_password(new_password, logout_all_sessions=0, key=None, old_password=None):
	...
	if key and not isinstance(key, str):
		stylo.throw(_("Invalid key type"))
	...
	logout_all_sessions = cint(logout_all_sessions) or stylo.db.get_single_value(
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				stylo.db.get_single_value("System Settings", "reset_password_link_expiry_duration")
			)
			if (
...
def sign_up(email, full_name, redirect_to):
	...
	if user:
		...
		if stylo.db.get_creation_count("User", 60) > 300:
		...
		default_role = stylo.db.get_single_value("Portal Settings", "default_role")
		if default_role:
...
def get_system_users(exclude_users=None, limit=None):
	...
	system_users = stylo.db.sql_list(
...
def get_role_profile(role_profile):
	roles = stylo.get_doc("Role Profile", {"role_profile": role_profile})
	return roles.roles
...
def generate_keys(user):
	...
	user_details = stylo.get_doc("User", user)
