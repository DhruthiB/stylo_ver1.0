...
import stylo.share
from stylo import STANDARD_USERS, _, msgprint, throw
from stylo.apps import get_default_path
from stylo.auth import MAX_PASSWORD_SIZE
from stylo.core.doctype.user_type.user_type import user_linked_with_permission_on_doctype
...
from stylo.sessions import clear_sessions
...
from stylo.utils.data import sha256_hash
from stylo.utils.html_utils import sanitize_html
from stylo.utils.password import check_password, get_password_reset_limit
from stylo.utils.password import update_password as _update_password
from stylo.utils.user import get_system_managers
from stylo.website.utils import get_home_page, is_signup_disabled
...
class User(Document):
	...
	if TYPE_CHECKING:
		from stylo.core.doctype.block_module.block_module import BlockModule
		from stylo.core.doctype.defaultvalue.defaultvalue import DefaultValue
		from stylo.core.doctype.has_role.has_role import HasRole
		from stylo.core.doctype.user_email.user_email import UserEmail
		from stylo.core.doctype.user_role_profile.user_role_profile import UserRoleProfile
		from stylo.core.doctype.user_session_display.user_session_display import UserSessionDisplay
		from stylo.core.doctype.user_social_login.user_social_login import UserSocialLogin
	...
	def active_sessions(self):
		...
		if self.name != stylo.session.user:
		...
		for session in sessions_data:
			data = stylo.parse_json(session.sessiondata)
			...
			session_docs.append(
				{
					...
					"is_current": session.sid == stylo.session.sid,
	...
	def onload(self):
		from stylo.utils.modules import get_modules_from_all_apps
	...
	def after_insert(self):
		...
		stylo.cache.delete_key("users_for_mentions")
		stylo.cache.delete_key("enabled_users")
	...
	def validate(self):
		...
		if (self.name not in ["Administrator", "Guest"]) and (not self.get_social_login_userid("stylo")):
			self.set_social_login_userid("stylo", stylo.generate_hash(length=39))
	...
	def populate_role_profile_roles(self):
		...
		for role_profile in self.role_profiles:
			role_profile = stylo.get_cached_doc("Role Profile", role_profile.role_profile)
	...
	def move_role_profile_name_to_role_profiles(self):
		...
		from stylo.deprecation_dumpster import deprecation_warning
	...
	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = stylo.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			stylo.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		stylo.clear_cache(user=self.name)
		now = stylo.in_test or stylo.flags.in_install
		self.send_password_notification(self.__new_password)
		stylo.enqueue(
			"stylo.core.doctype.user.user.create_contact",
			user=self,
		...
		if self.name not in STANDARD_USERS and not self.user_image:
			stylo.enqueue(
				"stylo.core.doctype.user.user.update_gravatar",
				name=self.name,
				now=now,
				enqueue_after_commit=True,
			)
		...
		if self.time_zone:
			stylo.defaults.set_default("time_zone", self.time_zone, self.name)

		if self.has_value_changed("language"):
			locale_keys = ("date_format", "time_format", "number_format", "first_day_of_the_week")
			if self.language:
				language = stylo.get_doc("Language", self.language)
				for key in locale_keys:
					value = language.get(key)
					if value:
						stylo.defaults.set_default(key, value, self.name)
			else:
				for key in locale_keys:
					stylo.defaults.clear_default(key, parent=self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == stylo.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			stylo.throw(_("User {0} cannot be disabled").format(self.name))

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(stylo.local, "login_manager", None):
			stylo.local.login_manager.logout(user=self.name)
	...
	def set_system_user(self):
		...
		if self.user_type and not stylo.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = stylo.get_cached_doc("User Type", self.user_type)
	...
	def validate_share(self, docshare):
		...
		# 		if docshare.share != 1:
		# 			stylo.throw(_("Sorry! User should have complete access to their own record."))
		# 	else:
		# 		stylo.throw(_("Sorry! Sharing with Website User is prohibited."))
	...
	def send_password_notification(self, new_password):
		...
		except stylo.OutgoingEmailError:
	...
	def password_reset_mail(self, link):
		reset_password_template = stylo.db.get_system_setting("reset_password_template")
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = stylo.get_hooks("welcome_email")
		if method:
			subject = stylo.get_attr(method[-1])()
		if not subject:
			site_name = stylo.db.get_default("site_name") or stylo.get_conf().get("site_name")
			if site_name:
				subject = _("Welcome to {0}").format(site_name)
			else:
				subject = _("Complete Registration")

		welcome_email_template = stylo.db.get_system_setting("welcome_email_template")
	...
	def send_login_mail(self, subject, template, add_args, now=None, custom_template=None):
		...
		from stylo.utils import get_url
		from stylo.utils.user import get_user_fullname

		created_by = get_user_fullname(stylo.session["user"])
		if created_by == "Guest":
		...
		sender = (
			stylo.session.user not in STANDARD_USERS and get_formatted_email(stylo.session.user)
		) or None

		if custom_template:
			from stylo.email.doctype.email_template.email_template import get_email_template
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(stylo.local, "login_manager", None):
			stylo.local.login_manager.logout(user=self.name)

		# delete todos
		stylo.db.delete("ToDo", {"allocated_to": self.name})
		todo_table = DocType("ToDo")
		(
			stylo.qb.update(todo_table)
			.set(todo_table.assigned_by, None)
			.where(todo_table.assigned_by == self.name)
		).run()

		# delete events
		stylo.db.delete("Event", {"owner": self.name, "event_type": "Private"})

		# delete shares
		stylo.db.delete("DocShare", {"user": self.name})
		# unlink contact
		table = DocType("Contact")
		stylo.qb.update(table).where(table.user == self.name).set(table.user, None).run()

		# delete notification settings
		stylo.delete_doc("Notification Settings", self.name, ignore_permissions=True)
		...
		stylo.db.delete("User Permission", {"user": self.name})

		# Delete OAuth data
		stylo.db.delete("OAuth Authorization Code", {"user": self.name})
		stylo.db.delete("Token Cache", {"user": self.name})

		# Remove user link from Workflow Action
		stylo.db.set_value("Workflow Action", {"user": self.name}, "user", None)

		# Delete user's List Filters
		stylo.db.delete("List Filter", {"for_user": self.name})

		# Remove user from Note's Seen By table
		seen_notes = stylo.get_all("Note", filters=[["Note Seen By", "user", "=", self.name]], pluck="name")
		for note_id in seen_notes:
			note = stylo.get_doc("Note", note_id)
		...
		invites = stylo.db.get_all("User Invitation", filters={"email": self.name}, pluck="name")
		for invite in invites:
			invite_doc = stylo.get_doc("User Invitation", invite)
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		if merge:
			stylo.delete_doc("Notification Settings", old_name, ignore_permissions=True)

		stylo.clear_cache(user=old_name)
	...
	def after_rename(self, old_name, new_name, merge=False):
		tables = stylo.db.get_tables()
		for tab in tables:
			desc = stylo.db.get_table_columns_description(tab)
		...
		if stylo.db.exists("Notification Settings", old_name):
			stylo.rename_doc("Notification Settings", old_name, new_name, force=True, show_alert=False)

		# set email
		stylo.db.set_value("User", new_name, "email", new_name)
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in stylo.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = stylo.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				stylo.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(stylo.scrub(self.first_name))

		if not username:
			# @firstname_last_name
			username = _check_suggestion(stylo.scrub("{} {}".format(self.first_name, self.last_name or "")))

		if username:
			stylo.msgprint(_("Suggested Username: {0}").format(username))

		return username

	def username_exists(self, username=None):
		return stylo.db.get_value("User", {"username": username or self.username, "name": ("!=", self.name)})
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			stylo.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(stylo.get_system_settings("allow_login_using_mobile_number"))
		login_with_username = cint(stylo.get_system_settings("allow_login_using_user_name"))
		...
		users = stylo.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
		...
		if validate_password:
			...
			except stylo.AuthenticationError:
	...
	def check_roles_added(self):
		...
		stylo.msgprint(
			_("Newly created user {0} has no roles enabled.").format(stylo.bold(self.name)),
			title=_("No Roles Specified"),
			indicator="orange",
			primary_action={
				"label": _("Add Roles"),
				"client_action": "stylo.set_route",
...
def get_all_roles():
	...
	roles = stylo.get_all(
		"Role",
		filters={
			"name": ("not in", stylo.permissions.AUTOMATIC_ROLES),
			"disabled": 0,
		},
...
def get_roles(arg=None):
	...
	return stylo.get_roles(stylo.form_dict.get("uid", stylo.session.user))


@stylo.whitelist()
def get_perm_info(role):
	"""get permission info"""
	from stylo.permissions import get_all_perms

	return get_all_perms(role)


@stylo.whitelist(allow_guest=True, methods=["POST"])
def update_password(
	...
	if len(new_password) > MAX_PASSWORD_SIZE:
		stylo.throw(_("Password size exceeded the maximum allowed size."))
	...
	if res.get("message"):
		stylo.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or stylo.get_system_settings("logout_on_password_reset")
	...
	redirect_to = stylo.cache.hget("redirect_after_login", user)
	if redirect_to:
		redirect_url = redirect_to
		stylo.cache.hdel("redirect_after_login", user)

	stylo.local.login_manager.login_as(user)

	stylo.db.set_value("User", user, "last_password_reset_date", today())
	stylo.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
	from stylo.utils.password_strength import test_password_strength as _test_password_strength
	...
	enable_password_policy = stylo.get_system_settings("enable_password_policy")

	if not enable_password_policy:
		return {}

	if not user_data:
		user_data = stylo.db.get_value(
			"User", stylo.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)

	if new_password:
		result = _test_password_strength(new_password, user_inputs=user_data)
		password_policy_validation_passed = False
		minimum_password_score = cint(stylo.get_system_settings("minimum_password_score"))
...
def has_email_account(email: str):
	return stylo.get_list("Email Account", filters={"email_id": email})


@stylo.whitelist(allow_guest=False)
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				stylo.get_system_settings("reset_password_link_expiry_duration")
			)
			if (
		...
		# verify old password
		stylo.local.login_manager.check_password(stylo.session.user, old_password)
...
def verify_password(password):
	stylo.local.login_manager.check_password(stylo.session.user, password)


@stylo.whitelist(allow_guest=True)
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	if is_signup_disabled():
		stylo.throw(_("Sign Up is disabled"), title=_("Not Allowed"))

	user = stylo.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		max_signups_allowed_per_hour = cint(stylo.get_system_settings("max_signups_allowed_per_hour") or 300)
		users_created_past_hour = stylo.db.get_creation_count("User", 60)
		...
		default_role = stylo.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			stylo.cache.hset("redirect_after_login", user.name, redirect_to)
...
def reset_password(user: str) -> str:
	try:
		user: User = stylo.get_doc("User", user)
		...
		stylo.local.response["http_status_code"] = 404
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters:
		if not (filters.get("ignore_user_type") and stylo.session.data.user_type == "System User"):
...
def get_website_users():
	...
	return stylo.db.count("User", filters={"enabled": True, "user_type": "Website User"})
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(stylo.db.escape(user) for user in STANDARD_USERS)
		)
...
def notify_admin_access_to_system_manager(login_manager=None):
	if (
		...
		and stylo.local.conf.notify_admin_access_to_system_manager
	):
		site = '<a href="{0}" target="_blank">{0}</a>'.format(stylo.local.request.host_url)
		date_and_time = "<b>{}</b>".format(format_datetime(now_datetime(), format_string="medium"))
		ip_address = stylo.local.request_ip
...
def handle_password_test_fail(feedback: dict):
	...
	stylo.throw(msg=" ".join([warning, *suggestions]), title=_("Invalid Password"))


def update_gravatar(name):
	gravatar = has_gravatar(name)
	if gravatar:
		stylo.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if stylo.db.get_creation_count("User", 60) > stylo.local.conf.get("throttle_user_limit", 60):
		stylo.throw(_("Throttled"))


@stylo.whitelist()
def get_module_profile(module_profile: str):
	module_profile = stylo.get_doc("Module Profile", {"module_profile_name": module_profile})
...
def create_contact(user, ignore_links=False, ignore_mandatory=False):
	...
	if not contact_name:
		...
		except stylo.TimestampMismatchError:
			raise stylo.RetryBackgroundJobError
...
def generate_keys(user: str):
	...
	user_details: User = stylo.get_doc("User", user)
	api_secret = stylo.generate_hash(length=15)
	# if api key is not set generate api key
	if not user_details.api_key:
		api_key = stylo.generate_hash(length=15)
...
def switch_theme(theme):
	if theme in ["Dark", "Light", "Automatic"]:
		stylo.db.set_value("User", stylo.session.user, "desk_theme", theme)


def get_enabled_users():
	def _get_enabled_users():
		enabled_users = stylo.get_all("User", filters={"enabled": "1"}, pluck="name")
		return enabled_users

	return stylo.cache.get_value("enabled_users", _get_enabled_users)
...
def impersonate(user: str, reason: str):
	...
	impersonator = stylo.session.user
	stylo.get_doc(
		{
	...
	stylo.local.login_manager.impersonate(user)


@stylo.whitelist()
@rate_limit(limit=10, seconds=60 * 60, methods="POST")
def clear_session(sid_hash: str):
	from stylo.sessions import delete_session

	sessions = stylo.qb.DocType("Sessions")
	sessions_data = (
		stylo.qb.from_(sessions).select(sessions.sid).where(sessions.user == stylo.session.user)
	).run(pluck=True)

	for session in sessions_data:
		if sha256_hash(session) == sid_hash:
			delete_session(sid=session, reason="Force Logged out by the user", user=stylo.session.user)
			stylo.toast(_("Successfully signed out"))
			return
