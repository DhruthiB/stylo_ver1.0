stylo.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = stylo.ui.make_app_page({
		parent: wrapper,
	...
	$(stylo.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new stylo.PermissionEngine(wrapper);
};

stylo.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

stylo.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			change: function () {
				stylo.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return stylo.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (stylo.get_route()[1]) {
			this.doctype_select.set_value(stylo.get_route()[1]);
		} else if (stylo.route_options) {
			if (stylo.route_options.doctype) {
				this.doctype_select.set_value(stylo.route_options.doctype);
			}
			if (stylo.route_options.role) {
				this.role_select.set_value(stylo.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = stylo.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return stylo
				.call({
					module: "stylo.core",
					page: "permission_manager",
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(stylo.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${stylo.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return stylo.call({
					...
					callback: (r) => {
						if (r.exc) {
							stylo.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			stylo.route_options = { allow: this.get_doctype() || "" };
	...
	get_link_fields(doctype) {
		return stylo.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
