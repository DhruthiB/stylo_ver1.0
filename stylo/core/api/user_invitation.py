...
import stylo.utils
from stylo import _
from stylo.core.doctype.user_invitation.user_invitation import UserInvitation


@stylo.whitelist(methods=["POST"])
def invite_by_email(
	emails: str, roles: list[str], redirect_to_path: str, app_name: str = "stylo"
) -> dict[str, list[str]]:
	UserInvitation.validate_role(app_name)

	# validate emails
	stylo.utils.validate_email_address(emails, throw=True)
	email_list = stylo.utils.split_emails(emails)
	if not email_list:
		stylo.throw(title=_("Invalid input"), msg=_("No email addresses to invite"))

	# get relevant data from the database
	disabled_user_emails = stylo.db.get_all(
		"User",
		filters={"email": ["in", email_list], "enabled": 0},
		pluck="email",
	)
	accepted_invite_emails = stylo.db.get_all(
		"User Invitation",
		filters={
			"email": ["in", email_list],
			"status": "Accepted",
	...
	pending_invite_emails = stylo.db.get_all(
		"User Invitation",
...
@stylo.whitelist(methods=["PATCH", "POST"])
def cancel_invitation(name: str, app_name: str):
	UserInvitation.validate_role(app_name)

	if not stylo.db.exists("User Invitation", name):
		stylo.throw(title=_("Error"), msg=_("Invitation not found"))

	invitation = stylo.get_doc("User Invitation", name)
...
def _accept_invitation(key: str, in_test: bool) -> None:
	# get invitation
	hashed_key = stylo.utils.sha256_hash(key)
	invitation_name = stylo.db.get_value("User Invitation", filters={"key": hashed_key})
	if not invitation_name:
		stylo.throw(title=_("Error"), msg=_("Invalid key"))
	invitation = stylo.get_doc("User Invitation", invitation_name)

	# accept invitation
	invitation.accept(ignore_permissions=True)

	user = stylo.get_doc("User", invitation.email)
	should_update_password = not user.last_password_reset_date and not bool(
		stylo.get_system_settings("disable_user_pass_login")
	)

	# set redirect_to
	redirect_to = stylo.utils.get_url(invitation.get_redirect_to_path())
	...
	if not in_test and not should_update_password:
		stylo.local.login_manager.login_as(invitation.email)
