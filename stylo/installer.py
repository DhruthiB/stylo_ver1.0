...
import stylo
from stylo.defaults import _clear_cache
from stylo.utils import cint, is_git_url
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		stylo.init(site)
		stylo.connect()
		enable_scheduler = cint(stylo.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		stylo.db.close()
...
def _new_site(
	...
	"""Install a new Stylo site"""

	from stylo.utils import scheduler
	...
	if not db_name:
		db_name = f"_{stylo.generate_hash(length=16)}"
	...
	if rollback_callback:
		rollback_callback.add(lambda: shutil.rmtree(stylo.get_site_path()))

	with filelock("forge_new_site", timeout=1):
		install_db(
		...
		apps_to_install = ["stylo"] + (stylo.conf.get("install_apps") or []) + (list(install_apps or []))
	...
	scheduler_status = "disabled" if stylo.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from stylo.database import bootstrap_database, drop_user_and_database, setup_database
	...
	stylo.flags.in_install_db = True

	if root_login:
		stylo.flags.root_login = root_login

	if root_password:
		stylo.flags.root_password = root_password
	...
	stylo.conf.admin_password = stylo.conf.admin_password or admin_password

	remove_missing_apps()

	stylo.db.create_auth_table()
	stylo.db.create_global_search_table()
	stylo.db.create_user_settings_table()

	stylo.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/stylo/forge/blob/develop/forge/utils/__init__.py#L390
	...
	from stylo.exceptions import InvalidRemoteException

	for org in ["stylo", "erpnext"]:
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/stylo/forge/blob/develop/forge/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/stylo/forge/blob/develop/forge/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from stylo.model.sync import sync_for
	from stylo.modules.utils import sync_customizations
	from stylo.utils.fixtures import sync_fixtures

	stylo.flags.in_install = name
	stylo.flags.ignore_in_install = False

	stylo.clear_cache()
	app_hooks = stylo.get_hooks(app_name=name)
	...
	if name not in stylo.get_all_apps():
	...
	other_class_overrides = stylo.get_hooks("override_doctype_class")
	if (
	...
	for before_install in app_hooks.before_install or []:
		out = stylo.get_attr(before_install)()
		if out is False:
			return

	for fn in stylo.get_hooks("before_app_install"):
		stylo.get_attr(fn)(name)
	...
	stylo.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		stylo.get_attr(after_install)()

	for fn in stylo.get_hooks("after_app_install"):
		stylo.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		stylo.get_attr(after_sync)()  #

	stylo.clear_cache()
	stylo.client_cache.erase_persistent_caches()
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		stylo.db.set_global("installed_apps", json.dumps(installed_apps))
		stylo.db.commit()
...
def remove_from_installed_apps(app_name):
	...
	if app_name in installed_apps:
		...
		stylo.local.doc_events_hooks = None
		stylo.get_single("Installed Applications").update_versions()
		stylo.db.commit()
		if stylo.flags.in_install:
			post_install()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = stylo.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in stylo.get_installed_apps():
	...
	for app in stylo.get_installed_apps():
		if app != app_name:
			hooks = stylo.get_hooks(app_name=app)
	...
	if not (dry_run or no_backup):
		from stylo.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	stylo.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		stylo.get_attr(before_uninstall)()

	for fn in stylo.get_hooks("before_app_uninstall"):
		stylo.get_attr(fn)(app_name)

	modules = stylo.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		stylo.get_attr(after_uninstall)()

	for fn in stylo.get_hooks("after_app_uninstall"):
		stylo.get_attr(fn)(app_name)

	stylo.client_cache.erase_persistent_caches()

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	stylo.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in stylo.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					stylo.delete_doc(doctype.name, doctype.name, ignore_on_trash=True, force=True)
					stylo.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			stylo.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in stylo.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				stylo.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = stylo.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if stylo.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			stylo.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			stylo.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from stylo.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		stylo.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	stylo.db.commit()


def init_singles():
	singles = stylo.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if stylo.db.get_singles_dict(single):
			continue

		try:
			doc = stylo.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, stylo.DoesNotExistError):
...
def make_conf(
	...
	sites_path = stylo.local.sites_path
	stylo.destroy()
	stylo.init(site, sites_path=sites_path)
...
def make_site_config(
	...
	stylo.create_folder(os.path.join(stylo.local.site_path))
...
def update_site_config(key, value, validate=True, site_config_path=None):
	...
	from stylo.config import clear_site_config_cache
	from stylo.utils.synchronization import filelock
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(stylo.local, "conf"):
		stylo.local.conf[key] = value


def get_site_config_path():
	return os.path.join(stylo.local.site_path, "site_config.json")
...
def clear_site_locks():
	...
	path = Path(stylo.get_site_path("locks"))
...
def make_site_dirs():
	for dir_path in [
		...
		path = stylo.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = stylo.get_module_list(app)
	for module in modules:
		d = stylo.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("stylo_subscription", "shopping_cart")
	installed_apps = json.loads(stylo.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_files(site_name, file_path):
	...
	from stylo.utils import get_forge_relative_path

	file_path = get_forge_relative_path(file_path)

	# Need to do stylo.init to maintain the site locals
	stylo.init(site_name)
	abs_site_path = os.path.abspath(stylo.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	"""Check if input db backup will get downgraded on current forge
	...
	current_version = Version(stylo.__version__)
	...
	if verbose and is_downgrade:
		print(f"Your site is currently on Stylo {current_version} and your backup is {backup_version}.")

	return is_downgrade


def get_old_backup_version(sql_file_path: str) -> Version | None:
	"""Return the stylo version used to create the specified database dump.

	This methods supports older versions of Stylo wich used a different format.
	"""
	header = get_db_dump_header(sql_file_path).split("\n")
	if match := re.search(r"Stylo (\d+\.\d+\.\d+)", header[0]):
...
def get_backup_version(sql_file_path: str) -> Version | None:
	...
	header = get_db_dump_header(sql_file_path).split("\n")
	metadata = ""
	if "begin stylo metadata" in header[0]:
		for line in header[1:]:
			if "end stylo metadata" in line:
				break
			metadata += line.replace("--", "").strip() + "\n"
		parser = configparser.ConfigParser()
		parser.read_string(metadata)
		return Version(parser["stylo"]["version"])
...
def partial_restore(sql_file_path, verbose=False):
	if stylo.conf.db_type == "mariadb":
		from stylo.database.mariadb.setup_db import import_db_from_sql
	elif stylo.conf.db_type == "postgres":
		import warnings

		from stylo.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		if (executable := which(executable_name)) is None:
			stylo.throw(
				...
				exc=stylo.ExecutableNotFound,
			)
		try:
			stylo.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise stylo.InvalidDatabaseFile
