...
def _new_site(
	...
	from stylo.utils import get_site_path, scheduler, touch_file
	...
	if not db_name:
		...
		db_name = "_" + hashlib.sha1(os.path.realpath(stylo.get_site_path()).encode()).hexdigest()[:16]
...
def install_db(
	...
	from stylo.database import setup_database

	if not db_type:
		db_type = stylo.conf.db_type or "mariadb"
		...
		for doctype in stylo.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					...
					stylo.delete_doc("DocType", doctype.name, ignore_on_trash=True)
				else:
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			stylo.delete_doc("DocType", doctype, ignore_on_trash=True)
...
def extract_sql_from_archive(sql_file_path):
	...
	SQL file or an SQL file path. The path may be absolute or relative from the forge
	...
	sql_file_path = get_forge_relative_path(sql_file_path)
...
def is_downgrade(sql_file_path, verbose=False):
	"""checks if input db backup will get downgraded on current forge"""

	# This function is only tested with mariadb
	# TODO: Add postgres support
	if stylo.conf.db_type not in (None, "mariadb"):
		return False
	...
	with open(sql_file_path) as f:
		header = f.readline()
		# Example first line:
		# -- Backup generated by Stylo 15.1.0 on branch fix-backup-restore

		if match := re.search(r"Stylo (\d+\.\d+\.\d+)", header):
			...
			if verbose and downgrade:
				print(f"Your site will be downgraded from Stylo {backup_version} to {current_version}")
...
def partial_restore(sql_file_path, verbose=False):
	...
	if stylo.conf.db_type in (None, "mariadb"):
