...
class OAuthWebRequestValidator(RequestValidator):
	...
	def get_default_redirect_uri(self, client_id, request, *args, **kwargs):
		...
		return stylo.db.get_value("OAuth Client", client_id, "default_redirect_uri")
		...
		if code in [vcode["name"] for vcode in validcodes]:
			...
			if code_challenge and not request.code_verifier:
				if stylo.db.exists("OAuth Authorization Code", code):
					stylo.delete_doc("OAuth Authorization Code", code, ignore_permissions=True, force=True)
					stylo.db.commit()
				return False
	...
	def save_bearer_token(self, token, request, *args, **kwargs):
		...
		return stylo.db.get_value("OAuth Client", request.client["name"], "default_redirect_uri")
...
def delete_oauth2_data():
	stylo.db.delete("OAuth Authorization Code", {"validity": "Invalid"})
	stylo.db.delete("OAuth Bearer Token", {"status": "Revoked"})
