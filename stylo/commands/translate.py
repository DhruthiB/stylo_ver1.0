...
def build_message_files(context: CliCtxObj):
	...
	for site in context.sites:
		try:
			stylo.init(site)
			stylo.connect()
			stylo.translate.rebuild_all_translation_files()
		finally:
			stylo.destroy()
...
@click.argument("app")  # , help="App name eg. stylo")
@pass_context
def new_language(context: CliCtxObj, lang_code, app):
	...
	# init site
	stylo.init(site=context.sites[0])
	stylo.connect()
	stylo.translate.write_translations_file(app, lang_code)

	print(f"File created at ./apps/{app}/{app}/translations/{lang_code}.csv")
	print("You will need to add the language in stylo/geo/languages.csv, if you haven't done it already.")
...
def get_untranslated(context: CliCtxObj, lang, untranslated_file, app="_ALL_APPS", all=None):
	...
	try:
		stylo.init(site)
		stylo.connect()
		stylo.translate.get_untranslated(lang, untranslated_file, get_all=all, app=app)
	finally:
		stylo.destroy()
...
def update_translations(context: CliCtxObj, lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	try:
		stylo.init(site)
		stylo.connect()
		stylo.translate.update_translations(lang, untranslated_file, translated_file, app=app)
	finally:
		stylo.destroy()
...
def import_translations(context: CliCtxObj, lang, path):
	...
	try:
		stylo.init(site)
		stylo.connect()
		stylo.translate.import_translations(lang, path)
	finally:
		stylo.destroy()
...
def migrate_translations(context: CliCtxObj, source_app, target_app):
	...
	try:
		stylo.init(site)
		stylo.connect()
		stylo.translate.migrate_translations(source_app, target_app)
	finally:
		stylo.destroy()
