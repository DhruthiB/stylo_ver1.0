...
if TYPE_CHECKING:
	from stylo.testing import TestRunner
...
def main(
	...
	if lightmode:
		from stylo.testing.config import TestParameters
	...
	from stylo.testing.environment import _cleanup_after_tests, _initialize_test_environment
	...
	testing_module_logger = logging.getLogger("stylo.testing")
...
def run_tests_in_light_mode(test_params):
	from stylo.testing.loader import StyloTestLoader
	from stylo.testing.result import StyloTestResult
	from stylo.tests.utils import toggle_test_mode

	# init environment
	stylo.init(test_params.site)
	if not stylo.db:
		stylo.connect()

	# disable scheduler
	global scheduler_disabled_by_user
	scheduler_disabled_by_user = stylo.utils.scheduler.is_scheduler_disabled(verbose=False)
	if not scheduler_disabled_by_user:
		stylo.utils.scheduler.disable_scheduler()
	stylo.clear_cache()

	toggle_test_mode(True)
	suite = StyloTestLoader().discover_tests(test_params)
	result = unittest.TextTestRunner(failfast=test_params.failfast, resultclass=StyloTestResult).run(suite)
...
def _load_doctype_list(doctype_list_path):
	...
	with open(stylo.get_app_path(app, path)) as f:
...
def _run_module_def_tests(app, module_def, runner: "TestRunner", force) -> "TestRunner":
	...
	from stylo.testing import discover_doctype_tests
...
def _get_doctypes_for_module_def(app, module_def):
	...
	doctypes = []
	doctypes_ = stylo.get_list(
		"DocType",
		filters={"module": module_def, "istable": 0},
		fields=["name", "module"],
		as_list=True,
	)
	from stylo.modules import get_module_name
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		...
		stylo.init(site)
		allow_tests = stylo.conf.allow_tests
...
def run_ui_tests(
	...
	site = get_site(context)
	stylo.init(site)
	app_base_path = stylo.get_app_source_path(app)
	site_url = stylo.utils.get_site_url(site)
	admin_password = stylo.get_conf().admin_password
	...
	node_bin = subprocess.getoutput("(cd ../stylo && yarn bin)")
	...
	if not (
		...
		stylo.commands.popen(f"(cd ../stylo && yarn add {packages} --no-lockfile)")
