...
def trigger_scheduler_event(context: CliCtxObj, event):
	...
	for site in context.sites:
		try:
			...
			stylo.connect()
			try:
				stylo.get_doc("Scheduled Job Type", {"method": event}).execute()
...
def scheduler(context: CliCtxObj, state: str, format: str, verbose: bool = False, site: str | None = None):
	...
	import stylo
	from stylo.utils.scheduler import is_scheduler_inactive, toggle_scheduler
	...
	with stylo.init_site(site=site):
		match state:
			case "status":
				stylo.connect()
...
def doctor(context: CliCtxObj, site=None):
	...
	from stylo.utils.doctor import doctor as _doctor
...
def show_pending_jobs(context: CliCtxObj, site=None):
	...
	from stylo.utils.doctor import pending_jobs as _pending_jobs
...
def start_worker(queue, quiet=False, rq_username=None, rq_password=None, burst=False, strategy=None):
	...
	from stylo.utils.background_jobs import start_worker
...
def start_worker_pool(queue, quiet=False, num_workers=2, burst=False):
	...
	from stylo.utils.background_jobs import start_worker_pool
...
def ready_for_migration(context: CliCtxObj, site=None):
	...
	from stylo.utils.doctor import any_job_pending
