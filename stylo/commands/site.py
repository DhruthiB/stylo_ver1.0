...
import stylo
from stylo.commands import get_site, pass_context
from stylo.exceptions import SiteNotSpecifiedError
from stylo.utils import CallbackManager
from stylo.utils.forge_helper import CliCtxObj
...
def new_site(
	...
	"Create a new site"
	from stylo.installer import _new_site

	stylo.init(site, new_site=True)

	if site in stylo.get_all_apps():
		click.secho(
			f"Your forge has an app called {site}, please choose another name for the site.", fg="red"
		)
		sys.exit(1)
...
def _restore(
	*,
	...
	from stylo.installer import extract_files
	from stylo.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key

	# Check for the backup file in the backup directory, as well as the main forge directory
	...
	err, out = stylo.utils.execute_in_shell(f"file {sql_file_path}", check_exit_code=True)
	if err:
...
def restore_backup(
	...
	from stylo.installer import _new_site, is_downgrade, is_partial, validate_database_sql

	if is_partial(sql_file_path):
		click.secho(
			"Partial Backup file detected. You cannot use a partial file to restore a Stylo site.",
			fg="red",
		)
		click.secho(
			"Use `forge partial-restore` to restore a partial backup to an existing site.",
			fg="yellow",
		)
		sys.exit(1)

	# Check if the backup is of an older version of stylo and the user hasn't specified force
...
def partial_restore(context: CliCtxObj, sql_file_path, verbose, encryption_key=None):
	from stylo.installer import is_partial, partial_restore
	from stylo.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
	...
	if "cipher" in out.decode().split(":")[-1].strip():
		...
		with decrypt_backup(sql_file_path, key):
			if not is_partial(sql_file_path):
				click.secho(
					"Full backup file detected. Use `forge restore` to restore a Stylo Site.",
					fg="red",
				)
				sys.exit(1)
...
def _reinstall(
	...
	try:
		...
		stylo.connect()
		stylo.clear_cache()
		installed = stylo.get_installed_apps()
		stylo.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context: CliCtxObj, apps, force=False):
	...
	from stylo.installer import install_app as _install_app
	from stylo.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except stylo.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {err!s}\n{stylo.get_traceback(with_context=True)}"
...
def list_apps(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		apps = stylo.get_single("Installed Applications").installed_applications
		if apps:
			installed_apps_info.extend(format_app(app) for app in apps)
		else:
			installed_apps_info.extend(stylo.get_installed_apps())
	...
	if format == "json":
		click.echo(stylo.as_json(summary_dict))
...
def add_db_index(context: CliCtxObj, doctype, column):
	...
	for site in context.sites:
		...
		stylo.connect()
		try:
			stylo.db.add_index(doctype, columns)
			stylo.db.commit()
		finally:
			stylo.destroy()
...
def describe_database_table(context, doctype, column):
	...
	from stylo.core.doctype.recorder.recorder import _fetch_table_stats
...
def add_system_manager(context: CliCtxObj, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		...
		stylo.connect()
		try:
			stylo.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			stylo.db.commit()
		finally:
			stylo.destroy()
	if not context.sites:
...
def disable_user(context: CliCtxObj, email):
	...
	site = get_site(context)
	with stylo.init_site(site):
		stylo.connect()
		user = stylo.get_doc("User", email)
		user.enabled = 0
...
def migrate(context: CliCtxObj, skip_failing=False, skip_search_index=False, skip_fixtures=False):
	...
	from stylo.migrate import SiteMigration
...
def migrate_to():
	...
	from stylo.integrations.stylo_providers import migrate_to
...
def run_patch(context: CliCtxObj, module, force):
	...
	for site in context.sites:
		...
		try:
			stylo.connect()
			stylo.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			stylo.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context: CliCtxObj, module, doctype, docname):
	...
	for site in context.sites:
		try:
			stylo.init(site)
			stylo.connect()
			stylo.reload_doc(module, doctype, docname, force=context.force)
			stylo.db.commit()
		finally:
			stylo.destroy()
	if not context.sites:
...
def reload_doctype(context: CliCtxObj, doctype):
	...
	for site in context.sites:
		try:
			stylo.init(site)
			stylo.connect()
			stylo.reload_doctype(doctype, force=context.force)
			stylo.db.commit()
		finally:
			stylo.destroy()
	if not context.sites:
...
def add_to_hosts(context: CliCtxObj):
	"Add site to hosts"
	for site in context.sites:
		stylo.commands.popen(f"echo '127.0.0.1\t{site}\n::1\t{site}' | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
	...
	for site in context.sites:
		try:
			...
			if rollback_callback:
				...
				rollback_callback = None
			if verbose:
				print(stylo.get_traceback(with_context=True))
			exit_code = 1
			continue
		if stylo.get_system_settings("encrypt_backup") and stylo.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context: CliCtxObj, app):
	...
	ensure_app_not_stylo(app)
	from stylo.installer import remove_from_installed_apps
...
def uninstall(context: CliCtxObj, app, dry_run, yes, no_backup, force):
	...
	ensure_app_not_stylo(app)
	from stylo.installer import remove_app
	from stylo.utils.synchronization import filelock
...
def _drop_site(
	...
	from stylo.database import drop_user_and_database
	from stylo.utils.backups import scheduled_backup
	...
	try:
		...
		if force:
			...
			messages = [
				...
				f"Hint: Use 'forge drop-site {site} --force' to force the removal of {site}",
			]
			click.echo("\n".join(messages))
			sys.exit(1)
	...
	stylo.flags.root_login = db_root_username
	stylo.flags.root_password = db_root_password

	drop_user_and_database(stylo.conf.db_name, stylo.conf.db_user)

	archived_sites_path = archived_sites_path or os.path.join(
		stylo.utils.get_forge_path(), "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	stylo.init(site)
	old_path = stylo.utils.get_site_path()
...
def set_last_active_for_user(context: CliCtxObj, user=None):
	...
	from stylo.core.doctype.user.user import get_system_users
	...
	with stylo.init_site(site):
		...
		stylo.db.set_value("User", user, "last_active", now_datetime())
		stylo.db.commit()
...
def publish_realtime(context: CliCtxObj, event, message, room, user, doctype, docname, after_commit):
	"Publish realtime event from forge"
	from stylo import publish_realtime
...
def browse(
	...
	if site not in stylo.utils.get_sites():
	...
	if user:
		...
		if stylo.conf.developer_mode or user == "Administrator":
			stylo.utils.set_request(path="/")
			stylo.local.cookie_manager = CookieManager()
			stylo.local.login_manager = LoginManager()
			stylo.local.login_manager.login_as(user, session_end, user_for_audit)
			sid = f"/app?sid={stylo.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{stylo.utils.get_site_url(site)}{sid}"
...
def start_ngrok(context: CliCtxObj, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = stylo.conf.ngrok_authtoken
...
def clear_log_table(context: CliCtxObj, doctype, days, no_backup):
	...
	"""
	from stylo.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from stylo.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from stylo.utils.backups import scheduled_backup
...
def trim_database(context: CliCtxObj, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		database_tables: list[str] = (
			stylo.qb.from_(information_schema.tables)
			.select(table_name)
			.where(information_schema.tables.table_schema == stylo.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run(pluck=True)
		)
		doctype_tables = stylo.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			...
			for table in TABLES_TO_DROP:
				...
				stylo.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[stylo.local.site] = TABLES_TO_DROP
		stylo.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"database",
		stylo.conf.db_type,
		f"framework_{stylo.conf.db_type}.sql",
	)
...
def trim_tables(context: CliCtxObj, dry_run, format, no_backup):
	...
	from stylo.model.meta import trim_tables
	from stylo.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {stylo.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {stylo.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({stylo.local.site: data}, indent=1, sort_keys=True))
	else:
		from stylo.utils.commands import render_table
...
def ensure_app_not_stylo(app: str) -> None:
	"""
	Ensure that the app name passed is not 'stylo'

	:param app: Name of the app
	:return: Nothing
	"""
	if app == "stylo":
		click.secho("You cannot remove or uninstall the app `stylo`", fg="red")
		sys.exit(1)
...
def bypass_patch(context: CliCtxObj, patch_name: str, yes: bool):
	...
	from stylo.modules.patch_handler import update_patch_log
