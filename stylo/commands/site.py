...
def new_site(
	...
	"Create a new site"
	from stylo.installer import _new_site, extract_sql_from_archive

	stylo.init(site=site, new_site=True)
...
def restore(
	...
	from stylo.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	try:
		...
		if is_partial(decompressed_file_name):
			...
			click.secho(
				"Use `forge partial-restore` to restore a partial backup to an existing site.", fg="yellow"
			)
	...
	# dont allow downgrading to older versions of stylo without force
...
def partial_restore(context, sql_file_path, verbose, encryption_key=None):
	from stylo.installer import extract_sql_from_archive, partial_restore
	from stylo.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	try:
		...
		with open(decompressed_file_name) as f:
			...
			if "Partial Backup" not in header:
				click.secho(
					"Full backup file detected.Use `forge restore` to restore a Stylo Site.", fg="red"
				)
				_backup.decryption_rollback()
				sys.exit(1)
...
def list_apps(context, format):
	...
	for site in context.sites:
		...
		if apps:
			...
			installed_applications = stylo.get_installed_apps()
			applications_summary = "\n".join(installed_applications)
			summary = f"{site_title}\n{applications_summary}\n"
...
def _extract_table_stats(doctype: str, columns: list[str]) -> dict:
	from stylo.utils import cint, cstr, get_table_name
	...
	schema = []
	for field in stylo.db.sql(f"describe {table}", as_dict=True):
		schema.append(
			{
				"column": field["Field"],
				"type": field["Type"],
	...
	indexes = []
	for idx in stylo.db.sql(f"show index from {table}", as_dict=True):
		indexes.append(
			{
	...
	for column in columns:
		cardinality = stylo.db.sql(f"select count(distinct {column}) from {table}")[0][0]
...
def add_to_hosts(context):
	"Add site to hosts"
	for site in context.sites:
		stylo.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
...
def _drop_site(
	...
	drop_user_and_database(stylo.conf.db_name, db_root_username, db_root_password)

	archived_sites_path = archived_sites_path or os.path.join(
		stylo.get_app_path("stylo"), "..", "..", "..", "archived", "sites"
	)
...
def trim_database(context, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		if not TABLES_TO_DROP:
			if format == "text":
				click.secho(f"No ghost tables found in {stylo.local.site}...Great!", fg="green")
		else:
			if not yes:
