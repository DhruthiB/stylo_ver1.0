...
import stylo
import stylo.commands
from stylo.commands import get_site, pass_context
from stylo.coverage import CodeCoverage
from stylo.exceptions import SiteNotSpecifiedError
from stylo.utils import cint, update_progress_bar
...
def build(
	...
	from stylo.build import bundle, download_stylo_assets
	...
	if not (force or apps or os.environ.get("CI")):
		# skip building stylo if assets exist remotely
		skip_stylo = download_stylo_assets(verbose=verbose)
	else:
		skip_stylo = False
	...
	if make_copy or restore:
		...
		click.secho(
			"forge build: --make-copy and --restore options are deprecated in favour of --hard-link",
			fg="yellow",
		)

	bundle(mode, apps=apps, hard_link=hard_link, verbose=verbose, skip_stylo=skip_stylo)
...
def destroy_all_sessions(context, reason=None):
	...
	for site in context.sites:
		try:
			stylo.init(site=site)
			stylo.connect()
			stylo.sessions.clear_all_sessions(reason)
			stylo.db.commit()
		finally:
			stylo.destroy()
	if not context.sites:
...
def show_config(context, format):
	...
	for site in context.sites:
		...
		configuration = stylo.get_site_config(sites_path=sites_path, site_path=site)
	...
	if format == "json":
		click.echo(stylo.as_json(sites_config))
...
def reset_perms(context):
	...
	from stylo.permissions import reset_perms

	for site in context.sites:
		try:
			stylo.init(site=site)
			stylo.connect()
			for d in stylo.db.sql_list(
				"""select name from `tabDocType`
				where istable=0 and custom=0"""
			):
				stylo.clear_cache(doctype=d)
				reset_perms(d)
		finally:
			stylo.destroy()
	if not context.sites:
...
def execute(context, method, args=None, kwargs=None, profile=False):
	...
	for site in context.sites:
		...
		try:
			...
			try:
				ret = stylo.get_attr(method)(*args, **kwargs)
			except Exception:
...
def export_doc(context, doctype, docname):
	...
	for site in context.sites:
		try:
			stylo.init(site=site)
			stylo.connect()
			stylo.modules.export_doc(doctype, docname)
		finally:
			stylo.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def export_json(context, doctype, path, name=None):
	...
	from stylo.core.doctype.data_import.data_import import export_json
...
def export_csv(context, doctype, path):
	...
	from stylo.core.doctype.data_import.data_import import export_csv
...
def export_fixtures(context, app=None):
	"Export fixtures"
	from stylo.utils.fixtures import export_fixtures
...
def data_import(context, file_path, doctype, import_type=None, submit_after_import=False, mute_emails=True):
	...
	from stylo.core.doctype.data_import.data_import import import_file
...
def bulk_rename(context, doctype, path):
	...
	from stylo.model.rename_doc import bulk_rename
	from stylo.utils.csvutils import read_csv_content
...
def database(context):
	...
	if not site:
		raise SiteNotSpecifiedError
	stylo.init(site=site)
	if not stylo.conf.db_type or stylo.conf.db_type == "mariadb":
		_mariadb()
...
def mariadb(context):
	...
	"""
	from stylo.utils import get_site_path
...
def _mariadb():
	...
	command = [
		mysql,
		"--port",
		str(stylo.conf.db_port or MariaDBDatabase.default_port),
		"-u",
		stylo.conf.db_name,
		f"-p{stylo.conf.db_password}",
		stylo.conf.db_name,
		"-h",
		stylo.conf.db_host or "localhost",
		"--pager=less -SFX",
		"--safe-updates",
		"-A",
	]
...
def _psql():
	...
	host = stylo.conf.db_host or "127.0.0.1"
	port = stylo.conf.db_port or "5432"
	env = os.environ.copy()
	env["PGPASSWORD"] = stylo.conf.db_password
	conn_string = f"postgresql://{stylo.conf.db_name}@{host}:{port}/{stylo.conf.db_name}"
...
def jupyter(context):
	...
	jupyter_notebooks_path = os.path.abspath(stylo.get_site_path("jupyter_notebooks"))
	sites_path = os.path.abspath(stylo.get_site_path(".."))
...
Run the following in your first cell to connect notebook to stylo
```
import stylo
stylo.init(site='{site}', sites_path='{sites_path}')
stylo.connect()
stylo.local.lang = stylo.db.get_default('lang')
stylo.db.connect()
```
	"""
	)
...
def store_logs(terminal: "InteractiveShellEmbed") -> None:
	...
	stylo.log_level = 20  # info
	with suppress(Exception):
		logger = stylo.logger("ipython")
		logger.info("=== forge console session ===")
...
def console(context, autoreload=False):
	...
	all_apps = stylo.get_installed_apps()
	failed_to_import = []
...
def transform_database(context, table, engine, row_format, failfast):
	...
	if stylo.conf.db_type and stylo.conf.db_type != "mariadb":
	...
	if table == "all":
		...
		queried_tables = (
			...
			.where(
				...
				& (information_schema.tables.table_schema == stylo.conf.db_name)
			)
			.run()
		)
	...
	for current, table in enumerate(tables):
		...
		try:
			stylo.db.sql(f"ALTER TABLE `{table}`{values_to_set}")
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		import stylo
		import stylo.test_runner

		tests = test
		site = get_site(context)

		allow_tests = stylo.get_conf(site).allow_tests

		if not (allow_tests or os.environ.get("CI")):
			click.secho("Testing is disabled for the site!", bold=True)
			click.secho("You can enable tests by entering following command:")
			click.secho(f"forge --site {site} set-config allow_tests true", fg="green")
			return

		stylo.init(site=site)

		stylo.flags.skip_before_tests = skip_before_tests
		stylo.flags.skip_test_records = skip_test_records

		ret = stylo.test_runner.main(
			app,
			module,
			doctype,
			module_def,
...
@click.option("--app", help="For App", default="stylo")
...
def run_parallel_tests(
	...
	with CodeCoverage(with_coverage, app):
		site = get_site(context)
		if use_orchestrator:
			from stylo.parallel_test_runner import ParallelTestWithOrchestrator

			ParallelTestWithOrchestrator(app, site=site)
		else:
			from stylo.parallel_test_runner import ParallelTestRunner
...
def run_ui_tests(
	...
	"Run UI tests"
	site = get_site(context)
	app_base_path = os.path.abspath(os.path.join(stylo.get_app_path(app), ".."))
	site_url = stylo.utils.get_site_url(site)
	admin_password = stylo.get_conf(site).admin_password
	...
	if not (
		...
		stylo.commands.popen(f"yarn add {packages} --no-lockfile", raise_err=True)
	...
	try:
		stylo.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)
...
def serve(
	...
	with CodeCoverage(with_coverage, "stylo"):
		if with_coverage:
		...
		try:
			...
			if args:
				...
				if args.startswith("/api/method"):
					stylo.local.form_dict.cmd = args.split("?", 1)[0].split("/")[-1]
			elif path:
			...
			stylo.handler.execute_cmd(stylo.form_dict.cmd)

			print(stylo.response)
		finally:
			stylo.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def make_app(destination, app_name, no_git=False):
	...
	if app_name in get_sites():
		click.secho(
			f"Your forge has a site called {app_name}, please choose another name for the app.", fg="red"
		)
		sys.exit(1)

	from stylo.utils.boilerplate import make_boilerplate
...
def create_patch():
	...
	from stylo.utils.boilerplate import PatchCreator
...
@click.option("-g", "--global", "global_", is_flag=True, default=False, help="Set value in forge config")
...
def get_version(output):
	...
	for app in sorted(stylo.get_all_apps()):
		module = stylo.get_module(app)
		app_hooks = stylo.get_module(app + ".hooks")

		app_info = stylo._dict()

		try:
			app_info.commit = Repo(stylo.get_app_path(app, "..")).head.object.hexsha[:7]
...
def rebuild_global_search(context, static_pages=False):
	...
	from stylo.utils.global_search import (
