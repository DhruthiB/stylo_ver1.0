...
@click.option(
	...
	default=False,
	envvar="USING_CACHED",
	help="Skips build and uses cached build artifacts (cache is set by Forge). Ignored if developer_mode enabled.",
)
def build(
	app=None,
	apps=None,
	...
	from stylo.gettext.translate import compile_translations
	...
	with filelock("forge_build", is_global=True, timeout=10):
		...
		development = stylo.local.conf.developer_mode or stylo._dev_server
		esbuild_target = stylo.local.conf.get("esbuild_target") or os.environ.get("ESBUILD_TARGET")
...
def execute(context: CliCtxObj, method, args=None, kwargs=None, profile=False):
	...
	for site in context.sites:
		...
		try:
			...
			try:
				ret = stylo.get_attr(method)(*fn_args, **fn_kwargs)
			except Exception:
				# eval is safe here because input is from console
				code = compile(method, "<forge execute>", "eval")
				ret = eval(code, globals(), locals())  # nosemgrep
				if callable(ret):
					suffix = "(*fn_args, **fn_kwargs)"
					code = compile(method + suffix, "<forge execute>", "eval")
					ret = eval(code, globals(), locals())  # nosemgrep
...
def _enter_console(extra_args=None):
	from stylo.database import get_command
	...
	elif stylo.conf.db_type == "sqlite":
	...
	bin, args, bin_name = get_command(
		socket=stylo.conf.db_socket,
		host=stylo.conf.db_host,
		port=stylo.conf.db_port,
		user=stylo.conf.db_user,
		password=stylo.conf.db_password,
...
stylo.connect()
stylo.local.lang = stylo.get_system_settings('language')
stylo.db.connect()
```
	"""
	)
...
def get_version(output):
	...
	for app in sorted(stylo.get_all_apps()):
		...
		try:
			app_info.commit = Repo(stylo.get_app_source_path(app)).head.object.hexsha[:7]
...
def list_sites(context: CliCtxObj, output_json=False):
	"List all the sites in current forge"
	site_dir = os.getcwd()
...
def setup_chrome():
	from stylo.utils.print_utils import setup_chromium
