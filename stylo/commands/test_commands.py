...
import stylo
import stylo.commands.scheduler
...
class BaseTestCommands(IntegrationTestCase):
	...
	def setup_test_site(cls):
		cmd_config = {
			...
			"db_root_username": stylo.conf.root_login,
			"db_root_password": stylo.conf.root_password,
		}

		if not os.path.exists(os.path.join(TEST_SITE, "site_config.json")):
			cls.execute(
				"forge new-site {test_site} "
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("forge --site {site} execute stylo.lacol.site")
		self.assertEqual(self.returncode, 1)
		self.assertIsNotNone(self.stderr)

		# test 4: execute a command with kwargs
		self.execute(
			"forge --site {site} execute stylo.bold --kwargs '{put_here}'",
			{"put_here": '{"text": "DocType"}'},  # avoid escaping errors
		)
		self.assertEqual(self.returncode, 0)
		self.assertEqual(self.stdout, stylo.bold(text="DocType"))
	...
	def test_restore(self):
		...
		global_config = {
			...
			"root_password": stylo.conf.mariadb_root_password or stylo.conf.root_password,
			"db_type": stylo.conf.db_type,
		}
		...
		with self.switch_site(TEST_SITE):
			public_file = stylo.new_doc(
				"File", file_name=f"test_{stylo.generate_hash()}", content=stylo.generate_hash()
			).insert()
			private_file = stylo.new_doc(
				"File", file_name=f"test_{stylo.generate_hash()}", content=stylo.generate_hash()
			).insert()

		# test 1: forge restore from full backup
		self.execute("forge --site {test_site} backup --ignore-backup-conf --with-files", site_data)
		self.execute(
		...
		self.execute(
			"forge --site {test_site} restore {database} --with-public-files {public} --with-private-files {private} ",
			site_data,
		)
		self.assertEqual(self.returncode, 0)

		with self.switch_site(TEST_SITE):
			self.assertTrue(stylo.db.table_exists("ToDo", cached=False))
		...
		self.execute("forge --site {test_site} restore {database}", site_data)
		self.assertEqual(self.returncode, 1)

	@skipIf(
		stylo.conf.db_type == "sqlite",
	...
	def test_partial_restore(self):
		...
		self.execute("forge --site {site} backup --only 'ToDo'")
	...
	def test_remove_from_installed_apps(self):
		...
		self.execute("forge --site {site} remove-from-installed-apps {app}", {"app": app})
		self.assertEqual(self.returncode, 0)
		self.execute("forge --site {site} list-apps")
	...
	def test_list_apps(self):
		...
		self.execute("forge --site all list-apps")
		...
		self.execute("forge --site {site} list-apps --format json")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(json.loads(self.stdout), dict)

		self.execute("forge --site {site} list-apps -f json")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(json.loads(self.stdout), dict)

	def test_show_config(self):
		# test 1: sanity check for command
		self.execute("forge --site all show-config")
		self.assertEqual(self.returncode, 0)

		# test 2: test keys in table text
		self.execute(
			"forge --site {site} set-config test_key '{second_order}' --parse",
			{"second_order": json.dumps({"test_key": "test_value"})},
		)
		self.execute("forge --site {site} show-config")
		self.assertEqual(self.returncode, 0)
		self.assertIn("test_key.test_key", self.stdout.split())
		self.assertIn("test_value", self.stdout.split())

		# test 3: parse json format
		self.execute("forge --site all show-config --format json")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(json.loads(self.stdout), dict)

		self.execute("forge --site {site} show-config --format json")
		self.assertIsInstance(json.loads(self.stdout), dict)

		self.execute("forge --site {site} show-config -f json")
		self.assertIsInstance(json.loads(self.stdout), dict)

	def test_get_forge_relative_path(self):
		forge_path = get_forge_path()
		test1_path = os.path.join(forge_path, "test1.txt")
		test2_path = os.path.join(forge_path, "sites", "test2.txt")
		...
		self.assertTrue("test1.txt" in get_forge_relative_path("test1.txt"))
		self.assertTrue("sites/test2.txt" in get_forge_relative_path("test2.txt"))
		with self.assertRaises(SystemExit):
			get_forge_relative_path("test3.txt")

		os.remove(test1_path)
		os.remove(test2_path)

	def test_stylo_site_env(self):
		os.putenv("FRAPPE_SITE", stylo.local.site)
		self.execute("forge execute stylo.ping")
		self.assertEqual(self.returncode, 0)
		self.assertIn("pong", self.stdout)

	def test_version(self):
		self.execute("forge version")
		self.assertEqual(self.returncode, 0)

		for output in ["legacy", "plain", "table", "json"]:
			self.execute(f"forge version -f {output}")
			self.assertEqual(self.returncode, 0)

		self.execute("forge version -f invalid")
	...
	def test_set_password(self):
		...
		self.execute("forge --site {site} set-admin-password test2")
		self.assertEqual(self.returncode, 0)
		...
		self.execute("forge --site {{site}} set-admin-password {}".format(original_password))
		self.assertEqual(self.returncode, 0)
		self.assertEqual(check_password("Administrator", original_password), "Administrator")

	@skipIf(
		not (stylo.conf.root_password and stylo.conf.admin_password and stylo.conf.db_type != "sqlite"),
	...
	def test_forge_drop_site_should_archive_site(self):
		...
		self.execute(
			...
			f"--db-root-username {stylo.conf.root_login} "
			f"--db-root-password {stylo.conf.root_password} "
			f"--db-type {stylo.conf.db_type} "
		)
		self.assertEqual(self.returncode, 0)

		self.execute(
			f"forge drop-site {site} --force "
			f"--db-root-username {stylo.conf.root_login} "
			f"--db-root-password {stylo.conf.root_password} "
		)
		self.assertEqual(self.returncode, 0)
		...
		forge_path = get_forge_path()
		site_directory = os.path.join(forge_path, f"sites/{site}")
		self.assertFalse(os.path.exists(site_directory))
		archive_directory = os.path.join(forge_path, f"archived/sites/{site}")
	...
	def test_set_global_conf(self):
		key = "answer"
		value = stylo.generate_hash()
		_ = stylo.get_site_config()
		self.execute(f"forge set-config {key} {value} -g")
	...
	def test_different_db_username(self):
		...
		kwargs = {
			...
			"db_root_username": stylo.conf.root_login,
			"db_root_password": stylo.conf.root_password or "",
		}
		self.execute(
			"forge new-site {new_site} --force --verbose "
			"--admin-password {admin_password} "
			"--db-root-username {db_root_username} "
			"--db-root-password {db_root_password} "
			"--db-type {db_type} "
	...
	def test_existing_db_username(self):
		...
		if stylo.conf.db_type == "mariadb":
			from stylo.database.mariadb.setup_db import get_root_connection

			root_conn = get_root_connection()
			root_conn.sql(f"CREATE USER '{user}'@'localhost'")
		else:
			from stylo.database.postgres.setup_db import get_root_connection
...
class TestBackups(BaseTestCommands):
	...
	def test_backup_no_options(self):
		...
		time.sleep(1)
		self.execute("forge --site {site} backup")
	...
	def test_backup_extract_restore(self):
		...
		self.execute(
			"forge --site {site} restore {backup_sql}",
			{
	...
	def test_old_backup_restore(self):
		"""Restore a backup after extracting"""
		self.execute("forge --site {site} backup --old-backup-metadata")
		self.assertEqual(self.returncode, 0)
		backup = fetch_latest_backups()
		self.execute(
			"forge --site {site} restore {database}",
			backup,
		)
		self.assertEqual(self.returncode, 0)
	...
	def test_backup_fails_with_exit_code(self):
		...
		odb = BackupGenerator(
			...
			db_socket=stylo.conf.db_socket,
			db_host=stylo.conf.db_host,
	...
	def test_backup_with_files(self):
		...
		self.execute("forge --site {site} backup --with-files")
	...
	def test_backup_with_custom_path(self):
		...
		self.execute("forge --site {site} backup --backup-path {backup_path}", {"backup_path": backup_path})
	...
	def test_backup_compress_files(self):
		...
		self.execute("forge --site {site} backup --with-files --compress")
		self.assertEqual(self.returncode, 0)
	...
	def test_backup_verbose(self):
		...
		self.execute("forge --site {site} backup --verbose")
		self.assertEqual(self.returncode, 0)
	...
	def test_backup_excluding_specific_doctypes(self):
		...
		self.execute(
			"forge --site {site} backup --exclude '{exclude}'",
...
class TestForgeBuild(IntegrationTestCase):
...
class TestSchedulerUtils(BaseTestCommands):
	...
	def test_ready_for_migrate(self):
		with cli(stylo.commands.scheduler.ready_for_migration) as result:
			self.assertEqual(result.exit_code, 0)
...
class TestDBCli(BaseTestCommands):
	...
	def test_db_cli(self):
		if stylo.conf.db_type == "sqlite":
			...
			cmd_input = rb"\q"
		self.execute("forge --site {site} db-console", kwargs={"cmd_input": cmd_input})
		self.assertEqual(self.returncode, 0)

	def test_db_cli_with_sql(self):
		if stylo.db.db_type == "postgres":
			self.execute("forge --site {site} db-console -c 'select 1'")
		elif stylo.db.db_type == "mariadb":
			self.execute("forge --site {site} db-console -e 'select 1'")
		self.assertEqual(self.returncode, 0)
...
class TestSchedulerCLI(BaseTestCommands):
	...
	def test_scheduler_status(self):
		self.execute("forge --site {site} scheduler status")
		self.assertEqual(self.returncode, 0)
...
class TestCLIImplementation(BaseTestCommands):
	def test_missing_commands(self):
		self.execute("forge --site {site} migrat")
...
class TestRQWorker(IntegrationTestCase):
	def spawn_rq(self, args=None, pool=False):
		self.handle = subprocess.Popen(
			["forge", "worker-pool" if pool else "worker", *(args or ())],
		)
