...
class TestAutoAssign(IntegrationTestCase):
	...
	def test_load_balancing(self):
		...
		for user in ("test@example.com", "test1@example.com", "test2@example.com"):
			self.assertEqual(
				len(stylo.get_all("ToDo", dict(allocated_to=user, reference_type=TEST_DOCTYPE))), 10
			)
...
def clear_assignments():
	stylo.db.delete("ToDo", {"reference_type": TEST_DOCTYPE})


def get_assignment_rule(days, assign=None):
	stylo.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 1")
	...
	stylo.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 2")
...
def _make_test_record(
	...
	**kwargs,
	...
	doc = stylo.new_doc(TEST_DOCTYPE)
