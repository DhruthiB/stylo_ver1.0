...
import stylo
from stylo.automation.doctype.auto_repeat.auto_repeat import (
...
from stylo.utils import add_days, add_months, getdate, today

if TYPE_CHECKING:
	from stylo.custom.doctype.custom_field.custom_field import CustomField
...
def add_custom_fields() -> "CustomField":
	...
	return create_custom_field("ToDo", df) or stylo.get_doc(
...
class TestAutoRepeat(IntegrationTestCase):
	...
	def test_daily_auto_repeat(self):
		...
		todo = stylo.get_doc(doc.reference_doctype, doc.reference_document)
		self.assertEqual(todo.auto_repeat, doc.name)

		new_todo = stylo.db.get_value("ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name")

		new_todo = stylo.get_doc("ToDo", new_todo)
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date=None):
		...
		data = get_auto_repeat_entries(getdate(today()))
		create_repeated_entries(data)
		docnames = stylo.get_all(doc.reference_doctype, {"auto_repeat": doc.name})
		self.assertEqual(len(docnames), 1)

		doc = stylo.get_doc("Auto Repeat", doc.name)
	...
	def test_email_notification(self):
		...
		email_queue = stylo.db.exists("Email Queue", dict(reference_doctype="ToDo", reference_name=new_todo))
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = stylo.get_doc(doctype=doctype, test="test submit on creation").insert()
	...
	def test_auto_repeat_assignee_with_separate_documents(self):
		...
		new_todo_count = stylo.db.count("ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name")
	...
	return stylo.get_doc(
		{
			...
			"reference_document": args.reference_document or stylo.db.get_value("ToDo", "name"),
...
def create_submittable_doctype(doctype, submit_perms=1):
	if stylo.db.exists("DocType", doctype):
		return
	else:
		doc = stylo.get_doc(
			{
