...
class AutoRepeat(Document):
	...
	def validate_reference_doctype(self):
		if stylo.flags.in_test or stylo.flags.in_patch:
			return
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not stylo.get_meta(self.reference_doctype).is_submittable:
			stylo.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(stylo.bold("Submit on Creation"))
			)
	...
	def create_documents(self):
		try:
			...
			if self.reference_document and not stylo.flags.in_test:
...
def make_auto_repeat_entry():
	...
	if not jobs or enqueued_method not in jobs[stylo.local.site]:
		...
		stylo.enqueue(enqueued_method, data=data)
...
def set_auto_repeat_as_completed():
	auto_repeat = stylo.get_all("Auto Repeat", filters={"status": ["!=", "Disabled"]})
	for entry in auto_repeat:
		doc = stylo.get_doc("Auto Repeat", entry.name)
		if doc.is_completed():
