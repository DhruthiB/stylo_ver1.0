...
import stylo
from stylo import _
from stylo.automation.doctype.assignment_rule.assignment_rule import get_repeated
from stylo.contacts.doctype.contact.contact import (
	get_contacts_linked_from,
	get_contacts_linking_to,
)
from stylo.core.doctype.communication.email import make
from stylo.desk.form.assign_to import add as assign_to
...
class AutoRepeat(Document):
	...
	if TYPE_CHECKING:
		from stylo.automation.doctype.auto_repeat_day.auto_repeat_day import AutoRepeatDay
		from stylo.automation.doctype.auto_repeat_user.auto_repeat_user import AutoRepeatUser
	...
	def on_update(self):
		stylo.get_doc(self.reference_doctype, self.reference_document).notify_update()

	def on_trash(self):
		stylo.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", "")
		stylo.get_doc(self.reference_doctype, self.reference_document).notify_update()
	...
	def set_dates(self):
		if self.disabled:
			...
			if self.end_date and getdate(self.end_date) < getdate(self.next_schedule_date):
				stylo.throw(_("The Next Scheduled Date cannot be later than the End Date."))
	...
	def validate_reference_doctype(self):
		if stylo.in_test or stylo.flags.in_patch:
			return
		if not stylo.get_meta(self.reference_doctype).allow_auto_repeat:
			stylo.throw(
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not stylo.get_meta(self.reference_doctype).is_submittable:
			stylo.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(
					stylo.bold(_("Submit on Creation"))
				)
			)
	...
	def validate_dates(self):
		...
		if self.end_date:
			...
			if end_date == getdate():
				stylo.throw(_("End Date cannot be today."))
			if end_date == getdate(self.start_date):
				stylo.throw(
					_("{0} should not be same as {1}").format(
						stylo.bold(_("End Date")), stylo.bold(_("Start Date"))
					)
				)
			...
			if self.recipients:
				...
				for email in email_list:
					if not validate_email_address(email):
						stylo.throw(_("{0} is an invalid email address in 'Recipients'").format(email))
			else:
				stylo.throw(_("'Recipients' not specified"))
		...
		if len(set(auto_repeat_days)) != len(auto_repeat_days):
			...
			stylo.throw(
				_("Auto Repeat Day{0} {1} has been repeated.").format(
					plural, stylo.bold(", ".join(repeated_days))
				)
			)

	def update_auto_repeat_id(self):
		# check if document is already on auto repeat
		auto_repeat = stylo.db.get_value(self.reference_doctype, self.reference_document, "auto_repeat")
		if auto_repeat and auto_repeat != self.name and not stylo.flags.in_patch:
			stylo.throw(
				_("The {0} is already on auto repeat {1}").format(self.reference_document, auto_repeat)
			)
		else:
			stylo.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", self.name)
	...
	def create_documents(self):
		try:
			...
			if self.reference_document and not stylo.in_test:
	...
	def make_new_document(self, assignee=None):
		reference_doc = stylo.get_doc(self.reference_doctype, self.reference_document)
		new_doc = stylo.copy_doc(reference_doc, ignore_no_copy=False)
	...
	def update_doc(self, new_doc, reference_doc):
		...
		auto_repeat_doc = stylo.get_doc("Auto Repeat", self.name)
	...
	def send_notification(self, new_doc):
		...
		if not self.subject:
			...
			subject = stylo.render_template(self.subject, {"doc": new_doc})
		...
		try:
			...
			).format(stylo.bold(_("Note")), stylo.bold(_("Allow Print for Draft")))
			attachments = None
		...
		if error_string:
			...
			message = stylo.render_template(self.message, {"doc": new_doc})
	...
	def fetch_linked_contacts(self):
		if self.reference_doctype and self.reference_document:
			...
			if not email_ids:
				stylo.msgprint(_("No contacts linked to document"), alert=True)
			else:
				self.recipients = ", ".join(email_ids)

	def disable_auto_repeat(self):
		stylo.db.set_value("Auto Repeat", self.name, "disabled", 1)
	...
	def notify_error_to_user(self, error_log):
		...
		form_link = stylo.utils.get_link_to_form(self.reference_doctype, self.reference_document)
		auto_repeat_failed_for = _("Auto Repeat failed for {0}").format(form_link)

		error_log_link = stylo.utils.get_link_to_form("Error Log", error_log.name)
...
def make_auto_repeat_entry():
	enqueued_method = "stylo.automation.doctype.auto_repeat.auto_repeat.create_repeated_entries"
	jobs = get_jobs()

	if not jobs or enqueued_method not in jobs[stylo.local.site]:
		date = getdate(today())
		data = get_auto_repeat_entries(date)
		stylo.enqueue(enqueued_method, data=data, queue="long")


def create_repeated_entries(data):
	for d in data:
		doc = stylo.get_doc("Auto Repeat", d.name)
		...
		if schedule_date == current_date and not doc.disabled:
			...
			if schedule_date and not doc.disabled:
				stylo.db.set_value("Auto Repeat", doc.name, "next_schedule_date", schedule_date)
...
def get_auto_repeat_entries(date=None):
	...
	auto_repeat = stylo.qb.DocType("Auto Repeat")
	query = stylo.qb.from_(auto_repeat)
...
def update_reference(docname: str, reference: str):
	doc = stylo.get_doc("Auto Repeat", str(docname))
...
def generate_message_preview(name: str):
	stylo.has_permission("Auto Repeat", "write", throw=True)
	auto_repeat = stylo.get_doc("Auto Repeat", str(name))
	doc = stylo.get_doc(auto_repeat.reference_doctype, auto_repeat.reference_document)
	doc.check_permission()
	subject_preview = _("Please add a subject to your email")
	msg_preview = stylo.render_template(auto_repeat.message, {"doc": doc})
	if auto_repeat.subject:
		subject_preview = stylo.render_template(auto_repeat.subject, {"doc": doc})
