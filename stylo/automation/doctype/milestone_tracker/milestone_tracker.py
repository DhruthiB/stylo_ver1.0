...
class MilestoneTracker(Document):
	...
	def on_update(self):
		stylo.cache_manager.clear_doctype_map("Milestone Tracker", self.document_type)
...
def evaluate_milestone(doc, event):
	...
	for d in get_milestone_trackers(doc.doctype):
		stylo.get_doc("Milestone Tracker", d.get("name")).apply(doc)


def get_milestone_trackers(doctype):
	return stylo.cache_manager.get_doctype_map(
