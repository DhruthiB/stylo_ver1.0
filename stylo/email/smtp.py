...
from stylo.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(stylo.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			stylo.msgprint(
				...
				raise_exception=stylo.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				stylo.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=stylo.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					stylo.msgprint(res[1], raise_exception=stylo.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if stylo.request and hasattr(stylo.request, "after_response"):
			stylo.request.after_response.add(self.quit)
		elif stylo.job:
			stylo.job.after_job.add(self.quit)
		elif not stylo.in_test:
			# Console?
			import atexit
