...
class TestSMTP(IntegrationTestCase):
	...
	def test_get_email_account(self):
		existing_email_accounts = stylo.get_all(
		...
		for email_account in existing_email_accounts:
			stylo.db.set_value("Email Account", email_account["name"], unset_details)

		# remove mail_server config so that test@example.com is not created
		mail_server = stylo.conf.get("mail_server")
		if "mail_server" in stylo.conf:
			del stylo.conf["mail_server"]

		stylo.local.outgoing_email_account = {}
		...
		stylo.conf["mail_server"] = mail_server
		for email_account in existing_email_accounts:
			set_details = {
			...
			stylo.db.set_value("Email Account", email_account["name"], set_details)
...
def create_email_account(email_id, password, enable_outgoing, default_outgoing=0, append_to=None):
	...
	email_account = stylo.new_doc("Email Account")
