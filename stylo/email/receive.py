...
from stylo.core.doctype.file.exceptions import MaxFileSizeReachedError
from stylo.core.doctype.file.utils import get_random_filename
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = imaplib.IMAP4_SSL(
					...
					timeout=stylo.conf.pop_timeout,
					ssl_context=ssl.create_default_context(),
				)
			else:
				self.imap = imaplib.IMAP4(
					self.settings.host, self.settings.incoming_port, timeout=stylo.conf.pop_timeout
				)
