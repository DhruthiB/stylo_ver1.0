...
import stylo
from stylo import _, safe_decode, safe_encode
from stylo.core.doctype.file import MaxFileSizeReachedError, get_random_filename
from stylo.email.oauth import Oauth
...
class EmailSizeExceededError(stylo.ValidationError):
	pass


class EmailTimeoutError(stylo.ValidationError):
	pass


class LoginLimitExceeded(stylo.ValidationError):
	pass
...
class EmailServer:
	...
	def __init__(self, args=None):
		self.settings = args or stylo._dict()
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = Timed_IMAP4_SSL(
					...
					timeout=stylo.conf.get("pop_timeout"),
					ssl_context=ssl.create_default_context(),
				)
			else:
				self.imap = Timed_IMAP4(
					self.settings.host, self.settings.incoming_port, timeout=stylo.conf.get("pop_timeout")
				)
			...
			stylo.msgprint(_("Invalid Mail Server. Please rectify and try again."))
			raise
	...
	def connect_pop(self):
		...
		try:
			...
			stylo.log_error("POP: Unable to connect")
			...
			if self.is_temporary_system_problem(e):
				...
				stylo.msgprint(_("Invalid User Name or Support Password. Please rectify and try again."))
				raise
	...
	def check_imap_uidvalidity(self, folder):
		...
		stylo.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)
		...
		if not uid_validity or uid_validity != current_uid_validity:
			...
			stylo.qb.update(Communication).set(Communication.uid, -1).where(
			...
			if self.settings.use_imap:
				...
				IMAPFolder = stylo.qb.DocType("IMAP Folder")
				stylo.qb.update(IMAPFolder).set(IMAPFolder.uidvalidity, current_uid_validity).set(
					IMAPFolder.uidnext, uidnext
				).where(IMAPFolder.parent == self.settings.email_account_name).where(
					IMAPFolder.folder_name == folder
				).run()
			else:
				EmailAccount = stylo.qb.DocType("Email Account")
				stylo.qb.update(EmailAccount).set(EmailAccount.uidvalidity, current_uid_validity).set(
	...
	def retrieve_message(self, uid, msg_num):
		try:
			...
			stylo.log_error("Unable to fetch email", self.make_error_msg(uid, msg_num))
	...
	def get_email_seen_status(self, uid, flag_string):
		...
		for flag in imaplib.ParseFlags(flag_string) or []:
			match = WORDS_PATTERN.search(stylo.as_unicode(flag))
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not stylo.db.get_value(
	...
	def decode_email(self, email):
		if not email:
			return
		decoded = ""
		for part, encoding in decode_header(stylo.as_unicode(email).replace('"', " ").replace("'", " ")):
			if encoding:
	...
	def save_attachments_in_doc(self, doc):
		...
		for attachment in self.attachments:
			try:
				...
				pass
			except stylo.FileAlreadyAttachedException:
				pass
...
class InboundMail(Email):
	...
	def _build_communication_doc(self):
		...
		communication = stylo.get_doc(data)
	...
	def is_reply_to_system_sent_mail(self):
		...
		return self.is_reply() and stylo.local.site in self.in_reply_to
	...
	def match_record_by_subject_and_sender(self, doctype):
		...
		3. Find by matching subject alone (Special case)
		        ...
		        the system user is replying to via the common email account in Stylo. This fix bypasses
		...
		if not record:
			...
			name = stylo.db.get_value(self.email_account.append_to, filters=filters)
			record = self.get_doc(doctype, name, ignore_error=True) if name else None
		return record

	def _create_reference_document(self, doctype):
		"""Create reference document if it does not exist in the system."""
		parent = stylo.new_doc(doctype)
		email_fields = self.get_email_fields(doctype)

		if email_fields.subject_field:
			parent.set(email_fields.subject_field, stylo.as_unicode(self.subject)[:140])

		if email_fields.sender_field:
			parent.set(email_fields.sender_field, stylo.as_unicode(self.from_email))

		if email_fields.sender_name_field:
			parent.set(email_fields.sender_name_field, stylo.as_unicode(self.from_real_name))
		...
		try:
			...
			return stylo.db.get_value(doctype, {email_fields.sender_field: self.from_email})

	@staticmethod
	def get_doc(doctype, docname, ignore_error=False):
		try:
			return stylo.get_doc(doctype, docname)
	...
	def get_users_linked_to_account(email_account):
		...
		users = stylo.get_all("User Email", filters={"email_account": email_account.name}, fields=["parent"])
	...
	def clean_subject(subject):
		...
		return stylo.as_unicode(strip(re.sub(regex, "", subject, count=0, flags=re.IGNORECASE)))
	...
	def get_document(self, doctype, name):
		"""Is same as stylo.get_doc but suppresses the DoesNotExist error."""
		try:
			return stylo.get_doc(doctype, name)
		except stylo.DoesNotExistError:
			return None
