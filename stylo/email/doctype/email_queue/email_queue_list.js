stylo.listview_settings["Email Queue"] = {
...
function show_toggle_sending_button(list_view) {
	...
	const sending_disabled = cint(stylo.sys_defaults.suspend_email_queue);
	const label = sending_disabled ? __("Resume Sending") : __("Suspend Sending");

	list_view.page.add_inner_button(label, async () => {
		await stylo.xcall(
			"stylo.email.doctype.email_queue.email_queue.toggle_sending",
		...
		stylo.sys_defaults.suspend_email_queue = sending_disabled ? 0 : 1;
...
function add_bulk_retry_button_to_actions(list_view) {
	list_view.page.add_actions_menu_item(__("Retry Sending"), () => {
		...
		stylo.call({
			method: "stylo.email.doctype.email_queue.email_queue.retry_sending",
			args: {
