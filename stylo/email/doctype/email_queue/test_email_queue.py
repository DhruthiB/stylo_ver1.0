...
import stylo
from stylo.email.doctype.email_queue.email_queue import SendMailContext, get_email_retry_limit
...
class TestEmailQueue(IntegrationTestCase):
	...
	def test_failed_email_notification(self):
		subject = stylo.generate_hash()
		email_record = stylo.new_doc("Email Queue")
		email_record.sender = "Test <test@example.com>"
		email_record.message = textwrap.dedent(
			f"""\
		MIME-Version: 1.0
		Message-Id: {stylo.generate_hash()}
		X-Original-From: Test <test@example.com>
		Subject: {subject}
		From: Test <test@example.com>
		To: <!--recipient-->
		Date: {stylo.utils.now_datetime().strftime("%a, %d %b %Y %H:%M:%S %z")}
		Reply-To: test@example.com
		X-Stylo-Site: {stylo.local.site}
		"""
		)
		...
		notification_log = stylo.db.get_value(
			"Notification Log",
	...
	def test_perf_reusing_smtp_server(self):
		...
		self.assertTrue(stylo.new_doc("Email Queue").get_email_account()._from_site_config)

		def get_server(q):
			return q.get_email_account().get_smtp_server()

		self.assertIs(get_server(stylo.new_doc("Email Queue")), get_server(stylo.new_doc("Email Queue")))

		q1 = stylo.new_doc("Email Queue", email_account="_Test Email Account 1")
		q2 = stylo.new_doc("Email Queue", email_account="_Test Email Account 1")
		self.assertIsNot(get_server(stylo.new_doc("Email Queue")), get_server(q1))
