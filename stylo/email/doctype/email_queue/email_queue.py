...
import stylo
from stylo import _, safe_encode, task
...
class EmailQueue(Document):
		...
		with SendMailContext(self, smtp_server_instance) as ctx:
			...
			for recipient in self.recipients:
				...
				if method := get_hook_method("override_email_send"):
					...
					if not stylo.flags.in_test or stylo.flags.testing_email:
			...
			if stylo.flags.in_test and not stylo.flags.testing_email:
	...
	def clear_old_logs(days=30):
		...
		# Delete queue table
		(
			stylo.qb.from_(email_queue).delete().where(email_queue.modified < (Now() - Interval(days=days)))
		).run()
...
class SendMailContext:
	...
	def get_tracker_str(self) -> str:
		if (
			...
			tracker_url_html = f'<img src="{get_url()}/api/method/stylo.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"/>'
		...
		if not queue_separately:
			...
			for recipients in stylo.utils.create_batch(final_recipients, 1000):
				stylo.enqueue(
					...
					now=stylo.flags.in_test or send_now,
					queue="long",
				)
