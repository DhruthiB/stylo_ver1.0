...
import stylo
from stylo import _, are_emails_muted, safe_encode, task
...
from stylo.email.email_body import add_attachment, get_email, get_formatted_html
from stylo.email.stylomail import StyloMail
from stylo.email.queue import get_unsubcribed_url, get_unsubscribe_message
...
from stylo.utils.verified_command import get_signed_params
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from stylo.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if stylo.session.user != "Administrator":
			stylo.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = stylo.get_doc(values)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = stylo.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return stylo.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			stylo.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(stylo.db.get_default("suspend_email_queue")) == 1
		):
			return False
	...
	def send(
		...
		stylo_mail_client: StyloMail = None,
		...
		with SendMailContext(self, smtp_server_instance, stylo_mail_client) as ctx:
			...
			for recipient in self.recipients:
				...
				elif not stylo.in_test or stylo.flags.testing_email:
					if ctx.email_account_doc.service == "Stylo Mail":
						is_newsletter = self.reference_doctype == "Newsletter"
						ctx.stylo_mail_client.send_raw(
							sender=self.sender,
			...
			if stylo.in_test and not stylo.flags.testing_email:
				stylo.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = stylo.qb.DocType("Email Queue")
		email_recipient = stylo.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			stylo.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
		...
		(
			stylo.qb.from_(email_recipient)
			.delete()
			.where(email_recipient.creation < (Now() - Interval(days=days)))
		).run()


from stylo.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	def __init__(
		...
		self.stylo_mail_client: StyloMail = stylo_mail_client
	...
	def fetch_outgoing_server(self):
		...
		if self.email_account_doc.service == "Stylo Mail":
			if not self.stylo_mail_client:
				self.stylo_mail_client = self.email_account_doc.get_stylo_mail_client()
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": stylo.get_traceback()}
	...
	def notify_failed_email(self):
		...
		notification = stylo.new_doc("Notification Log")
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		if self.queue_doc.get("email_read_tracker_url"):
			...
			tracker_url = f"{get_url()}/api/method/stylo.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = stylo.get_doc("File", file_filters)
				...
				print_format_file = stylo.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not stylo.get_system_settings("store_attached_pdf_document"):
			return

		file_data = stylo._dict(file_name=file_name, is_private=1)
		...
		if stylo.db.exists("File", file_data):
			return

		file = stylo.new_doc("File", **file_data)
...
def retry_sending(queues: str | list[str]):
	if not stylo.has_permission("Email Queue", throw=True):
		return
	...
	email_queue = stylo.qb.DocType("Email Queue")
	stylo.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, stylo.session.user
...
def toggle_sending(enable):
	...
	stylo.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	stylo.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	stylo.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(stylo.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/stylo.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/stylo.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				stylo.qb.from_(EmailUnsubscribe)
	...
	def prepare_email_content(self):
		...
		if self.in_reply_to:
			if message_id := stylo.db.get_value("Communication", self.in_reply_to, "message_id"):
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in stylo.utils.create_batch(final_recipients, 1000):
				stylo.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=stylo.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=stylo.in_test or send_now,
					queue="long",
				)
	...
	def send_emails(self, queue_data, final_recipients):
		...
		stylo_mail_client = None
		smtp_server_instance = None
		for r in final_recipients:
			recipients = list(set([r, *self.final_cc(), *self.bcc]))
			q = EmailQueue.new({**queue_data, **{"recipients": recipients}}, ignore_permissions=True)
			if not stylo_mail_client and not smtp_server_instance:
				email_account = q.get_email_account(raise_error=True)

				if email_account.service == "Stylo Mail":
					stylo_mail_client = email_account.get_stylo_mail_client()
				else:
					smtp_server_instance = email_account.get_smtp_server()

			with suppress(Exception):
				q.send(smtp_server_instance=smtp_server_instance, stylo_mail_client=stylo_mail_client)
	...
	def as_dict(self, include_recipients=True):
		...
		except stylo.InvalidEmailAddressError:
