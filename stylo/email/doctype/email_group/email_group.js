	...
	refresh: function (frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(
				...
				function () {
					stylo.prompt(
						...
						function (data) {
							stylo.call({
								method: "stylo.email.doctype.email_group.email_group.import_from",
								args: {
									name: frm.doc.name,
			...
			frm.add_custom_button(
				...
				function () {
					stylo.prompt(
						...
						function (data) {
							stylo.call({
								method: "stylo.email.doctype.email_group.email_group.add_subscribers",
								args: {
									name: frm.doc.name,
			...
			frm.add_custom_button(
				__("New Newsletter"),
				function () {
					stylo.route_options = { email_group: frm.doc.name };
					stylo.new_doc("Newsletter");
				},
				__("Action")
			);
		}
