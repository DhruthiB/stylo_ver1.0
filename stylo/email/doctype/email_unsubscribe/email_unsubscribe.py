...
class EmailUnsubscribe(Document):
	...
	def validate(self):
		if not self.global_unsubscribe and not (self.reference_doctype and self.reference_name):
			stylo.throw(_("Reference DocType and Reference Name are required"), stylo.MandatoryError)

		if not self.global_unsubscribe and stylo.db.get_value(self.doctype, self.name, "global_unsubscribe"):
			stylo.throw(_("Delete this record to allow sending to this email address"))
		...
		if self.global_unsubscribe:
			if stylo.get_all(
				...
				stylo.throw(_("{0} already unsubscribed").format(self.email), stylo.DuplicateEntryError)
	...
	def on_update(self):
		if self.reference_doctype and self.reference_name:
			doc = stylo.get_doc(self.reference_doctype, self.reference_name)
