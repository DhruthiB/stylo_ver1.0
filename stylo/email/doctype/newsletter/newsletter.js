...
stylo.ui.form.on("Newsletter", {
	refresh(frm) {
		let doc = frm.doc;
		let can_write = in_list(stylo.boot.user.can_write, doc.doctype);
		...
		if (frm.is_new() && !doc.sender_email) {
			let { fullname, email } = stylo.user_info(doc.owner);
	...
	send_emails(frm) {
		stylo.dom.freeze(__("Queuing emails..."));
		frm.call("send_emails").then(() => {
			frm.refresh();
			stylo.dom.unfreeze();
			stylo.show_alert(
				__("Queued {0} emails", [stylo.utils.shorten_number(frm.doc.total_recipients)])
			);
		});
	},

	schedule_send_dialog(frm) {
		let hours = stylo.utils.range(24);
	...
	update_schedule_message(frm) {
		if (!frm.doc.email_sent && frm.doc.schedule_send) {
			let datetime = stylo.datetime.global_date_format(frm.doc.schedule_send);
