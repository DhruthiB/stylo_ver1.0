...
import stylo.utils
from stylo import _
from stylo.email.doctype.email_group.email_group import add_subscribers
from stylo.rate_limiter import rate_limit
from stylo.utils.safe_exec import is_job_queued
...
class Newsletter(WebsiteGenerator):
	...
	def get_sending_status(self):
		...
		emails_queued = is_job_queued(
			job_name=stylo.utils.get_job_name("send_bulk_emails_for", self.doctype, self.name),
			queue="long",
		)
	...
	def send_test_email(self, email):
		test_emails = stylo.utils.validate_email_address(email, throw=True)
		self.send_newsletter(emails=test_emails)
		stylo.msgprint(_("Test email sent to {0}").format(email), alert=True)
	...
	def validate_newsletter_status(self):
		if self.email_sent:
			stylo.throw(_("Newsletter has already been sent"), exc=NewsletterAlreadySentError)

		if self.get("__islocal"):
			stylo.throw(_("Please save the Newsletter before sending"), exc=NewsletterNotSavedError)

	def validate_newsletter_recipients(self):
		if not self.newsletter_recipients:
			stylo.throw(_("Newsletter should have atleast one recipient"), exc=NoRecipientFoundError)
		self.validate_recipient_address()

	def validate_sender_address(self):
		"""Validate self.send_from is a valid email address or not."""
		if self.sender_email:
			stylo.utils.validate_email_address(self.sender_email, throw=True)
			self.send_from = (
	...
	def validate_recipient_address(self):
		...
		for recipient in self.newsletter_recipients:
			stylo.utils.validate_email_address(recipient, throw=True)

	def validate_publishing(self):
		if self.send_webview_link and not self.published:
			stylo.throw(_("Newsletter must be published to send webview link in email"))
	...
	def queue_all(self):
		...
		self.email_sent_at = stylo.utils.now()
	...
	def send_newsletter(self, emails: list[str]):
		...
		sender = self.send_from or stylo.utils.get_formatted_email(self.owner)
		args = self.as_dict()
		args["message"] = self.get_message()

		is_auto_commit_set = bool(stylo.db.auto_commit_on_many_writes)
		stylo.db.auto_commit_on_many_writes = not stylo.flags.in_test
		...
		stylo.db.auto_commit_on_many_writes = is_auto_commit_set

	def get_message(self) -> str:
		message = self.message
		if self.content_type == "Markdown":
			message = stylo.utils.md_to_html(self.message_md)
		if self.content_type == "HTML":
			message = self.message_html

		return stylo.render_template(message, {"doc": self.as_dict()})

	def get_recipients(self) -> list[str]:
		"""Get recipients from Email Group"""
		emails = stylo.get_all(
			"Email Group Member",
	...
	def get_email_groups(self) -> list[str]:
		...
		return [x.email_group for x in self.email_group] or stylo.get_all(
...
def confirmed_unsubscribe(email, group):
	...
	doc = stylo.get_doc("Email Group Member", {"email": email, "email_group": group})
...
def subscribe(email, email_group=None):
	...
	api_endpoint = stylo.utils.get_url(
		"/api/method/stylo.email.doctype.newsletter.newsletter.confirm_subscription"
	)
	...
	email_confirmation_template = stylo.db.get_value(
	...
	if email_confirmation_template:
		...
		email_template = stylo.get_doc("Email Template", email_confirmation_template)
		email_subject = email_template.subject
		content = stylo.render_template(email_template.response, args)
	else:
...
def confirm_subscription(email, email_group=None):
	...
	try:
		group = stylo.get_doc("Email Group", email_group)
	except stylo.DoesNotExistError:
		group = stylo.get_doc({"doctype": "Email Group", "title": email_group}).insert(
	...
	if welcome_url:
		...
		stylo.local.response["location"] = welcome_url
	else:
...
def send_scheduled_email():
	...
	scheduled_newsletter = stylo.get_all(
		"Newsletter",
		filters={
			"schedule_send": ("<=", stylo.utils.now_datetime()),
			"email_sent": False,
	...
	for newsletter_name in scheduled_newsletter:
		try:
			newsletter = stylo.get_doc("Newsletter", newsletter_name)
			...
			stylo.db.set_value("Newsletter", newsletter_name, "email_sent", 0)
...
def get_default_email_group():
	return _("Website", lang=stylo.db.get_default("language"))
