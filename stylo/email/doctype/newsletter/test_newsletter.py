...
import stylo
from stylo.email.doctype.newsletter.exceptions import (
	NewsletterAlreadySentError,
	NoRecipientFoundError,
)
from stylo.email.doctype.newsletter.newsletter import (
	Newsletter,
...
from stylo.utils import add_days, getdate
...
class TestNewsletterMixin:
	...
	def tearDown(self):
		...
		for newsletter in newsletters:
			...
			stylo.delete_doc("Newsletter", newsletter)
			stylo.db.delete("Newsletter Email Group", {"parent": newsletter})
			newsletters.remove(newsletter)

	def setup_email_group(self):
		if not stylo.db.exists("Email Group", "_Test Email Group"):
			stylo.get_doc({"doctype": "Email Group", "title": "_Test Email Group"}).insert()
		...
		for email in emails:
			...
			try:
				...
				stylo.db.update(doctype, email_filters, "unsubscribed", 0)
	...
	def get_newsletter(**kwargs) -> "Newsletter":
		...
		similar_newsletters = stylo.get_all(doctype, newsletter_content, pluck="name")

		for similar_newsletter in similar_newsletters:
			stylo.delete_doc(doctype, similar_newsletter)

		newsletter = stylo.get_doc({"doctype": doctype, **newsletter_content, **kwargs})
		...
		for file in attached_files:
			stylo.delete_doc("File", file)

		return newsletter


class TestNewsletter(TestNewsletterMixin, StyloTestCase):
	def test_send(self):
		self.send_newsletter()

		email_queue_list = [stylo.get_doc("Email Queue", e.name) for e in stylo.get_all("Email Queue")]
	...
	def test_unsubscribe(self):
		...
		group = stylo.get_all("Newsletter Email Group", filters={"parent": name}, fields=["email_group"])
	...
	def test_send_scheduled_email_error_handling(self):
		...
		job_path = "stylo.email.doctype.newsletter.newsletter.Newsletter.queue_all"
		m = MagicMock(side_effect=stylo.OutgoingEmailError)

		with self.assertRaises(stylo.OutgoingEmailError):
