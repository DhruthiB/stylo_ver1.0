...
import stylo
from stylo import _, are_emails_muted, safe_encode
from stylo.desk.doctype.notification_log.notification_log import enqueue_create_notification
from stylo.desk.form import assign_to
from stylo.email.doctype.email_domain.email_domain import EMAIL_DOMAIN_FIELDS
from stylo.email.stylomail import StyloMail
from stylo.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from stylo.email.smtp import SMTPServer
from stylo.email.utils import get_port
from stylo.model.document import Document
from stylo.utils import cint, comma_or, cstr, parse_addr, validate_email_address
...
def cache_email_account(cache_name):
	def decorator_cache_email_account(func):
		...
		def wrapper_cache_email_account(*args, **kwargs):
			if not hasattr(stylo.local, cache_name):
				setattr(stylo.local, cache_name, {})

			cached_accounts = getattr(stylo.local, cache_name)
...
class EmailAccount(Document):
	...
	if TYPE_CHECKING:
		from stylo.email.doctype.imap_folder.imap_folder import IMAPFolder
		...
		service: DF.Literal[
			"", "Stylo Mail", "GMail", "Sendgrid", "SparkPost", "Yahoo Mail", "Outlook.com", "Yandex.Mail"
		]
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				stylo.throw(_("Login Id is required"))
		else:
			self.login_id = None
		...
		if self.service == "Stylo Mail":
			self.use_imap = 0
			self.always_use_account_email_id_as_sender = 1

			if self.auth_method == "Basic" or self.get_oauth_token():
				self.validate_stylo_mail_settings()

		# validate the imap settings
		if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
			stylo.throw(_("You need to set one IMAP folder for {0}").format(stylo.bold(self.email_id)))

		if stylo.local.flags.in_patch or stylo.in_test:
			return
		...
		if (
			not stylo.local.flags.in_install
			and not self.awaiting_password
			and not self.service == "Stylo Mail"
		):
			if validate_oauth or self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					if not use_oauth:
						stylo.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				stylo.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
			...
			for folder in self.imap_folder:
				if folder.append_to:
					...
					if folder.append_to not in valid_doctypes:
						stylo.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))

	@stylo.whitelist()
	def validate_stylo_mail_settings(self):
		if self.service == "Stylo Mail":
			stylo_mail_client = self.get_stylo_mail_client()
			stylo_mail_client.validate()

	def validate_smtp_conn(self):
		if not self.smtp_server:
			stylo.throw(_("SMTP Server is required"))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					stylo.bold(_("Default Incoming")),
					stylo.bold(_("Enable Incoming")),
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					stylo.bold(_("Default Outgoing")),
					stylo.bold(_("Enable Outgoing")),
				)
			)
		if messages:
			if len(messages) == 1:
	...
	def clear_cache(self):
		super().clear_cache()
		stylo.client_cache.delete_value("automatic_linking_email")
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in stylo.get_all("Email Account", filters={field: 1}):
				if email_account.name == self.name:
					continue

				email_account = stylo.get_doc("Email Account", email_account.name)
				email_account.set(field, 0)
				email_account.save()

	@stylo.whitelist()
	def get_domain_values(self, domain: str):
		return stylo.db.get_value("Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True)
	...
	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		...
		if not args.get("host"):
			stylo.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(stylo._dict(args))
	...
	def check_email_server_connection(self, email_server, in_receive):
		...
		try:
			...
			if in_receive:
				...
				description = stylo.message_log.pop() if stylo.message_log else "Socket Error"
	...
	@property
	def _password(self):
		raise_exception = not (self.auth_method == "OAuth" or self.no_smtp_authentication or stylo.in_test)
	...
	def from_record(cls, record):
		email_account = stylo.new_doc(cls.DOCTYPE)
	...
	def find_one_by_filters(cls, **kwargs) -> "EmailAccount":
		name = stylo.db.get_value(cls.DOCTYPE, kwargs)
	...
	def get_account_details_from_site_config(cls):
		if not stylo.conf.get("mail_server"):
			return {}
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "Stylo"},
		...
		for doc_field_name, d in field_to_conf_name_map.items():
			...
			value = [stylo.conf.get(k) for k in conf_names if stylo.conf.get(k)]
	...
	def get_stylo_mail_client(self):
		return self._stylo_mail_client

	@functools.cached_property
	def _stylo_mail_client(self):
		if self.auth_method == "OAuth":
			if access_token := self.get_access_token():
				return StyloMail(self.stylo_mail_site, self.email_id, access_token=access_token)

			stylo.throw(
				_("Please Authorize OAuth for Email Account {0}").format(
					stylo.bold(self.email_account_name)
				),
				title=_("Stylo Mail OAuth Error"),
			)
		else:
			return StyloMail(
				self.stylo_mail_site, self.email_id, self.api_key, self.get_password("api_secret")
			)
	...
	def handle_incoming_connect_error(self, description):
		if self.get_failed_attempts_count() > 5:
			...
			stylo.enqueue(self._disable_broken_incoming_account, description=description)
		else:
	...
	def set_failed_attempts_count(self, value):
		stylo.cache.set_value(
			f"{self.name}:email-account-failed-attempts", value, expires_in_sec=2 * 60 * 60
		)

	def get_failed_attempts_count(self):
		return cint(stylo.cache.get_value(f"{self.name}:email-account-failed-attempts"))
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				try:
					...
					if self.use_imap:
						self.handle_bad_emails(mail.uid, mail.raw_message, stylo.get_traceback())
					exceptions.append(stylo.get_traceback())
				except Exception:
					stylo.db.rollback()
				else:
					stylo.db.commit()
		...
		if exceptions:
			raise Exception(stylo.as_json(exceptions))
	...
	def get_inbound_mails(self) -> list[InboundMail]:
		...
		try:
			if self.service == "Stylo Mail":
				stylo_mail_client = self.get_stylo_mail_client()
				messages = stylo_mail_client.pull_raw(last_received_at=self.last_received_at)
	...
	def send_auto_reply(self, communication, email):
		...
		from stylo.core.doctype.communication.email import (
		...
		if self.enable_auto_reply:
			...
			stylo.sendmail(
				...
				or stylo.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		Communication = stylo.qb.DocType("Communication")
		stylo.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def after_rename(self, old, new, merge=False):
		stylo.db.set_value("Email Account", new, "email_account_name", new)
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				stylo.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if stylo.db.exists(
				"Email Account",
				{"enable_automatic_linking": 1, "name": ("!=", self.name)},
			):
				stylo.throw(_("Automatic Linking can be activated only for one Email Account."))
	...
	def get_oauth_token(self):
		if self.auth_method == "OAuth":
			connected_app = stylo.get_doc("Connected App", self.connected_app)
...
def get_append_to(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	...
	email_append_to_list = [
		dt.name for dt in stylo.get_all("DocType", filters=filters, fields=["name", "email_append_to"])
	]
	# Set Email Append To DocTypes set via Customize Form
	email_append_to_list.extend(
		dt.doc_type
		for dt in stylo.get_list(
			"Property Setter",
...
def notify_unreplied():
	...
	for email_account in stylo.get_all(
		"Email Account",
		"name",
		...
		if append_to:
			...
			for comm in stylo.get_all(
				...
				if stylo.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	from stylo.integrations.doctype.connected_app.connected_app import has_token

	doctype = stylo.qb.DocType("Email Account")
	email_accounts = (
	...
	for email_account in email_accounts:
		...
		if now:
			...
			queued_jobs = get_jobs(site=stylo.local.site, key="job_name")[stylo.local.site]
...
def pull_emails(email_account: str) -> None:
	...
	stylo.has_permission("Email Account", "read", throw=True)
	...
	if job_name not in queued_jobs:
		...
		stylo.msgprint(_("Emails are already being pulled from this account."))


def pull_from_email_account(email_account):
	"""Runs within a worker process"""
	email_account = stylo.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing, used_oauth):
	...
	from stylo.core.doctype.user.user import ask_pass_update
	...
	user_names = stylo.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		UserEmail = stylo.qb.DocType("User Email")
		stylo.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
		...
		users = " and ".join([stylo.bold(user.get("name")) for user in user_names])
		stylo.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def remove_user_email_inbox(email_account):
	...
	for user in users:
		doc = stylo.get_doc("User", user.get("name"))
...
def set_email_password(email_account, password):
	account = stylo.get_doc("Email Account", email_account)
...
def get_automatic_email_link():
	...
	return stylo.client_cache.get_value("automatic_linking_email", generator=check_db)
