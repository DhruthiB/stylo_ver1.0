...
class EmailAccount(Document):
	...
	def validate(self):
		...
		if stylo.local.flags.in_patch or stylo.local.flags.in_test:
			return
		...
		if not stylo.local.flags.in_install and not self.awaiting_password:
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					stylo.bold(_("Default Incoming")), stylo.bold(_("Enable Incoming"))
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					stylo.bold(_("Default Outgoing")), stylo.bold(_("Enable Outgoing"))
				)
			)
		if messages:
			if len(messages) == 1:
				(as_list, messages) = (0, messages[0])
			stylo.msgprint(messages, as_list=as_list, indicator="orange", title=_("Defaults Updated"))
	...
	def _password(self):
		raise_exception = not (
			self.auth_method == "OAuth" or self.no_smtp_authentication or stylo.flags.in_test
		)
	...
	def set_failed_attempts_count(self, value):
		stylo.cache().set_value(f"{self.name}:email-account-failed-attempts", value)
		...
		def process_mail(messages, append_to=None):
			for index, message in enumerate(messages.get("latest_messages", [])):
				...
				if self.email_sync_option != "UNSEEN" or seen_status != "SEEN":
					...
					mails.append(InboundMail(message, self, stylo.safe_decode(uid), seen_status, append_to))
	...
	def send_auto_reply(self, communication, email):
		...
		from stylo.core.doctype.communication.email import set_incoming_outgoing_accounts
	...
	def mark_emails_as_read_unread(self, email_server=None, folder_name="INBOX"):
		...
		EmailFlagQ = stylo.qb.DocType("Email Flag Queue")
		flags = (
			stylo.qb.from_(EmailFlagQ)
			.select(EmailFlagQ.name, EmailFlagQ.communication, EmailFlagQ.uid, EmailFlagQ.action)
			.where(EmailFlagQ.is_completed == 0)
			.where(EmailFlagQ.email_account == stylo.db.escape(self.name))
		).run(as_dict=True)
		...
		if flags and uid_list:
			...
			EmailFlagQueue = stylo.qb.DocType("Email Flag Queue")
			stylo.qb.update(EmailFlagQueue).set(EmailFlagQueue.is_completed, 1).where(
	...
	def set_communication_seen_status(self, docnames, seen=0):
		...
		if not docnames:
			return
		Communication = stylo.qb.from_("Communication")
		stylo.qb.update(Communication).set(Communication.seen == seen).where(
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			...
			if stylo.db.exists("Email Account", {"enable_automatic_linking": 1, "name": ("!=", self.name)}):
...
def get_append_to(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	...
	for dt in stylo.get_all("DocType", filters=filters, fields=["name", "email_append_to"]):
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing, used_oauth):
	...
	for user in user_names:
		...
		user_inbox = (
			stylo.db.get_value("User Email", {"email_account": email_account, "parent": user_name}, ["name"])
			or None
		)
...
def remove_user_email_inbox(email_account):
	...
	users = stylo.get_all("User Email", filters={"email_account": email_account}, fields=["parent as name"])
