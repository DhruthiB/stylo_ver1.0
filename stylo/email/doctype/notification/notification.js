...
stylo.notification = {
	setup_fieldname_select: function (frm) {
		...
		stylo.model.with_doctype(frm.doc.document_type, function () {
			...
			let get_receiver_fields = function (
				...
				let extract_receiver_field = function (df) {
					...
					if (stylo.model.table_fields.includes(df.fieldtype)) {
						let child_fields = stylo.get_doc("DocType", df.options).fields;
			...
			let fields = stylo.get_doc("DocType", frm.doc.document_type).fields;
			let options = $.map(fields, function (d) {
				return stylo.model.no_value_type.includes(d.fieldtype)
					? null
...
• Amount: {{ doc.grand_total }}
		...
		if (template) {
			...
			if (frm.doc.channel === "Email") {
				stylo.utils.highlight_pre(message_examples_field.$wrapper);
			}
		}
	},
};

stylo.ui.form.on("Notification", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		stylo.notification.setup_fieldname_select(frm);
		stylo.notification.setup_example_message(frm);
		...
		frm.get_field("is_standard").toggle(stylo.boot.developer_mode);
		frm.trigger("event");
		if (frm.doc.document_type) {
			frm.add_custom_button(__("Preview"), () => {
				...
				let dialog = new stylo.views.RenderPreviewer(args);
				return dialog;
			});
		}
	...
	view_properties: function (frm) {
		stylo.route_options = { doc_type: frm.doc.document_type };
	...
	event: function (frm) {
		...
		frm.add_custom_button(__("Get Alerts for Today"), function () {
			stylo.call({
				method: "stylo.email.doctype.notification.notification.get_documents_for_today",
				args: {
					notification: frm.doc.name,
				},
				callback: function (r) {
					if (r.message && r.message.length > 0) {
						stylo.msgprint(r.message.toString());
					} else {
						stylo.msgprint(__("No alerts for today"));
					}
				},
			});
		});
	...
	set_up_filters_editor(frm) {
		...
		stylo.model.with_doctype(frm.doc.document_type, () => {
			const filter_group = new stylo.ui.FilterGroup({
				parent: parent,
				doctype: frm.doc.document_type,
				on_change: () => {
