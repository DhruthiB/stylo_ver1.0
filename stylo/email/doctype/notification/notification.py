...
from stylo.utils import add_to_date, cast, nowdate, validate_email_address
...
class Notification(Document):
	...
	def validate(self):
		...
		stylo.cache().hdel("notifications", self.document_type)
...
"""
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = stylo.get_doc(self.document_type, d.name)

			if self.condition and not stylo.safe_eval(self.condition, None, get_context(doc)):
				continue
			...
			return [
				{
					...
					"lang": stylo.db.get_value("Print Format", self.print_format, "default_print_language")
...
def evaluate_alert(doc: Document, alert, event):
	...
	try:
		...
		title = str(e)
		message = stylo.get_traceback()
		stylo.log_error(message=message, title=title)
...
def get_context(doc):
	Stylo = namedtuple("stylo", ["utils"])
	return {
		"doc": doc,
		"nowdate": nowdate,
		"stylo": Stylo(utils=get_safe_globals().get("stylo").get("utils")),
	}
