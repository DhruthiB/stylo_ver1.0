...
import stylo
from stylo import _
from stylo.core.doctype.role.role import get_info_based_on_role, get_user_info
from stylo.core.doctype.sms_settings.sms_settings import send_sms
from stylo.desk.doctype.notification_log.notification_log import enqueue_create_notification
from stylo.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
from stylo.model.document import Document
from stylo.modules.utils import export_module_json, get_doc_module
from stylo.utils import add_to_date, cast, now_datetime, nowdate, validate_email_address
...
class Notification(Document):
	...
	if TYPE_CHECKING:
		from stylo.email.doctype.notification_recipient.notification_recipient import NotificationRecipient
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition and not self.filters:
			return _("Yes")
		try:
			doc = stylo.get_cached_doc(self.document_type, preview_document)
			if self.condition_type == "Python":
				context = get_context(doc)
				return _("Yes") if stylo.safe_eval(self.condition, eval_locals=context) else _("No")
	...
	def preview_message(self, preview_document):
		try:
			...
			msg = stylo.render_template(self.message, context)
			if self.channel == "SMS":
				return stylo.utils.strip_html_tags(msg)
			return msg
		except Exception as e:
	...
	def preview_subject(self, preview_document):
		try:
			...
			if not self.subject:
				return _("No subject")
			if "{" in self.subject:
				return stylo.render_template(self.subject, context)
			return self.subject
		except Exception as e:
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			stylo.throw(_("Please specify which date field must be checked"))

		if self.event in ("Minutes Before", "Minutes After"):
			if not self.datetime_changed:
				stylo.throw(_("Please specify which datetime field must be checked"))
			if not self.minutes_offset:
				stylo.throw(_("Please specify the minutes offset"))
		...
		if self.event == "Value Change" and not self.value_changed:
			stylo.throw(_("Please specify which value field must be checked"))

		if self.attach_files == "From Field" and not self.from_attach_field:
			stylo.throw(_("Please specify the field from which to attach files"))
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not stylo.conf.developer_mode:
			stylo.throw(
	...
	def validate_condition(self):
		...
		temp_doc = stylo.new_doc(self.document_type)
		try:
			stylo.safe_eval(self.condition, None, get_context(temp_doc.as_dict()))
		except Exception:
			stylo.throw(_("The Condition '{0}' is invalid").format(self.condition))
	...
	def validate_filters(self):
		...
		dummy_doc = stylo.new_doc(self.document_type)
	...
	def validate_forbidden_document_types(self):
		if self.document_type in FORBIDDEN_DOCUMENT_TYPES or (
			stylo.get_meta(self.document_type).istable and self.event not in DATE_BASED_EVENTS
		):
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = stylo.get_lazy_doc(self.document_type, d.name)

			if (
				self.condition_type == "Python"
				and self.condition
				and not stylo.safe_eval(self.condition, None, get_context(doc))
			):
				continue
	...
	def queue_send(self, doc, enqueue_after_commit=True):
		...
		return enqueue(
			"stylo.email.doctype.notification.notification.evaluate_alert",
			doc=doc,
			alert=self,
			now=stylo.in_test,
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in stylo.model.numeric_fieldtypes:
						value = stylo.utils.cint(value)
	...
	def create_system_notification(self, doc, context):
		...
		if "{" in subject:
			subject = stylo.render_template(self.subject, context)
		...
		notification_doc = {
			...
			"email_content": stylo.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from stylo.core.doctype.communication.email import _make as make_communication
		...
		sender = None
		message = stylo.render_template(self.message, context)
		...
		if doc.doctype != "Communication":
			...
			comm = stylo.get_lazy_doc("Communication", communication)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=stylo.utils.strip_html_tags(stylo.render_template(self.message, context)),
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not stylo.safe_eval(recipient.condition, None, context):
					continue
	...
	def get_print(self, doc):
		...
		print_settings = stylo.get_doc("Print Settings", "Print Settings")
		if (doc.docstatus == 0 and not print_settings.allow_print_for_draft) or (
			...
			return {
				...
				or stylo.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self, md_as_html=False):
		...
		path = os.path.join(os.path.dirname(module.__file__), stylo.scrub(self.name))
		extension = FORMATS.get(self.message_type, ".md")
		file_path = path + extension
		...
		if extension == ".md":
			return stylo.utils.md_to_html(template)
...
def clear_notification_cache():
	stylo.client_cache.delete_keys("notifications::")


@stylo.whitelist()
def get_documents_for_today(notification):
	notification = stylo.get_doc("Notification", notification)
...
def trigger_notifications(doc, method=None):
	if stylo.flags.in_import or stylo.flags.in_patch:
	...
	if method == "daily":
		...
		for d in doc_list:
			alert = stylo.get_doc("Notification", d.name)
...
def evaluate_alert(doc: Document, alert, event=None):
	...
	try:
		if isinstance(alert, str):
			alert = stylo.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition_type == "Python" and alert.condition:
			if not stylo.safe_eval(alert.condition, None, context):
				return
		...
		if event == "Value Change" and not doc.is_new():
			if not stylo.db.has_column(doc.doctype, alert.value_changed):
		...
		message = _("Error while evaluating Notification {0}. Please fix your template.").format(
			stylo.utils.get_link_to_form("Notification", alert.name)
		)
		stylo.throw(message, title=_("Error in Notification"))
	except Exception as e:
		title = str(e)
		message = stylo.get_traceback(with_context=True)
		stylo.log_error(title=title, message=message)
		msg = f"<details><summary>{title}</summary>{message}</details>"
		stylo.throw(msg, title=_("Error in Notification"))


def get_context(doc):
	Stylo = namedtuple("Stylo", ["stylo"])
	stylo = Stylo(stylo=get_safe_globals().get("stylo"))
	return {
		"doc": doc,
		"nowdate": nowdate,
		"stylo": stylo.stylo,
...
def get_emails_from_template(template, context):
	...
	emails = stylo.render_template(template, context) if "{" in template else template
