...
def get_test_notification(config):
	try:
		notification = stylo.get_doc(doctype="Notification", **config).insert()
		yield notification
	finally:
		notification.delete()
		stylo.db.commit()
...
def get_test_doctype_with_attach_field(doctype_name="Test Attach Doctype"):
	...
	try:
		...
		if not stylo.db.exists("DocType", doctype_name):
			stylo.get_doc(
				{
					"doctype": "DocType",
					"name": doctype_name,
		...
		stylo.db.delete(doctype_name)
		stylo.delete_doc_if_exists("DocType", doctype_name, force=1)
		stylo.db.commit()
...
def create_test_file(file_name="test_attachment.txt", content="Test file content"):
	...
	try:
		...
		stylo.delete_doc_if_exists("File", file_doc.name, force=1)
		stylo.db.commit()
...
class TestNotification(IntegrationTestCase):
	def setUp(self):
		...
		if not stylo.db.exists("Notification", {"name": "ToDo Status Update"}, "name"):
			notification = stylo.new_doc("Notification")
		...
		if not stylo.db.exists("Notification", {"name": "Contact Status Update"}, "name"):
			notification = stylo.new_doc("Notification")
	...
	def test_new_and_save(self):
		...
		self.assertEqual(stylo.db.get_value("Communication", communication.name, "subject"), "__testing__")

	def test_condition(self):
		"""Check notification is triggered based on a condition."""
		event = stylo.new_doc("Event")
		event.subject = "test"
	...
	def test_invalid_condition(self):
		...
		recipent = stylo.new_doc("Notification Recipient")
		recipent.receiver_by_document_field = "owner"

		notification.recipents = recipent
		notification.condition = "test"

		self.assertRaises(stylo.ValidationError, notification.save)
		notification.delete()
	...
	def test_minutes_positive_offset(self):
		...
		with get_test_notification(notification) as n:
			stylo.db.delete("Notification Log", {"subject": n.subject})
			# Run the offset alerts
			trigger_notifications(None, "offset")

			# Check if the notification was triggered
			self.assertEqual(1, stylo.db.count("Notification Log", {"subject": n.subject}))
	...
	def test_minutes_offset_validation(self):
		...
		self.assertRaises(stylo.ValidationError, notification.insert)
	...
	def test_date_changed(self):
		...
		stylo.get_doc(
			"Scheduled Job Type",
			dict(method="stylo.email.doctype.notification.notification.trigger_daily_alerts"),
		).execute()
		...
		event.starts_on = stylo.utils.add_days(stylo.utils.nowdate(), 2) + " 12:00:00"
		event.save()
	...
	def test_cc_jinja(self):
		stylo.db.delete("User", {"email": "test_jinja@example.com"})
		stylo.db.delete("Email Queue")
		stylo.db.delete("Email Queue Recipient")

		test_user = stylo.new_doc("User")
		...
		self.assertTrue(stylo.db.get_value("Email Queue Recipient", {"recipient": "test_jinja@example.com"}))
	...
	def test_notification_value_change_casted_types(self):
		...
		with get_test_notification(notification) as n:
			...
			user.birth_date = stylo.utils.add_days(user.birth_date, 1).date()
			user.save()

			user.reload()
			user.birth_date = stylo.utils.getdate(user.birth_date)
			user.save()
			self.assertEqual(1, stylo.db.count("Notification Log", {"subject": n.subject}))
	...
	def tearDownClass(cls):
		stylo.delete_doc_if_exists("Notification", "ToDo Status Update")
		stylo.delete_doc_if_exists("Notification", "Contact Status Update")
		...
		notification = stylo.get_doc(
			{
				...
				"message": "{% set val = stylo.get_doc('ToDo', doc.name) %} ToDo allocated to {{ doc.allocated_to }}",
				"channel": "Email",
	...
	def test_filters_condition(self):
		...
		stylo.delete_doc_if_exists("Notification", "Test Filters Condition")
...
❯ forge run-tests --module stylo.email.doctype.notification.test_notification --case TestNotificationOffsetRange
...
# 		self.event = stylo.new_doc("Event")
...
# 		stylo.db.delete("Event", {"name": self.event.name})
# 		stylo.db.delete("Notification", {"name": self.notification["name"]})
# 		stylo.db.delete("Notification Log", {"subject": self.notification["subject"]})
...
# 						self.assertEqual(
# 							1,
# 							stylo.db.count("Notification Log", {"subject": n.subject}),
