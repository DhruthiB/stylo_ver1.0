...
from stylo.utils import add_to_date, get_link_to_form, today
from stylo.utils.data import is_html

# test_records = stylo.get_test_records('Auto Email Report')


class TestAutoEmailReport(StyloTestCase):
	def test_auto_email(self):
		stylo.delete_doc("Auto Email Report", "Permitted Documents For User")
...
def get_auto_email_report():
	if not stylo.db.exists("Auto Email Report", "Permitted Documents For User"):
		auto_email_report = stylo.get_doc(
			dict(
		...
		auto_email_report = stylo.get_doc("Auto Email Report", "Permitted Documents For User")
