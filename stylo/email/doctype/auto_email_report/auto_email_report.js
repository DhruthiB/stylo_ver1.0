...
stylo.ui.form.on("Auto Email Report", {
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Download"), function () {
				var w = window.open(
					stylo.urllib.get_full_url(
						"/api/method/stylo.email.doctype.auto_email_report.auto_email_report.download?" +
							"name=" +
			...
			frm.add_custom_button(__("Send Now"), function () {
				stylo.call({
					method: "stylo.email.doctype.auto_email_report.auto_email_report.send_now",
					args: { name: frm.doc.name },
					callback: function () {
						stylo.msgprint(__("Scheduled to send"));
					},
				});
			});
		} else {
			if (!frm.doc.user) {
				frm.set_value("user", stylo.session.user);
			}
			if (!frm.doc.email_to) {
				frm.set_value("email_to", stylo.session.user);
			}
		}
	...
	fetch_report_filters(frm) {
		if (
			...
			stylo.call({
				method: "stylo.desk.query_report.get_script",
				args: {
					report_name: frm.doc.report,
				},
				callback: function (r) {
					stylo.dom.eval(r.message.script || "");
					frm.script_setup_for = frm.doc.report;
	...
	show_filters: async function (frm) {
		...
		wrapper.empty();
		let reference_report = stylo.query_reports[frm.doc.report];
		if (!reference_report || !reference_report.filters) {
			reference_report = await stylo.model.with_doc("Report", frm.doc.report);
		}
		if (
			...
			if (
				...
				if (frm.doc.filters) {
					...
					stylo.db.get_value("Report", frm.doc.report, "json", (r) => {
						if (r && r.json) {
				...
				report_filters = stylo.query_reports[frm.doc.reference_report].filters;
			} else {
				filters = JSON.parse(frm.doc.filters || "{}");
				...
				if (val.fieldtype != "Break") {
					if (val.fieldtype === "MultiSelectList") {
						val.get_data = (txt) => {
							...
							const doctype_link =
								stylo.scrub(val.options) === val.options
									? dialog.get_value(val.options)
									: val.options;

							return doctype_link
								? stylo.db.get_link_options(doctype_link, txt)
								: [];
						};
					}
			...
			report_filters.forEach((f) => {
				...
				$("<td>" + stylo.format(filters[f.fieldname], f) + "</td>")
					.css(css)
					.appendTo(row);
			});
