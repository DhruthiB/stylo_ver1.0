...
import stylo
from stylo import _
from stylo.styloclient import StyloClient, StyloOAuth2Client
from stylo.utils import convert_utc_to_system_timezone, get_datetime, get_system_timezone

CHUNK_SIZE = 5 * 1024 * 1024  # 5MB


class StyloMail:
	"""Class to interact with the Stylo Mail API."""
	...
	def get_client(
		...
		api_secret: str | None = None,
		access_token: str | None = None,
	) -> StyloClient | StyloOAuth2Client:
		"""Returns a StyloClient or StyloOAuth2Client instance."""

		if hasattr(stylo.local, "stylo_mail_clients"):
			if client := stylo.local.stylo_mail_clients.get(email):
				return client
		else:
			stylo.local.stylo_mail_clients = {}

		client = (
			StyloOAuth2Client(url=site, access_token=access_token)
			if access_token
			else StyloClient(url=site, api_key=api_key, api_secret=api_secret)
		)
		stylo.local.stylo_mail_clients[email] = client
	...
	def request(
		...
		"""Makes a request to the Stylo Mail API."""
	...
	def send_raw(
		...
		"""Sends an email using the Stylo Mail API."""
	...
	def pull_raw(
		...
		"""Pull emails for the account using the Stylo Mail API."""
		...
		headers = {"X-Site": stylo.utils.get_url()}
