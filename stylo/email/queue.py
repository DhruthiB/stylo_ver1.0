...
from stylo.utils import cint, cstr, get_url, now_datetime
from stylo.utils.data import getdate
...
def get_emails_sent_this_month(email_account=None):
	...
	return stylo.db.count("Email Queue", filters=filters)
...
def get_emails_sent_today(email_account=None):
	...
	return stylo.db.sql(q, q_args)[0][0]


def get_unsubscribe_message(unsubscribe_message: str, expose_recipients: str) -> "stylo._dict[str, str]":
	...
	return stylo._dict(html=html, text=text)
...
def get_unsubcribed_url(reference_doctype, reference_name, email, unsubscribe_method, unsubscribe_params):
	...
	if unsubscribe_params:
		params.update(stylo.parse_json(unsubscribe_params))
...
def unsubscribe(doctype, name, email):
	...
	if not stylo.in_test and not verify_request():
		return
...
def flush():
	...
	if stylo.are_emails_muted():
		msgprint(_("Emails are muted"))

	if cint(stylo.db.get_default("suspend_email_queue")) == 1:
		return
	...
	for row in email_queue_batch:
		try:
			email_queue: EmailQueue = stylo.get_doc("Email Queue", row.name)
			email_queue.send()
		except Exception:
			stylo.get_doc("Email Queue", row.name).log_error()
			...
			if (
				...
				stylo.throw(_("Email Queue flushing aborted due to too many failures."))


def get_queue():
	batch_size = cint(stylo.conf.email_queue_batch_size) or 500
...
def retry_sending_emails():
	emails_in_sending = stylo.get_all(
	...
	for e in emails_in_sending:
		if now_datetime() - e["modified"] > timedelta(minutes=15):
			update_fields = {}
			email_queue = stylo.get_doc("Email Queue", e["name"])
			sent_to_atleast_one_recipient = any(
				rec.recipient for rec in email_queue.recipients if rec.is_mail_sent()
			)
			if email_queue.retry < cint(stylo.db.get_system_setting("email_retry_limit")) or 3:
