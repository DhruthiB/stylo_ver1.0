...
def add_docshare(
	...
	if share_name:
		doc = stylo.get_doc("DocShare", share_name)
	else:
		doc = stylo.new_doc("DocShare")
	...
	if stylo.get_cached_value("User", user, "follow_shared_documents"):
...
def remove(doctype, name, user, flags=None):
	share_name = stylo.db.get_value("DocShare", {"user": user, "share_name": name, "share_doctype": doctype})

	if share_name:
		stylo.delete_doc("DocShare", share_name, flags=flags)
...
def set_docshare_permission(doctype, name, user, permission_to, value=1, everyone=0, flags=None):
	...
	if not share_name:
		...
		share = stylo.get_doc("DocShare", share_name)
		if flags:
...
def get_users(doctype: str, name: str) -> list:
	...
	doc = stylo.get_lazy_doc(doctype, name)
...
def get_shared_doctypes(user=None):
	...
	if not user:
		user = stylo.session.user
	table = stylo.qb.DocType("DocShare")
	query = (
...
def check_share_permission(doctype, name):
	...
	if not stylo.has_permission(doctype, ptype="share", doc=name):
		stylo.throw(
			_("No permission to {0} {1} {2}").format("share", _(doctype), name), stylo.PermissionError
		)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(stylo.session.user)
	notification_message = _("{0} shared a document {1} {2} with you").format(
		stylo.bold(reference_user), stylo.bold(_(doctype)), get_title_html(title)
	)
