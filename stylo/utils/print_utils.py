...
def get_print(
	...
	from stylo.website.serve import get_response_without_exception_handling
	...
	if "pdf_generator" not in local.form_dict:
		...
		if pdf_generator is None:
			pdf_generator = (
				stylo.get_cached_value("Print Format", print_format, "pdf_generator") or "wkhtmltopdf"
			)
	...
	if local.form_dict.pdf_generator != "wkhtmltopdf":
		hook_func = stylo.get_hooks("pdf_generator")
	...
	for hook in stylo.get_hooks("on_print_pdf"):
		stylo.call(hook, doctype=doctype, name=name, print_format=print_format)
...
def attach_print(
	...
	stylo.local.flags.ignore_print_permissions = True

	with print_language(lang or stylo.local.lang):
		content = ""
	...
	stylo.local.flags.ignore_print_permissions = False
...
def setup_chromium():
	"""Setup Chromium at the forge level."""
...
def find_or_download_chromium_executable():
	...
	chromium_dir = os.path.join(forge_path, "chromium")
...
def download_chromium():
	...
	forge_path = stylo.utils.get_forge_path()
	"""Download and extract Chromium for the specific version at the forge level."""
	chromium_dir = os.path.join(forge_path, "chromium")
...
def get_chromium_download_url():
	...
	common_config = stylo.get_common_site_config()
