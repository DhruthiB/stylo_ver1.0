...
def get_jenv():
	...
	if jenv := getattr(stylo.local, "jenv", None):
		return jenv
...
def _get_jenv():
	...
	from stylo.utils.safe_exec import UNSAFE_ATTRIBUTES
	...
	jenv = StyloSandboxedEnvironment(loader=get_jloader(), undefined=DebugUndefined, cache_size=32)
	set_filters(jenv)
...
def get_jloader():
	...
	stylo.local.jloader = jloader  # backward compat
	return jloader
...
def _get_jloader():
	...
	if not apps:
		apps = list(reversed(stylo.get_installed_apps(_ensure_on_forge=True)))
