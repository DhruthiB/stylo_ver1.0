...
def get_secret():
	from stylo.utils.password import get_encryption_key

	return stylo.local.conf.get("secret") or get_encryption_key()


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = stylo.safe_decode(
		stylo.local.flags.signed_query_string or getattr(stylo.request, "query_string", None)
	)
	...
	if signature_string in query_string:
		...
		valid_method = stylo.request.method == "GET"
		valid_request_data = not (stylo.request.form or stylo.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return stylo.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(stylo.utils.cstr(p) for p in params.values())
	if not secret:
		secret = stylo.local.conf.get("secret") or "secret"
