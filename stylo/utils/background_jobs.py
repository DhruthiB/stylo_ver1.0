...
from stylo import _
from stylo.utils import CallbackManager, cint, get_forge_id, get_sites
...
MAX_QUEUED_JOBS = 500  # stylo.enqueue will start failing when these many jobs exist in queue.
...
def enqueue(
	...
	**kwargs,
	...
	if deduplicate:
		if not job_id:
			stylo.throw(_("`job_id` paramater is required for deduplication."))
		job = get_job(job_id)
		if job and job.get_status(refresh=False) in (JobStatus.QUEUED, JobStatus.STARTED):
			stylo.logger().error(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not stylo.in_test:
		from stylo.deprecation_dumpster import deprecation_warning
	...
	call_directly = now or (not is_async and not stylo.in_test)
	if call_directly:
	...
	if enqueue_after_commit:
		stylo.db.after_commit.add(enqueue_call)
		return
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	if is_async:
		...
		if os.environ.get("CI"):
			from stylo.tests.utils import toggle_test_mode
	...
	try:
		...
		stylo.db.rollback(chain=True)
		stylo.log_error(title=method_name)
		stylo.monitor.add_data_to_monitor(exception=e.__class__.__name__)
		stylo.db.commit(chain=True)
...
class StyloWorker(Worker):
	def work(self, *args, **kwargs):
		self.start_stylo_scheduler()
	...
	def start_stylo_scheduler(self):
		from stylo.utils.scheduler import start_scheduler

		# TODO: switch to multiprocessing.Process() after further investigating of fork -> forkserver
		Thread(target=start_scheduler, daemon=True).start()


class StyloWorkerNoFork(StyloWorker):
...
def start_worker_pool(
	...
	import stylo.database.query  # sqlparse and indirect imports
	import stylo.query_builder  # pypika
	import stylo.utils  # common utils
	...
	if sbool(os.environ.get("FRAPPE_BACKGROUND_WORKERS_NOFORK", False)):
		worker_klass = StyloWorkerNoFork
	else:
		if sys.version_info >= (3, 14):
			import multiprocessing

			multiprocessing.set_start_method("fork", force=True)
		worker_klass = StyloWorker
...
def get_redis_conn(username=None, password=None):
	...
	if conf.get("use_rq_auth"):
		if username:
			...
			cred["username"] = conf.rq_username or get_forge_id()
	...
	try:
		...
		log(
			...
			"You can reset credentials using `forge create-rq-users` CLI and restart the server",
			colour="red",
		)
		raise
	except Exception as e:
...
def get_queues(connection=None) -> list[Queue]:
	"""Get all the queues linked to the current forge."""
...
def generate_qname(qtype: str) -> str:
	"""Generate qname by combining forge ID and queue type.

	qnames are useful to define namespaces of customers.
	"""
	if isinstance(qtype, list):
		qtype = ",".join(qtype)
	return f"{get_forge_id()}:{qtype}"


def is_queue_accessible(qobj: Queue) -> bool:
	"""Checks whether queue is relate to current forge or not."""
...
def _check_queue_size(q: Queue):
	max_jobs = cint(stylo.conf.max_queued_jobs) or MAX_QUEUED_JOBS
	# Workaround for arbitrarily sized forgees,
	...
	if cint(q.count) >= max_jobs:
		...
		stylo.throw(
			...
			exc=stylo.QueueOverloaded,
			primary_action=primary_action if stylo.has_permission("System Health Report") else None,
		)
