...
from stylo.database.database import Database, EmptyQueryValues


class StyloIntegration(Integration):
	identifier = "stylo"
...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = stylo._dict({"scheduled": False, "wait": 0})
		...
		if stylo.form_dict.cmd:
			path = f"/api/method/{stylo.form_dict.cmd}"
		else:
	...
	scope.set_user({"id": stylo.local.site})
	user = getattr(stylo.session, "user", "Unidentified")
	scope.set_tag("stylo_user", user)
	# Extract `X-Stylo-Request-ID` to store as a separate field if its present
	if trace_id := stylo.monitor.get_trace_id():
		scope.set_tag("stylo_trace_id", trace_id)
	...
	try:
		...
		with hub.configure_scope() as scope:
			if (
				...
				set_scope(scope)
			if stylo.request:
				evt_processor = _make_wsgi_event_processor(stylo.request.environ, False)
				scope.add_event_processor(evt_processor)
				if stylo.request.is_json:
					scope.set_context("JSON Body", stylo.request.json)
				elif stylo.request.form:
					scope.set_context("Form Data", stylo.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], stylo.ValidationError | stylo.PermissionError):
					return
		...
		stylo.logger().error("Failed to capture exception", exc_info=True)
