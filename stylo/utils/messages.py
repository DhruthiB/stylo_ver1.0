...
from stylo.utils.data import safe_decode
...
def msgprint(
	...
	*,
	...
	def _raise_exception():
		if raise_exception:
			if inspect.isclass(raise_exception) and issubclass(raise_exception, Exception):
				...
				exc = stylo.ValidationError(msg)
			if out.__stylo_exc_id:
				exc.__stylo_exc_id = out.__stylo_exc_id
			raise exc

	if stylo.flags.mute_messages:
		_raise_exception()
		return
	...
	if stylo.flags.print_messages and out.message:
	...
	if raise_exception:
		...
		out.__stylo_exc_id = stylo.generate_hash()
	...
	if realtime:
		stylo.publish_realtime(event="msgprint", message=out)
	else:
		stylo.message_log.append(out)
	_raise_exception()
...
def get_message_log() -> list[dict]:
	return [msg_out for msg_out in stylo.local.message_log]


def clear_last_message():
	if len(stylo.local.message_log) > 0:
		stylo.local.message_log = stylo.local.message_log[:-1]


def throw(
	msg: str,
	exc: type[Exception] | Exception = stylo.ValidationError,
	title: str | None = None,
	is_minimizable: bool = False,
	wide: bool = False,
	as_list: bool = False,
...
def throw_permission_error():
	throw(_("Not permitted"), stylo.PermissionError)
