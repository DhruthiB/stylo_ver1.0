...
cssutils.log.setLog(stylo.logger("cssutils"))
...
def get_chrome_pdf(print_format, html, options, output, pdf_generator=None):
	from stylo.utils.pdf_generator.browser import Browser
	from stylo.utils.pdf_generator.chrome_pdf_generator import ChromePDFGenerator
	from stylo.utils.pdf_generator.pdf_merge import PDFTransformer
...
def prepare_header_footer(soup: BeautifulSoup):
	...
	for html_id in ("header-html", "footer-html"):
		if content := soup.find(id=html_id):
			...
			hook_func = stylo.get_hooks(id_map.get(html_id))
			html = stylo.call(
				hook_func[-1],
				soup=soup,
				head=head,
