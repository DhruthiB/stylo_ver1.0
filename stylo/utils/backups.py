...
class BackupGenerator:
	...
	def _set_existing_tables(self):
		...
		if not hasattr(self, "_existing_tables"):
			self._existing_tables = stylo.db.get_tables()
	...
	def set_backup_tables_from_config(self):
		...
		backup_conf = stylo.conf.get("backup", {})
		self._set_existing_tables()
	...
	def backup_files(self):
		for folder in ("public", "private"):
			...
			except stylo.CommandFailedError as e:
	...
	def take_dump(self):
		if self.db_type == "sqlite":
			...
			db_path = Path(stylo.get_site_path()) / "db" / f"{self.db_name}.db"
		...
		if not gzip_exc:
			stylo.throw(
				_("gzip not found in PATH! This is required to take a backup."), exc=stylo.ExecutableNotFound
			)
		...
		if self.old_backup_metadata:
			...
			database_header_content = [
				"begin stylo metadata",
				"[stylo]",
				f"version = {stylo.__version__}",
				f"branch = {get_app_branch('stylo') or 'N/A'}",
				"end stylo metadata",
				"",
			]
...
def get_backup_path():
	return stylo.utils.get_site_path(conf.get("backup_path", "private/backups"))
