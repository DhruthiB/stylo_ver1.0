...
def check_for_update():
	if stylo.get_system_settings("disable_system_update_notification"):
		return
...
def has_app_update_notifications() -> bool:
	return bool(stylo.cache.sismember("changelog-update-user-set", stylo.session.user))
		...
		for vuln in advisory["vulnerabilities"]:
			with suppress(Exception):
				...
				if (
					...
					# XXX: this is not 100% correct, but works for stylo
...
def add_message_to_redis(update_json):
	...
	stylo.cache.set_value("changelog-update-info", json.dumps(update_json))
	...
	stylo.cache.sadd("changelog-update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = stylo.cache.get_value("changelog-update-info")
	if not update_info:
		return
	...
	update_message = ""
	if stylo.cache.sismember("changelog-update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				...
				if app.security_issues:
					...
					security_msg = security_msg.format(stylo.bold(app.security_issues))
	...
	if on_stylocloud():
		primary_action = {
			"label": _("Update from Stylo Cloud"),
			"client_action": "window.open",
			"args": f"https://stylocloud.com/dashboard/sites/{stylo.local.site}",
		}
	...
	if update_message:
		...
		stylo.cache.srem("changelog-update-user-set", user)


def get_pyproject(app: str) -> dict | None:
	from tomli import load

	pyproject_path = stylo.get_app_path(app, "..", "pyproject.toml")
