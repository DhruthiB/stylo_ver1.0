...
import stylo
from stylo import _
from stylo.query_builder import Table
...
def set_encrypted_password(doctype, name, pwd, fieldname="password"):
	...
	# TODO: Simplify this via aliasing methods in `stylo.qb`
	...
	except stylo.db.DataError as e:
		if stylo.db.is_data_too_long(e):
			stylo.throw(_("Most probably your password is too long."), exc=e)
		raise e


def remove_encrypted_password(doctype, name, fieldname="password"):
	stylo.db.delete("__Auth", {"doctype": doctype, "name": name, "fieldname": fieldname})


def check_password(user, pwd, doctype="User", fieldname="password", delete_tracker_cache=True):
	"""Checks if user and password are correct, else raises stylo.AuthenticationError"""
	...
	if not result or not passlibctx.verify(pwd, result[0].password):
		raise stylo.AuthenticationError(_("Incorrect User or Password"))
...
def delete_login_failed_cache(user):
	stylo.cache().hdel("last_login_tried", user)
	stylo.cache().hdel("login_failed_count", user)
	stylo.cache().hdel("locked_account_time", user)
...
def delete_all_passwords_for(doctype, name):
	try:
		stylo.db.delete("__Auth", {"doctype": doctype, "name": name})
...
def rename_password(doctype, old_name, new_name):
	...
	stylo.qb.update(Auth).set(Auth.name, new_name).where(
...
def encrypt(txt, encryption_key=None):
	...
	try:
		...
		stylo.throw(_("Encryption key is in invalid format!"))
...
def decrypt(txt, encryption_key=None, key: str | None = None):
	...
	try:
		...
		stylo.throw(
			...
			+ "<br><br>"
			+ _(
				"Please visit https://stylocloud.com/docs/sites/migrate-an-existing-site#encryption-key for more information."
			),
		)


def get_encryption_key():
	if "encryption_key" not in stylo.local.conf:
		from stylo.installer import update_site_config

		encryption_key = Fernet.generate_key().decode()
		update_site_config("encryption_key", encryption_key)
		stylo.local.conf.encryption_key = encryption_key

	return stylo.local.conf.encryption_key


def get_password_reset_limit():
	return stylo.get_system_settings("password_reset_limit") or 3
