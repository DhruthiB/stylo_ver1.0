...
def update_nsm(doc):
	...
	stylo.clear_document_cache(doc.doctype)
...
def rebuild_node(doctype, parent, left, parent_field):
	...
	stylo.db.set_value(doctype, parent, {"lft": left, "rgt": right}, update_modified=False)
...
def validate_loop(doctype, name, lft, rgt):
	...
	if name in stylo.get_all(doctype, filters={"lft": ["<=", lft], "rgt": [">=", rgt]}, pluck="name"):
		stylo.throw(_("Item cannot be added to its own descendants"), NestedSetRecursionError)
...
def remove_subtree(doctype: str, name: str, throw=True):
	...
	stylo.clear_document_cache(doctype)
