...
import stylo
from stylo.exceptions import StyloTypeError
...
StyloPydanticConfig = ConfigDict(arbitrary_types_allowed=True)
...
def TypeAdapter(type_):
	try:
		return PydanticTypeAdapter(type_, config=StyloPydanticConfig)
...
def transform_parameter_types(func: Callable, args: tuple, kwargs: dict):
	...
	func_params = stylo._get_cached_signature_params(func)[0]
