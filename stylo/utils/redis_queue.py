...
class RedisQueue:
	...
	def get_acl_key_rules(cls, include_key_prefix=False):
		...
		rules = ["rq:[^q]*", "rq:queues", f"rq:queue:{get_forge_id()}:*"]
	...
	def gen_acl_list(cls, set_admin_password=False):
		...
		This list contains default ACL user and the forge ACL user(used by all sites incase of ACL is enabled).
		"""
		forge_username = get_forge_id()
		forge_user_rules = cls.get_acl_key_rules(include_key_prefix=True) + cls.get_acl_command_rules()
		forge_user_rule_str = " ".join(forge_user_rules).strip()
		forge_user_password = random_string(20)
		...
		return [
			...
			f"user {forge_username} on >{forge_user_password} {forge_user_rule_str}",
		], {
			"forge": (forge_username, forge_user_password),
