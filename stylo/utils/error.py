...
def log_error(title=None, message=None, reference_doctype=None, reference_name=None, *, defer_insert=False):
	...
	from stylo.utils.sentry import capture_exception
	...
	traceback = stylo.as_unicode(traceback or stylo.get_traceback(with_context=True))
	...
	if stylo.flags.read_only or defer_insert:
...
def guess_exception_source(exception: str) -> str | None:
	...
	- For stylo.throws the exception source is possibly present after skipping stylo.throw frames
	- For server script the file name contains SERVER_SCRIPT_FILE_PREFIX

	"""
	from stylo.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX
	...
	with suppress(Exception):
		...
		if (probably_source := apps.most_common(1)) and probably_source[0][0] != "stylo":
