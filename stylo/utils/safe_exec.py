...
class StyloTransformer(RestrictingNodeTransformer):
...
class StyloPrintCollector(PrintCollector):
...
def safe_exec(
	...
	*,
	...
	if not is_safe_exec_enabled():
		msg = _("Server Scripts are disabled. Please enable server scripts from forge configuration.")
...
def _compile_code(script: str, filename: str, mode: str = "exec"):
	return compile_restricted(script, filename=filename, policy=StyloTransformer, mode=mode)
...
def get_safe_globals():
	...
	out._print_ = StyloPrintCollector
