...
from stylo.utils import cint, strip_html_tags
...
def reset():
	...
	:return:
	"""
	stylo.db.delete("__global_search")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in stylo.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = stylo.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = stylo.get_installed_apps()
		module_app = stylo.local.module_app

		doctypes = [
			d.name
			for d in global_search_doctypes
			if module_app.get(stylo.scrub(d.module)) and module_app[stylo.scrub(d.module)] in installed_apps
		]

		return doctypes

	return stylo.cache.get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if stylo.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = stylo._dict({"docstatus": ["!=", 2]})
	...
	all_records = stylo.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = stylo.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": stylo.db.escape(doctype),
					"name": stylo.db.escape(doc.name),
					"content": stylo.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": stylo.db.escape((title or "")[: int(stylo.db.VARCHAR_LEN)]),
					"route": stylo.db.escape((route or "")[: int(stylo.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def delete_global_search_records_for_doctype(doctype):
	stylo.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = stylo._dict()
	child_search_fields = stylo._dict()

	for child in meta.get_table_fields():
		child_meta = stylo.get_meta(child.options)
		...
		if search_fields:
			...
			for record in child_records:
				all_children.setdefault(record.parent, stylo._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	content = [
		...
		if doc.get(field.fieldname) and field.fieldtype not in stylo.model.table_fields
	]
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(stylo.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if stylo.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	apps = stylo.get_installed_apps()

	routes_to_index = []
	for app in apps:
		base = stylo.get_app_path(app, "www")
		path_to_index = stylo.get_app_path(app, "www")
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(stylo.safe_decode(value))
...
def sync_global_search():
	...
	def get_search_queue_item_generator():
		while value := stylo.cache.rpop("global_search_queue"):
			yield value
...
def sync_values(values: list):
	...
	GlobalSearch = stylo.qb.Table("__global_search")
	conflict_fields = ["content", "published", "title", "route"]

	query = stylo.qb.into(GlobalSearch).columns(["doctype", "name", *conflict_fields]).insert(*values)
...
def sync_value_in_queue(value):
	try:
		...
		stylo.cache.lpush("global_search_queue", json.dumps(value))
	except redis.exceptions.ConnectionError:
		# not connected, sync directly
		assert not stylo.flags.in_test, "Should not fail silently in tests"
		sync_value(value)
...
def delete_for_document(doc):
	...
	"""
	stylo.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	from stylo.query_builder.functions import Match

	results = []
	sorted_results = []

	allowed_doctypes = set(get_doctypes_for_global_search()) & set(stylo.get_user().get_can_read())
	...
	for doctype in allowed_doctypes:
		for r in results:
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = stylo.get_meta(r.doctype)
					if meta.image_field:
						r.image = stylo.db.get_value(r.doctype, r.name, meta.image_field)
					if meta.title_field:
						r.title = stylo.db.get_value(r.doctype, r.name, meta.title_field)
				except Exception:
...
def web_search(text: str, scope: str | None = None, start: int = 0, limit: int = 20):
	...
	for text in texts:
		...
		mariadb_conditions += "MATCH(`content`) AGAINST ({} IN BOOLEAN MODE)".format(
			stylo.db.escape("+" + text + "*")
		)
		postgres_conditions += f'TO_TSVECTOR("content") @@ PLAINTO_TSQUERY({stylo.db.escape(text)})'

		values = {"scope": "".join([scope, "%"]) if scope else "", "limit": limit, "start": start}

		result = stylo.db.multisql(
			{
