...
from stylo.deprecation_dumpster import gzip_compress, gzip_decompress, make_esc
...
def validate_name(name, throw=False):
	...
	* anything else is considered invalid
	...
	Note: "Name" here is name of a person, not the primary key in Stylo doctypes.
	"""
	if not name:
		return False
...
def validate_iban(iban: str, throw: bool = False) -> bool:
	...
	if not valid and throw:
		stylo.throw(stylo._("'{0}' is not a valid IBAN").format(stylo.bold(iban)))
...
def has_gravatar(email: str) -> str:
	...
	if stylo.flags.in_import or stylo.flags.in_install or stylo.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	...
	"""
	hexdigest = hashlib.md5(stylo.as_unicode(email).encode("utf-8"), usedforsecurity=False).hexdigest()
...
def execute_in_shell(cmd, verbose=False, low_priority=False, check_exit_code=False):
	...
	if failed:
		raise stylo.CommandFailedError(
...
def get_forge_path():
	return os.environ.get("FRAPPE_BENCH_ROOT") or os.path.realpath(
		os.path.join(os.path.dirname(stylo.__file__), "..", "..", "..")
	)
...
def get_test_client(use_cookies=True) -> Client:
	"""Return an test instance of the Stylo WSGI."""
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(stylo.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
...
def get_site_info():
	...
	site_info = {
		...
		"setup_complete": stylo.is_setup_complete(),
	...
	return orjson.loads(stylo.as_json(site_info))
...
def get_db_count(*args):
	...
	return orjson.loads(stylo.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	"""
	return orjson.loads(stylo.as_json(stylo.call(fn, *args, **kwargs)))
...
def get_html_for_route(route):
	...
	return stylo.safe_decode(response.get_data())
...
def get_assets_json():
	...
	if not stylo.conf.developer_mode:
		return stylo.client_cache.get_value(
			"assets_json",
			shared=True,
...
def get_forge_relative_path(file_path):
	"""Fix paths relative to the forge root directory if exists and return the absolute path.
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
	...
	for info in missing_info:
		user_info.setdefault(info.name, stylo._dict()).update(
...
class CallbackManager:
	...
	Example usage: stylo.db.after_commit
	"""
