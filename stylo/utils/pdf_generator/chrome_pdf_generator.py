...
class ChromePDFGenerator:
	...
	def _initialize_chromium(self):
		...
		# only when we want to use chromium from a specific path ( incase we don't have chromium in forge folder )
	...
	def _find_chromium_executable(self):
		...
		if not exec_path.exists():
			frappe.throw(
				f"Chromium executable not found: {exec_path}. please run forge setup-new-pdf-backend"
			)
