...
import stylo
from stylo.core.doctype.data_import.data_import import export_json, import_doc
...
def sync_fixtures(app=None):
	...
	stylo.flags.in_fixtures = True

	for app in apps:
		import_fixtures(app)
		import_custom_scripts(app)

	stylo.flags.in_fixtures = False


def import_fixtures(app):
	fixtures_path = stylo.get_app_path(app, "fixtures")
	...
	for fname in fixture_files:
		...
		file_path = stylo.get_app_path(app, "fixtures", fname)
		try:
			import_doc(file_path)
		except (ImportError, stylo.DoesNotExistError) as e:
...
def import_custom_scripts(app):
	...
	scripts_folder = stylo.get_app_path(app, "fixtures", "custom_scripts")
	...
	for fname in os.listdir(scripts_folder):
		...
		with open(file_path) as f:
			script = f.read()
			if stylo.db.exists("Client Script", {"dt": doctype}):
				client_script = stylo.get_doc("Client Script", {"dt": doctype})
				client_script.script = script
				client_script.save()
			else:
				client_script = stylo.new_doc("Client Script")
...
def export_fixtures(app=None):
	...
	if app:
		...
		apps = stylo.get_installed_apps()
	for app in apps:
		for fixture in stylo.get_hooks("fixtures", app_name=app):
			filters = None
			or_filters = None
			...
			if not os.path.exists(stylo.get_app_path(app, "fixtures")):
				os.mkdir(stylo.get_app_path(app, "fixtures"))

			export_json(
				fixture,
				stylo.get_app_path(app, "fixtures", stylo.scrub(fixture) + ".json"),
				filters=filters,
