...
def download_multi_pdf_async(
	...
	stylo.local.response["http_status_code"] = http.HTTPStatus.CREATED
...
def _download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		for idx, ss in enumerate(result):
			try:
				...
				if task_id:
					stylo.publish_realtime(task_id=task_id, message={"message": "Failed"})
		...
		for doctype_name in doctype:
			...
			for doc_name in doctype[doctype_name]:
				try:
					...
					if task_id:
						stylo.publish_realtime(task_id=task_id, message="Failed")
					stylo.log_error(
	...
	with BytesIO() as merged_pdf:
		...
		if task_id:
			...
			_file.save()
			stylo.publish_realtime(f"task_complete:{task_id}", message={"file_url": _file.unique_url})
		else:
			stylo.local.response.filecontent = merged_pdf.getvalue()
			stylo.local.response.type = "pdf"


from stylo.deprecation_dumpster import read_multi_pdf
