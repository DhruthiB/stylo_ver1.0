...
import stylo
from stylo.utils import cint, get_forge_path, get_datetime, get_sites, now_datetime
from stylo.utils.background_jobs import set_niceness
...
def start_scheduler() -> NoReturn:
	...
	try:
		...
		stylo.logger("scheduler").debug("Scheduler already running")
		return
...
def _get_scheduler_lock_file() -> True:
	return os.path.abspath(os.path.join(get_forge_path(), "config", "scheduler_process"))
...
def enqueue_events_for_all_sites() -> None:
	...
	for site in sites:
		try:
			...
			stylo.logger("scheduler").debug(f"Failed to enqueue events for site: {site}", exc_info=True)


def enqueue_events_for_site(site: str) -> None:
	def log_exc():
		stylo.logger("scheduler").error(f"Exception in Enqueue Events for Site {site}", exc_info=True)
...
def enqueue_events() -> list[str] | None:
	if schedule_jobs_based_on_activity():
		enqueued_jobs = []
		all_jobs = stylo.get_all("Scheduled Job Type", filters={"stopped": 0}, fields="*")
		random.shuffle(all_jobs)
		for job_type in all_jobs:
			job_type = stylo.get_doc(doctype="Scheduled Job Type", **job_type)
			try:
				if job_type.enqueue():
...
def is_scheduler_disabled(verbose=True) -> bool:
	...
	scheduler_disabled = not stylo.get_system_settings("enable_scheduler")
...
def is_dormant(check_time=None):
	from stylo.utils.stylocloud import on_stylocloud

	if stylo.conf.developer_mode or not on_stylocloud():
		return False
	threshold = cint(stylo.get_system_settings("dormant_days")) * 86400
	if not threshold:
		return False

	last_activity = stylo.db.get_value(
...
def _get_last_creation_timestamp(doctype):
	timestamp = stylo.db.get_value(doctype, filters={}, fieldname="creation", order_by="creation desc")
	if timestamp:
