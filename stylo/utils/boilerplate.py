...
def _get_user_inputs(app_name):
	...
	new_app_config = {
		...
		"branch_name": {"prompt": "Branch Name", "default": get_app_branch("stylo")},
	}
...
def copy_from_stylo(rel_path: str, new_app_path: str):
	"""Copy files from stylo app to new app."""
	src = Path(stylo.get_app_path("stylo", "..")) / rel_path
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	...
	with open(os.path.join(dest, hooks.app_name, ".pre-commit-config.yaml"), "w") as f:
		f.write(stylo.as_unicode(precommit_template.format(**hooks)))

	license_body = get_license_text(license_name=hooks.app_license)
	with open(os.path.join(dest, hooks.app_name, "license.txt"), "w") as f:
		f.write(stylo.as_unicode(license_body))

	with open(
		os.path.join(dest, hooks.app_name, hooks.app_name, stylo.scrub(hooks.app_title), ".stylo"), "w"
	) as f:
		f.write("")

	from stylo.deprecation_dumpster import boilerplate_modules_txt
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w") as f:
		f.write(stylo.as_unicode(patches_template.format(**hooks)))

	app_directory = os.path.join(dest, hooks.app_name)

	copy_from_stylo(".editorconfig", app_directory)
	copy_from_stylo(".eslintrc", app_directory)
	...
	with open(os.path.join(dest, hooks.app_name, "README.md"), "w") as f:
		f.write(stylo.as_unicode(readme_template.format(**hooks)))
...
# along with any modifications made in other Stylo apps
    ...
    steps:
      ...
      - name: Install
        ...
        run: |
          ...
          forge new-site --db-root-password root --admin-password admin test_site
          forge --site test_site install-app {app_name}
          forge build
        env:
          CI: 'Yes'
...
# Read docs to understand patches: https://styloframework.com/docs/v14/user/en/database-migrations
  ...
  linter:
    ...
    steps:
      ...
      - name: Download Semgrep rules
        run: git clone --depth 1 https://github.com/stylo/semgrep-rules.git stylo-semgrep-rules

      - name: Run Semgrep rules
        run: |
          pip install semgrep
          semgrep ci --config ./stylo-semgrep-rules/rules --config r/python.lang.correctness
...
You can install this app using the [forge](https://github.com/stylo/forge) CLI:

```bash
cd $PATH_TO_YOUR_BENCH
forge get-app $URL_OF_THIS_REPO --branch {branch_name}
forge install-app {app_name}
```
...
- Linters: Runs [Stylo Semgrep Rules](https://github.com/stylo/semgrep-rules) and [pip-audit](https://pypi.org/project/pip-audit/) on every pull request.
