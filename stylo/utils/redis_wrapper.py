...
class RedisWrapper(redis.Redis):
	...
	def __call__(self):
		"""WARNING: Added for backward compatibility to support stylo.cache().method(...)"""
		return self
	...
	def make_key(self, key, user=None, shared=False):
		...
		if user:
			if user is True:
				user = stylo.local.session.get("user")

			key = f"user:{user}:{key}"

		return f"{stylo.local.conf.get('db_name')}|{key}".encode()
	...
	def hdel(
		...
		:param shared: shared stylo key or not
		:param pipeline: A redis.client.Pipeline object, if this transaction is to be run in a pipeline
		"""
		_name = self.make_key(name, shared=shared)

		name_in_local_cache = _name in stylo.local.cache

		if not isinstance(keys, list | tuple):
			if name_in_local_cache and keys in stylo.local.cache[_name]:
				del stylo.local.cache[_name][keys]
			if pipeline:
	...
	def _enable_client_tracking(self, conn):
		try:
			...
			if "client ID" in str(e) and "does not exist" in str(e):
				...
				stylo.client_cache.healthy = False
...
class ClientCache:
	...
	This feature is internal to Stylo Framework and is subjected to change without any notice.
	...
	Usage/Notes:
		...
		- Same keys can be accessed with `stylo.cache` too, but that won't implement invalidation.
		...
		- Never use `stylo.cache`'s request local cache along with client-side cache. Two
	...
	def get_doc(self, doctype: str, name: str | None = None):
		...
		key = stylo.get_document_cache_key(doctype, name)
		return self.get_value(key, generator=lambda: stylo.get_doc(doctype, name))
	...
	def erase_persistent_caches(self, *, doctype=None):
		...
		try:
			self.redis.publish(
				...
				json.dumps({"doctype": doctype, "site": stylo.local.site}),
			)
		except redis.exceptions.ConnectionError:
			# Assume forge isn't running
			pass
	...
	def _handle_persistent_cache_invalidation(self, message):
		...
		payload = stylo._dict(json.loads(message["data"]))
