...
from stylo.modules.patch_handler import _patch_mode
...
def sync_for(app_name, force=0, reset_permissions=False):
	...
	if app_name == "stylo":
		...
		Stylo_PATH = stylo.get_app_path("stylo")
		...
		for core_module in [
			...
			files.append(os.path.join(Stylo_PATH, "core", "doctype", core_module, f"{core_module}.json"))

		for custom_module in ["custom_field", "property_setter"]:
			files.append(
				os.path.join(Stylo_PATH, "custom", "doctype", custom_module, f"{custom_module}.json")
			)

		for website_module in ["web_form", "web_template", "web_form_field", "portal_menu_item"]:
			files.append(
				os.path.join(Stylo_PATH, "website", "doctype", website_module, f"{website_module}.json")
			)
		...
		for desk_module in [
			...
			files.append(os.path.join(Stylo_PATH, "desk", "doctype", desk_module, f"{desk_module}.json"))

		for module_name, document_type in IMPORTABLE_DOCTYPES:
			file = os.path.join(Stylo_PATH, module_name, "doctype", document_type, f"{document_type}.json")
			if file not in files:
				files.append(file)

	for module_name in stylo.local.app_modules.get(app_name) or []:
		folder = os.path.dirname(stylo.get_module(app_name + "." + module_name).__file__)
