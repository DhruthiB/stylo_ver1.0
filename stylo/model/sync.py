...
def sync_for(app_name, force=0, reset_permissions=False):
	...
	if app_name == "stylo":
		...
		FRAPPE_PATH = stylo.get_app_path("stylo")
...
def get_doc_files(files, start_path):
	...
	for _module, doctype in IMPORTABLE_DOCTYPES + [
		(None, stylo.scrub(dt)) for dt in stylo.get_hooks("importable_doctypes")
	]:
...
def remove_orphan_doctypes():
	...
	doctype_names = stylo.get_all("DocType", {"custom": 0}, pluck="name")
	orphan_doctypes = []
	...
	for i, name in enumerate(orphan_doctypes):
		stylo.delete_doc("DocType", name, force=True, ignore_missing=True)
