...
import stylo
from stylo import _, bold
from stylo.model.document import Document
from stylo.model.dynamic_links import get_dynamic_link_map
from stylo.model.naming import validate_name
from stylo.model.utils.user_settings import sync_user_settings, update_user_settings_data
from stylo.query_builder import Field
from stylo.utils.data import sbool
from stylo.utils.password import rename_password
...
def update_document_title(
	...
	**kwargs,
	...
	# TODO: omit this after runtime type checking (ref: https://github.com/stylo/stylo/pull/14927)
	for obj in [docname, updated_title, updated_name]:
		if not isinstance(obj, str | NoneType):
			stylo.throw(f"{obj=} must be of type str or None")
	...
	if title_updated:
		try:
			...
			doc.save()
			stylo.msgprint(_("Saved"), alert=True, indicator="green")
		except Exception as e:
			if stylo.db.is_duplicate_entry(e):
				stylo.throw(
					_("{0} {1} already exists").format(doctype, stylo.bold(docname)),
					title=_("Duplicate Name"),
					exc=stylo.DuplicateEntryError,
				)
			raise
...
def rename_doc(
	...
	if not (new_usage_style or old_usage_style):
		raise TypeError(
			"{doctype, old, new} or {doc, new} are required arguments for stylo.model.rename_doc"
		)
	...
	if validate:
		old_doc = doc or stylo.get_doc(doctype, old)
	...
	# call after_rename
	new_doc = stylo.get_doc(doctype, new)
	...
	if merge:
		new_doc.add_comment("Edit", _("merged {0} into {1}").format(stylo.bold(old), stylo.bold(new)))
	else:
		new_doc.add_comment("Edit", _("renamed from {0} to {1}").format(stylo.bold(old), stylo.bold(new)))

	if merge:
		stylo.delete_doc(doctype, old)

	new_doc.clear_cache()
	stylo.clear_cache()
	if rebuild_search:
		stylo.enqueue("stylo.utils.global_search.rebuild_for_doctype", doctype=doctype)
...
def update_assignments(old: str, new: str, doctype: str) -> None:
	old_assignments = stylo.parse_json(stylo.db.get_value(doctype, old, "_assign")) or []
	new_assignments = stylo.parse_json(stylo.db.get_value(doctype, new, "_assign")) or []
	...
	for user in common_assignments:
		...
		for todo in todos:
			stylo.delete_doc("ToDo", todo.name)

	unique_assignments = list(set(old_assignments + new_assignments))
	stylo.db.set_value(doctype, new, "_assign", stylo.as_json(unique_assignments, indent=0))
...
def update_user_settings(old: str, new: str, link_fields: list[dict]) -> None:
	...
	UserSettings = stylo.qb.Table("__UserSettings")

	user_settings_details = (
		stylo.qb.from_(UserSettings)
...
def update_customizations(old: str, new: str) -> None:
	stylo.db.set_value("Custom DocPerm", {"parent": old}, "parent", new, update_modified=False)


def update_attachments(doctype: str, old: str, new: str) -> None:
	if doctype != "DocType":
		File = stylo.qb.DocType("File")

		stylo.qb.update(File).set(File.attached_to_name, new).where(
			(File.attached_to_name == old) & (File.attached_to_doctype == doctype)
		).run()


def rename_versions(doctype: str, old: str, new: str) -> None:
	Version = stylo.qb.DocType("Version")

	stylo.qb.update(Version).set(Version.docname, new).where(
		(Version.docname == old) & (Version.ref_doctype == doctype)
	).run()


def rename_eps_records(doctype: str, old: str, new: str) -> None:
	EPL = stylo.qb.DocType("Energy Point Log")

	stylo.qb.update(EPL).set(EPL.reference_name, new).where(
...
def rename_parent_and_child(doctype: str, old: str, new: str, meta: "Meta") -> None:
	stylo.qb.update(doctype).set("name", new).where(Field("name") == old).run()
...
def update_autoname_field(doctype: str, new: str, meta: "Meta") -> None:
	...
	if meta.get("autoname"):
		...
		if field and field[0] == "field":
			stylo.qb.update(doctype).set(field[1], new).where(Field("name") == new).run()
...
def validate_rename(
	...
	if save_point:
		_SAVE_POINT = f"validate_rename_{stylo.generate_hash(length=8)}"
		stylo.db.savepoint(_SAVE_POINT)

	exists = stylo.qb.from_(doctype).where(Field("name") == new).for_update().select("name").run(pluck=True)
	exists = exists[0] if exists else None

	if not stylo.db.exists(doctype, old):
		stylo.throw(_("Can't rename {0} to {1} because {0} doesn't exist.").format(old, new))

	if old == new:
		stylo.throw(_("No changes made because old and new name are the same.").format(old, new))
	...
	if merge and not exists:
		stylo.throw(_("{0} {1} does not exist, select a new target to merge").format(doctype, new))

	if not merge and exists and not ignore_if_exists:
		stylo.throw(_("Another {0} with name {1} exists, select another name").format(doctype, new))

	kwargs = {"doctype": doctype, "ptype": "write", "raise_exception": False}
	if old_doc:
		kwargs["doc"] = old_doc

	if not (ignore_permissions or stylo.permissions.has_permission(**kwargs)):
		stylo.throw(_("You need write permission on {0} {1} to rename").format(doctype, old))

	if merge:
		kwargs["doc"] = stylo.get_doc(doctype, new)
		if not (ignore_permissions or stylo.permissions.has_permission(**kwargs)):
			stylo.throw(_("You need write permission on {0} {1} to merge").format(doctype, new))

	if not (force or ignore_permissions) and not meta.allow_rename:
		stylo.throw(_("{0} not allowed to be renamed").format(_(doctype)))

	# validate naming like it's done in doc.py
	new = validate_name(doctype, new)

	if save_point:
		stylo.db.rollback(save_point=_SAVE_POINT)
...
def rename_doctype(doctype: str, old: str, new: str) -> None:
	...
	fields_with_options = ("Link", *stylo.model.table_fields)
...
def update_link_field_values(link_fields: list[dict], old: str, new: str, doctype: str) -> None:
	for field in link_fields:
		if field["issingle"]:
			try:
				single_doc = stylo.get_doc(field["parent"])
			...
			stylo.db.set_value(parent, {docfield: old}, docfield, new, update_modified=False)
...
def get_link_fields(doctype: str) -> list[dict]:
	...
	if not stylo.flags.link_fields:
		stylo.flags.link_fields = {}

	if doctype not in stylo.flags.link_fields:
		dt = stylo.qb.DocType("DocType")
		df = stylo.qb.DocType("DocField")
		cf = stylo.qb.DocType("Custom Field")
		ps = stylo.qb.DocType("Property Setter")
		...
		if stylo.db.has_column("DocField", "is_virtual"):
			standard_fields_query = standard_fields_query.where(df.is_virtual == 0)

		virtual_doctypes = []
		if stylo.db.has_column("DocType", "is_virtual"):
		...
		cf_issingle = stylo.qb.from_(dt).select(dt.issingle).where(dt.name == cf.dt).as_("issingle")
		custom_fields = (
			stylo.qb.from_(cf)
		...
		ps_issingle = stylo.qb.from_(dt).select(dt.issingle).where(dt.name == ps.doc_type).as_("issingle")
		...
		stylo.flags.link_fields[doctype] = standard_fields + custom_fields + property_setter_fields

	return stylo.flags.link_fields[doctype]


def update_options_for_fieldtype(fieldtype: str, old: str, new: str) -> None:
	CustomField = stylo.qb.DocType("Custom Field")
	PropertySetter = stylo.qb.DocType("Property Setter")

	if stylo.conf.developer_mode:
		for name in stylo.get_all("DocField", filters={"options": old}, pluck="parent"):
			if name in (old, new):
				continue

			doctype = stylo.get_doc("DocType", name)
			save = False
	...
	DocField = stylo.qb.DocType("DocField")
	stylo.qb.update(DocField).set(DocField.options, new).where(
		(DocField.fieldtype == fieldtype) & (DocField.options == old)
	).run()

	stylo.qb.update(CustomField).set(CustomField.options, new).where(
		(CustomField.fieldtype == fieldtype) & (CustomField.options == old)
	).run()

	stylo.qb.update(PropertySetter).set(PropertySetter.value, new).where(
...
def get_select_fields(old: str, new: str) -> list[dict]:
	...
	# get link fields from tabDocField
	st_issingle = stylo.qb.from_(dt).select(dt.issingle).where(dt.name == df.parent).as_("issingle")
	standard_fields = (
		stylo.qb.from_(df)
...
def update_select_field_values(old: str, new: str):
	from stylo.query_builder.functions import Replace

	DocField = stylo.qb.DocType("DocField")
	CustomField = stylo.qb.DocType("Custom Field")
	PropertySetter = stylo.qb.DocType("Property Setter")

	stylo.qb.update(DocField).set(DocField.options, Replace(DocField.options, old, new)).where(
		(DocField.fieldtype == "Select")
		& (DocField.parent != new)
		& (DocField.options.like(f"%\n{old}%") | DocField.options.like(f"%{old}\n%"))
	).run()

	stylo.qb.update(CustomField).set(CustomField.options, Replace(CustomField.options, old, new)).where(
		(CustomField.fieldtype == "Select")
		& (CustomField.dt != new)
		& (CustomField.options.like(f"%\n{old}%") | CustomField.options.like(f"%{old}\n%"))
	).run()

	stylo.qb.update(PropertySetter).set(PropertySetter.value, Replace(PropertySetter.value, old, new)).where(
...
def update_parenttype_values(old: str, new: str):
	child_doctypes = stylo.get_all(
		"DocField",
		fields=["options", "fieldname"],
		filters={"parent": new, "fieldtype": ["in", stylo.model.table_fields]},
	)

	custom_child_doctypes = stylo.get_all(
		"Custom Field",
		fields=["options", "fieldname"],
		filters={"dt": new, "fieldtype": ["in", stylo.model.table_fields]},
	)

	child_doctypes += custom_child_doctypes
	fields = [d["fieldname"] for d in child_doctypes]

	property_setter_child_doctypes = stylo.get_all(
		"Property Setter",
	...
	for doctype in child_doctypes:
		...
		stylo.qb.update(table).set(table.parenttype, new).where(table.parenttype == old).run()


def rename_dynamic_links(doctype: str, old: str, new: str):
	Singles = stylo.qb.DocType("Singles")
	for df in get_dynamic_link_map().get(doctype, []):
		# dynamic link in single, just one value to check
		meta = stylo.get_meta(df.parent)
		if meta.is_virtual:
			continue
		if meta.issingle:
			refdoc = stylo.db.get_singles_dict(df.parent)
			if refdoc.get(df.options) == doctype and refdoc.get(df.fieldname) == old:
				stylo.qb.update(Singles).set(Singles.value, new).where(
			...
			stylo.qb.update(parent).set(df.fieldname, new).where(
...
def bulk_rename(doctype: str, rows: list[list] | None = None, via_console: bool = False) -> list[str] | None:
	...
	if not rows:
		stylo.throw(_("Please select a valid csv file with data"))

	if not via_console:
		max_rows = 500
		if len(rows) > max_rows:
			stylo.throw(_("Maximum {0} rows allowed").format(max_rows))
...
def update_linked_doctypes(
	...
	from stylo.model.utils.rename_doc import update_linked_doctypes
...
def get_fetch_fields(doctype: str, linked_to: str, ignore_doctypes: list | None = None) -> list[dict]:
	from stylo.model.utils.rename_doc import get_fetch_fields
...
def show_deprecation_warning(funct: str) -> None:
	...
	message = (
		f"Function stylo.model.rename_doc.{funct} has been deprecated and "
		"moved to the stylo.model.utils.rename_doc"
	)
