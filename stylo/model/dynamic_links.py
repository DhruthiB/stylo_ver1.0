...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(stylo.local, "dynamic_link_map", None) is None or stylo.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		...
		stylo.local.dynamic_link_map = dynamic_link_map
	return stylo.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += stylo.db.sql(query, as_dict=True)
	return df
...
def fetch_distinct_link_doctypes(doctype: str, fieldname: str):
	...
	doctypes = stylo.cache().get_value(key)

	if doctypes is None:
		doctypes = stylo.db.sql(f"""select distinct `{fieldname}` from `tab{doctype}`""", pluck=True)
		stylo.cache().set_value(key, doctypes, expires_in_sec=12 * 60 * 60)
...
def invalidate_distinct_link_doctypes(doctype: str, fieldname: str, linked_doctype: str):
	...
	if linked_doctype not in doctypes:
		...
		stylo.db.add_before_commit(lambda: stylo.cache().delete_value(key))
