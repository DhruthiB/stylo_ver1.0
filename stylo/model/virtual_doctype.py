...
class VirtualDoctype(Protocol):
	...
	- DocType controller has to inherit from `stylo.model.document.Document` class
	...
	@staticmethod
	def get_list(**kwargs) -> list[stylo._dict]:
...
def validate_controller(doctype: str) -> None:
	try:
		...
		stylo.msgprint(_("Failed to import virtual doctype {}, is controller file present?").format(doctype))
		return
		...
		if not isinstance(method, staticmethod):
			stylo.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					stylo.bold(doctype), stylo.bold(m), stylo.bold(_as_str(method))
				),
