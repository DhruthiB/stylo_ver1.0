...
from stylo.utils.defaults import get_not_null_defaults
...
def get_controller(doctype):
	...
	For `custom` type, return `stylo.model.document.Document`.
...
def import_controller(doctype):
	...
	if doctype not in DOCTYPES_FOR_DOCTYPE:
		doctype_info = stylo.db.get_value("DocType", doctype, ("module", "custom", "is_tree"), as_dict=True)
		if doctype_info:
	...
	if class_overrides and class_overrides.get(doctype):
		...
		custom_module = stylo.get_module(module_path)
		custom_class_ = getattr(custom_module, custom_classname, None)
		if not issubclass(custom_class_, class_):
			original_class_path = stylo.bold(f"{class_.__module__}.{class_.__name__}")
			stylo.throw(
				f"{doctype}: {stylo.bold(import_path)} must be a subclass of {original_class_path}",
...
def _get_extended_class(base_class, doctype):
	...
	extensions = stylo.get_hooks("extend_doctype_class", {}).get(doctype)
	if not extensions:
		return base_class

	# Get extension classes in reverse order using stylo.get_attr
	extension_classes = []
	for extension_path in reversed(extensions):
		try:
			extension_class = stylo.get_attr(extension_path)
...
class BaseDocument:
	...
	def _init_child(self, value, key):
		...
		if not __dict.get("name"):
			__dict["__islocal"] = 1
			__dict["__temporary_name"] = stylo.generate_hash(length=10)
		...
		for fieldname in self.meta.get_valid_fields():
			...
			if df:
				...
				if isinstance(value, list) and fieldtype not in table_fields:
					stylo.throw(_("Value for {0} cannot be a list").format(_(df.label, context=df.parent)))
	...
	def get_valid_columns(self) -> list[str]:
		valid_columns_cache = stylo.local.valid_columns
	...
	def update_modified(self):
		...
		if getattr(self.meta, "issingle", False):
			stylo.db.set_single_value(self.doctype, "modified", self.modified, update_modified=False)
		else:
	...
	def get_invalid_links(self, is_submittable=False):
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if df.fieldtype == "Link":
				...
				if not doctype:
					stylo.throw(_("{0} must be set first").format(_(self.meta.get_label(df.options))))
			...
			check_docstatus = is_submittable and stylo.get_meta(doctype).is_submittable
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (stylo.in_test and value.startswith("_T-")):
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		stylo.throw(
			_("{0}: '{1}' ({3}) will get truncated, as max characters allowed is {2}").format(
				reference, stylo.bold(_(df.label, context=df.parent)), max_length, value
			),
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields, mask_fields=None):
		...
		if not self.is_new() and masked_fieldnames:
			ref_values = stylo.db.get_value(self.doctype, self.name, masked_fieldnames, as_dict=True) or {}
