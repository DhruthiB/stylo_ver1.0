...
from stylo.model.dynamic_links import invalidate_distinct_link_doctypes
from stylo.model.naming import set_new_name
from stylo.model.utils.link_count import notify_link_count
...
def get_controller(doctype):
	...
	For `custom` type, returns `stylo.model.document.Document`.

	:param doctype: DocType name as string."""

	def _get_controller():
		from stylo.model.document import Document
		from stylo.utils.nestedset import NestedSet

		module_name, custom = stylo.db.get_value(
			"DocType", doctype, ("module", "custom"), cache=not stylo.flags.in_migrate
		) or ("Core", False)

		if custom:
			is_tree = stylo.db.get_value("DocType", doctype, "is_tree", ignore=True, cache=True)
			_class = NestedSet if is_tree else Document
		else:
			class_overrides = stylo.get_hooks("override_doctype_class")
			if class_overrides and class_overrides.get(doctype):
				import_path = class_overrides[doctype][-1]
				module_path, classname = import_path.rsplit(".", 1)
				module = stylo.get_module(module_path)
	...
	if stylo.local.dev_server or stylo.flags.in_migrate:
		return _get_controller()

	site_controllers = stylo.controllers.setdefault(stylo.local.site, {})
...
class BaseDocument:
	...
	@cached_property
	def meta(self):
		return stylo.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return stylo.db.get_value(self.doctype, self.name, key)
			...
			if df:
				if is_virtual_field:
					...
					if (prop := getattr(type(self), fieldname, None)) and is_a_property(prop):
						...
						from stylo.utils.safe_exec import get_safe_globals

						value = stylo.safe_eval(
							code=options,
				...
				if isinstance(value, list) and df.fieldtype not in table_fields:
					stylo.throw(_("Value for {0} cannot be a list").format(_(df.label)))
	...
	def get_valid_columns(self) -> list[str]:
		if self.doctype not in stylo.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from stylo.model.meta import get_table_columns

				valid = get_table_columns(self.doctype)
			else:
				valid = self.meta.get_valid_columns()

			stylo.local.valid_columns[self.doctype] = valid

		return stylo.local.valid_columns[self.doctype]
	...
	def as_json(self):
		return stylo.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and stylo.db.db_type == "postgres":
			conflict_handler = "on conflict (name) do nothing"

		if not self.creation:
			self.creation = self.modified = now()
			self.owner = self.modified_by = stylo.session.user
		...
		try:
			...
			if stylo.db.is_primary_key_violation(e):
				...
				if not ignore_if_duplicate:
					stylo.msgprint(
						_("{0} {1} already exists").format(_(self.doctype), stylo.bold(self.name)),
						title=_("Duplicate Name"),
						indicator="red",
					)
					raise stylo.DuplicateEntryError(self.doctype, self.name, e)

			elif stylo.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if stylo.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if stylo.db.db_type != "postgres":
			...
			stylo.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise stylo.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		stylo.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if docname:
				if df.fieldtype == "Link":
					doctype = df.options
					if not doctype:
						stylo.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						stylo.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not meta.get("is_virtual"):
					if not fields_to_fetch:
						...
						values = _dict(name=stylo.db.get_value(doctype, docname, "name", cache=True))
					else:
						...
						values = stylo.db.get_value(doctype, docname, values_to_fetch, as_dict=True)

				if getattr(meta, "issingle", 0):
					values.name = doctype

				if meta.get("is_virtual"):
					values = stylo.get_doc(doctype, docname).as_dict()
				...
				if values:
					...
					if not values.name:
						...
						and stylo.get_meta(doctype).is_submittable
						and cint(stylo.db.get_value(doctype, docname, "docstatus")) == DocStatus.cancelled()
					):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			from stylo.model.meta import get_default_df

			fetch_from_df = get_default_df(fetch_from_fieldname) or stylo.get_meta(doctype).get_field(
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (stylo.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		# data_field options defined in stylo.model.data_field_options
		for phone_field in self.meta.get_phone_fields():
			phone = self.get(phone_field.fieldname)
			stylo.utils.validate_phone_number_with_country_code(phone, phone_field.fieldname)
		...
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in stylo.STANDARD_USERS) and (data in stylo.STANDARD_USERS):
					continue
				for email_address in stylo.utils.split_emails(data):
					stylo.utils.validate_email_address(email_address, throw=True)

			if data_field_options == "Name":
				stylo.utils.validate_name(data, throw=True)

			if data_field_options == "Phone":
				stylo.utils.validate_phone_number(data, throw=True)

			if data_field_options == "URL":
				if not data:
					continue

				stylo.utils.validate_url(data, throw=True)

	def _validate_constants(self):
		if stylo.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = stylo.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				stylo.throw(
					...
					stylo.CannotChangeConstantError,
				)
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				stylo.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				stylo.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		stylo.throw(
			_("{0}: '{1}' ({3}) will get truncated, as max characters allowed is {2}").format(
				reference, stylo.bold(_(df.label)), max_length, value
			),
			stylo.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = stylo.get_doc(self.doctype, self.name).as_dict()
				...
				if self_value != db_value:
					stylo.throw(
						_("{0} Not allowed to change {1} after submission from {2} to {3}").format(
							...
							stylo.bold(db_value),
							stylo.bold(self_value),
						),
						stylo.UpdateAfterSubmitError,
	...
	def _save_passwords(self):
		...
		from stylo.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def get_formatted(
		...
		if (
			...
			currency = stylo.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = stylo.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_text_editor(self):
		from stylo.core.doctype.file.utils import extract_images_from_doc
...
def _filter(data, filters, limit=None):
	...
	for d in data:
		for f, fval in _filters.items():
			if not stylo.compare(getattr(d, f, None), fval[0], fval[1]):
				break
		else:
			out.append(d)
