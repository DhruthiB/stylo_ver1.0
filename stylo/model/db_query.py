...
from stylo.model import get_permitted_fields, optional_fields
...
class DatabaseQuery:
	...
	def doctype_meta(self):
		if not hasattr(self, "_doctype_meta"):
			self._doctype_meta = stylo.get_meta(self.doctype)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		"""
		if self.flags.ignore_permissions or not stylo.get_system_settings("apply_perm_level_on_api_calls"):
			return
		...
		for i, field in enumerate(self.fields):
			...
			# handle child / joined table fields
				...
				if wrap_grave_quotes(table) in self.query_tables:
					...
					raise stylo.PermissionError(ch_doctype)
	...
	def prepare_filter_condition(self, f):
		...
		if f.operator.lower() in NestedSetHierarchy:
			...
			fallback = "''"
			value = [stylo.db.escape((cstr(v.name) or "").strip(), percent=False) for v in result]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
