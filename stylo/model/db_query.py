...
from stylo.database.utils import DefaultOrderBy, FallBackDateTimeStr, NestedSetHierarchy
from stylo.model import OPTIONAL_FIELDS, get_permitted_fields
from stylo.model.meta import get_table_columns
from stylo.model.utils import is_virtual_doctype
from stylo.model.utils.user_settings import get_user_settings, update_user_settings
from stylo.query_builder.utils import Column
from stylo.types import Filters, FilterSignature, FilterTuple
...
from stylo.utils.data import DateTimeLikeObject, get_datetime, getdate, sbool
...
class DatabaseQuery:
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or stylo.session.user
	...
	def get_meta(self, doctype: str):
		if doctype not in self._metas:
			self._metas[doctype] = stylo.get_meta(doctype)
	...
	def execute(
		...
		*,
		...
		if as_list and not isinstance(self.fields, (Sequence | str)) and len(self.fields) > 1:
			stylo.throw(_("Fields must be a list or tuple when as_list is enabled"))
		...
		if is_virtual_doctype(self.doctype):
			...
			return stylo.call(controller.get_list, args=kwargs, **kwargs)
	...
	def build_and_run(self):
		...
		if stylo.db.db_type == "postgres" and args.order_by and args.group_by:
...
{limit}""".format(**args)
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {stylo.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			stylo.throw(_("Use of sub-query or function is restricted"), stylo.DataError)
		...
		for field in self.fields:
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					stylo.throw(_("Illegal SQL Query"))
	...
	def apply_fieldlevel_read_permissions(self):
		...
		Note: Does not apply to `stylo.model.core_doctype_list`
		...
		Example:
		        ...
		        - Query: fields=["*"]
		        - Result: fields=["title", ...] // will also include Stylo's meta field like `name`, `owner`, etc.
		"""
		from stylo.desk.reportview import extract_fieldnames
		...
		for i, field in fields_to_check:
			...
			# labels / pseudo columns or stylo internals
			if column[0] in {"'", '"'}:
				continue
			...
			if doctype and doctype != self.doctype:
				if wrap_grave_quotes(table) not in self.query_tables:
					raise stylo.PermissionError(doctype)
	...
	def prepare_filter_condition(self, ft: FilterTuple) -> str:
		...
		from stylo.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in NestedSetHierarchy:
			...
			lft, rgt = "", ""
			if f.value:
				lft, rgt = stylo.db.get_value(ref_doctype, f.value, ["lft", "rgt"]) or (0, 0)
			...
			fallback = "''"
			value = [stylo.db.escape((cstr(v)).strip(), percent=False) for v in nodes]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
		...
		if f.operator.lower() in ("in", "not in"):
			...
			if value is None:
				...
				fallback = "''"
				value = [stylo.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
				if len(value):
					value = f"({', '.join(value)})"
				else:
					value = "('')"
			...
			if f.operator in (">", "<", ">=", "<=") and (f.fieldname in ("creation", "modified")):
				...
				df
				and (db_type := cstr(stylo.db.type_map.get(df.fieldtype, " ")[0]))
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if stylo.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif escape and isinstance(value, str):
				value = f"{stylo.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and stylo.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True) -> str | list:
		...
		role_permissions = stylo.permissions.get_role_permissions(self.doctype_meta, user=self.user)
		if (
			...
			only_if_shared = True
			self.shared = stylo.share.get_shared(self.doctype, self.user)
			if not self.shared:
				stylo.throw(_("No permission to read {0}").format(_(self.doctype)), stylo.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				...
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {stylo.db.escape(self.user, percent=False)}"
				)

			# add user permission only if role has read perm
			elif role_permissions.get("read") or role_permissions.get("select"):
				# get user permissions
				user_permissions = stylo.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(stylo.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if stylo.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(stylo.db.escape(doc, percent=False) for doc in docs)
	...
	def get_permission_query_conditions(self) -> str:
		conditions = []
		hooks = stylo.get_hooks("permission_query_conditions", {})
		condition_methods = hooks.get(self.doctype, []) + hooks.get("*", [])
		for method in condition_methods:
			if c := stylo.call(stylo.get_attr(method), self.user, doctype=self.doctype):
				conditions.append(c)

		if permission_script_name := get_server_script_map().get("permission_query", {}).get(self.doctype):
			script = stylo.get_doc("Server Script", permission_script_name)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if any(re.search(r"\b" + pattern + r"\b", sanitized) for pattern in subquery_indicators):
			stylo.throw(_("Cannot use sub-query here."))
		...
		for field in parameters.split(","):
			...
			if full_field_name:
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					stylo.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))
			...
			for func in blacklisted_sql_functions:
				if re.search(r"\b" + re.escape(func) + r"\W*\(", field.lower()):
					stylo.throw(_("Cannot use {0} in order/group by").format(field))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
			stylo.db.exists("DocField", {"parent": parent, "options": child_doctype})
			or stylo.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
			raise stylo.PermissionError

		if stylo.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = stylo.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	from_date = stylo.utils.nowdate()
	to_date = stylo.utils.nowdate()
	...
	if fieldtype == "Datetime":
		cond = f"'{stylo.db.format_datetime(from_date)}' AND '{stylo.db.format_datetime(to_date)}'"
	else:
		cond = f"'{stylo.db.format_date(from_date)}' AND '{stylo.db.format_date(to_date)}'"
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = stylo._dict(stylo.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = stylo._dict(option)
