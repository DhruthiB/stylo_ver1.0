...
import stylo
from stylo import _
from stylo.model import child_table_fields, default_fields, table_fields
...
def make_mapped_doc(method, source_name, selected_children=None, args=None):
	...
	method = stylo.get_attr(stylo.override_whitelisted_method(method))

	stylo.is_whitelisted(method)

	if selected_children:
		selected_children = json.loads(selected_children)

	if args:
		stylo.flags.args = stylo._dict(json.loads(args))

	stylo.flags.selected_children = selected_children or None
...
def get_mapped_doc(
	...
	apply_strict_user_permissions = stylo.get_system_settings("apply_strict_user_permissions")
	...
	if not target_doc:
		...
		if table_maps[from_doctype].get("on_parent"):
			...
			if isinstance(target_parent, str):
				target_parent = stylo.get_doc(json.loads(target_parent))
			target_parentfield = target_parent.get_parentfield_of_doctype(target_doctype)
			target_doc = stylo.new_doc(
			...
			target_doc = stylo.new_doc(target_doctype)
			ret_doc = target_doc
	elif isinstance(target_doc, str):
		target_doc = stylo.get_doc(json.loads(target_doc))
		ret_doc = target_doc
	else:
		ret_doc = target_doc
	...
	if cached:
		source_doc = stylo.get_cached_doc(from_doctype, from_docname)
	else:
		source_doc = stylo.get_doc(from_doctype, from_docname)
		...
		for df in source_doc.meta.get_table_fields():
			...
			if table_map:
				...
				for source_d in source_doc.get(df.fieldname):
					...
					if (
						stylo.flags.selected_children
						and (df.fieldname in stylo.flags.selected_children)
						and source_d.name not in stylo.flags.selected_children[df.fieldname]
					):
						continue
...
def map_fetch_fields(target_doc, df, no_copy_fields):
	...
	for fetch_df in target_doc.meta.get("fields", {"fetch_from": f"^{df.fieldname}."}):
		...
		if (
			...
			if not linked_doc:
				try:
					linked_doc = stylo.get_doc(df.options, target_doc.get(df.fieldname))
				except Exception:
					return
...
def map_child_doc(source_d, target_parent, table_map, source_parent=None):
	...
	target_d = stylo.new_doc(target_child_doctype, parent_doc=target_parent, parentfield=target_parentfield)
