...
from stylo import _, get_module_path
from stylo.desk.doctype.tag.tag import delete_tags_for_document
from stylo.model.docstatus import DocStatus
from stylo.model.dynamic_links import get_dynamic_link_map
from stylo.model.naming import revert_series_if_last
from stylo.model.utils import is_virtual_doctype
from stylo.utils.data import get_link_to_form
from stylo.utils.file_manager import remove_all
from stylo.utils.global_search import delete_for_document
from stylo.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	Args:
		doctype (str, optional): The document type to delete. If not provided,
			retrieved from stylo.form_dict.get("dt"). Defaults to None.
		name (str | int | list, optional): The name/ID of the document(s) to delete.
			...
			retrieved from stylo.form_dict.get("dn"). Defaults to None.
		...
		ignore_missing (bool, optional): When True, doesn't raise an error if the
			...
			stylo.DoesNotExistError if document is missing. Defaults to True.
	...
	Raises:
		stylo.DoesNotExistError: When document doesn't exist and ignore_missing=False.
		stylo.LinkExistsError: When document is linked to other records and force=False.
		stylo.PermissionError: When user doesn't have delete permissions and ignore_permissions=False.
		stylo.ValidationError: When trying to delete a submitted document.
		stylo.QueryTimeoutError: When document is locked by another user.
	...
	# get from form
	if not doctype:
		doctype = stylo.form_dict.get("dt")
		name = stylo.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			stylo.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				doc = stylo.get_doc(doctype, name)
				if not (doc.custom or stylo.conf.developer_mode or stylo.flags.in_patch or force):
					stylo.throw(_("Standard DocType can not be deleted."))

				update_flags(doc, flags, ignore_permissions)
				check_permission_and_not_submitted(doc)
				# delete custom table fields using this doctype.
				stylo.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", stylo.model.table_fields)}
				)
				stylo.db.delete("__global_search", {"doctype": name})

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				stylo.conf.developer_mode
				and not doc.custom
				and not (
					for_reload
			...
			stylo.clear_cache(doctype=name)

		else:
			# Lock the doc without waiting
			try:
				stylo.db.get_value(doctype, name, for_update=True, wait=False)
			except (stylo.QueryTimeoutError, stylo.QueryDeadlockError):
				stylo.throw(
					_(
						"This document can not be deleted right now as it's being modified by another user. Please try again after some time."
					),
					exc=stylo.QueryTimeoutError,
				)
			doc = stylo.get_doc(doctype, name)
			...
			if not for_reload:
				...
				if not force:
					...
					except stylo.LinkExistsError as e:
			...
			if not for_reload:
				...
				stylo.enqueue(
					"stylo.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
					now=stylo.in_test,
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and stylo.flags.in_install != "stylo":
		stylo.get_doc(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		stylo.db.delete("Singles", {"doctype": name})
	else:
		stylo.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if stylo.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = stylo.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", stylo.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		stylo.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	# permission
	if not doc.flags.ignore_permissions and stylo.session.user != "Administrator":
		if doc.doctype == "DocType" and not doc.custom:
			stylo.throw(_("Only the Administrator can delete a standard DocType."))
		else:
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from stylo.model.rename_doc import get_link_fields
	...
	if method == "Delete":
		ignored_doctypes.update(stylo.get_hooks("ignore_links_on_delete"))
	...
	for lf in link_fields:
		...
		if issingle:
			if stylo.db.get_single_value(link_dt, link_field) == doc.name:
		...
		for item in stylo.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
...
def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `stylo.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):
		ignore_linked_doctypes = doc.get("ignore_linked_doctypes") or []

		if df.parent in stylo.get_hooks("ignore_links_on_delete") or (
		...
		if meta.issingle:
			...
			for refdoc in stylo.db.sql(
				...
				if (method == "Delete" and not DocStatus(refdoc.docstatus).is_cancelled()) or (
					...
					if reference_doctype in stylo.get_hooks("ignore_links_on_delete") or (
...
def clear_timeline_references(link_doctype, link_name):
	stylo.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", stylo.scrub(doctype))
