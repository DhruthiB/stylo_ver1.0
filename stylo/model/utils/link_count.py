...
def notify_link_count(doctype, name):
	...
	if doctype in ignore_doctypes or not stylo.request or random() < 0.9:  # Sample 10%
		return

	if not hasattr(stylo.local, "_link_count"):
		stylo.local._link_count = defaultdict(int)
		stylo.db.after_commit.add(flush_local_link_count)

	stylo.local._link_count[(doctype, name)] += 1


def flush_local_link_count():
	"""flush from local before ending request"""
	new_links = getattr(stylo.local, "_link_count", None)
	if not new_links:
		return

	link_count = stylo.cache.get_value("_link_count") or {}
	...
	if link_count:
		for (doctype, name), count in link_count.items():
			try:
				...
				stylo.qb.update(table).set(table.idx, table.idx + count).where(table.name == name).run()
				stylo.db.commit()
