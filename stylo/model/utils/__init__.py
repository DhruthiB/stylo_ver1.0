...
def set_field_property(filters, key, value):
	...
	docs = [
		stylo.get_doc("DocType", d.parent)
		for d in stylo.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(stylo.ValidationError):
	pass
	...
	for _ in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with open(stylo.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def is_virtual_doctype(doctype: str):
	if stylo.flags.in_install or stylo.flags.in_migrate:
		return (
			stylo.db.get_value("DocType", doctype, "is_virtual")
			if stylo.db.has_column("DocType", "is_virtual")
			else False
		)

	return getattr(stylo.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if stylo.flags.in_install or stylo.flags.in_migrate:
		...
		return getattr(stylo.get_meta(doctype), "issingle", False)
