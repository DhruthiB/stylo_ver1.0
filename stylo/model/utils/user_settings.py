...
def get_user_settings(doctype, for_update=False):
	user_settings = stylo.cache.hget("_user_settings", f"{doctype}::{stylo.session.user}")

	if user_settings is None:
		user_settings = stylo.db.sql(
			"""select data from `__UserSettings`
			where `user`=%s and `doctype`=%s""",
			(stylo.session.user, doctype),
		)
...
def update_user_settings(doctype, user_settings, for_update=False):
	...
	stylo.cache.hset("_user_settings", f"{doctype}::{stylo.session.user}", json.dumps(current))


def sync_user_settings():
	"""Sync from cache to database (called asynchronously via the browser)"""
	for key, data in stylo.cache.hgetall("_user_settings").items():
		key = safe_decode(key)
...
def update_user_settings_data(
	...
	if data:
		...
		if update:
			...
			stylo.cache.hset("_user_settings", f"{user_setting.doctype}::{user_setting.user}", None)
