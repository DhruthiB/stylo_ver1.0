...
from stylo.utils import cast, cint, cstr
...
def get_meta(doctype, cached=True) -> "Meta":
	...
	if meta := stylo.cache().hget("doctype_meta", doctype):
		return meta

	meta = Meta(doctype)
	stylo.cache().hset("doctype_meta", doctype, meta)
	return meta
...
class Meta(Document):
	...
	def process(self):
		...
		self.is_large_table = stylo.db.estimate_count(self.name) > LARGE_TABLE_THRESHOLD
...
def is_single(doctype):
	try:
		return stylo.db.get_value("DocType", doctype, "issingle")
	except IndexError:
...
def get_field_precision(df, doc=None, currency=None):
	...
	from stylo.utils import get_number_format_info
	...
	if df.precision:
		...
		if not precision:
			number_format = stylo.db.get_default("number_format") or "#,###.##"
...
def trim_table(doctype, dry_run=True):
	stylo.cache().hdel("table_columns", f"tab{doctype}")
