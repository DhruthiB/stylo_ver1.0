...
from stylo.model.base_document import (
	DOCTYPE_TABLE_FIELDS,
	TABLE_DOCTYPES_FOR_DOCTYPE,
	BaseDocument,
)
from stylo.model.document import Document
from stylo.model.utils import is_single_doctype
from stylo.model.workflow import get_workflow_name
from stylo.modules import load_doctype_module
from stylo.utils import cached_property, cast, cint, cstr
from stylo.utils.caching import site_cache
from stylo.utils.data import add_to_date, get_datetime
...
def get_meta(doctype: "str | DocType", cached: bool = True) -> "_Meta":
	...
	if (
		...
		and (meta := stylo.client_cache.get_value(f"doctype_meta::{doctype}"))
	):
		return meta
	...
	stylo.client_cache.set_value(key, meta)
	return meta
...
def clear_meta_cache(doctype: str = "*"):
	...
	if doctype == "*":
		stylo.client_cache.delete_keys(key)
	else:
		stylo.client_cache.delete_value(key)
...
def get_table_columns(doctype):
	return stylo.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = stylo.scrub(doctype)
	with open(stylo.get_app_path("stylo", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = (
		stylo._dict(fieldname="creation", fieldtype="Datetime"),
		stylo._dict(fieldname="owner", fieldtype="Data"),
	)
	...
	def get_masked_fields(self):
		...
		if stylo.session.user == "Administrator":
			return []
		cache_key = f"masked_fields::{self.name}::{stylo.session.user}"
		masked_fields = stylo.cache.get_value(cache_key)
		...
		if masked_fields is None:
			...
			stylo.cache.set_value(cache_key, masked_fields)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(stylo._dict(fieldtype="Data", fieldname="name", label="Name"))
	...
	def _valid_columns(self):
		table_exists = stylo.db.table_exists(self.name)
	...
	def _valid_fields(self):
		if (stylo.flags.in_install or stylo.flags.in_migrate) and self.name in self.special_doctypes:
	...
	def add_custom_fields(self):
		if not stylo.db.table_exists("Custom Field"):
			return

		custom_fields = stylo.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not stylo.db.table_exists("Property Setter"):
			return

		property_setters = stylo.db.get_values(
			"Property Setter",
	...
	def check_if_large_table(self):
		...
		if self.istable or not stylo.db.table_exists(self.name):  # During install, new migrate
			return

		if stylo.db.estimate_count(self.name) > LARGE_TABLE_SIZE_THRESHOLD:
			recent_change = stylo.db.get_value(self.name, {}, "creation", order_by="creation desc")
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(stylo._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if not permission_type:
			permission_type = "select" if stylo.only_has_select_perm(self.name, user=user) else "read"
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if stylo.share.get_shared(self.name, user, rights=[permission_type], limit=1):
	...
	def get_permlevel_access(self, permission_type="read", parenttype=None, *, user=None):
		has_access_to = []
		roles = set(stylo.get_roles(user))
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = stylo.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other Stylo applications via hooks.
		"""
		data = stylo._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = stylo._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in stylo.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = stylo._dict(stylo.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = stylo.scrub(self.module)
		doctype = stylo.scrub(self.name)
		template_path = stylo.get_module_path(
...
def get_parent_dt(dt):
	if not stylo.is_table(dt):
		return ""

	return (
		stylo.db.get_value(
			"DocField",
			{"fieldtype": ("in", stylo.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	stylo.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(stylo.local, "field_currency", None):
		stylo.local.field_currency = stylo._dict()

	if not (
		stylo.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and stylo.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = stylo.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if stylo.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = stylo.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			stylo.local.field_currency.setdefault((doc.doctype, ref_docname), stylo._dict()).setdefault(
				df.fieldname, currency
			)

	return stylo.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent") and stylo.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)


def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from stylo.locale import get_number_format

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(stylo.db.get_default("currency_precision"))
		if not precision:
			number_format = get_number_format()
			precision = number_format.precision
	else:
		precision = cint(stylo.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return stylo._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return stylo._dict(fieldname=fieldname, fieldtype="Int")

		elif fieldname in ("owner", "modified_by"):
			return stylo._dict(fieldname=fieldname, fieldtype="Link", options="User")

		return stylo._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in stylo.get_all("DocType", filters=filters, pluck="name"):
		try:
			dropped_columns = trim_table(doctype, dry_run=dry_run)
			if dropped_columns:
				UPDATED_TABLES[doctype] = dropped_columns
		except stylo.db.TableMissingError:
			if quiet:
				continue
...
def trim_table(doctype, dry_run=True):
	...
	fields = stylo.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		stylo.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
...
if typing.TYPE_CHECKING:
	...
	from stylo.core.doctype.doctype.doctype import DocType
