...
import stylo
from stylo import _, is_whitelisted, msgprint
from stylo.core.doctype.file.utils import relink_mismatched_files
from stylo.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from stylo.database.utils import commit_after_response
from stylo.desk.form.document_follow import follow_document
from stylo.integrations.doctype.webhook import run_webhooks
from stylo.model import optional_fields, table_fields
from stylo.model.base_document import BaseDocument, D, get_controller
from stylo.model.docstatus import DocStatus
from stylo.model.naming import set_new_name, validate_name
from stylo.model.utils import is_virtual_doctype, simple_singledispatch
from stylo.model.workflow import set_workflow_state_on_action, validate_workflow
from stylo.types import DF
from stylo.types.filter import FilterSignature
from stylo.utils import compare, cstr, date_diff, file_lock, flt, get_table_name, now
from stylo.utils.data import get_absolute_url, get_datetime, get_timedelta, getdate
from stylo.utils.global_search import update_global_search

if TYPE_CHECKING:
	from typing_extensions import Self

	from stylo.core.doctype.docfield.docfield import DocField
...
def get_doc(**kwargs: dict) -> "_NewDocument":
	...
	Not recommended. Use `stylo.new_doc` instead."""
	pass
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `stylo.model.Document` object.
...
class Document(BaseDocument):
	...
	def load_from_db(self) -> "Self":
		...
		if not is_doctype and self.meta.issingle:
			single_doc = stylo.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = stylo.new_doc(self.doctype, as_dict=True)
			...
			if not is_doctype and isinstance(self.name, str | int):
				for_update = ""
				if self.flags.for_update and stylo.db.db_type != "sqlite":
			...
			if not d:
				stylo.throw(
					...
					stylo.DoesNotExistError(doctype=self.doctype),
				)
	...
	def mask_fields(self):
		from stylo.model.db_query import mask_field_value

		mask_fields = stylo.get_meta(self.doctype).get_masked_fields()
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `stylo.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self._handle_permission_failure(permtype)

	def has_permission(self, permtype="read", *, debug=False, user=None) -> bool:
		"""
		Call `stylo.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return stylo.permissions.has_permission(self.doctype, permtype, self, debug=debug, user=user)
	...
	def insert(
		...
		if not (stylo.flags.in_migrate or stylo.local.flags.in_install or stylo.flags.in_setup_wizard):
			if stylo.get_cached_value("User", stylo.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, stylo.session.user)
		return self
	...
	def check_if_locked(self):
		...
		if file_lock.lock_age(self.get_signature()) > DOCUMENT_LOCK_SOFT_EXPIRY:
			primary_action = {
				"label": "Force Unlock",
				"server_action": "stylo.model.document.unlock_document",
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Self":
		...
		self.flags.ignore_version = stylo.in_test if ignore_version is None else ignore_version
	...
	def validate_amended_from(self):
		if stylo.db.get_value(self.doctype, self.get("amended_from"), "docstatus") != 2:
			message = _(
				"{0} cannot be amended because it is not cancelled. Please cancel the document before creating an amendment."
			).format(stylo.utils.get_link_to_form(self.doctype, self.get("amended_from")))
			stylo.throw(message, title=_("Amendment Not Allowed"))
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `stylo.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		stylo.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in stylo.db.value_cache:
			stylo.db.value_cache.pop(self.doctype, None)

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = stylo.session.user
		...
		if self.is_new() and not (
			stylo.flags.in_install or stylo.flags.in_patch or stylo.flags.in_migrate
		):
		...
		stylo.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					...
					stylo.bold(_(df.label, context=df.parent)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), stylo.bold(_(df.label, context=df.parent))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				stylo.throw(msg, stylo.NonNegativeError, title=_("Negative Value"))
				...
				if fail:
					stylo.throw(
						_("Value cannot be changed for {0}").format(
							stylo.bold(_(self.meta.get_label(field.fieldname)))
						),
						exc=stylo.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields(include_computed=True):
			all_fields += stylo.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields(include_computed=True):
			for df in stylo.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or stylo.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = stylo.get_meta(df.options).get_high_permlevel_fields()
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = stylo.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		if self.is_new():
			new_doc = stylo.new_doc(self.doctype, as_dict=True)
			self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = stylo.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			stylo.msgprint(
				...
				raise_exception=stylo.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.DRAFT:
			if self.docstatus.is_draft():
				...
				raise stylo.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise stylo.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise stylo.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if stylo.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise stylo.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			stylo.throw(_("Could not find {0}").format(msg), stylo.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			stylo.throw(_("Cannot link cancelled document: {0}").format(msg), stylo.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			method == "onload"
			or (stylo.flags.in_import and stylo.flags.mute_emails)
		...
		from stylo.email.doctype.notification.notification import evaluate_alert
		...
		notifications = stylo.client_cache.get_value(
	...
	def _rename(
		...
		"""Rename the document. Triggers stylo.rename_doc, then reloads."""
	...
	def discard(self):
		...
		if not self.docstatus.is_draft():
			raise stylo.ValidationError(_("Only draft documents can be discarded"), self.docstatus)
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = stylo.get_doc(self.doctype, self.name, for_update=True)
		except stylo.DoesNotExistError:
			if raise_exception:
				raise

			return stylo.clear_last_message()
	...
	def run_post_save_methods(self):
		...
		if not (stylo.flags.in_import and self.is_new()):
			self.clear_cache()
		...
		if (self.doctype, self.name) in stylo.flags.currently_saving:
			stylo.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		stylo.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			stylo.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([stylo.session.user]), update_modified=False
			)
	...
	def notify_update(self):
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and not self.meta.get("istable"):
			data = {"doctype": self.doctype, "name": self.name, "user": stylo.session.user}
			stylo.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run stylo.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in stylo.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", stylo.session.user)
	...
	def db_get(self, fieldname):
		...
		return stylo.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from stylo.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def save_version(self):
		...
		if (
			...
			or stylo.flags.in_install
			or (not self._doc_before_save and stylo.flags.in_patch)
		):
			return

		doc_to_compare = self._doc_before_save
		if not doc_to_compare and (amended_from := self.get("amended_from")):
			doc_to_compare = stylo.get_doc(self.doctype, amended_from)
	...
	def hook(f):
		...
		def compose(fn, *hooks):
			def runner(self, method, *args, **kwargs):
				...
				for f in hooks:
					try:
						stylo.db._disable_transaction_control += 1
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			stylo.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or stylo.EmptyTableError
			)
	...
	def round_floats_in(self, doc, fieldnames=None):
		...
		rounding_method = stylo.get_system_settings("rounding_method")
		...
		return stylo.get_doc(
			{
				...
				"comment_email": comment_email or stylo.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not stylo.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = stylo.parse_json(_seen)

			if user not in _seen:
				_seen.append(user)
				commit_after_response(
					lambda: stylo.db.set_value(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		if existing_key:
			doc = stylo.get_doc("Document Share Key", existing_key)
		else:
			doc = stylo.new_doc("Document Share Key")
	...
	def queue_action(self, action, **kwargs):
		...
		return enqueue(
			"stylo.model.document.execute_action",
			...
			if timeout:
				for _ in range(timeout):
					...
					if not file_lock.lock_exists(signature):
						...
						break
			if lock_exists:
				raise stylo.DocumentLockedError
		file_lock.create_lock(signature)
		stylo.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in stylo.local.locked_documents:
			stylo.local.locked_documents.remove(self)
	...
	def validate_from_to_dates(self, from_date_field: str, to_date_field: str) -> None:
		...
		if date_diff(to_date, from_date) < 0:
			...
			if self.meta.istable:
				table_row = (
					_("{0} row #{1}:").format(
						_(stylo.unscrub(self.parentfield)),
						self.idx,
					)
					+ " "
				)
			...
			stylo.throw(
				...
				+ _("{0} must be after {1}").format(
					stylo.bold(_(self.meta.get_label(to_date_field))),
					stylo.bold(_(self.meta.get_label(from_date_field))),
				),
				stylo.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = stylo.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from stylo.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from stylo.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = stylo.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		if stylo.message_log:
			msg = stylo.message_log[-1].get("message")
		else:
			msg = "<pre><code>" + stylo.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	for child_table in child_table_fields:
		valid_column_map[child_table.options] = stylo.get_meta(child_table.options).get_valid_columns()
	...
	while document_batch := list(itertools.islice(documents, chunk_size)):
		...
		for dt, docs in values_map.items():
			stylo.db.bulk_insert(dt, valid_column_map[dt], docs, ignore_duplicates=ignore_duplicates)
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.owner = doc.modified_by = stylo.session.user
...
def unlock_document(doctype: str, name: str):
	stylo.get_lazy_doc(doctype, name).unlock()
	stylo.msgprint(stylo._("Document Unlocked"), alert=True)


def get_lazy_controller(doctype):
	lazy_controllers = stylo.lazy_controllers.setdefault(stylo.local.site, {})
...
def new_doc(
	...
	**kwargs,
	...
	from stylo.model.create_new import get_new_doc
...
def get_cached_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Identical to `stylo.get_doc`, but return from cache if available."""
	if (key := can_cache_doc(args)) and (doc := stylo.cache.get_value(key)):
		return doc
...
def _set_document_in_cache(key: str, doc: "Document") -> None:
	stylo.cache.set_value(key, doc, expires_in_sec=3600)
...
def clear_document_cache(doctype: str, name: str | None = None) -> None:
	stylo.db.value_cache.pop(doctype, None)

	def clear_in_redis():
		if name is not None:
			stylo.cache.delete_value(get_document_cache_key(doctype, name))
		else:
			stylo.cache.delete_keys(get_document_cache_key(doctype, ""))

	clear_in_redis()
	if hasattr(stylo.db, "after_commit"):
		stylo.db.after_commit.add(clear_in_redis)
		stylo.db.after_rollback.add(clear_in_redis)

	if doctype == "System Settings" and hasattr(stylo.local, "system_settings"):
		delattr(stylo.local, "system_settings")

	if doctype == "Website Settings" and hasattr(stylo.local, "website_settings"):
		delattr(stylo.local, "website_settings")
...
def get_cached_value(
	...
	if isinstance(fieldname, str):
		if as_dict:
			stylo.throw("Cannot make dict for single fieldname")
		return doc.get(fieldname)

	values = [doc.get(f) for f in fieldname]
	if as_dict:
		return stylo._dict(zip(fieldname, values, strict=False))
	return values
...
def get_single_value(setting: str, fieldname: str, /, *, as_dict: bool = False):
	...
	Usage:
		telemetry_enabled = stylo.get_single_value("System Settings", "telemetry_enabled")
	"""
...
def get_last_doc(
	...
	*,
	...
	d = stylo.get_all(doctype, filters=filters, limit_page_length=1, order_by=order_by, pluck="name")
	if d:
...
def get_single(doctype):
	"""Return a `stylo.model.document.Document` object of the given Single doctype."""
