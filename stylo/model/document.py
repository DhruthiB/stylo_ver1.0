...
from stylo.model.base_document import BaseDocument, get_controller
...
from stylo.utils import cstr, date_diff, file_lock, flt, get_datetime_str, now
...
def get_doc(*args, **kwargs):
	"""returns a stylo.model.Document object.
...
class Document(BaseDocument):
	...
	def load_from_db(self):
		...
		for df in self._get_table_fields():
			...
			# During stylo is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to stylo.db.get_value() as it would crash otherwise.
	...
	def has_permission(self, permtype="read", verbose=False) -> bool:
		...
		return stylo.permissions.has_permission(self.doctype, permtype, self)
	...
	def raise_no_permission_to(self, perm_type):
		...
		stylo.flags.error_message = (
			_("Insufficient Permission for {0}").format(self.doctype) + f" ({stylo.bold(_(perm_type))})"
		)
		raise stylo.PermissionError
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Document":
		...
		self.flags.ignore_version = stylo.flags.in_test if ignore_version is None else ignore_version
	...
	def update_single(self, d):
		...
		if self.doctype in stylo.db.value_cache:
			del stylo.db.value_cache[self.doctype]
		...
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					...
					stylo.bold(_(df.label)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), stylo.bold(_(df.label))
				)
				...
				if fail:
					stylo.throw(
						_("Value cannot be changed for {0}").format(
							stylo.bold(self.meta.get_label(field.fieldname))
						),
	...
	def run_notifications(self, method):
		...
		if (
			(stylo.flags.in_import and stylo.flags.mute_emails)
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = stylo.cache().hget("notifications", self.doctype, _get_notifications)
	...
	def validate_value(self, fieldname, condition, val2, doc=None, raise_exception=None):
		...
		if not stylo.compare(val1, condition, val2):
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not stylo.flags.read_only:
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", stylo._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", stylo._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		try:
			self.lock()
		except stylo.DocumentLockedError:
		...
		if date_diff(self.get(to_date_field), self.get(from_date_field)) < 0:
			stylo.throw(
				_("{0} must be after {1}").format(
					stylo.bold(self.meta.get_label(to_date_field)),
					stylo.bold(self.meta.get_label(from_date_field)),
				),
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	try:
		...
		if stylo.local.message_log:
			msg = json.loads(stylo.local.message_log[-1]).get("message")
		else:
...
def unlock_document(doctype: str | None = None, name: str | None = None, args=None):
	...
	if not doctype and not name and args:
		...
		name = str(args["name"])
	stylo.get_doc(doctype, name).unlock()
