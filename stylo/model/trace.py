"""
Traced Fields for Stylo

This module provides utilities for creating traced fields in Stylo documents,
...
- Seamlessly integrate with Stylo's document model

Example of standard usage:
		from stylo.model.trace import TracedDocument, traced_field
...
See stylo.tests.classes.context_managers for a context manager built into test classes.
"""
...
class TracedValue:
	"""
	A descriptor class for creating traced fields in Stylo documents.

	This class allows for monitoring and validating changes to specific fields
	in a Stylo document. It can enforce forbidden values and apply custom
	validation logic.
	...
	def __set__(self, obj, value):
		...
		if value in self.forbidden_values:
			if stylo.in_test:
				stylo.throw(f"{self.field_name} cannot be set to {value}", AssertionError)
			else:
				stylo.throw(f"{self.field_name} cannot be set to {value}")
		...
		if self.custom_validation:
			try:
				...
				if stylo.in_test:
					stylo.throw(str(e), AssertionError)
				else:
					stylo.throw(str(e))
...
class TracedDocument(Document):
	"""
	A base class for Stylo documents with traced fields.

	This class extends Stylo's Document class to provide support for
	traced fields created with TracedValue.

	Attributes:
	        Inherits all attributes from stylo.model.document.Document
	"""
...
def traced_field(*args, **kwargs):
	...
	This function simplifies the creation of traced fields in Stylo documents.
...
from stylo.deprecation_dumpster import model_trace_traced_field_context as traced_field_context
