...
if TYPE_CHECKING:
	...
	from stylo.workflow.doctype.workflow.workflow import Workflow


DEFAULT_WORKFLOW_TASKS = ["Webhook", "Server Script"]


class WorkflowStateError(stylo.ValidationError):
	pass


class WorkflowTransitionError(stylo.ValidationError):
	pass


class WorkflowPermissionError(stylo.ValidationError):
	pass


def get_workflow_name(doctype):
	workflow_name = stylo.cache.hget("workflow", doctype)
	if workflow_name is None:
		workflow_name = stylo.db.get_value("Workflow", {"document_type": doctype, "is_active": 1}, "name")
		stylo.cache.hset("workflow", doctype, workflow_name or "")
...
def get_transitions(
	...
	if not isinstance(doc, Document):
		doc = stylo.get_doc(stylo.parse_json(doc))
		doc.load_from_db()
	...
	if not current_state:
		if raise_exception:
			...
			stylo.throw(_("Workflow State not set"), WorkflowStateError)
...
def get_workflow_safe_globals():
	# access to stylo.db.get_value, stylo.db.get_list, and date time utils.
	return dict(
		stylo=stylo._dict(
			db=stylo._dict(get_value=stylo.db.get_value, get_list=stylo.db.get_list),
			session=stylo.session,
			utils=stylo._dict(
				now_datetime=stylo.utils.now_datetime,
				add_to_date=stylo.utils.add_to_date,
				get_datetime=stylo.utils.get_datetime,
				now=stylo.utils.now,
			),
		)
	)
...
def is_transition_condition_satisfied(transition, doc) -> bool:
	if not transition.condition:
		...
		return stylo.safe_eval(transition.condition, get_workflow_safe_globals(), dict(doc=doc.as_dict()))
...
def apply_workflow(doc, action):
	...
	if not transition:
		stylo.throw(_("Not a valid Workflow Action"), WorkflowTransitionError)

	if not has_approval_access(user, doc, transition):
		stylo.throw(_("Self approval is not allowed"))
	...
	if transition.transition_tasks:
		workflow_transitions = stylo.db.get_all(
		...
		workflow_methods = [{"name": "Create a customer", "method":
					 		"stylo.dotted.path.create_customer"}]
		"""

		tasks = {i["name"]: i["method"] for i in stylo.get_hooks("workflow_methods")}
		...
		for workflow_transition in workflow_transitions:
			...
			if workflow_transition.task in DEFAULT_WORKFLOW_TASKS:
				match workflow_transition.task:
					case "Webhook":
						webhook = stylo.get_doc("Webhook", workflow_transition.link)
						task_method = webhook.execute_for_doc

					case "Server Script":
						server_script = stylo.get_doc("Server Script", workflow_transition.link)
						task_method = server_script.execute_workflow_task

			else:  # normal app-defined tasks
				try:
					task_method = stylo.get_attr(tasks[workflow_transition.task])
				except KeyError:
					stylo.throw(_('There is no task called "{}"').format(workflow_transition.task))
		...
		for async_task in async_tasks:
			stylo.enqueue(async_task, doc=doc, enqueue_after_commit=True)
	...
	if doc.docstatus.is_draft() and new_docstatus.is_draft():
		...
		stylo.throw(_("Illegal Document Status for {0}").format(next_state.state))
...
def validate_workflow(doc):
	...
	if current_state != next_state:
		bold_current = stylo.bold(current_state)
		bold_next = stylo.bold(next_state)
...
def get_workflow(doctype) -> "Workflow":
	return stylo.get_cached_doc("Workflow", get_workflow_name(doctype))
...
def get_workflow_field_value(workflow_name, field):
	return stylo.get_cached_value("Workflow", workflow_name, field)
...
def bulk_workflow_approval(docnames, doctype, action):
	...
	if len(docnames) < 20:
		...
		stylo.msgprint(_("Bulk {0} is enqueued in background.").format(action), alert=True)
		stylo.enqueue(
		...
		stylo.throw(_("Bulk approval only support up to 500 documents."), title=_("Too Many Documents"))
...
def _bulk_workflow_action(docnames, doctype, action):
	...
	for idx, docname in enumerate(docnames, 1):
		...
		try:
			...
			apply_workflow(stylo.get_doc(doctype, docname), action)
			stylo.db.commit()
		except Exception as e:
			if not stylo.message_log:
			...
			if not message_dict:
				if stylo.message_log:
					messages = stylo.get_message_log()
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		for doc in messages.keys():
			if len(messages[doc]):
				html = f"<details><summary>{stylo.utils.get_link_to_form(doctype, doc)}</summary>"
	...
	try:
		for i, doc in enumerate(docs, 1):
			...
			actions = [
				...
				if has_approval_access(stylo.session.user, doc, t)
			]
			if not actions:
				return []
