...
from stylo.exceptions import DoesNotExistError, ValidationError
from stylo.model.base_document import get_controller
from stylo.model.rename_doc import (
	bulk_rename,
	get_fetch_fields,
	update_document_title,
	update_linked_doctypes,
)
from stylo.modules.utils import get_doc_path
...
def patch_db(endpoints: list[str] | None = None):
	...
	for point in endpoints:
		x = patch(f"stylo.db.{point}", new=lambda: True)
		patched_endpoints.append(x)

	savepoint = "SAVEPOINT_for_test_bulk_rename"
	stylo.db.savepoint(save_point=savepoint)
	try:
...
class TestRenameDoc(StyloTestCase):
	...
	def tearDownClass(self):
		...
		stylo.delete_doc_if_exists("DocType", "Renamed Doc")
		stylo.db.sql_ddl("drop table if exists `tabRenamed Doc`")

		# delete the documents created
		for docname in self.available_documents:
			stylo.delete_doc(self.test_doctype, docname)

		for dt in self.doctype.values():
			if stylo.db.exists("DocType", dt):
				stylo.delete_doc("DocType", dt)
				stylo.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{dt}`")

		# reset original value of developer_mode conf
		stylo.conf.developer_mode = self._original_developer_flag

	def setUp(self):
		stylo.flags.link_fields = {}
		if self._testMethodName == "test_doc_rename_method":
			self.property_setter = stylo.get_doc(
				{
	...
	def test_rename_doc(self):
		"""Rename an existing document via stylo.rename_doc"""
		old_name = choice(self.available_documents)
		new_name = old_name + ".new"
		self.assertEqual(new_name, stylo.rename_doc(self.test_doctype, old_name, new_name, force=True))
		self.available_documents.remove(old_name)
		self.available_documents.append(new_name)

	def test_merging_docs(self):
		"""Merge two documents via stylo.rename_doc"""
		first_todo, second_todo = sample(self.available_documents, 2)

		second_todo_doc = stylo.get_doc(self.test_doctype, second_todo)
		second_todo_doc.priority = "High"
		second_todo_doc.save()

		merged_todo = stylo.rename_doc(self.test_doctype, first_todo, second_todo, merge=True, force=True)
		merged_todo_doc = stylo.get_doc(self.test_doctype, merged_todo)
		self.available_documents.remove(first_todo)

		with self.assertRaises(DoesNotExistError):
			stylo.get_doc(self.test_doctype, first_todo)
	...
	def test_rename_controllers(self):
		...
		self.assertNotEqual(get_controller(self.doctype.old), stylo.model.document.Document)

		old_doctype_path = get_doc_path("Custom", "DocType", self.doctype.old)

		# rename doc via wrapper API accessible via /desk
		stylo.rename_doc("DocType", self.doctype.old, self.doctype.new)

		# check if database and controllers are updated
		self.assertTrue(stylo.db.exists("DocType", self.doctype.new))
		self.assertFalse(stylo.db.exists("DocType", self.doctype.old))
		self.assertFalse(os.path.exists(old_doctype_path))

	def test_rename_doctype(self):
		"""Rename DocType via stylo.rename_doc"""
		from stylo.core.doctype.doctype.test_doctype import new_doctype

		if not stylo.db.exists("DocType", "Rename This"):
			new_doctype(
				"Rename This",
				fields=[
					{
		...
		to_rename_record = stylo.get_doc(
			{"doctype": "Rename This", "linked_to_doctype": "Rename This"}
		).insert()

		# Rename doctype
		self.assertEqual(
			"Renamed Doc", stylo.rename_doc("DocType", "Rename This", "Renamed Doc", force=True)
		)

		# Test if Doctype value has changed in Link field
		linked_to_doctype = stylo.db.get_value("Renamed Doc", to_rename_record.name, "linked_to_doctype")
		...
		new_name = "ToDo"
		self.assertEqual(new_name, stylo.rename_doc("Renamed Doc", old_name, new_name, force=True))

	def test_update_document_title_api(self):
		test_doctype = "Module Def"
		test_doc = stylo.get_doc(
			{
				"doctype": test_doctype,
				"module_name": f"Test-test_update_document_title_api-{stylo.generate_hash()}",
				"custom": True,
			}
		)
		...
		doc_before = stylo.get_doc(test_doctype, dn)
		return_value = update_document_title(doctype=dt, docname=dn, new_name=new_name)
		doc_after = stylo.get_doc(test_doctype, return_value)
	...
	def test_bulk_rename(self):
		...
		with patch_db(["commit", "rollback"]), patch("stylo.enqueue") as enqueue:
	...
	def test_deprecated_utils(self):
		...
		with redirect_stdout(stdout), patch_db(["set_value"]):
			...
			self.assertTrue("Function stylo.model.rename_doc.get_fetch_fields" in stdout.getvalue())

			update_linked_doctypes("User", "ToDo", "str", "str")
			self.assertTrue("Function stylo.model.rename_doc.update_linked_doctypes" in stdout.getvalue())

	def test_doc_rename_method(self):
		name = choice(self.available_documents)
		new_name = f"{name}-{stylo.generate_hash(length=4)}"
		doc = stylo.get_doc(self.test_doctype, name)
		doc.rename(new_name, merge=stylo.db.exists(self.test_doctype, new_name))
	...
	def test_parenttype(self):
		...
		parent_a_instance = stylo.get_doc(
			doctype=parent_a.name, test_table=[{"some_fieldname": "x"}], name="XYZ"
		).insert()

		parent_b_instance = stylo.get_doc(
