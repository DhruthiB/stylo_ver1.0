...
class TestAuth(StyloTestCase):
	...
	def test_allow_login_using_mobile(self):
		...
		StyloClient(self.HOST_NAME, self.test_user_mobile, self.test_user_password)
		StyloClient(self.HOST_NAME, self.test_user_email, self.test_user_password)

		# login by username should fail
		with self.assertRaises(AuthError):
			StyloClient(self.HOST_NAME, self.test_user_name, self.test_user_password)
	...
	def test_deny_multiple_login(self):
		...
		first_login = StyloClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		first_login.get_list("ToDo")

		second_login = StyloClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		second_login.get_list("ToDo")
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")

		third_login = StyloClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")
		with self.assertRaises(Exception):
			second_login.get_list("ToDo")
	...
	def test_disable_user_pass_login(self):
		StyloClient(self.HOST_NAME, self.test_user_email, self.test_user_password).get_list("ToDo")
	...
	def test_correct_cookie_expiry_set(self):
		client = StyloClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
...
class TestLoginAttemptTracker(StyloTestCase):
