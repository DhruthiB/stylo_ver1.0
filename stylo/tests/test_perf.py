...
- Time bound tests: Forgemarks are done on GHA before adding numbers
...
class TestPerformance(StyloTestCase):
	def reset_request_specific_caches(self):
		# To simulate close to request level of handling
		stylo.destroy()  # releases everything on stylo.local
		stylo.init(site=self.TEST_SITE)
		stylo.connect()
		stylo.clear_cache()

	def setUp(self) -> None:
		self.HOST = stylo.utils.get_site_url(stylo.local.site)
	...
	def test_permitted_fieldnames(self):
		...
		doc = stylo.new_doc("Prepared Report")
		# load permitted fieldnames once
		doc.permitted_fieldnames

		with patch("stylo.model.base_document.get_permitted_fields") as mocked:
			doc.as_dict()
			# get_permitted_fields should not be called again
			mocked.assert_not_called()

	def test_set_value_query_count(self):
		stylo.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			stylo.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			stylo.db.set_value("User", {"user_type": "System User"}, "interest", "Nothing")
	...
	def test_get_value_limits(self):
		...
		stylo.db.get_values("User", filters=filters[1], limit=1)
		for filter in filters:
			with self.assertRowsRead(1):
				self.assertEqual(1, len(stylo.db.get_values("User", filters=filter, limit=1)))
			with self.assertRowsRead(2):
				self.assertEqual(2, len(stylo.db.get_values("User", filters=filter, limit=2)))

			self.assertEqual(
				len(stylo.db.get_values("User", filters=filter)), stylo.db.count("User", filter)
			)

			with self.assertRowsRead(1):
				stylo.db.get_value("User", filters=filter)

			with self.assertRowsRead(1):
				stylo.db.exists("User", filter)
	...
	def test_req_per_seconds_basic(self):
		...
		req_count = 1000
		client = StyloClient(self.HOST, "Administrator", self.ADMIN_PASSWORD)
	...
	def test_no_stale_ref_sql(self):
		...
		for kwargs in ({}, {"as_dict": True}, {"as_list": True}):
			result = stylo.db.sql(query, **kwargs)
