...
class TestTestUtils(StyloTestCase):
	...
	def test_thread_locals(self):
		stylo.flags.temp_flag_to_be_discarded = True

	def test_temp_setting_changes(self):
		current_setting = stylo.get_system_settings("logout_on_password_reset")

		with change_settings("System Settings", {"logout_on_password_reset": int(not current_setting)}):
			updated_settings = stylo.get_system_settings("logout_on_password_reset")
			self.assertNotEqual(current_setting, updated_settings)

		restored_settings = stylo.get_system_settings("logout_on_password_reset")
...
def tearDownModule():
	...
	assert "temp_flag_to_be_discarded" not in stylo.flags
	assert not stylo.db.exists("Currency", "STONKS")
