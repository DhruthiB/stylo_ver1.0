...
from stylo.desk.page.setup_wizard.install_fixtures import update_global_search_doctypes
from stylo.test_runner import make_test_objects
from stylo.tests.utils import StyloTestCase
from stylo.utils import global_search, now_datetime


class TestGlobalSearch(StyloTestCase):
	def setUp(self):
		update_global_search_doctypes()
		global_search.setup_global_search_table()
		self.assertTrue("__global_search" in stylo.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		stylo.db.delete("Property Setter", {"doc_type": "Event"})
	...
	def test_update_doc(self):
		...
		event = stylo.get_doc("Event", stylo.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		self.insert_test_events()
		event_name = stylo.get_all("Event")[0].name
		event = stylo.get_doc("Event", event_name)
		...
		stylo.delete_doc("Event", event_name)
		global_search.sync_global_search()
		stylo.db.commit()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = stylo.get_doc({"doctype": "Event", "subject": text, "starts_on": now_datetime()})
			doc.insert()
