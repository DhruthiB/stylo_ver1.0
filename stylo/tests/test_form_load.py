...
from stylo.desk.form.load import get_docinfo, getdoc, getdoctype
from stylo.tests.utils import StyloTestCase
from stylo.utils.file_manager import save_file

test_dependencies = ["Blog Category", "Blogger"]


class TestFormLoad(StyloTestCase):
	def test_load(self):
		getdoctype("DocType")
		meta = next(filter(lambda d: d.name == "DocType", stylo.response.docs))
		self.assertEqual(meta.name, "DocType")
		self.assertTrue(meta.get("__js"))

		stylo.response.docs = []
		getdoctype("Event")
		meta = next(filter(lambda d: d.name == "Event", stylo.response.docs))
	...
	def test_fieldlevel_permissions_in_load(self):
		blog = stylo.get_doc(
			{
				...
				"title": f"_Test Blog Post {stylo.utils.now()}",
				"published": 0,
			}
		)
		...
		doc = stylo.get_doc("Blog Post", blog.name)
		doc.published = 1
		doc.save()
		...
		blog_doc.delete()
		stylo.delete_doc(blog_post_property_setter.doctype, blog_post_property_setter.name)
	...
	def test_fieldlevel_permissions_in_load_for_child_table(self):
		...
		contact = stylo.get_doc("Contact", "_Test Contact 1")
	...
	def test_get_doc_info(self):
		...
		get_docinfo(note)
		docinfo = stylo.response["docinfo"]
