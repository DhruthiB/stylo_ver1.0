...
class TestFixtureImport(StyloTestCase):
	def create_new_doctype(self, DocType: str) -> None:
		file = stylo.get_app_path("stylo", "custom", "fixtures", f"{DocType}.json")
	...
	def insert_dummy_data_and_export(self, DocType: str, dummy_name_list: list[str]) -> str:
		for name in dummy_name_list:
			doc = stylo.get_doc({"doctype": DocType, "member_name": name})
			doc.insert()
	...
	def test_fixtures_import(self):
		self.assertFalse(stylo.db.exists("DocType", "temp_doctype"))
		...
		stylo.db.truncate("temp_doctype")
		...
		self.assertEqual(stylo.db.count("temp_doctype"), len(dummy_name_list))

		data = stylo.get_all("temp_doctype", "member_name")
		stylo.db.truncate("temp_doctype")
	...
	def test_singles_fixtures_import(self):
		self.assertFalse(stylo.db.exists("DocType", "temp_singles"))
		...
		singles_doctype = stylo.qb.DocType("Singles")
		truncate_query = (
			stylo.qb.from_(singles_doctype).delete().where(singles_doctype.doctype == "temp_singles")
		)
		truncate_query.run()
		...
		data = stylo.db.get_single_value("temp_singles", "member_name")
		truncate_query.run()
