...
import stylo
from stylo.query_builder import Case
from stylo.query_builder.builder import Function
from stylo.query_builder.custom import ConstantColumn
from stylo.query_builder.functions import (
	Cast_,
	Coalesce,
	CombineDatetime,
	Date,
...
def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(stylo.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")


@run_only_if(db_type_is.MARIADB)
class TestCustomFunctionsMariaDB(StyloTestCase):
	...
	def test_constant_column(self):
		query = stylo.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
		self.assertEqual(query.get_sql(), "SELECT `name`,'John' `User` FROM `tabDocType`")

	def test_timestamp(self):
		note = stylo.qb.DocType("Note")
		self.assertEqual(
	...
	def test_time(self):
		...
		select_query = stylo.qb.from_(note).select(CombineDatetime(note.posting_date, note.posting_time))
	...
	def test_cast(self):
		...
		self.assertEqual(
			stylo.qb.from_("red").from_(note).select("other", Cast_(note.name, "varchar")).get_sql(),
...
class TestBuilderBase:
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", stylo.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", stylo.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", stylo.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = stylo.qb.from_("ToDo").select("*").limit(1)
	...
	def test_agg_funcs(self):
		stylo.db.truncate("Communication")
		sample_data = {
		...
		stylo.get_doc(sample_data).insert()
		sample_data["rating"] = 3
		stylo.get_doc(sample_data).insert()
		sample_data["rating"] = 4
		stylo.get_doc(sample_data).insert()
		self.assertEqual(stylo.qb.max("Communication", "rating"), 4)
		self.assertEqual(stylo.qb.min("Communication", "rating"), 1)
		self.assertAlmostEqual(stylo.qb.avg("Communication", "rating"), 2.666, places=2)
		self.assertEqual(stylo.qb.sum("Communication", "rating"), 8.0)
		stylo.db.rollback()


class TestParameterization(StyloTestCase):
	def test_where_conditions(self):
		DocType = stylo.qb.DocType("DocType")
		query = stylo.qb.from_(DocType).select(DocType.name).where(DocType.owner == "Administrator' --")
	...
	def test_set_conditions(self):
		DocType = stylo.qb.DocType("DocType")
		query = stylo.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_where_conditions_functions(self):
		DocType = stylo.qb.DocType("DocType")
		query = (
			stylo.qb.from_(DocType).select(DocType.name).where(Coalesce(DocType.search_fields == "subject"))
		)
	...
	def test_case(self):
		DocType = stylo.qb.DocType("DocType")
		query = stylo.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = stylo.qb.DocType("DocType")
		query = stylo.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
	...
	def test_named_parameter_wrapper(self):
		from stylo.query_builder.terms import NamedParameterWrapper
...
@run_only_if(db_type_is.MARIADB)
class TestBuilderMaria(StyloTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual("SELECT * FROM `tabNotes`", stylo.qb.from_("Notes").select("*").get_sql())
		self.assertEqual("SELECT * FROM `__Auth`", stylo.qb.from_("__Auth").select("*").get_sql())

	def test_get_qb_type(self):
		from stylo.query_builder import get_query_builder

		qb = get_query_builder(stylo.db.db_type)
		self.assertEqual("SELECT * FROM `tabDocType`", qb().from_("DocType").select("*").get_sql())


@run_only_if(db_type_is.POSTGRES)
class TestBuilderPostgres(StyloTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual('SELECT * FROM "tabNotes"', stylo.qb.from_("Notes").select("*").get_sql())
		self.assertEqual('SELECT * FROM "__Auth"', stylo.qb.from_("__Auth").select("*").get_sql())

	def test_replace_tables(self):
		info_schema = stylo.qb.Schema("information_schema")
		self.assertEqual(
			'SELECT * FROM "pg_stat_all_tables"',
			stylo.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", stylo.qb.Field("table_name").get_sql())
