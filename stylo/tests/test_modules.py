...
from stylo.modules import export_customizations, export_module_json, get_module_path
from stylo.modules.utils import export_doc, sync_customizations
...
class TestUtils(StyloTestCase):
	def setUp(self):
		self._dev_mode = stylo.local.conf.developer_mode
		stylo.local.conf.developer_mode = True

	def tearDown(self):
		stylo.db.rollback()
		stylo.local.conf.developer_mode = self._dev_mode
		stylo.local.flags.pop("in_import", None)

	def test_export_module_json_no_export(self):
		stylo.local.flags.in_import = True
		doc = stylo.get_last_doc("DocType")
	...
	def test_export_module_json(self):
		doc = stylo.get_last_doc("DocType", {"issingle": 0, "custom": 0})
		...
		with open(export_doc_path) as f:
			export_doc_before = stylo.parse_json(f.read())

		last_modified_before = os.path.getmtime(export_doc_path)
		self.addCleanup(write_file, path=export_doc_path, content=stylo.as_json(export_doc_before))

		stylo.flags.in_import = False
		stylo.conf.developer_mode = True
		...
		with open(f"{export_path}.json") as f:
			stylo.parse_json(f.read())  # export_doc_after
	...
	def test_reload_doc(self):
		stylo.db.set_value("DocType", "Note", "migration_hash", "", update_modified=False)
		self.assertFalse(stylo.db.get_value("DocType", "Note", "migration_hash"))
		stylo.db.set_value(
			"DocField",
		...
		self.assertEqual(
			stylo.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Text",
		)
		stylo.reload_doctype("Note")
		self.assertEqual(
			stylo.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Data",
		)
		self.assertTrue(stylo.db.get_value("DocType", "Note", "migration_hash"))
	...
	def test_export_doc(self):
		...
		note.title = stylo.generate_hash(length=10)
		note.save()
		export_doc(doctype="Note", name=note.name)
		exported_doc_path = Path(
			stylo.get_app_path("stylo", "desk", "note", note.name, f"{note.name}.json")
		)
	...
	def test_make_boilerplate(self):
		with temp_doctype() as doctype:
			scrubbed = stylo.scrub(doctype.name)
			path = stylo.get_app_path("stylo", "core", "doctype", scrubbed, f"{scrubbed}.json")
...
def temp_doctype():
	try:
		...
		stylo.db.sql_ddl(f"DROP TABLE `tab{doctype.name}`")
...
def note_customizations():
	try:
		...
		delete_path(stylo.get_module_path("Desk", "Note"))
