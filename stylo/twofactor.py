...
from stylo.utils import cint, get_datetime, get_url, time_diff_in_seconds
from stylo.utils.background_jobs import enqueue
from stylo.utils.password import decrypt, encrypt

PARENT_FOR_DEFAULTS = "__2fa"


def get_default(key):
	return stylo.db.get_default(key, parent=PARENT_FOR_DEFAULTS)


def set_default(key, value):
	stylo.db.set_default(key, value, parent=PARENT_FOR_DEFAULTS)


def clear_default(key):
	stylo.defaults.clear_default(key, parent=PARENT_FOR_DEFAULTS)
...
def toggle_two_factor_auth(state, roles=None):
	...
	for role in roles or []:
		role = stylo.get_doc("Role", {"role_name": role})
...
def two_factor_is_enabled(user=None):
	...
	enabled = int(stylo.db.get_single_value("System Settings", "enable_two_factor_auth") or 0)
	if enabled:
		bypass_two_factor_auth = int(
			stylo.db.get_single_value("System Settings", "bypass_2fa_for_retricted_ip_users") or 0
		)
		if bypass_two_factor_auth and user:
			...
			if restrict_ip_list and stylo.local.request_ip:
				for ip in restrict_ip_list:
					if stylo.local.request_ip.startswith(ip):
						enabled = False
						break
...
def get_cached_user_pass():
	...
	user = pwd = None
	tmp_id = stylo.form_dict.get("tmp_id")
	if tmp_id:
		user = stylo.safe_decode(stylo.cache().get(tmp_id + "_usr"))
		pwd = stylo.safe_decode(stylo.cache().get(tmp_id + "_pwd"))
	return (user, pwd)


def authenticate_for_2factor(user):
	"""Authenticate two factor for enabled user before login."""
	if stylo.form_dict.get("otp"):
		return
	otp_secret = get_otpsecret_for_(user)
	token = int(pyotp.TOTP(otp_secret).now())
	tmp_id = stylo.generate_hash(length=8)
	cache_2fa_data(user, token, otp_secret, tmp_id)
	verification_obj = get_verification_obj(user, token, otp_secret)
	# Save data in local
	stylo.local.response["verification"] = verification_obj
	stylo.local.response["tmp_id"] = tmp_id


def cache_2fa_data(user, token, otp_secret, tmp_id):
	"""Cache and set expiry for data."""
	pwd = stylo.form_dict.get("pwd")
	...
	if verification_method in ["SMS", "Email"]:
		expiry_time = stylo.flags.token_expiry or 300
		stylo.cache().set(tmp_id + "_token", token)
		stylo.cache().expire(tmp_id + "_token", expiry_time)
	else:
		expiry_time = stylo.flags.otp_expiry or 180
	for k, v in {"_usr": user, "_pwd": pwd, "_otp_secret": otp_secret}.items():
		stylo.cache().set(f"{tmp_id}{k}", v)
		stylo.cache().expire(f"{tmp_id}{k}", expiry_time)
...
def two_factor_is_enabled_for_(user):
	...
	role_doctype = stylo.qb.DocType("Role")
	no_of_users = stylo.db.count(
		role_doctype,
...
def get_verification_method():
	return stylo.db.get_single_value("System Settings", "two_factor_method")


def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	"""Confirm otp matches."""
	from stylo.auth import get_login_attempt_tracker

	if not otp:
		otp = stylo.form_dict.get("otp")
	if not otp:
	...
	if not tmp_id:
		tmp_id = stylo.form_dict.get("tmp_id")
	hotp_token = stylo.cache().get(tmp_id + "_token")
	otp_secret = stylo.cache().get(tmp_id + "_otp_secret")
	if not otp_secret:
	...
	if hotp_token:
		if hotp.verify(otp, int(hotp_token)):
			stylo.cache().delete(tmp_id + "_token")
...
def get_verification_obj(user, token, otp_secret):
	otp_issuer = stylo.db.get_single_value("System Settings", "otp_issuer_name")
	verification_method = get_verification_method()
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = stylo.db.get_value("User", user, ["phone", "mobile_no"], as_dict=1)
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		stylo.db.get_single_value("System Settings", "otp_issuer_name")
	)
	subject = stylo.render_template(subject_template, kwargs_dict)
	return subject
...
def get_email_body_for_2fa(kwargs_dict):
	...
	"""
	body = stylo.render_template(body_template, kwargs_dict)
	return body
...
def get_link_for_qrcode(user, totp_uri):
	...
	key = stylo.generate_hash(length=20)
	key_user = f"{key}_user"
	key_uri = f"{key}_uri"
	lifespan = int(stylo.db.get_single_value("System Settings", "lifespan_qrcode_image")) or 240
	stylo.cache().set_value(key_uri, totp_uri, expires_in_sec=lifespan)
	stylo.cache().set_value(key_user, user, expires_in_sec=lifespan)
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	try:
		from stylo.core.doctype.sms_settings.sms_settings import send_request
	except Exception:
		return False
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	user_email = stylo.db.get_value("User", user, "email")
	if not user_email:
		return False
	hotp = pyotp.HOTP(otp_secret)
...
def qrcode_as_png(user, totp_uri):
	...
	png_file_name = f"{stylo.generate_hash(length=20)}.png"
	_file = stylo.get_doc(
		{
			"doctype": "File",
	...
	file_path = os.path.join(stylo.get_site_path("public", "files"), _file.file_name)
	url = qrcreate(totp_uri)
...
def create_barcode_folder():
	...
	folder = stylo.db.exists("File", {"file_name": folder_name})
	if folder:
		return folder
	folder = stylo.get_doc({"doctype": "File", "file_name": folder_name, "is_folder": 1, "folder": "Home"})
...
def delete_qrimage(user, check_expiry=False):
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = stylo.get_doc("File", barcode.name)
		stylo.delete_doc("File", barcode.name, ignore_permissions=True)


def delete_all_barcodes_for_users():
	"""Task to delete all barcodes for user."""

	users = stylo.get_all("User", {"enabled": 1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, str):
		barcode = stylo.get_doc("File", barcode)
	lifespan = stylo.db.get_single_value("System Settings", "lifespan_qrcode_image") or 240
...
def disable():
	stylo.db.set_single_value("System Settings", "enable_two_factor_auth", 0)


@stylo.whitelist()
def reset_otp_secret(user: str):
	if stylo.session.user != user:
		stylo.only_for("System Manager", message=True)

	settings = stylo.get_cached_doc("System Settings")
	...
	otp_issuer = settings.otp_issuer_name or "Stylo Framework"
	...
	stylo.msgprint(_("OTP Secret has been reset. Re-registration will be required on next login."))
