...
def check_admin_or_system_manager(user=None):
	...
	warn(
		...
		'Please use stylo.only_for("System Manager") instead.',
	...
	if ("System Manager" not in stylo.get_roles(user)) and (user != "Administrator"):
...
def has_permission(
	...
	*,
	...
	:param raise_exception:
	        ...
	        If not False, will display a message using stylo.msgprint
	...
	if doc:
		if isinstance(doc, str):
			doc = stylo.get_doc(meta.name, doc)
		perm = get_doc_permissions(doc, user=user, ptype=ptype).get(ptype)
		if not perm:
			push_perm_check_log(_("User {0} does not have access to this document").format(stylo.bold(user)))
	else:
...
def get_role_permissions(doctype_meta, user=None, is_owner=None):
	...
	if not stylo.local.role_permissions.get(cache_key):
...
def has_user_permission(doc, user=None):
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = stylo.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None):
	...
	for method in reversed(methods):
		controller_permission = stylo.call(stylo.get_attr(method), doc=doc, ptype=ptype, user=user)
...
def set_user_permission_if_allowed(doctype, name, user, with_message=False):
	if get_role_permissions(stylo.get_meta(doctype), user).set_user_permissions != 1:
...
def remove_user_permission(doctype, name, user):
	...
	stylo.delete_doc("User Permission", user_permission_name)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	for d in user_permissions_for_doctype:
		stylo.delete_doc("User Permission", d.name)
...
def reset_perms(doctype):
	...
	stylo.db.delete("Custom DocPerm", {"parent": doctype})
...
def has_child_permission(
	...
	if child_doc:
		...
		if permlevel > 0 and permlevel not in parent_meta.get_permlevel_access(ptype, user=user):
			push_perm_check_log(
				_("Insufficient Permission Level for {0}").format(stylo.bold(parent_doctype))
			)
			return False
