...
def export_module_json(doc, is_standard, module):
	...
	if not stylo.flags.in_import and getattr(stylo.get_conf(), "developer_mode", 0) and is_standard:
...
def export_customizations(module, doctype, sync_on_migrate=0, with_permissions=0):
	...
	if not stylo.get_conf().developer_mode:
	...
	def sync(key, custom_doctype, doctype_fieldname):
		...
		for doc_type in doctypes:
			...
			if doc_type == doctype or not os.path.exists(
				os.path.join(folder, stylo.scrub(doc_type) + ".json")
			):
...
def get_module_path(module):
	...
	return stylo.get_module_path(module)
...
def get_doctype_module(doctype) -> str:
	...
	def make_modules_dict():
		return dict(stylo.db.sql("select name, module from tabDocType"))

	doctype_module_map = stylo.cache().get_value("doctype_modules", make_modules_dict)
...
def get_app_publisher(module: str) -> str:
	app = get_module_app(module)
	if not app:
		stylo.throw(_("App not found"))
	app_publisher = stylo.get_hooks(hook="app_publisher", app_name=app)[0]
	...
	def get_stats(args):
		...
		with open(target_file_path, "w") as target:
			with open(
				...
				target.write(
					stylo.as_unicode(
						stylo.utils.cstr(source.read()).format(
							app_publisher=app_publisher,
							year=stylo.utils.nowdate()[:4],
