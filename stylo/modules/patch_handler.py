...
def run_all(skip_failing: bool = False, patch_type: PatchType | None = None) -> None:
	...
	executed = set(stylo.get_all("Patch Log", filters={"skipped": 0}, fields="patch", pluck="patch"))
	...
	for patch in stylo.flags.final_patches:
...
def get_all_patches(patch_type: PatchType | None = None) -> list[str]:
	if patch_type and not isinstance(patch_type, PatchType):
		stylo.throw(f"Unsupported patch type specified: {patch_type}")
...
def get_patches_from_app(app: str, patch_type: PatchType | None = None) -> list[str]:
	...
	patches_txt = stylo.get_pymodule_path(app, "patches.txt")
	...
	try:
		...
		if patch_type.value in parser.sections():
			...
			stylo.throw(stylo._("Patch type {} not found in patches.txt").format(patch_type))
		...
		if not patch_type or patch_type == PatchType.pre_model_sync:
			return stylo.get_file_items(patches_txt)
...
def reload_doc(args):
	...
	run_single(method=stylo.modules.reload_doc, methodargs=args)
...
def execute_patch(patchmodule: str, method=None, methodargs=None):
	...
	print(
		f"Executing {patchmodule or methodargs} in {stylo.local.site} ({stylo.db.cur_db_name}){docstring}"
	)
	...
	try:
		if patchmodule:
			if patchmodule.startswith("finally:"):
				...
				stylo.flags.final_patches.append(patchmodule)
			else:
				if has_patch_file:
					_patch()
				else:
...
def update_patch_log(patchmodule, skipped=False):
	...
	patch = stylo.get_doc({"doctype": "Patch Log", "patch": patchmodule})
...
def executed(patchmodule):
	...
	return stylo.db.get_value("Patch Log", {"patch": patchmodule, "skipped": 0})


def _patch_mode(enable):
	"""stop/start execution till patch is run"""
	stylo.local.flags.in_patch = enable
	stylo.db.commit()


def check_session_stopped():
	"""This function is deprecated. Use maintenance_mode in site config instead."""
	if stylo.db.get_global("__session_status") == "stop":
		stylo.msgprint(stylo.db.get_global("__session_status_message"))
