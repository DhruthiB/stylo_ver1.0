import stylo
from stylo.utils.number_format import NumberFormat
...
def get_locale_value(key: str, language: str | None = None) -> str | None:
	...
	:param language: The language code to get the value for. Defaults to the current user's language.
	"""
	lang = language or stylo.local.lang
	if lang:
		value = stylo.client_cache.get_doc("Language", lang).get(key)

	return value or stylo.db.get_default(key)
