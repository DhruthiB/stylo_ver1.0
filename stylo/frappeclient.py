"""
StyloClient is a library that helps you connect with other stylo systems
"""
import base64
import json
...
class StyloClient:
	def __init__(
		...
		api_key=None,
		api_secret=None,
		stylo_authorization_source=None,
	):
		import requests
		...
		self.stylo_authorization_source = stylo_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.stylo_authorization_source:
				auth_source = {"Stylo-Authorization-Source": self.stylo_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": stylo.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return stylo._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "stylo.client.insert_many", "docs": stylo.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": stylo.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return stylo._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "stylo.client.bulk_update", "docs": stylo.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "stylo.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "stylo.client.submit", "doc": stylo.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "stylo.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": stylo.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "stylo.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not stylo.db.exists("User", doc.get("owner")):
				stylo.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = stylo.db.get_single_value(doctype, "modified")
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "StyloClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise StyloException(exc)
		if "message" in rjson:
...
class StyloOAuth2Client(StyloClient):
