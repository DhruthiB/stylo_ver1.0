...
stylo.provide("website");
stylo.provide("stylo.awesome_bar_path");
window.cur_frm = null;
...
$.extend(stylo, {
	...
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (stylo._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				stylo._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					stylo.msgprint(__("Not found"));
				},
				403: function () {
					stylo.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			stylo.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				stylo.msgprint(server_messages);
			}
		}
	...
	send_message: function (opts, btn) {
		return stylo.call({
			type: "POST",
			method: "stylo.www.contact.send_message",
			btn: btn,
			args: opts,
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return stylo.call({
			type: "GET",
			method: "stylo.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (stylo.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", stylo.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				stylo.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				stylo.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				stylo.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	...
	unfreeze: function () {
		if (!stylo.freeze_count) return; // anything open?
		stylo.freeze_count--;
		if (!stylo.freeze_count) {
	...
	trigger_ready: function () {
		stylo.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = stylo.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		stylo.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				stylo.do_search(val);
	...
	do_search: function (val) {
		var path =
			(stylo.awesome_bar_path && stylo.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		stylo.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return stylo.get_cookie("user_id") !== "Guest" && stylo.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (stylo.session.user === "Guest" && window.show_language_picker) {
			stylo
				.call("stylo.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = stylo.get_cookie("preferred_language");
					let language_codes = [];
...
stylo.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		stylo.utils.debounce(() => {
			...
			stylo
				.call({
					method: "stylo.search.web_search",
					args: {
...
window.valid_email = function (id) {
	...
	// copied regex from stylo/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = stylo.get_cookie("full_name");
	var logged_in = stylo.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	stylo.bind_navbar_search();

	// switch to app link
	if (stylo.get_cookie("system_user") === "yes" && logged_in) {
		stylo.add_switch_to_desk();
	}

	stylo.render_user();
	stylo.setup_lazy_images();
...
$(document).on("page-change", function () {
	...
	$.extend(stylo, stylo.get_cookies());
	stylo.session = { user: stylo.user_id };

	stylo.datetime.refresh_when();
	stylo.trigger_ready();
	stylo.bind_filters();
	stylo.highlight_code_blocks();
	stylo.add_link_to_headings();
	stylo.make_navbar_active();
	// scroll to hash
...
stylo.ready(function () {
	stylo.show_language_picker();
	stylo.setup_videos();
	stylo.socketio.init(window.socketio_port);
});
