...
from stylo.utils import random_string, set_request
from stylo.website.doctype.blog_post.blog_post import get_blog_list
...
class TestBlogPost(StyloTestCase):
	def setUp(self):
	...
	def test_generator_not_found(self):
		pages = stylo.get_all("Blog Post", fields=["name", "route"], filters={"published": 0}, limit=1)

		route = f"test-route-{stylo.generate_hash(length=5)}"

		stylo.db.set_value("Blog Post", pages[0].name, "route", route)
	...
	def test_category_link(self):
		...
		blog_page_html = stylo.safe_decode(blog_page_response.get_data())
		...
		cached_value = stylo.db.value_cache.get(("DocType", "Blog Post", "name"))
		stylo.db.value_cache[("DocType", "Blog Post", "name")] = (("Blog Post",),)
		...
		category_page_html = stylo.safe_decode(category_page_response.get_data())
		# Category page should contain the blog post title
		self.assertIn(blog.title, category_page_html)

		# Cleanup
		stylo.db.value_cache[("DocType", "Blog Post", "name")] = cached_value
		stylo.delete_doc("Blog Post", blog.name)
		stylo.delete_doc("Blog Category", blog.blog_category)
	...
	def test_blog_pagination(self):
		...
		filters = stylo._dict({"blog_category": scrub(category_title)})
		...
		for blog in blogs:
			stylo.delete_doc(blog.doctype, blog.name)
		stylo.delete_doc("Blog Category", blogs[0].blog_category)

	def test_caching(self):
		# to enable caching
		stylo.flags.force_website_cache = True
		print(stylo.session.user)
	...
	def test_like_dislike(self):
		...
		stylo.db.delete("Comment", {"comment_type": "Like", "reference_doctype": "Blog Post"})

		from stylo.templates.includes.likes.likes import like

		stylo.form_dict.reference_doctype = "Blog Post"
		stylo.form_dict.reference_name = test_blog.name
		stylo.form_dict.like = True
		stylo.local.request_ip = "127.0.0.1"

		liked = like()
		self.assertEqual(liked, True)

		stylo.form_dict.like = False
...
def make_test_blog(category_title="Test Blog Category"):
	...
	if not stylo.db.exists("Blog Category", category_name):
		stylo.get_doc(dict(doctype="Blog Category", title=category_title)).insert()
	if not stylo.db.exists("Blogger", "test-blogger"):
		stylo.get_doc(dict(doctype="Blogger", short_name="test-blogger", full_name="Test Blogger")).insert()

	test_blog = stylo.get_doc(
		dict(
			doctype="Blog Post",
