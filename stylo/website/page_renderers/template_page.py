...
from stylo.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def set_template_path(self):
		...
		for app in stylo.get_installed_apps(stylo_last=True):
	...
	def get_index_path_options(search_path):
		return (
			stylo.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = stylo.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = stylo.get_module(self.pymodule_name)
		...
		self.context.developer_mode = stylo.conf.developer_mode
	...
	def render_template(self):
		if self.template_path.endswith("min.js"):
			...
			html = stylo.render_template(self.source, self.context, safe_render=safe_render)
	...
	def get_raw_template(self):
		return stylo.get_jloader().get_source(stylo.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = stylo.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "stylo"
		self.app_path = stylo.get_app_path("stylo")
		self.path = path
	...
	def set_user_info(self):
		from stylo.utils.user import get_fullname_and_avatar

		info = get_fullname_and_avatar(stylo.session.user)
