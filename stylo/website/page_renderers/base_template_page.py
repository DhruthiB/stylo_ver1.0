...
from stylo.website.website_components.metatags import MetaTags
...
class BaseTemplatePage(BaseRenderer):
	...
	def init_context(self):
		self.context = stylo._dict()
		self.context.update(get_website_settings())
		self.context.update(stylo.local.conf.get("website_context") or {})

	def add_csrf_token(self, html):
		if stylo.local.session and getattr(stylo.local.session, "data", None):
			csrf_token = stylo.local.session.data.csrf_token
			return html.replace(
				"<!-- csrf_token -->", f'<script>stylo.csrf_token = "{csrf_token}";</script>'
			)
	...
	def set_base_template_if_missing(self):
		if not self.context.base_template_path:
			app_base = stylo.get_hooks("base_template")
	...
	def set_missing_values(self):
		# set using stylo.respond_as_web_page
		if hasattr(stylo.local, "response") and stylo.local.response.get("context"):
			self.context.update(stylo.local.response.context)

		# to be able to inspect the context dict
		# Use the macro "inspect" from macros.html
		self.context.canonical = stylo.utils.get_url(stylo.utils.escape_html(self.path))
		...
		self.context.pathname = getattr(stylo.local, "path", None) if hasattr(stylo, "local") else self.path

	def update_website_context(self):
		# apply context from hooks
		update_website_context = stylo.get_hooks("update_website_context")
		for method in update_website_context:
			values = stylo.get_attr(method)(self.context)
			if values:
