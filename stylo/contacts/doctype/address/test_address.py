...
import stylo
from stylo.contacts.doctype.address.address import address_query, get_address_display
from stylo.tests.utils import StyloTestCase


class TestAddress(StyloTestCase):
	def test_template_works(self):
		if not stylo.db.exists("Address Template", "India"):
			stylo.get_doc({"doctype": "Address Template", "country": "India", "is_default": 1}).insert()

		if not stylo.db.exists("Address", "_Test Address-Office"):
			stylo.get_doc(
				{
		...
		address = stylo.get_list("Address")[0].name
		display = get_address_display(stylo.get_doc("Address", address).as_dict())
