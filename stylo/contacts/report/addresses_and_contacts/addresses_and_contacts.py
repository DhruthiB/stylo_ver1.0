# Copyright (c) 2013, Stylo Technologies Pvt. Ltd. and contributors
...
def get_reference_addresses_and_contact(reference_doctype, reference_name):
	...
	reference_list = [
		d[0] for d in stylo.get_list(reference_doctype, filters=filters, fields=["name"], as_list=True)
	]

	for d in reference_list:
		reference_details.setdefault(d, stylo._dict())
...
def get_reference_details(reference_doctype, doctype, reference_list, reference_details):
	...
	records = stylo.get_list(doctype, filters=filters, fields=fields, as_list=True)
	temp_records = list()
	...
	if not reference_list:
		stylo.throw(_("No records present in {0}").format(reference_doctype))

	reference_details[reference_list[0]][stylo.scrub(doctype)] = temp_records
