...
def get_context(context):
	...
	for token in active_tokens:
		...
		app = {
			...
			"app_name": stylo.db.get_value("OAuth Client", token.get("client"), "app_name"),
			"creation": creation,
		}
		client_apps.append(app)

	app = None
	if "app" in stylo.form_dict:
		app = stylo.get_doc("OAuth Client", stylo.form_dict.app)
		app = app.__dict__
...
def get_first_login(client):
	login_date = stylo.get_all(
		"OAuth Bearer Token",
		filters=[["user", "=", stylo.session.user], ["client", "=", client]],
		fields=["creation"],
		order_by="creation",
		limit=1,
	)
...
def delete_client(client_id):
	active_client_id_tokens = stylo.get_all(
		"OAuth Bearer Token", filters=[["user", "=", stylo.session.user], ["client", "=", client_id]]
	)
	for token in active_client_id_tokens:
		stylo.delete_doc("OAuth Bearer Token", token.get("name"), ignore_permissions=True)
