...
import stylo
from stylo import _
from stylo.model.document import Document, get_controller
from stylo.utils import cint, quoted
from stylo.website.path_resolver import resolve_path
...
def get_context(context, **dict_params):
	...
	stylo.local.form_dict.update(dict_params)
	doctype = stylo.local.form_dict.doctype
	context.parents = [{"route": "me", "title": _("My Account")}]
	context.meta = stylo.get_meta(doctype)
	context.update(get_list_context(context, doctype) or {})
	context.doctype = doctype
	context.txt = stylo.local.form_dict.txt
	context.update(get(**stylo.local.form_dict))
...
def get(
	...
	**kwargs,
	...
	list_context = stylo.flags.list_context
	...
	for doc in raw_result:
		doc.doctype = doctype
		new_context = stylo._dict(doc=doc, meta=meta, list_view_fields=list_view_fields)

		if not list_context.get_list and not isinstance(new_context.doc, Document):
			new_context.doc = stylo.get_doc(doc.doctype, doc.name)
			new_context.update(new_context.doc.as_dict())

		if not stylo.flags.in_test:
			pathname = pathname or stylo.local.request.path
			new_context["pathname"] = pathname.strip("/ ")
		new_context.update(list_context)
		set_route(new_context)
		rendered_row = stylo.render_template(row_template, new_context, is_path=True)
...
def get_list_data(
	...
	**kwargs,
	...
	if stylo.is_table(doctype):
		stylo.throw(_("Child DocTypes are not allowed"), title=_("Invalid DocType"))

	if not txt and stylo.form_dict.search:
		txt = stylo.form_dict.search
		del stylo.form_dict["search"]

	controller = get_controller(doctype)
	meta = stylo.get_meta(doctype)

	filters = prepare_filters(doctype, controller, kwargs)
	list_context = get_list_context(stylo._dict(), doctype, web_form_name)
	...
	stylo.flags.list_context = list_context
...
def prepare_filters(doctype, controller, kwargs):
	...
	if filters.pathname:
		...
		for key, val in stylo.local.form_dict.items():
			if key not in filters and key != "flags":
				filters[key] = val

	# filter the filters to include valid fields only
	from stylo.model.meta import DEFAULT_FIELD_LABELS
...
def get_list_context(context, doctype, web_form_name=None):
	...
	from stylo.website.doctype.web_form.web_form import get_web_form_module

	list_context = context or stylo._dict()
	meta = stylo.get_meta(doctype)
	...
	def update_context_from_module(module, list_context):
		...
		if hasattr(module, "get_list_context"):
			out = stylo._dict(module.get_list_context(list_context) or {})
			if out:
				list_context = out
		return list_context
	...
	if meta.custom and web_form_name:
		webform_list_contexts = stylo.get_hooks("webform_list_context")
		if webform_list_contexts and not stylo.get_doc("Module Def", meta.module).custom:
			out = stylo._dict(stylo.get_attr(webform_list_contexts[0])(meta.module) or {})
			if out:
				list_context = out
