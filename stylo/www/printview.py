...
import stylo
from stylo import _, cstr, get_module_path
from stylo.core.doctype.access_log.access_log import make_access_log
from stylo.core.doctype.document_share_key.document_share_key import is_expired
from stylo.utils import cint, escape_html, strip_html
...
def get_context(context):
	...
	if not ((stylo.form_dict.doctype and stylo.form_dict.name) or stylo.form_dict.doc):
		return {
			"body": f"""
				<h1>Error</h1>
				<p>Parameters doctype and name required</p>
				<pre>{escape_html(stylo.as_json(stylo.form_dict, indent=2))}</pre>
				"""
		}

	if stylo.form_dict.doc:
		doc = stylo.form_dict.doc
	else:
		doc = stylo.get_doc(stylo.form_dict.doctype, stylo.form_dict.name)

	set_link_titles(doc)

	settings = stylo.parse_json(stylo.form_dict.settings)

	letterhead = stylo.form_dict.letterhead or None
	...
	make_access_log(
		doctype=stylo.form_dict.doctype, document=stylo.form_dict.name, file_type="PDF", method="Print"
	)

	if print_format and print_format.get("print_format_builder_beta"):
		from stylo.utils.weasyprint import get_html

		body = get_html(
			doctype=stylo.form_dict.doctype, name=stylo.form_dict.name, print_format=print_format.name
		)
		...
		body = get_rendered_template(
			...
			meta=meta,
			trigger_print=stylo.form_dict.trigger_print,
			no_letterhead=stylo.form_dict.no_letterhead,
	...
	return {
		"body": body,
		"print_style": get_print_style(stylo.form_dict.style, print_format),
		"comment": stylo.session.user,
		"title": stylo.utils.strip_html(cstr(doc.get_title() or doc.name)),
		"lang": stylo.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"doctype": stylo.form_dict.doctype,
		"name": stylo.form_dict.name,
		"key": stylo.form_dict.get("key"),
...
def get_print_format_doc(print_format_name, meta):
	...
	if not print_format_name:
		print_format_name = stylo.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return stylo.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	if not stylo.flags.ignore_print_permissions:
		validate_print_permission(doc)

	print_settings = stylo.get_single("Print Settings").as_dict()
	...
	if doc.meta.is_submittable:
		if doc.docstatus.is_draft() and not cint(print_settings.allow_print_for_draft):
			stylo.throw(_("Not allowed to print draft documents"), stylo.PermissionError)

		if doc.docstatus.is_cancelled() and not cint(print_settings.allow_print_for_cancelled):
			stylo.throw(_("Not allowed to print cancelled documents"), stylo.PermissionError)
	...
	letter_head = stylo._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = stylo.utils.jinja.render_template(letter_head.content, {"doc": doc.as_dict()})
	...
	if letter_head.footer:
		letter_head.footer = stylo.utils.jinja.render_template(letter_head.footer, {"doc": doc.as_dict()})
...
def set_title_values_for_link_and_dynamic_link_fields(meta, doc, parent_doc=None):
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = stylo.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta, doc):
	for field in meta.get_table_fields():
		...
		_meta = stylo.get_meta(field.options)
	...
	for field in meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, stylo.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(doc, str) and isinstance(name, str):
		doc = stylo.get_doc(doc, name)

	if isinstance(doc, str):
		doc = stylo.get_doc(json.loads(doc))

	print_format = get_print_format_doc(print_format, meta=meta or stylo.get_meta(doc.doctype))
	set_link_titles(doc)
	...
	try:
		html = get_rendered_template(
			...
			settings=stylo.parse_json(settings),
		)
	except stylo.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc, name=None, print_format=None, meta=None, lang=None):
	...
	if not print_format or (print_format and not print_format.raw_printing):
		stylo.throw(
			_("{0} is not a raw printing format.").format(print_format), stylo.TemplateNotFoundError
		)
...
def validate_print_permission(doc):
	for ptype in ("read", "print"):
		if stylo.has_permission(doc.doctype, ptype, doc):
			return

	if stylo.has_website_permission(doc):
		return

	if (key := stylo.form_dict.key) and isinstance(key, str) and validate_key(key, doc) is not False:
		return
...
def validate_key(key, doc):
	document_key_expiry = stylo.get_cached_value(
		"Document Share Key",
	...
	if document_key_expiry is not None:
		if is_expired(document_key_expiry[0]):
			raise stylo.exceptions.LinkExpired
		else:
			return

	# TODO: Deprecate this! kept it for backward compatibility
	if stylo.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return
...
def get_print_format(doctype, print_format):
	if print_format.disabled:
		stylo.throw(_("Print Format {0} is disabled").format(print_format.name), stylo.DoesNotExistError)

	# server, find template
	module = print_format.module or stylo.db.get_value("DocType", doctype, "module")
	...
	if not is_custom_module:
		path = os.path.join(
			...
			stylo.scrub(print_format.name) + ".html",
		)
	...
	stylo.throw(_("No template found at path: {0}").format(path), stylo.TemplateNotFoundError)
...
def make_layout(doc, meta, format_data=None):
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/stylo/images/signature-placeholder.png"
...
def get_print_style(style=None, print_format=None, for_legacy=False):
	...
	css = stylo.get_template("templates/styles/standard.css").render(context)

	if style and stylo.db.exists("Print Style", style):
		css = css + "\n" + stylo.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data, table_meta, df):
	...
	columns = []
	doc = data[0] or stylo.new_doc(df.options)
