...
from stylo.utils import escape_html, validate_email_address

sitemap = 1


def get_context(context):
	doc = stylo.get_doc("Contact Us Settings", "Contact Us Settings")
...
def send_message(sender, message, subject="Website Query"):
	...
	with suppress(stylo.OutgoingEmailError):
		if forward_to_email := stylo.db.get_single_value("Contact Us Settings", "forward_to_email"):
			stylo.sendmail(recipients=forward_to_email, reply_to=sender, content=message, subject=subject)
...
{0}"""
	...
	system_language = stylo.db.get_single_value("System Settings", "language")
	# add to to-do ?
	stylo.get_doc(
		dict(
