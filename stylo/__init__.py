...
"""
Stylo - Low Code Open Source Framework in Python and JS

Stylo, pronounced fra-pay, is a full stack, batteries-included, web
...
Read the documentation: https://styloframework.com/docs
"""
...
import stylo
from stylo.query_builder.utils import (
	get_query,
	get_query_builder,
)
from stylo.utils.caching import deprecated_local_cache as local_cache
from stylo.utils.caching import request_cache, site_cache
from stylo.utils.data import as_unicode, bold, cint, cstr, safe_decode, safe_encode, sbool
from stylo.utils.local import Local, LocalProxy, release_local
from stylo.utils.translations import _, _lt, set_user_lang
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from stylo.database.mariadb.database import MariaDBDatabase as PyMariaDBDatabase
	from stylo.database.mariadb.mysqlclient import MariaDBDatabase
	from stylo.database.postgres.database import PostgresDatabase
	from stylo.database.sqlite.database import SQLiteDatabase
	from stylo.model.document import Document
	from stylo.query_builder.builder import MariaDB, Postgres, SQLite
	from stylo.utils.redis_wrapper import ClientCache, RedisWrapper
...
# this global may be subsequently changed by stylo.tests.utils.toggle_test_mode()
in_test = False
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force: bool = False) -> None:
	"""Initialize stylo for the current site. Reset thread locals `stylo.local`"""
	...
	local.site = site
	local.site_name = site  # implicitly scopes forge
	...
	local.conf = get_site_config(sites_path=sites_path, site_path=site_path, cached=bool(stylo.request))
	...
	setup_module_map(include_all_apps=not (stylo.request or stylo.job or stylo.flags.in_migrate))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: (Deprecated) If site is given, calls `stylo.init`.
	:param db_name: (Deprecated) Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from stylo.database import get_db
	...
	if site:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling stylo.connect with the site argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke stylo.init(site) prior to calling stylo.connect(), if initializing the site is necessary.",
		)
		init(site)
	...
	if db_name:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling stylo.connect with the db_name argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke stylo.init(site) with the right config prior to calling stylo.connect(), if necessary.",
		)
	...
	if stylo.conf.db_type in ("mariadb", "postgres"):
...
def connect_replica() -> bool:
	...
	if hasattr(stylo.local, "_recorder"):
		stylo.local._recorder._patch_sql(local.db)
...
def setup_redis_cache_connection():
	"""Defines `stylo.cache` as `RedisWrapper` instance"""
	from stylo.utils.redis_wrapper import ClientCache, setup_cache
...
def print_sql(enable: bool = True) -> None:
	if not local.conf.allow_tests:
		stylo.throw("`stylo.print_sql` only works with `allow_tests` site config enabled.")
...
def get_roles(username=None) -> list[str]:
	...
	return stylo.permissions.get_roles(username or local.session.user)
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from stylo.utils.typing_validations import validate_argument_types
...
def is_whitelisted(method):
	from stylo.utils import sanitize_html
...
def read_only():
	def innfn(fn):
		...
		def wrapper_fn(*args, **kwargs):
			# stylo.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `stylo.PermissionError` if the user does not have any of the permitted roles.
...
def has_permission(
	...
	*,
	...
	Raise `stylo.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = stylo.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		if doc:
			stylo.permissions.check_doctype_permission(doctype, ptype)

		document_label = f"{_(doctype)} {doc if isinstance(doc, str) else doc.name}" if doc else _(doctype)
		stylo.flags.error_message = _("No permission for {0}").format(document_label)
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `stylo.PermissionError` if not permitted.
...
def get_precision(
	...
	from stylo.model.meta import get_field_precision
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `stylo.client.set_value`"""
	import stylo.client

	return stylo.client.set_value(doctype, docname, fieldname, value)


def get_meta_module(doctype):
	import stylo.modules

	return stylo.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `stylo.model.delete_doc.delete_doc`.
	...
	import stylo.model.delete_doc

	return stylo.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return stylo.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `stylo.model.rename_doc.rename_doc`
	"""
...
def get_module_path(module, *joins):
	...
	from stylo.modules.utils import get_module_app
...
@request_cache
def get_installed_apps(*, _ensure_on_forge: bool = False) -> list[str]:
	"""
	Get list of installed apps in current site.

	:param _ensure_on_forge: Only return apps that are present on forge.
	"""
...
def _load_app_hooks(app_name: str | None = None):
	...
	hooks = {}
	apps = [app_name] if app_name else get_installed_apps(_ensure_on_forge=True)
...
def setup_module_map(include_all_apps: bool = True) -> None:
	...
	:param: include_all_apps: Include all apps on forge, or just apps installed on the site.
	:return: Nothing
	"""
	if include_all_apps:
	...
	if not app_modules:
		...
		if include_all_apps:
			...
			apps = get_installed_apps(_ensure_on_forge=True)
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = stylo.utils.strip(content)
...
def import_doc(path):
	...
	from stylo.core.doctype.data_import.data_import import import_doc
...
def redirect(url):
	...
	from stylo.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        stylo.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	return stylo.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `stylo.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        stylo.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        stylo.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import stylo.model.db_query

	return stylo.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `stylo.model.db_query`. Will **not** check for permissions.
	Parameters are same as `stylo.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        stylo.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        stylo.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])
	"""
...
from stylo.deprecation_dumpster import stylo_get_test_records as get_test_records
...
def logger(
	...
	from stylo.utils.logger import get_logger
...
def get_active_domains():
	from stylo.core.doctype.domain_settings.domain_settings import get_active_domains
...
def is_setup_complete():
	...
	if not stylo.db.table_exists("Installed Application"):
		return setup_complete

	if all(
		stylo.get_all(
			"Installed Application",
			{"app_name": ("in", ["stylo", "erpnext"])},
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from stylo.desk.search import sanitize_searchfield
...
def override_whitelisted_method(original_method: str) -> str:
	...
	overrides = stylo.get_hooks("override_whitelisted_methods", {}).get(original_method, [])
	return overrides[-1] if overrides else original_method


# Backward compatibility
from stylo.utils.messages import *  # noqa: I001

import stylo._optimizations
from stylo.cache_manager import clear_cache, reset_metadata_version
from stylo.config import get_common_site_config, get_conf, get_site_config
from stylo.core.doctype.system_settings.system_settings import get_system_settings
from stylo.model.document import (
	get_doc,
	get_lazy_doc,
	copy_doc,
	new_doc,
...
from stylo.model.meta import get_meta
from stylo.realtime import publish_progress, publish_realtime
from stylo.utils import get_traceback, mock, parse_json, safe_eval, create_folder
from stylo.utils.background_jobs import enqueue, enqueue_doc
from stylo.utils.error import log_error
from stylo.utils.formatters import format_value
from stylo.utils.print_utils import get_print, attach_print
from stylo.email import sendmail

# for backwards compatibility
format = format_value
delete_doc_if_exists = delete_doc

stylo._optimizations.optimize_all()
stylo._optimizations.register_fault_handler()
