...
from stylo.query_builder import (
	get_query,
	get_query_builder,
...
from stylo.utils.data import cint, cstr, sbool
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from stylo.translate import get_all_translations
...
def get_lang_dict(fortype: str, name: str | None = None) -> dict[str, str]:
	...
	from stylo.translate import get_dict

	return get_dict(fortype, name)


def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `stylo.local.lang`"""
	from stylo.translate import get_user_lang
...
if TYPE_CHECKING:
	from stylo.database.mariadb.database import MariaDBDatabase
	...
	from stylo.utils.redis_wrapper import RedisWrapper
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: If site is given, calls `stylo.init`.
...
def cache() -> "RedisWrapper":
	...
	global redis_server
	if not redis_server:
		from stylo.utils.redis_wrapper import setup_cache
...
def get_traceback(with_context: bool = False) -> str:
	...
	from stylo.utils import get_traceback
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `stylo.ValidationError`
	:param title: [optional] Message title. Default: "Message".
	:param is_minimizable: [optional] Allow users to minimize the modal
	:param wide: [optional] Show wide modal
	:param as_list: [optional] If `msg` is a list, render as un-ordered list.
...
def sendmail(
	...
	if as_markdown:
		from stylo.utils import md_to_html
...
def clear_cache(user: str | None = None, doctype: str | None = None):
	...
	import stylo.utils.caching

	if doctype:
		stylo.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		stylo.cache_manager.clear_user_cache(user)
	else:  # everything
		from stylo import translate

		stylo.cache_manager.clear_user_cache()
		stylo.cache_manager.clear_domain_cache()
	...
	stylo.utils.caching._SITE_CACHE.clear()
	local.role_permissions = {}
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	permissions = stylo.permissions.get_role_permissions(doctype, user=user)
...
def has_permission(
	...
	*,
	...
	Raises `stylo.PermissionError` if user isn't permitted and `throw` is truthy
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `stylo.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = stylo.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = stylo.get_doc("ToDo", "TD0001")

	"""
	import stylo.model.document

	doc = stylo.model.document.get_doc(*args, **kwargs)
...
def get_meta(doctype, cached=True):
	"""Get `stylo.model.meta.Meta` instance of given doctype name."""
	import stylo.model.meta

	return stylo.model.meta.get_meta(doctype, cached=cached)
...
@request_cache
def get_installed_apps(sort=False, stylo_last=False, *, _ensure_on_forge=False):
	"""
	Get list of installed apps in current site.

	:param sort: [DEPRECATED] Sort installed apps based on the sequence in sites/apps.txt
	:param stylo_last: [DEPRECATED] Keep stylo last. Do not use this, reverse the app list instead.
	:param ensure_on_forge: Only return apps that are present on forge.
	"""
	...
	if stylo_last:
		deprecation_warning("`stylo_last` argument is deprecated and will be removed in v15.")
		if "stylo" in installed:
			installed.remove("stylo")
		installed.append("stylo")
...
def compare(val1, condition, val2):
	"""Compare two values using `stylo.utils.compare`
	...
	return stylo.utils.compare(val1, condition, val2)
...
def get_list(doctype, *args, **kwargs):
	...
	Example usage:
	        ...
	        stylo.get_list("ToDo", fields="*", filters = {"description": ("like", "test%")})
	"""
...
def get_all(doctype, *args, **kwargs):
	...
	Example usage:
	        ...
	        # filter as a list of dicts
	        stylo.get_all("ToDo", fields=["*"], filters = {"description": ("like", "test%")})
	"""
...
def get_test_records(doctype):
	...
	from stylo.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import stylo.utils.formatters

	return stylo.utils.formatters.format_value(*args, **kwargs)
...
def attach_print(
	...
	from stylo.utils import scrub_urls
	from stylo.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import stylo.realtime

	return stylo.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return stylo.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: stylo.local.cache[namespace]
	:param key: stylo.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import stylo.utils.background_jobs

	return stylo.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return stylo.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from stylo.utils.safe_exec import safe_eval
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> stylo.get_version("User", "foobar@gmail.com")
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from stylo.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from stylo.utils import parse_json
...
if _tune_gc:
	...
	# This number is quite small for stylo where a single query can potentially create 700+
	# objects easily.
