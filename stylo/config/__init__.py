import stylo
from stylo import _
from stylo.desk.moduleview import (
	config_exists,
	get_data,
...
def get_modules_from_all_apps_for_user(user=None):
	...
	global_blocked_modules = stylo.get_doc("User", "Administrator").get_blocked_modules()
	...
	for module in allowed_modules_list:
		...
		# Set defaults links
		module["links"] = get_onboard_items(module["app"], stylo.scrub(module_name))[:5]
...
def get_modules_from_app(app):
	return stylo.get_all("Module Def", filters={"app_name": app}, fields=["module_name", "app_name as app"])


def get_all_empty_tables_by_module():
	table_rows = stylo.qb.Field("table_rows")
	table_name = stylo.qb.Field("table_name")
	information_schema = stylo.qb.Schema("information_schema")

	empty_tables = (
		stylo.qb.from_(information_schema.tables).select(table_name).where(table_rows == 0)
	).run()

	empty_tables = {r[0] for r in empty_tables}

	results = stylo.get_all("DocType", fields=["name", "module"])
