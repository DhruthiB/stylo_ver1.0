...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(stylo.conf.db_type or "mariadb")
...
def get_query(*args, **kwargs) -> QueryBuilder:
	from stylo.database.query import Engine
...
def DocType(*args, **kwargs):
	return stylo.qb.DocType(*args, **kwargs)


def Table(*args, **kwargs):
	return stylo.qb.Table(*args, **kwargs)


def patch_query_execute():
	"""Patch the Query Builder with helper execute method
	This excludes the use of `stylo.db.sql` method while
	...
	def execute_query(query, *args, **kwargs):
		...
		result = stylo.db.sql(query, params, *args, **kwargs)  # nosemgrep
	...
	def prepare_query(query):
		...
		if stylo.flags.in_safe_exec:
			from stylo.utils.safe_exec import check_safe_sql_query
			...
			if not check_safe_sql_query(query, throw=False):
				...
				if len(callstack) >= 3 and ".py" in callstack[2].filename:
					...
					raise stylo.PermissionError("Only SELECT SQL allowed in scripting")
		return query, param_collector.get_parameters()

	builder_class = stylo.qb._BuilderClasss
	...
	stylo._qb_patched[stylo.conf.db_type] = True


def patch_query_aggregation():
	"""Patch aggregation functions to stylo.qb"""
	from stylo.query_builder.functions import _avg, _max, _min, _sum
