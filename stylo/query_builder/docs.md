...
Check out some examples [here](https://styloframework.com/docs/v14/user/en/api/query-builder)
...
```python
stylo.db.sql("select `name` from `tabUser`")
```
...
stylo.qb.from_("User").select(Field("name"))
...
```python
stylo.db.get_values("User", fieldname="name", filters={})
```

This module is used to support the 3rd way of query generation in stylo.
...
This module supports almost all the features present in db_query which powers `stylo.get_all` and `stylo.get_list`
...
1. Dict Query
   ...
   ```python
       stylo.db.get_values("ToDo", fieldname="name", filters={"description": "Something Random"})
   ```

   and many other possible caveats to the dict representation such as

   ```python
      stylo.db.get_values("User", fieldname="name",
      filters={"name": ("like", "admin%")})

      stylo.db.get_values("ToDo", fieldname="name", filters={"description": ("in", ["somso%", "someome"])})
   ```
...
2. Misc Query
   ...
   - To support this
     ...
     ```python
     stylo.db.get_values("ToDo", fieldname="name", filters=["description", "=", "someone"])
     ```
...
3. Criterion Query
   ...
   stylo.db.get_values("User", fieldname="name", filters=Field("name") == "Administrator")
...
### 2. Implementing the missing features which are present in `stylo.get_list` and `stylo.get_all` (do we even need so much magic?)
