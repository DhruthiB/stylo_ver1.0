...
class TestFullTextSearch(StyloTestCase):
	def setUp(self):
		index = get_index()
		index.build()
		self.index = index
	...
	def test_update_index(self):
		...
		# Update new doc
		self.index.update_index({"name": "sw/stylobooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/stylobooks")
...
def get_index():
	return TestWrapper("test_stylo_index")
...
def get_documents():
	...
	docs.append(
		{
			...
			Stylo Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
	...
	docs.append(
		{
			"name": "sw/stylo",
			"content": """Stylo Framework is a full-stack web framework, that includes everything you need to build and
