...
stylo.provide("stylo.customize_form");

stylo.ui.form.on("Customize Form", {
	setup: function (frm) {
		...
		frm.set_query("doc_type", function () {
			return {
				filters: [
					...
					["DocType", "restrict_to_domain", "in", stylo.boot.active_domains],
				],
			};
		});
	...
	refresh: function (frm) {
		...
		if (frm.doc.doc_type) {
			stylo.model.with_doctype(frm.doc.doc_type).then(() => {
				frm.page.set_title(__("Customize Form - {0}", [__(frm.doc.doc_type)]));
				stylo.customize_form.set_primary_action(frm);

				frm.add_custom_button(
					__("Go to {0} List", [__(frm.doc.doc_type)]),
					function () {
						stylo.set_route("List", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					__("Set Permissions"),
					function () {
						stylo.set_route("permission-manager", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					...
					function () {
						stylo.customize_form.confirm(__("Remove all customizations?"), frm);
					},
					__("Actions")
				);
				...
				frm.toggle_display(
					["queue_in_background"],
					stylo.get_meta(frm.doc.doc_type).is_submittable || 0
				);
	...
	set_default_doc_type(frm) {
		let doc_type;
		if (stylo.route_options && stylo.route_options.doc_type) {
			doc_type = stylo.route_options.doc_type;
				...
				return frm.call({
					...
					callback: function (r) {
						if (!r.exc) {
							...
							stylo.customize_form.clear_locals_and_refresh(frm);
						}
					},
				});
			}
	...
	async trim_table(frm) {
		let dropped_columns = await stylo.xcall(
			"stylo.custom.doctype.customize_form.customize_form.get_orphaned_columns",
			{ doctype: frm.doc.doc_type }
		);

		if (!dropped_columns?.length) {
			stylo.toast(__("This doctype has no orphan fields to trim"));
			return;
		}
		let msg = __(
			...
			frm.add_custom_button(
				...
				function () {
					stylo.prompt(
						...
						function (data) {
							stylo.call({
								method: "stylo.modules.utils.export_customizations",
								args: {
									doctype: frm.doc.doc_type,
									module: data.module,
	...
	setup_sort_order(frm) {
		...
		if (frm.doc.doc_type) {
			var fields = $.map(frm.doc.fields, function (df) {
				return stylo.model.is_value_type(df.fieldtype) ? df.fieldname : null;
			});
	...
	setup_default_views(frm) {
		stylo.model.set_default_views_for_doctype(frm.doc.doc_type, frm);
	},
});

// can't delete standard fields
stylo.ui.form.on("Customize Form Field", {
	before_fields_remove: function (frm, doctype, name) {
		const row = stylo.get_doc(doctype, name);
	...
	fields_add: function (frm, cdt, cdn) {
		var f = stylo.model.get_doc(cdt, cdn);
...
stylo.ui.form.on("DocType Link", {
	before_links_remove: function (frm, doctype, name) {
		let row = stylo.get_doc(doctype, name);
		...
		if (!(row.custom || row.__islocal)) {
			stylo.msgprint(__("Cannot delete standard link. You can hide it if you want"));
			throw "cannot delete standard link";
		}
	},
	links_add: function (frm, cdt, cdn) {
		let f = stylo.model.get_doc(cdt, cdn);
		f.custom = 1;
	},
...
stylo.ui.form.on("DocType Action", {
	before_actions_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			stylo.msgprint(__("Cannot delete standard action. You can hide it if you want"));
...
stylo.ui.form.on("DocType State", {
	before_states_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			stylo.msgprint(__("Cannot delete standard document state."));
...
stylo.customize_form.save_customization = function (frm) {
	if (frm.doc.doc_type) {
		return frm.call({
			doc: frm.doc,
			freeze: true,
...
stylo.customize_form.update_fields_from_form_builder = function (frm) {
	let form_builder = stylo.form_builder;
...
stylo.customize_form.set_primary_action = function (frm) {
...
stylo.customize_form.confirm = function (msg, frm) {
	...
	stylo.customize_form.confirm.dialog = d;
	d.show();
};

stylo.customize_form.clear_locals_and_refresh = function (frm) {
	delete frm.doc.__unsaved;
	// clear doctype from locals
	stylo.model.clear_doc("DocType", frm.doc.doc_type);
	delete stylo.meta.docfield_copy[frm.doc.doc_type];
	frm.refresh();
};

function render_form_builder(frm) {
	if (stylo.form_builder && stylo.form_builder.doctype === frm.doc.doc_type) {
		stylo.form_builder.setup_page_actions();
		stylo.form_builder.store.fetch();
		return;
	}

	if (stylo.form_builder) {
		stylo.form_builder.wrapper = $(frm.fields_dict["form_builder"].wrapper);
		stylo.form_builder.frm = frm;
		stylo.form_builder.doctype = frm.doc.doc_type;
		stylo.form_builder.customize = true;
		stylo.form_builder.init(true);
		stylo.form_builder.store.fetch();
	} else {
		stylo.require("form_builder.bundle.js").then(() => {
			stylo.form_builder = new stylo.ui.FormBuilder({
