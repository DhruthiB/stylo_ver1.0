...
from stylo.custom.doctype.property_setter.property_setter import delete_property_setter
from stylo.model import core_doctypes_list, no_value_fields
from stylo.model.docfield import supports_translation
from stylo.model.document import Document
from stylo.model.meta import trim_table
...
class CustomizeForm(Document):
	...
	if TYPE_CHECKING:
		from stylo.core.doctype.doctype_action.doctype_action import DocTypeAction
		from stylo.core.doctype.doctype_link.doctype_link import DocTypeLink
		from stylo.core.doctype.doctype_state.doctype_state import DocTypeState
		from stylo.custom.doctype.customize_form_field.customize_form_field import CustomizeFormField
	...
	def on_update(self):
		stylo.db.delete("Singles", {"doctype": "Customize Form"})
		stylo.db.delete("Customize Form Field")
	...
	def fetch_to_customize(self):
		...
		meta = stylo.get_meta(self.doc_type, cached=False)
	...
	def validate_doctype(self, meta):
		...
		if self.doc_type in core_doctypes_list:
			stylo.throw(_("Core DocTypes cannot be customized."))

		if meta.issingle:
			stylo.throw(_("Single DocTypes cannot be customized."))

		if meta.custom:
			stylo.throw(_("Only standard DocTypes are allowed to be customized from Customize Form."))
	...
	def get_name_translation(self):
		...
		return stylo.get_value(
			"Translation",
			{"source_text": self.doc_type, "language": stylo.local.lang or "en"},
	...
	def set_name_translation(self):
		...
		if not self.label:
			if current:
				# clear translation
				stylo.delete_doc("Translation", current.name)
			return
			...
			stylo.get_doc(
				{
					...
					"language_code": stylo.local.lang or "en",
				}
			).insert()
			return

		if self.label != current.translated_text:
			stylo.db.set_value("Translation", current.name, "translated_text", self.label)
	...
	def save_customization(self):
		...
		if self.flags.update_db:
			try:
				stylo.db.updatedb(self.doc_type)
			except Exception as e:
				if stylo.db.is_db_table_size_limit(e):
					stylo.throw(
		...
		if not hasattr(self, "hide_success") or not self.hide_success:
			stylo.msgprint(_("{0} updated").format(_(self.doc_type)), alert=True)
		stylo.clear_cache(doctype=self.doc_type)
		self.fetch_to_customize()

		if self.flags.rebuild_doctype_for_global_search:
			stylo.enqueue(
				"stylo.utils.global_search.rebuild_for_doctype",
	...
	def allow_property_change(self, prop, meta_df, df):
		if prop == "fieldtype":
			...
			(
				stylo.db.get_value("DocField", {"parent": self.doc_type, "fieldname": df.fieldname}, "reqd")
				== 1
			)
			and (df.get(prop) == 0)
		):
			stylo.msgprint(_("Row {0}: Not allowed to disable Mandatory for standard fields").format(df.idx))
			return False
			...
			stylo.msgprint(_("You cannot unset 'Read Only' for field {0}").format(df.label))
			return False

		elif prop == "options" and df.get("fieldtype") not in ALLOWED_OPTIONS_CHANGE:
			stylo.msgprint(_("You can't set 'Options' for field {0}").format(df.label))
			return False

		elif prop == "translatable" and not supports_translation(df.get("fieldtype")):
			stylo.msgprint(_("You can't set 'Translatable' for field {0}").format(df.label))
			return False
	...
	def set_property_setters_for_actions_and_links(self, meta):
		...
		for doctype, fieldname, field_map in (
			...
			for i, d in enumerate(self.get(fieldname) or []):
				d.idx = i
				if stylo.db.exists(doctype, d.name) and not d.custom:
					# check property and apply property setter
					original = stylo.get_doc(doctype, d.name)
	...
	def clear_removed_items(self, doctype, items):
		...
		"""
		if items:
			stylo.db.delete(doctype, dict(parent=self.doc_type, custom=1, name=("not in", items)))
		else:
			stylo.db.delete(doctype, dict(parent=self.doc_type, custom=1))
	...
	def update_custom_fields(self):
		for i, df in enumerate(self.get("fields")):
			...
			if not stylo.db.exists("Custom Field", {"dt": self.doc_type, "fieldname": df.fieldname}):
	...
	def update_in_custom_field(self, df, i):
		...
		custom_field = stylo.get_doc("Custom Field", meta_df[0].name)
		changed = False
		for prop in docfield_properties:
	...
	def delete_custom_fields(self):
		...
		for fieldname in fields_to_remove:
			...
			if not is_standard_or_system_generated_field(df):
				stylo.delete_doc("Custom Field", df.name)
	...
	def get_existing_property_value(self, property_name, fieldname=None):
		...
		if fieldname:
			...
			if stylo.db.has_column("DocType", property_name):
				property_value = stylo.db.get_value("DocType", self.doc_type, property_name)
			else:
				property_value = None
	...
	def validate_fieldtype_change(self, df, old_value, new_value):
		...
		if allowed:
			old_value_length = cint(stylo.db.type_map.get(old_value)[1])
			new_value_length = cint(stylo.db.type_map.get(new_value)[1])
	...
	def validate_fieldtype_length(self):
		for field in self.check_length_for_fieldtypes:
			df = field.get("df")
			max_length = cint(stylo.db.type_map.get(df.fieldtype)[1])
			...
			label = df.label
			links_str = ", ".join(stylo.utils.get_link_to_form(self.doc_type, doc.name) for doc in docs)

			if docs:
				stylo.throw(
					_(
						"Value for field {0} is too long in {1}. Length should be lesser than {2} characters"
					).format(stylo.bold(label), links_str, stylo.bold(max_length)),
	...
	def reset_layout(self):
		...
		property_setters = stylo.get_all(
			"Property Setter",
		...
		stylo.db.delete("Property Setter", {"name": ("in", property_setters)})
		stylo.clear_cache(doctype=self.doc_type)
		self.fetch_to_customize()
...
def get_orphaned_columns(doctype: str):
	...
	stylo.db.begin(read_only=True)  # Avoid any potential bug from writing to db
	return trim_table(doctype, dry_run=True)


def reset_customization(doctype):
	setters = stylo.get_all(
		"Property Setter",
		filters={
			"doc_type": doctype,
	...
	for setter in setters:
		stylo.delete_doc("Property Setter", setter)
	...
	for field in custom_fields:
		stylo.delete_doc("Custom Field", field)
