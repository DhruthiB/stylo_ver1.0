...
stylo.ui.form.on("Client Script", {
		...
		if (frm.doc.view == "Form") {
			...
			if (!frm.is_new()) {
				frm.add_custom_button(__("Compare Versions"), () => {
					new stylo.ui.DiffView("Client Script", "script", frm.doc.name);
				});
			}
		}
	...
	dt(frm) {
		frm.toggle_display("view", !stylo.boot.single_types.includes(frm.doc.dt));
	...
	view(frm) {
		let has_form_boilerplate = frm.doc.script.includes("stylo.ui.form.on");
...
stylo.ui.form.on("Task", "validate", function(frm) {
...
stylo.ui.form.on("Sales Invoice",  {
    validate: function(frm) {
