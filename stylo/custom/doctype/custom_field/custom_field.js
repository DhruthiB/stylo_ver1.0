...
stylo.ui.form.on("Custom Field", {
	...
	dt: function (frm) {
		...
		return stylo.call({
			method: "stylo.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && stylo.utils.has_special_chars(frm.doc.label)) {
			...
			frm.add_custom_button(__("Rename Fieldname"), () => {
				stylo.prompt(
					...
					function (data) {
						stylo
							.xcall(
								"stylo.custom.doctype.custom_field.custom_field.rename_fieldname",
								{
...
stylo.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";
