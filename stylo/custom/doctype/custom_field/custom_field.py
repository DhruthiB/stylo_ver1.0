...
from stylo.model import core_doctypes_list
from stylo.model.docfield import supports_translation
from stylo.model.document import Document
from stylo.query_builder.functions import IfNull
from stylo.utils import cstr, random_string
...
class CustomField(Document):
		...
		if not self.fieldname:
			...
			if not label:
				if self.fieldtype in ["Section Break", "Column Break", "Tab Break"]:
					...
					stylo.throw(_("Label is mandatory"))
	...
	def validate(self):
		...
		from stylo.core.doctype.doctype.doctype import check_fieldname_conflicts
		...
		if not self.fieldname:
			stylo.throw(_("Fieldname not set for Custom Field"))
	...
	def on_trash(self):
		...
		doctype_layouts = stylo.get_all("DocType Layout", filters={"document_type": self.dt}, pluck="name")
	...
	def validate_insert_after(self, meta):
		if not meta.get_field(self.insert_after):
			stylo.throw(
				...
				stylo.DoesNotExistError,
			)

		if self.fieldname == self.insert_after:
			stylo.throw(_("Insert After cannot be set as {0}").format(meta.get_label(self.insert_after)))
...
def get_fields_label(doctype=None):
	...
	if doctype in core_doctypes_list:
		return stylo.msgprint(_("Custom Fields cannot be added to core DocTypes."))

	if meta.custom:
		return stylo.msgprint(_("Custom Fields can only be added to a standard DocType."))

	return [
		{"value": df.fieldname or "", "label": _(df.label, context=df.parent) if df.label else ""}
		for df in stylo.get_meta(doctype).get("fields")
	]


def create_custom_field_if_values_exist(doctype, df):
	df = stylo._dict(df)
	if df.fieldname in stylo.db.get_table_columns(doctype) and stylo.db.count(
...
def create_custom_field(doctype, df, ignore_validate=False, is_system_generated=True):
	...
	if not df.fieldname and df.label:
		df.fieldname = stylo.scrub(df.label)
	if not stylo.db.get_value("Custom Field", {"dt": doctype, "fieldname": df.fieldname}):
...
def create_custom_fields(custom_fields: dict, ignore_validate=False, update=True):
	...
	try:
		stylo.flags.in_create_custom_fields = True
		doctypes_to_update = set()

		if stylo.flags.in_setup_wizard:
			...
			for doctype in doctypes:
				...
				for df in fields:
					...
					if not field:
						...
						except stylo.exceptions.DuplicateEntryError:
							pass

					elif update:
						custom_field = stylo.get_doc({"doctype": "Custom Field", **field})
		...
		stylo.flags.in_create_custom_fields = False
...
def get_existing_custom_fields(custom_fields):
	...
	existing_fields = stylo.get_all("Custom Field", filters={"dt": ("in", doctypes_to_fetch)}, fields="*")
...
def rename_fieldname(custom_field: str, fieldname: str):
	...
	field: CustomField = stylo.get_doc("Custom Field", custom_field)
	...
	if field.is_system_generated:
		stylo.throw(_("System Generated Fields can not be renamed"))
	if stylo.db.has_column(parent_doctype, fieldname):
		stylo.throw(_("Can not rename as column {0} is already present on DocType.").format(fieldname))
	if old_fieldname == new_fieldname:
		stylo.msgprint(_("Old and new fieldnames are same."), alert=True)
		return

	if stylo.db.has_column(field.dt, old_fieldname):
		stylo.db.rename_column(parent_doctype, old_fieldname, new_fieldname)
	...
	stylo.msgprint(_("Custom field renamed to {0} successfully.").format(fieldname), alert=True)
	stylo.db.commit()
	stylo.clear_cache()
...
def _update_fieldname_references(field: CustomField, old_fieldname: str, new_fieldname: str) -> None:
	...
	if field.fieldtype == "Password":
		Auth = stylo.qb.Table("__Auth")
		stylo.qb.update(Auth).set(Auth.fieldname, new_fieldname).where(
