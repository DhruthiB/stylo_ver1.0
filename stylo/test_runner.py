...
from stylo.modules import get_module_name, load_doctype_module
...
def main(
	...
	if doctype_list_path:
		...
		with open(stylo.get_app_path(app, doctype_list_path)) as f:
			doctype = f.read().strip().splitlines()

	if ui_tests:
		print(
			"Selenium testing has been deprecated\nUse forge --site {site_name} run-ui-tests for Cypress tests"
		)
	...
	try:
		stylo.flags.print_messages = verbose
		...
		stylo.clear_cache()
		scheduler_disabled_by_user = stylo.utils.scheduler.is_scheduler_disabled()
		if not scheduler_disabled_by_user:
			stylo.utils.scheduler.disable_scheduler()

		if not stylo.flags.skip_before_tests:
			if verbose:
				print('Running "before_tests" hooks')
			for fn in stylo.get_hooks("before_tests", app_name=app):
				stylo.get_attr(fn)()
		...
		if not scheduler_disabled_by_user:
			stylo.utils.scheduler.enable_scheduler()
...
def run_tests_for_doctype(
	...
	for doctype in doctypes:
		module = stylo.db.get_value("DocType", doctype, "module")
		if not module:
		...
		if force:
			for name in stylo.db.sql_list("select name from `tab%s`" % doctype):
				stylo.delete_doc(doctype, name, force=True)
...
def _run_unittest(
	...
	stylo.flags.tests_verbose = verbose
...
def _add_test(app, path, filename, verbose, test_suite=None, ui_tests=False):
	...
	app_path = stylo.get_pymodule_path(app)
...
def make_test_records(doctype, verbose=0, force=False, commit=False):
	...
	if stylo.flags.skip_test_records:
		return
	...
	for options in get_dependencies(doctype):
		...
		if options not in stylo.local.test_objects:
			stylo.local.test_objects[options] = []
...
def make_test_records_for_doctype(doctype, verbose=0, force=False, commit=False):
	...
	if hasattr(test_module, "_make_test_records"):
		stylo.local.test_objects[doctype] += test_module._make_test_records(verbose)

	elif hasattr(test_module, "test_records"):
		if doctype in stylo.local.test_objects:
			stylo.local.test_objects[doctype] += make_test_objects(
		...
		test_records = stylo.get_test_records(doctype)
		if test_records:
			stylo.local.test_objects[doctype] += make_test_objects(
...
def make_test_objects(doctype, test_records=None, verbose=None, reset=False, commit=False):
	...
	for doc in test_records:
		...
		if stylo.db.exists(d.doctype, d.name) and not reset:
			stylo.db.rollback()
...
def add_to_test_record_log(doctype):
	...
	if doctype not in test_record_log:
		stylo.flags.test_record_log.append(doctype)
		with open(stylo.get_site_path(".test_log"), "w") as f:
			f.write("\n".join(filter(None, stylo.flags.test_record_log)))


def get_test_record_log():
	"""Return the list of doctypes for which test records have been created"""
	if "test_record_log" not in stylo.flags:
		if os.path.exists(stylo.get_site_path(".test_log")):
			with open(stylo.get_site_path(".test_log")) as f:
				stylo.flags.test_record_log = f.read().splitlines()
		else:
			stylo.flags.test_record_log = []

	return stylo.flags.test_record_log
