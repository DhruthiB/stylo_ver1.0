...
stylo.ui.form.on("Document Type Mapping", {
	...
	get_fields: function (frm) {
		let filtered_fields = [];
		stylo.model.with_doctype(frm.doc.local_doctype, () => {
			stylo.get_meta(frm.doc.local_doctype).fields.map((field) => {
				if (
					field.fieldname !== "remote_docname" &&
					field.fieldname !== "remote_site_name" &&
					stylo.model.is_value_type(field) &&
					!field.hidden
				) {
