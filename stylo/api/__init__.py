...
import stylo
from stylo import _
from stylo.pulse.app_heartbeat_event import capture_app_heartbeat
...
def handle(request: Request):
	...
	if stylo.get_system_settings("log_api_requests"):
		doc = stylo.get_doc(
			{
...
from stylo.api.v1 import url_rules as v1_rules
from stylo.api.v2 import url_rules as v2_rules
...
def get_api_version() -> ApiVersion | None:
	if not stylo.request:
		return

	if stylo.request.path.startswith(f"/api/{ApiVersion.V2.value}"):
