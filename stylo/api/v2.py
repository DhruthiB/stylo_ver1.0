...
import stylo
import stylo.client
from stylo import _, cint, cstr, get_newargs, is_whitelisted
from stylo.core.doctype.server_script.server_script_utils import get_server_script_map
from stylo.handler import is_valid_http_method, run_server_script, upload_file
...
def handle_rpc_call(method: str, doctype: str | None = None):
	from stylo.modules.utils import load_doctype_module
	...
	method = stylo.override_whitelisted_method(method)
	...
	try:
		...
		stylo.throw(_("Failed to get method {0} with {1}").format(method, e))
...
def document_list(doctype: str) -> list[dict[str, Any]]:
	...
	Query Parameters (accessible via stylo.form_dict):
	...
	Response:
		stylo.response["data"]: List of document records as dicts
		stylo.response["has_next_page"]: Indicates if more pages are available
	...
	Controller Customization:
		...
		Example:
			class Project(Document):
				@staticmethod
				def get_list(query):
					Project = stylo.qb.DocType("Project")
					if user_has_role("Project Owner"):
						query = query.where(Project.owner == stylo.session.user)
					else:
	...
	args = stylo.form_dict
	fields: list | None = stylo.parse_json(args.get("fields", None))
	filters: dict | None = stylo.parse_json(args.get("filters", None))
	...
	if hasattr(controller, "get_list"):
		try:
			...
			stylo.throw(_("Error in {0}.get_list: {1}").format(doctype, str(e)))

	data = query.run(as_dict=as_dict, debug=debug)
	stylo.response["has_next_page"] = len(data) > limit
	return data[:limit]


def count(doctype: str) -> int:
	from stylo.desk.reportview import get_count

	stylo.form_dict.doctype = doctype

	return get_count()


def create_doc(doctype: str):
	data = stylo.form_dict
	data.pop("doctype", None)

	doc = stylo.new_doc(doctype, **data)
...
def copy_doc(doctype: str, name: str, ignore_no_copy: bool = True):
	...
	copy = stylo.copy_doc(doc, ignore_no_copy=ignore_no_copy)
...
def delete_doc(doctype: str, name: str):
	stylo.client.delete_doc(doctype, name)
...
def execute_doc_method(doctype: str, name: str, method: str | None = None):
	...
	doc.check_permission(PERMISSION_MAP[stylo.request.method])
	result = doc.run_method(method, **stylo.form_dict)
	stylo.response.docs.append(doc.as_dict())
	return result
...
def run_doc_method(method: str, document: dict[str, Any] | str, kwargs=None):
	...
	if isinstance(document, str):
		document = stylo.parse_json(document)

	if kwargs is None:
		kwargs = {}

	doc = stylo.get_doc(document)
	...
	stylo.response.docs.append(doc)  # send modified document and result both.
	return response
...
url_rules = [
	...
	Rule("/method/ping", endpoint=stylo.ping),
	Rule("/method/upload_file", endpoint=upload_file),
	Rule("/method/<method>", endpoint=handle_rpc_call),
	Rule(
		"/method/run_doc_method",
		methods=["GET", "POST"],
		endpoint=lambda: stylo.call(run_doc_method, **stylo.form_dict),
	),
