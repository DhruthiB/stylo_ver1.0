...
def execute():
	if stylo.db.exists("DocType", "Desk Page"):
		if stylo.db.exists("DocType", "Workspace"):
			# this patch was not added initially, so this page might still exist
			stylo.delete_doc("DocType", "Desk Page")
		else:
	...
	stylo.reload_doc("desk", "doctype", "workspace", force=True)
	stylo.reload_doc("desk", "doctype", "workspace_link", force=True)
	stylo.reload_doc("desk", "doctype", "workspace_chart", force=True)
	stylo.reload_doc("desk", "doctype", "workspace_shortcut", force=True)
