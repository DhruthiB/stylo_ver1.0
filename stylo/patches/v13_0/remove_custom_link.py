...
def execute():
	...
	"""
	stylo.reload_doctype("DocType Link")
	if stylo.db.has_table("Custom Link"):
		for custom_link in stylo.get_all("Custom Link", ["name", "document_type"]):
			stylo.db.sql(
				"update `tabDocType Link` set custom=1, parent=%s where parent=%s",
				(custom_link.document_type, custom_link.name),
			)

		stylo.delete_doc("DocType", "Custom Link")
