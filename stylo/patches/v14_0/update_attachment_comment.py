...
def execute():
	...
	for name, content in stylo.get_all(
		...
		stylo.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in stylo.get_all(
		...
		if content and content.startswith("Removed "):
			stylo.db.set_value("Comment", name, "content", content[8:], update_modified=False)
