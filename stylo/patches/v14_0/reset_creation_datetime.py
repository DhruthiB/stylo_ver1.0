...
import stylo
from stylo.query_builder import DocType as _DocType


def execute():
	"""Resetting creation datetimes for DocTypes"""
	DocType = _DocType("DocType")
	doctype_jsons = glob.glob(os.path.join("..", "apps", "stylo", "stylo", "**", "doctype", "**", "*.json"))

	stylo_modules = stylo.get_all("Module Def", filters={"app_name": "stylo"}, pluck="name")
	site_doctypes = stylo.get_all(
		"DocType",
		filters={"module": ("in", stylo_modules), "custom": False},
	...
	for dt_path in doctype_jsons:
		with open(dt_path) as f:
			try:
				file_schema = stylo._dict(json.load(f))
			except Exception:
				continue
			...
			if file_schema.creation != _site_schema[0].creation:
				stylo.qb.update(DocType).set(DocType.creation, file_schema.creation).where(
