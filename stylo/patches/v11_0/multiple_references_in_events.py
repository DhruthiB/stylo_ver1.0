...
def execute():
	stylo.reload_doctype("Event")
	# Rename "Cancel" to "Cancelled"
	stylo.db.sql("""UPDATE tabEvent set event_type='Cancelled' where event_type='Cancel'""")
	...
	for event in events:
		if event.ref_type and event.ref_name:
			try:
				e = stylo.get_doc("Event", event.name)
				e.append(
					"event_participants",
				...
				stylo.log_error(stylo.get_traceback())
