...
def execute():
	...
	reports = stylo.get_all("Report", {"report_type": "Report Builder"})

	for report_name in reports:
		settings = stylo.db.get_value("Report", report_name, "json")
		if not settings:
			continue

		settings = stylo._dict(json.loads(settings))
		...
		stylo.db.set_value("Report", report_name, "json", json.dumps(settings))
