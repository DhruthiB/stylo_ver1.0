...
import stylo
from stylo.permissions import get_valid_perms
from stylo.utils import cint


def execute():
	stylo.reload_doctype("User Permission")
	user_permissions = stylo.get_all("User Permission", fields=["allow", "name", "user"])
	...
	for key, doctype_to_skip in doctype_to_skip_map.items():
		if not doctype_to_skip:
			continue
		if not stylo.db.has_column("User Permission", "applicable_for") and stylo.db.has_column(
...
def get_linked_doctypes(doctype):
	from stylo.permissions import get_linked_doctypes

	linked_doctypes = get_linked_doctypes(doctype)
	child_doctypes = [d.options for d in stylo.get_meta(doctype).get_table_fields()]
