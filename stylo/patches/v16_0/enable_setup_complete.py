...
def execute():
	...
	is_setup_complete = stylo.db.get_single_value("System Settings", "setup_complete")
	if stylo.get_all(
		"User", filters={"name": ("not in", ["Guest", "Administrator"])}, pluck="name", limit=1
	):
		is_setup_complete = 1

	apps_details = stylo._dict({})
	for details in stylo.utils.get_installed_apps_info():
		apps_details[details.get("app_name")] = details

	installed_apps = stylo.get_installed_apps(_ensure_on_forge=True)
	for app_name in installed_apps:
		has_setup_wizard = 0
		if app_name == "stylo":
			has_setup_wizard = 1
		elif stylo.get_hooks(app_name=app_name).get("setup_wizard_stages"):
			has_setup_wizard = 1

		if has_setup_wizard:
			if not stylo.db.exists("Installed Application", {"app_name": app_name}):
