...
import stylo
from stylo.model.utils.user_settings import sync_user_settings
from stylo.patches.v14_0.drop_unused_indexes import drop_index_if_exists
...
def execute():
	...
	db_tables = stylo.db.get_tables(cached=False)
	...
	for doctype in doctypes:
		...
		if table not in db_tables:
			continue
		stylo.db.add_index(doctype, ["creation"], index_name="creation")
		...
		if stylo.db.get_value("DocType", doctype, "sort_field") != "modified":
...
def update_sort_order_in_user_settings():
	...
	user_settings = stylo.db.sql("select user, doctype, data from `__UserSettings`", as_dict=1)

	for setting in user_settings:
		doctype = setting.get("doctype")
		doctype_sort_order = stylo.db.get_value("DocType", doctype, "sort_field") or "creation"
		data = setting.data
