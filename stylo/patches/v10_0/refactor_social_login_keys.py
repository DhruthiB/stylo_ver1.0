...
def execute():
	...
	stylo.reload_doc("integrations", "doctype", "social_login_key", force=True)

	if not stylo.db.exists("DocType", "Social Login Keys"):
		return

	social_login_keys = stylo.get_doc("Social Login Keys", "Social Login Keys")
	if social_login_keys.get("facebook_client_id") or social_login_keys.get("facebook_client_secret"):
		facebook_login_key = stylo.new_doc("Social Login Key")
	...
	if social_login_keys.get("stylo_server_url"):
		stylo_login_key = stylo.new_doc("Social Login Key")
		stylo_login_key.get_social_login_provider("Stylo", initialize=True)
		stylo_login_key.social_login_provider = "Stylo"
		stylo_login_key.base_url = social_login_keys.get("stylo_server_url")
		stylo_login_key.client_id = social_login_keys.get("stylo_client_id")
		stylo_login_key.client_secret = social_login_keys.get("stylo_client_secret")
		if not (stylo_login_key.client_secret and stylo_login_key.client_id and stylo_login_key.base_url):
	...
	if social_login_keys.get("github_client_id") or social_login_keys.get("github_client_secret"):
		github_login_key = stylo.new_doc("Social Login Key")
	...
	if social_login_keys.get("google_client_id") or social_login_keys.get("google_client_secret"):
		google_login_key = stylo.new_doc("Social Login Key")
	...
	stylo.delete_doc("DocType", "Social Login Keys")


def run_patch():
	stylo.reload_doc("core", "doctype", "user", force=True)
	stylo.reload_doc("core", "doctype", "user_social_login", force=True)

	users = stylo.get_all("User", fields=["*"], filters={"name": ("not in", ["Administrator", "Guest"])})

	for user in users:
		idx = 0
		if user.stylo_userid:
			insert_user_social_login(user.name, user.modified_by, "stylo", idx, userid=user.stylo_userid)
			idx += 1
...
def insert_user_social_login(user, modified_by, provider, idx, userid=None, username=None):
	...
	creation_time = stylo.utils.get_datetime_str(stylo.utils.get_datetime())
	values = [
		stylo.generate_hash(length=10),
		creation_time,
		creation_time,
		user,
		modified_by,
	...
	query = """INSERT INTO `tabUser Social Login` (`{source_cols}`)
		VALUES ({values})
	""".format(source_cols="`, `".join(source_cols), values=", ".join([stylo.db.escape(d) for d in values]))
