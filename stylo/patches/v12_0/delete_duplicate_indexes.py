...
def execute():
	...
	all_tables = stylo.db.get_tables()
	final_deletion_map = stylo._dict()

	for table in all_tables:
		indexes_to_keep_map = stylo._dict()
		indexes_to_delete = []
		index_info = stylo.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				stylo.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
