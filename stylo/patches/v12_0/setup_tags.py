...
def execute():
	stylo.delete_doc_if_exists("DocType", "Tag Category")
	stylo.delete_doc_if_exists("DocType", "Tag Doc Category")

	stylo.reload_doc("desk", "doctype", "tag")
	stylo.reload_doc("desk", "doctype", "tag_link")

	tag_list = []
	tag_links = []
	time = stylo.utils.get_datetime()

	for doctype in stylo.get_list("DocType", filters={"istable": 0, "issingle": 0, "is_virtual": 0}):
		if not stylo.db.count(doctype.name) or not stylo.db.has_column(doctype.name, "_user_tags"):
			continue

		for _user_tags in stylo.db.sql(
			...
			for tag in _user_tags.get("_user_tags").split(",") if _user_tags.get("_user_tags") else []:
				...
				tag_link_name = stylo.generate_hash(length=10)
				tag_links.append(
