...
def deferred_insert(doctype: str, records: list[Union[dict, "Document"]] | str):
	...
	try:
		stylo.cache().rpush(f"{queue_prefix}{doctype}", _records)
...
def save_to_db():
	queue_keys = stylo.cache().get_keys(queue_prefix)
	for key in queue_keys:
		record_count = 0
		queue_key = get_key_name(key)
		doctype = get_doctype_name(key)
		while stylo.cache().llen(queue_key) > 0 and record_count <= 500:
			records = stylo.cache().lpop(queue_key)
...
def insert_record(record: Union[dict, "Document"], doctype: str):
	try:
		...
		stylo.get_doc(record).insert()
	except Exception as e:
		stylo.logger().error(f"Error while inserting deferred {doctype} record: {e}")
