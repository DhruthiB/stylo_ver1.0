...
import stylo
from stylo.integrations.doctype.oauth_client.oauth_client import OAuthClient
from stylo.utils import get_request_session
...
def make_request(method: str, url: str, auth=None, headers=None, data=None, json=None, params=None):
	...
	try:
		...
		response = stylo.flags.integration_request = s.request(
		...
		if stylo.flags.integration_request_doc:
			stylo.flags.integration_request_doc.log_error()
		else:
			stylo.log_error()
		raise exc
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = stylo.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else stylo.as_json(obj, indent=1)
...
def validate_dynamic_client_metadata(client: OAuth2DynamicClientMetadata):
	...
	if not stylo.conf.developer_mode and any(c.scheme != "https" for c in client.redirect_uris):
...
def create_new_oauth_client(client: OAuth2DynamicClientMetadata):
	doc = cast(OAuthClient, stylo.get_doc({"doctype": "OAuth Client"}))
...
def get_oauth_settings():
	...
	settings: dict[str, Any] = stylo._dict({"skip_authorization": None})
	if stylo.get_cached_value("OAuth Settings", "OAuth Settings", "skip_authorization"):
		settings["skip_authorization"] = "Auto"  # based on legacy OAuth Provider Settings value

	elif value := stylo.get_cached_value(
