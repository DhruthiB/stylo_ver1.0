...
import stylo
from stylo.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/stylo.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "stylo.email.oauth.authorize_google_access",
	"contacts": "stylo.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"drive": "stylo.integrations.doctype.google_drive.google_drive.authorize_access",
	"indexing": "stylo.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = stylo.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			stylo.throw(stylo._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			stylo.throw(stylo._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		stylo.log_error(stylo._(error_title), stylo._(response.get("error_description", error_message)))

		if raise_err:
			stylo.throw(stylo._(error_title), GoogleAuthenticationError, stylo._(error_message))
...
def callback(state: str, code: str | None = None, error: str | None = None) -> None:
	...
	Invokes functions using `stylo.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to stylo site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			stylo.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
	...
	stylo.local.response["location"] = f"{redirect}?{failure_query_param if error else success_query_param}"
