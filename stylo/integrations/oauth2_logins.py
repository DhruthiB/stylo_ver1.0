...
import stylo
import stylo.utils
from stylo.utils.oauth import login_via_oauth2, login_via_oauth2_id_token
...
@stylo.whitelist(allow_guest=True)
def login_via_stylo(code: str, state: str):
	login_via_oauth2("stylo", code, state, decoder=decoder_compat)
...
def custom(code: str, state: str):
	...
	process social login from /api/method/stylo.integrations.oauth2_logins.custom/<provider>
	"""
	path = stylo.request.path[1:].split("/")
	if len(path) == 4 and path[3]:
		provider = path[3]
		# Validates if provider doctype exists
		if stylo.db.exists("Social Login Key", provider):
