...
import stylo
import stylo.utils
from stylo import oauth
from stylo.integrations.utils import (
...
ENDPOINTS = {
	"token_endpoint": "/api/method/stylo.integrations.oauth2.get_token",
	"userinfo_endpoint": "/api/method/stylo.integrations.oauth2.openid_profile",
	"revocation_endpoint": "/api/method/stylo.integrations.oauth2.revoke_token",
	"authorization_endpoint": "/api/method/stylo.integrations.oauth2.authorize",
	"introspection_endpoint": "/api/method/stylo.integrations.oauth2.introspect_token",
}
...
def authorize(**kwargs):
	...
	failure_url = stylo.form_dict.get("redirect_uri", "") + "?error=access_denied"
	...
	if stylo.session.user == "Guest":
		...
		try:
			...
			unrevoked_tokens = stylo.db.exists(
				"OAuth Bearer Token", {"status": "Active", "user": stylo.session.user}
			)
...
def get_openid_configuration():
	...
	response.data = stylo.as_json(
		{
			"issuer": stylo_server_url,
			"authorization_endpoint": f"{stylo_server_url}{ENDPOINTS['authorization_endpoint']}",
			"token_endpoint": f"{stylo_server_url}{ENDPOINTS['token_endpoint']}",
			"userinfo_endpoint": f"{stylo_server_url}{ENDPOINTS['userinfo_endpoint']}",
			"revocation_endpoint": f"{stylo_server_url}{ENDPOINTS['revocation_endpoint']}",
			"introspection_endpoint": f"{stylo_server_url}{ENDPOINTS['introspection_endpoint']}",
...
def get_authorization_server_metadata():
	...
	response = Response()
	response.mimetype = "application/json"
	response.data = stylo.as_json(_get_authorization_server_metadata())
	stylo.local.allow_cors = "*"
	return response
...
def _get_authorization_server_metadata():
	...
	metadata = dict(
		...
		service_documentation="https://docs.stylo.io/framework/user/en/guides/integration/how_to_set_up_oauth#add-a-client-app",
	...
	if stylo.get_cached_value("OAuth Settings", "OAuth Settings", "enable_dynamic_client_registration"):
		metadata["registration_endpoint"] = f"{issuer}/api/method/stylo.integrations.oauth2.register_client"
...
def register_client():
	...
	if not stylo.get_cached_value("OAuth Settings", "OAuth Settings", "enable_dynamic_client_registration"):
		raise NotFound

	response = Response()
	response.mimetype = "application/json"
	data = stylo.request.json
	...
	try:
		...
		response.status_code = 400
		response.data = stylo.as_json({"error": "invalid_client_metadata", "error_description": str(e)})
		return response
	...
	if error := validate_dynamic_client_metadata(client):
		response.status_code = 400
		response.data = stylo.as_json({"error": "invalid_client_metadata", "error_description": error})
		return response
	...
	response.data = stylo.as_json(response_data)
	return response
...
def get_protected_resource_metadata():
	...
	response = Response()
	response.mimetype = "application/json"
	response.data = stylo.as_json(_get_protected_resource_metadata())
	return response


def _get_protected_resource_metadata():
	from stylo.integrations.doctype.oauth_settings.oauth_settings import OAuthSettings

	oauth_settings = cast(OAuthSettings, stylo.get_cached_doc("OAuth Settings", ignore_permissions=True))
...
def set_cors_for_privileged_requests():
	...
	2. POST requests on /api/method/stylo.integrations.oauth2.register_client
	...
	if (
		...
		or not stylo.local.request
		or not stylo.local.request.headers.get("Origin")
	):
		return

	if stylo.request.path.startswith("/.well-known/") and stylo.request.method in ("GET", "OPTIONS"):
		stylo.local.allow_cors = "*"
		return

	if (
		stylo.request.path.startswith("/api/method/stylo.integrations.oauth2.register_client")
		and stylo.request.method in ("POST", "OPTIONS")
		and stylo.get_cached_value(
			"OAuth Settings",
			"OAuth Settings",
	...
	if (
		stylo.request.path.startswith(ENDPOINTS["token_endpoint"])
		or stylo.request.path.startswith(ENDPOINTS["revocation_endpoint"])
		or stylo.request.path.startswith(ENDPOINTS["introspection_endpoint"])
		or stylo.request.path.startswith(ENDPOINTS["userinfo_endpoint"])
	) and stylo.request.method in ("POST", "OPTIONS"):
		_set_allowed_cors()
		return
...
def _set_allowed_cors():
	allowed = stylo.get_cached_value(
		"OAuth Settings",
		"OAuth Settings",
	...
	if "*" in allowed:
		...
		stylo.local.allow_cors = allowed
