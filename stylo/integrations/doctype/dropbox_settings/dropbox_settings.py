...
import stylo
from stylo import _
from stylo.integrations.offsite_backup_utils import (
	get_chunk_site,
...
from stylo.utils import cint, encode, get_backups_path, get_files_path, get_request_site_address
from stylo.utils.background_jobs import enqueue
from stylo.utils.backups import new_backup
...
class DropboxSettings(Document):
	def onload(self):
		if not self.app_access_key and stylo.conf.dropbox_access_key:
			self.set_onload("dropbox_setup_via_site_config", 1)

	def validate(self):
		if self.enabled and self.limit_no_of_backups and self.no_of_backups < 1:
			stylo.throw(_("Number of DB backups cannot be less than 1"))
...
def take_backup():
	...
	enqueue(
		"stylo.integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox",
		queue="long",
		timeout=1500,
	)
	stylo.msgprint(_("Queued for backup. It may take a few minutes to an hour."))
...
def take_backups_if(freq):
	if stylo.db.get_single_value("Dropbox Settings", "backup_frequency") == freq:
...
def take_backup_to_dropbox(retry_count=0, upload_db_backup=True):
	...
	try:
		if cint(stylo.db.get_single_value("Dropbox Settings", "enabled")):
			validate_file_size()
			...
			if cint(stylo.db.get_single_value("Dropbox Settings", "send_email_for_successful_backup")):
...
def backup_to_dropbox(upload_db_backup=True):
	...
	if upload_db_backup:
		if stylo.flags.create_new_backup:
...
def upload_from_folder(path, is_private, dropbox_folder, dropbox_client, did_not_upload, error_log):
	...
	if is_fresh_upload():
		...
		response = stylo._dict({"entries": []})

	path = str(path)

	for f in stylo.get_all(
		"File",
		...
		for file_metadata in response.entries:
			try:
				...
				error_log.append(stylo.get_traceback())
...
def update_file_dropbox_status(file_name):
	stylo.db.set_value("File", file_name, "uploaded_to_dropbox", 1, update_modified=False)


def is_fresh_upload():
	file_name = stylo.db.get_value("File", {"uploaded_to_dropbox": 1}, "name")
...
def get_dropbox_settings(redirect_uri=False):
	...
	settings = stylo.get_doc("Dropbox Settings")
	app_details = {
		"app_key": settings.app_access_key or stylo.conf.dropbox_access_key,
		"app_secret": settings.get_password(fieldname="app_secret_key", raise_exception=False)
		if settings.app_secret_key
		else stylo.conf.dropbox_secret_key,
		...
		app_details.update(
			{
				...
				+ "/api/method/stylo.integrations.doctype.dropbox_settings.dropbox_settings.dropbox_auth_finish"
			}
		)
...
def dropbox_auth_finish():
	...
	if not callback.state or not callback.code:
		stylo.respond_as_web_page(
			...
			indicator_color="red",
			http_status_code=stylo.AuthenticationError.http_status_code,
		)
		return
	...
	stylo.local.response["location"] = "/app/dropbox-settings"
...
def set_dropbox_token(access_token, refresh_token=None):
	...
	dropbox_settings = stylo.get_single("Dropbox Settings")
