...
from stylo.utils.background_jobs import get_queues_timeout
from stylo.utils.jinja import validate_template
from stylo.utils.safe_exec import get_safe_globals

WEBHOOK_SECRET_HEADER = "X-Stylo-Webhook-Signature"
...
class Webhook(Document):
	...
	if TYPE_CHECKING:
		from stylo.integrations.doctype.webhook_data.webhook_data import WebhookData
		from stylo.integrations.doctype.webhook_header.webhook_header import WebhookHeader
	...
	def on_update(self):
		stylo.client_cache.delete_value("webhooks")
	...
	def validate_docevent(self):
		if self.webhook_doctype:
			is_submittable = stylo.get_value("DocType", self.webhook_doctype, "is_submittable")
			if not is_submittable and self.webhook_docevent in [
				"on_submit",
				"on_cancel",
				"on_update_after_submit",
			]:
				stylo.throw(_("DocType must be Submittable for the selected Doc Event"))

	def validate_condition(self):
		temp_doc = stylo.new_doc(self.webhook_doctype)
		if self.condition:
			try:
				stylo.safe_eval(self.condition, eval_locals=get_context(temp_doc))
			except Exception as e:
				stylo.throw(_("Invalid Condition: {}").format(e))
	...
	def validate_request_url(self):
		try:
			...
			stylo.throw(_("Check Request URL"), exc=e)
	...
	def validate_repeating_fields(self):
		...
		if len(webhook_data) != len(set(webhook_data)):
			stylo.throw(_("Same Field is entered more than once"))
	...
	def validate_secret(self):
		if self.enable_security:
			try:
				...
				stylo.throw(_("Invalid Webhook Secret"))
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition:
			return _("Yes")
		try:
			doc = stylo.get_cached_doc(self.webhook_doctype, preview_document)
			met_condition = stylo.safe_eval(self.condition, eval_locals=get_context(doc))
		except Exception as e:
	...
	def preview_request_body(self, preview_document):
		try:
			doc = stylo.get_cached_doc(self.webhook_doctype, preview_document)
			return stylo.as_json(get_webhook_data(doc, self))
		except Exception as e:
			stylo.local.message_log = []
...
def get_context(doc):
	return {"doc": doc, "utils": get_safe_globals().get("stylo").get("utils")}
...
def enqueue_webhook(doc, webhook) -> None:
	...
	try:
		if not isinstance(webhook, Document):
			webhook: Webhook = stylo.get_doc("Webhook", webhook.get("name"))
		request_url = webhook.request_url
		if webhook.is_dynamic_url:
			request_url = stylo.render_template(webhook.request_url, get_context(doc))
		headers = get_webhook_headers(doc, webhook)
		data = get_webhook_data(doc, webhook)

	except Exception as e:
		stylo.logger().debug({"enqueue_webhook_error": e})
	...
	for i in range(3):
		try:
			...
			r.raise_for_status()
			stylo.logger().debug({"webhook_success": r.text})
			log_request(webhook.name, doc.doctype, doc.name, request_url, headers, data, r)
			break

		except requests.exceptions.ReadTimeout as e:
			stylo.logger().debug({"webhook_error": e, "try": i + 1})
			log_request(webhook.name, doc.doctype, doc.name, request_url, headers, data)
...
def log_request(
	...
	request_log = stylo.get_doc(
		{
			"doctype": "Webhook Request Log",
			"webhook": webhook,
			"reference_doctype": doctype,
			"reference_document": docname,
			"user": stylo.session.user if stylo.session.user else None,
			"url": url,
			"headers": stylo.as_json(headers) if headers else None,
			"data": stylo.as_json(data) if data else None,
			"response": res.text if res is not None else None,
			"error": stylo.get_traceback(),
		}
	)
...
def get_webhook_data(doc, webhook):
	...
	if webhook.webhook_data:
		...
		data = stylo.render_template(webhook.webhook_json, get_context(doc))
