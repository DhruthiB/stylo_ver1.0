...
stylo.webhook = {
	set_fieldname_select: (frm) => {
		if (frm.doc.webhook_doctype) {
			stylo.model.with_doctype(frm.doc.webhook_doctype, () => {
				// get doctype fields
				let fields = $.map(
					stylo.get_doc("DocType", frm.doc.webhook_doctype).fields,
					(d) => {
						if (
							stylo.model.no_value_type.includes(d.fieldtype) &&
							!stylo.model.table_fields.includes(d.fieldtype)
						) {
							return null;
						} else {
							return {
				...
				// add meta fields
				for (let field of stylo.model.std_fields) {
...
stylo.ui.form.on("Webhook", {
	refresh: (frm) => {
		stylo.webhook.set_fieldname_select(frm);
		frm.set_query(
			"background_jobs_queue",
			"stylo.integrations.doctype.webhook.webhook.get_all_queues"
		);
	...
	request_structure: (frm) => {
		stylo.webhook.set_request_headers(frm);
	},
...
stylo.ui.form.on("Webhook Data", {
	fieldname: (frm, cdt, cdn) => {
		...
		if (!df.length) {
			...
			df = stylo.model.std_fields.filter((field) => field.fieldname == row.fieldname);
		}
