...
class TestSocialLoginKey(IntegrationTestCase):
	def setUp(self) -> None:
		...
		stylo.delete_doc("User", TEST_GITHUB_USER, force=True)
		super().setUp()
	...
	def test_normal_signup_and_github_login(self):
		...
		if not stylo.db.exists("User", TEST_GITHUB_USER):
			user = stylo.new_doc("User", email=TEST_GITHUB_USER, first_name="GitHub Login")
		...
		self.assertEqual(stylo.session.user, TEST_GITHUB_USER)
	...
	def test_force_disabled_signups(self):
		...
		with patch.object(OAuth2Service, "get_auth_session", return_value=mock_session):
			login_via_oauth2("github", "iwriu", {"token": "ewrwerwer"})
		self.assertEqual(stylo.session.user, "Guest")
