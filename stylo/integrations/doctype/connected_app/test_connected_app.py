...
import stylo
from stylo.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = stylo.new_doc(doctype)
	connected_app.provider_name = "stylo"
...
def get_oauth_client():
	oauth_client = stylo.new_doc("OAuth Client")
...
class TestConnectedApp(StyloTestCase):
	def setUp(self):
		"""Set up a Connected App that connects to our own oAuth provider.

		Stylo comes with it's own oAuth2 provider that we can test against. The
		...
		Stylo's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/stylo.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		stylo.db.commit()  # Avoid snapshot violation issues

		delete_if_exists("token_cache")
		delete_if_exists("connected_app")

		if getattr(self, "oauth_client", None):
			tokens = stylo.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = stylo.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = stylo.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = stylo.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
