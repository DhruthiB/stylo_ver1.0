...
import stylo
from stylo import _
from stylo.integrations.utils import make_get_request
from stylo.model.document import Document

if any((os.getenv("CI"), stylo.conf.developer_mode, stylo.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	if TYPE_CHECKING:
		from stylo.integrations.doctype.oauth_scope.oauth_scope import OAuthScope
		from stylo.integrations.doctype.query_parameters.query_parameters import QueryParameters
	...
	def get_openid_configuration(self):
		if not self.openid_configuration:
			stylo.throw(_("Please enter OpenID Configuration URL"))
		return make_get_request(self.openid_configuration)

	def validate(self):
		base_url = stylo.utils.get_url()
		callback_path = (
			"/api/method/stylo.integrations.doctype.connected_app.connected_app.callback/" + self.name
		)
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = stylo.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		stylo.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if stylo.db.exists("Token Cache", token_cache_name):
			token_cache = stylo.get_doc("Token Cache", token_cache_name)
...
@stylo.whitelist(methods=["GET"], allow_guest=True)
def callback(code=None, state=None):
	...
	if stylo.session.user == "Guest":
		stylo.local.response["type"] = "redirect"
		stylo.local.response["location"] = "/login?" + urlencode({"redirect-to": stylo.request.url})
		return

	path = stylo.request.path[1:].split("/")
	if len(path) != 4 or not path[3]:
		stylo.throw(_("Invalid Parameters."))

	connected_app = stylo.get_doc("Connected App", path[3])
	token_cache = stylo.get_doc("Token Cache", connected_app.name + "-" + stylo.session.user)

	if state != token_cache.state:
		stylo.throw(_("Invalid token state! Check if the token has been created by the OAuth user."))
	...
	stylo.local.response["type"] = "redirect"
	stylo.local.response["location"] = token_cache.get("success_uri") or connected_app.get_url()


@stylo.whitelist()
def has_token(connected_app, connected_user=None):
	app = stylo.get_doc("Connected App", connected_app)
	token_cache = app.get_token_cache(connected_user or stylo.session.user)
