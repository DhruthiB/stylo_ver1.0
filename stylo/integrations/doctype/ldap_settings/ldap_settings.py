...
import stylo
from stylo import _, safe_encode
from stylo.model.document import Document
from stylo.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from stylo.integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			stylo.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			stylo.throw(_("Invalid username or password"))
		except Exception as ex:
			stylo.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = stylo.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "stylo.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def create_or_update_user(self, user_data: dict, groups: list | None = None):
		...
		if stylo.db.exists("User", user_data["email"]):
			user = stylo.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = stylo.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			stylo.throw(_("LDAP is not enabled."))
		...
		try:
			...
			stylo.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					...
					stylo.msgprint(_("Password changed successfully."))
				else:
					stylo.throw(_("Failed to change password."))
			else:
				stylo.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			stylo.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	...
	ldap: LDAPSettings = stylo.get_doc("LDAP Settings")

	user = ldap.authenticate(stylo.as_unicode(args.usr), stylo.as_unicode(args.pwd))

	stylo.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(stylo.local.login_manager):
			return False
