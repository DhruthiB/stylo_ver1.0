...
from stylo.utils.password import set_encrypted_password

if TYPE_CHECKING:
	from stylo.desk.doctype.event.event import Event
...
class GoogleCalendar(Document):
	...
	def validate(self):
		google_settings = stylo.get_cached_doc("Google Settings")
		...
		if not google_settings.client_id or not google_settings.client_secret:
			stylo.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			raise stylo.ValidationError(
				_("Click on {0} to generate Refresh Token.").format(stylo.bold(allow_google_calendar_label))
			)
		...
		try:
			...
			stylo.throw(
				...
				).format(stylo.bold(allow_google_calendar_label))
			)
...
def authorize_access(g_calendar: str, reauthorize: bool = False):
	...
	"""
	google_calendar = stylo.get_doc("Google Calendar", g_calendar)
	...
	if not google_calendar.authorization_code or reauthorize:
		stylo.cache.hset("google_calendar", "google_calendar", google_calendar.name)
	...
	if "refresh_token" in r:
		stylo.db.set_value("Google Calendar", google_calendar.name, "refresh_token", r["refresh_token"])
		stylo.db.commit()

	stylo.local.response["type"] = "redirect"
	stylo.local.response["location"] = google_calendar.get_url()

	stylo.msgprint(_("Google Calendar has been configured."), indicator="green")
...
def google_callback(code=None):
	...
	"""
	google_calendar = stylo.cache.hget("google_calendar", "google_calendar")
	stylo.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	stylo.db.commit()
...
def sync(g_calendar: str | None = None):
	...
	for g in stylo.get_list("Google Calendar", filters=filters, pluck="name"):
...
def get_google_calendar_object(g_calendar):
	...
	google_settings = stylo.get_cached_doc("Google Settings")
	account: GoogleCalendar = stylo.get_doc("Google Calendar", g_calendar)
...
def check_google_calendar(account: GoogleCalendar, google_calendar):
	...
	calendar = {
		...
		"timeZone": stylo.get_system_settings("time_zone"),
	}
	try:
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				stylo.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				stylo.throw(msg)
	...
	for idx, event in enumerate(results):
		stylo.publish_realtime(
			"import_google_calendar", {"progress": idx + 1, "total": len(results)}, user=stylo.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			# NOTE: Skip if event is already synced; Stylo doesn't track individual
			# instances of recurring events, so we need to check if the event is already
			# synced in Stylo Calendar
			if event.get("recurringEventId"):
				...
			elif not stylo.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in Stylo Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	e: Event = stylo.get_doc(calendar_event)
...
def update_participants_in_event(calendar_event: "Event", google_event: dict):
	...
	in_system_participants = stylo.get_all(
...
def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in Stylo Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = stylo.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	if (
		...
		or not stylo.db.exists("Google Calendar", {"name": doc.google_calendar})
	):
		return
	...
	try:
		...
		stylo.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		stylo.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in Stylo Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if Stylo Event is deleted.
	"""

	if not stylo.db.exists("Google Calendar", {"name": doc.google_calendar, "push_to_google_calendar": 1}):
		return
...
def get_attendees(doc):
	...
	owner = stylo.db.get_value("Google Calendar", doc.google_calendar, "user")
