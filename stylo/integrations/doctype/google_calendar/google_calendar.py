...
class GoogleCalendar(Document):
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = stylo.bold(_("Allow Google Calendar Access"))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	"""
	google_settings = stylo.get_doc("Google Settings")
	google_calendar = stylo.get_doc("Google Calendar", g_calendar)

	redirect_uri = (
		get_request_site_address(True)
		+ "?cmd=stylo.integrations.doctype.google_calendar.google_calendar.google_callback"
	)
	...
	if not google_calendar.authorization_code or reauthorize:
		...
		try:
			...
			stylo.local.response["location"] = "/app/Form/{}/{}".format(
				quote("Google Calendar"), quote(google_calendar.name)
			)

			stylo.msgprint(_("Google Calendar has been configured."))
...
def sync(g_calendar=None):
	...
	google_calendars = stylo.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	"""
	google_settings = stylo.get_doc("Google Settings")
	account = stylo.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": stylo.db.get_single_value("System Settings", "time_zone"),
			}
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	for idx, event in enumerate(results):
		stylo.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=stylo.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not stylo.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			stylo.get_doc(
				{
					...
					"reference_name": stylo.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def insert_event_to_calendar(account, event, recurrence=None):
	...
	stylo.get_doc(calendar_event).insert(ignore_permissions=True)
...
def insert_event_in_google_calendar(doc, method=None):
	...
	"""
	if (
		not stylo.db.exists("Google Calendar", {"name": doc.google_calendar})
...
def delete_event_from_google_calendar(doc, method=None):
	...
	if not stylo.db.exists("Google Calendar", {"name": doc.google_calendar}):
		return
