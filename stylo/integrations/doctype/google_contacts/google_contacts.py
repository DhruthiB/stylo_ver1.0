...
class GoogleContacts(Document):
	def validate(self):
		if not stylo.db.get_single_value("Google Settings", "enable"):
			stylo.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = stylo.bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	oauth_code = stylo.db.get_value("Google Contacts", g_contact, "authorization_code") if not code else code
	oauth_obj = GoogleOAuth("contacts")
...
def get_google_contacts_object(g_contact):
	...
	"""
	account = stylo.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = stylo.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	stylo.db.set_value("Google Contacts", account.name, "last_sync_on", stylo.utils.now_datetime())

	for idx, connection in enumerate(results):
		stylo.publish_realtime(
			"import_google_contacts", dict(progress=idx + 1, total=len(results)), user=stylo.session.user
		)
		# Work-around to fix
		# https://github.com/stylo/stylo/issues/22648
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not stylo.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		stylo.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		stylo.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		stylo.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		stylo.msgprint(
