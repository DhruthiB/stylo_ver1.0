import stylo
from stylo import _
from stylo.database.schema import DBTable, get_definition
from stylo.model import log_types
from stylo.utils import cint, flt
...
class PostgresTable(DBTable):
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			stylo.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{stylo.db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				stylo.db.sql(final_alter_query)
			if create_contraint_query:
				# nosemgrep
				stylo.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				stylo.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if stylo.db.is_duplicate_fieldname(e):
				stylo.throw(str(e))
			elif stylo.db.is_duplicate_entry(e):
