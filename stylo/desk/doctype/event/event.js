...
stylo.ui.form.on("Event", {
	...
	refresh: function (frm) {
		if (frm.doc.event_participants) {
			frm.doc.event_participants.forEach((value) => {
				frm.add_custom_button(
					...
					function () {
						stylo.set_route("Form", value.reference_doctype, value.reference_docname);
					},
					__("Participants")
				);
			});
		...
		frm.add_custom_button(
			__("Add Contacts"),
			function () {
				new stylo.desk.eventParticipants(frm, "Contact");
			},
			__("Add Participants")
		);
		...
		if (
			...
			stylo.datetime.now_date() <= ends_on_date
		) {
...
stylo.ui.form.on("Event Participants", {
	event_participants_remove: function (frm, cdt, cdn) {
		if (cdt && !cdn.includes("new-event-participants")) {
			stylo.call({
				type: "POST",
				method: "stylo.desk.doctype.event.event.delete_communication",
				args: {
					event: frm.doc,
...
stylo.desk.eventParticipants = class eventParticipants {
