...
def save_listview_settings(doctype, listview_settings, removed_listview_fields):
	listview_settings = stylo.parse_json(listview_settings)
	removed_listview_fields = stylo.parse_json(removed_listview_fields)

	if stylo.get_all("List View Settings", filters={"name": doctype}):
	...
	return {"meta": stylo.get_meta(doctype, False), "listview_settings": doc}


def set_listview_fields(doctype, listview_fields, removed_listview_fields):
	meta = stylo.get_meta(doctype)

	listview_fields = [f.get("fieldname") for f in stylo.parse_json(listview_fields) if f.get("fieldname")]
...
def set_in_list_view_property(doctype, field, value):
	...
	property_setter = stylo.db.get_value(
		"Property Setter",
		{"doc_type": doctype, "field_name": field.fieldname, "property": "in_list_view"},
	)
	if property_setter:
		doc = stylo.get_doc("Property Setter", property_setter)
		doc.value = value
		doc.save()
	else:
...
def get_default_listview_fields(doctype):
	meta = stylo.get_meta(doctype)
	path = stylo.get_module_path(
		stylo.scrub(meta.module), "doctype", stylo.scrub(meta.name), stylo.scrub(meta.name) + ".json"
	)
	doctype_json = stylo.get_file_json(path)
