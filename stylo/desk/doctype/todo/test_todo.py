...
import stylo
from stylo.core.doctype.doctype.doctype import clear_permissions_cache
from stylo.model.db_query import DatabaseQuery
from stylo.permissions import add_permission, reset_perms
from stylo.tests.utils import StyloTestCase

test_dependencies = ["User"]


class TestToDo(StyloTestCase):
	def test_delete(self):
		todo = stylo.get_doc(
			dict(doctype="ToDo", description="test todo", assigned_by="Administrator")
		).insert()

		stylo.db.delete("Deleted Document")
		todo.delete()

		deleted = stylo.get_doc(
	...
	def test_fetch(self):
		...
		self.assertEqual(
			todo.assigned_by_full_name, stylo.db.get_value("User", todo.assigned_by, "full_name")
		)

	def test_fetch_setup(self):
		stylo.db.delete("ToDo")

		todo_meta = stylo.get_doc("DocType", "ToDo")
	...
	def test_doc_read_access(self):
		...
		test_user = stylo.get_doc("User", "test4@example.com")
...
def create_new_todo(description, assigned_by):
	...
	return stylo.get_doc(todo).insert()
