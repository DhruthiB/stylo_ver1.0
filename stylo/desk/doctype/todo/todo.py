...
from stylo.utils import get_fullname, parse_addr
...
class ToDo(Document):
	...
	def validate(self):
		...
		if self.is_new():
			if self.assigned_by == self.allocated_to:
				assignment_message = stylo._("{0} self assigned this task: {1}").format(
					get_fullname(self.assigned_by), self.description
				)
			else:
				assignment_message = stylo._("{0} assigned {1}: {2}").format(
			...
			if self.get_db_value("status") != self.status:
				if self.allocated_to == stylo.session.user:
					removal_message = stylo._("{0} removed their assignment.").format(
						get_fullname(stylo.session.user)
					)
				else:
					removal_message = stylo._("Assignment of {0} removed by {1}").format(
						get_fullname(self.allocated_to), get_fullname(stylo.session.user)
					)
	...
	def add_assign_comment(self, text, comment_type):
		...
		stylo.get_doc(self.reference_type, self.reference_name).add_comment(comment_type, text)

	def delete_communication_links(self):
		# unlink todo from linked comments
		return stylo.db.delete("Communication Link", {"link_doctype": self.doctype, "link_name": self.name})
	...
	def update_in_reference(self):
		...
		try:
			assignments = stylo.db.get_values(
				"ToDo",
				{
			...
			if stylo.db.is_table_missing(e) and stylo.flags.in_install:
				# no table
				return

			elif stylo.db.is_column_missing(e):
	...
	def get_owners(cls, filters=None):
		...
		rows = stylo.get_all(cls.DocType, filters=filters or {}, fields=["allocated_to"])
...
def on_doctype_update():
	stylo.db.add_index("ToDo", ["reference_type", "reference_name"])
...
def get_permission_query_conditions(user):
	...
	todo_roles = stylo.permissions.get_doctype_roles("ToDo")
	todo_roles = set(todo_roles) - set(AUTOMATIC_ROLES)

	if any(check in todo_roles for check in stylo.get_roles(user)):
		return None
	else:
		return """(`tabToDo`.allocated_to = {user} or `tabToDo`.assigned_by = {user})""".format(
			user=stylo.db.escape(user)
		)


def has_permission(doc, ptype="read", user=None):
	user = user or stylo.session.user
	todo_roles = stylo.permissions.get_doctype_roles("ToDo", ptype)
	todo_roles = set(todo_roles) - set(AUTOMATIC_ROLES)
