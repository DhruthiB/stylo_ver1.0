...
stylo.ui.form.on("Dashboard Chart", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		frm.is_disabled = !stylo.boot.developer_mode && frm.doc.is_standard;
		...
		if (!frm.is_new()) {
			frm.add_custom_button("Add Chart to Dashboard", () => {
				const dialog = stylo.dashboard_utils.get_add_to_dashboard_dialog(
					frm.doc.name,
					"Dashboard Chart",
					"stylo.desk.doctype.dashboard_chart.dashboard_chart.add_chart_to_dashboard"
				);

				if (!frm.doc.chart_name) {
					stylo.msgprint(__("Please create chart first"));
				} else {
					dialog.show();
				}
			});
	...
	set_heatmap_year_options: function (frm) {
		if (frm.doc.type == "Heatmap") {
			stylo.db.get_doc("System Settings").then((doc) => {
				const creation_date = doc.creation;
				frm.set_df_property(
					"heatmap_year",
					"options",
					stylo.dashboard_utils.get_years_since_creation(creation_date)
				);
			});
		}
	},
			...
			if (frm.doc.filters_json.length > 2) {
				...
				stylo.report_utils.get_report_filters(report_name).then((filters) => {
					if (filters) {
						...
						let filter_values = stylo.report_utils.get_filter_values(filters);
				...
				if (!frm.doc.use_report_chart) {
					if (data.result.length) {
						...
						if (!frm.field_options.numeric_fields.length) {
							...
							let y_field_df = stylo.meta.get_docfield(
	...
	show_filters: function (frm) {
		frm.chart_filters = [];
		stylo.dashboard_utils.get_filters_for_chart_type(frm.doc).then((filters) => {
			if (filters) {
				frm.chart_filters = filters;
			}
	...
	render_filters_table: function (frm) {
		...
		table.on("click", () => {
			frm.is_disabled && stylo.throw(__("Cannot edit filters for standard charts"));
			...
			stylo.dashboards.filters_dialog = dialog;
	...
	set_parent_document_type: async function (frm) {
		...
		stylo.model.with_doctype(document_type, async () => {
			let doc_is_table = stylo.get_meta(document_type).istable;
