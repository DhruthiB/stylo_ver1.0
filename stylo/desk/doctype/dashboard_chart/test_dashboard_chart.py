...
import stylo
from stylo.desk.doctype.dashboard_chart.dashboard_chart import get
from stylo.tests.utils import StyloTestCase
from stylo.utils import formatdate, get_last_day, getdate
from stylo.utils.dateutils import get_period, get_period_ending


class TestDashboardChart(StyloTestCase):
	...
	def test_dashboard_chart(self):
		if stylo.db.exists("Dashboard Chart", "Test Dashboard Chart"):
			stylo.delete_doc("Dashboard Chart", "Test Dashboard Chart")
	...
	def test_empty_dashboard_chart(self):
		if stylo.db.exists("Dashboard Chart", "Test Empty Dashboard Chart"):
			stylo.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart")

		stylo.db.delete("Error Log")
	...
	def test_chart_wih_one_value(self):
		if stylo.db.exists("Dashboard Chart", "Test Empty Dashboard Chart 2"):
			stylo.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart 2")

		stylo.db.delete("Error Log")

		# create one data point
		stylo.get_doc(dict(doctype="Error Log", creation="2018-06-01 00:00:00")).insert()
	...
	def test_group_by_chart_type(self):
		if stylo.db.exists("Dashboard Chart", "Test Group By Dashboard Chart"):
			stylo.delete_doc("Dashboard Chart", "Test Group By Dashboard Chart")

		stylo.get_doc({"doctype": "ToDo", "description": "test"}).insert()
		...
		todo_status_count = stylo.db.count("ToDo", {"status": result.get("labels")[0]})
	...
	def test_daily_dashboard_chart(self):
		...
		if stylo.db.exists("Dashboard Chart", "Test Daily Dashboard Chart"):
			stylo.delete_doc("Dashboard Chart", "Test Daily Dashboard Chart")
	...
	def test_weekly_dashboard_chart(self):
		...
		if stylo.db.exists("Dashboard Chart", "Test Weekly Dashboard Chart"):
			stylo.delete_doc("Dashboard Chart", "Test Weekly Dashboard Chart")
	...
	def test_avg_dashboard_chart(self):
		...
		if stylo.db.exists("Dashboard Chart", "Test Average Dashboard Chart"):
			stylo.delete_doc("Dashboard Chart", "Test Average Dashboard Chart")
	...
	def test_user_date_label_dashboard_chart(self):
		stylo.delete_doc_if_exists("Dashboard Chart", "Test Dashboard Chart Date Label")
		...
		with patch.object(stylo.utils.data, "get_user_date_format", return_value="dd.mm.yyyy"):
		...
		with patch.object(stylo.utils.data, "get_user_date_format", return_value="mm-dd-yyyy"):
			result = get(chart_name="Test Dashboard Chart Date Label")
...
def create_new_communication(subject, date, rating):
	...
	comm = stylo.get_doc(communication)
	if not stylo.db.exists("Communication", {"subject": comm.subject}):
		comm.insert()
