context("Realtime updates", () => {
	...
	it("Recieves custom messages from server", { scrollBehavior: false }, () => {
		...
		cy.window()
			.its("stylo")
			.then((stylo) => {
				stylo.realtime.on(event, () => handler.handle());
			});
	...
	it("Progress bar", { scrollBehavior: false }, () => {
		...
		cy.call("stylo.tests.ui_test_helpers.publish_progress", { title }).then(() => {
...
function publish_realtime(args) {
	return cy.call("stylo.tests.ui_test_helpers.publish_realtime", args);
}
