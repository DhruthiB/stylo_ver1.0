...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.then((stylo) => {
				return stylo.xcall(
					"stylo.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("stylo")
			.then((stylo) => {
				stylo.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.stylo-control[data-fieldname="child_table_1"]').as("table");
