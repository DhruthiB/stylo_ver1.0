...
context("Data Field Input Validation in New Form", () => {
	...
	function validateField(fieldname, invalid_value, valid_value) {
		...
		cy.get(`.stylo-control[data-fieldname="${fieldname}"]`).should("have.class", "has-error");
		// Valid value, should not have has-error class
		cy.get_field(fieldname).clear().type(valid_value);
		cy.get(`.stylo-control[data-fieldname="${fieldname}"]`).should(
			"not.have.class",
			"has-error"
		);
	}
	...
	describe("Data Field Options", () => {
		...
		it("should validate URL", () => {
			validateField("url", "jkl", "https://stylo.io");
			validateField("url", "abcd.com", "http://google.com/home");
			validateField("url", "&&http://google.uae", "gopher://stylo.io");
			validateField("url", "ftt2:://google.in?q=news", "ftps2://stylo.io/__/#home");
