...
const vue_style_plugin = require("./stylo-vue-style");
...
const argv = yargs
	...
	.option("using-cached", {
		type: "boolean",
		description:
			"Skips build and uses cached build artifacts to update assets.json (used by Forge)",
	})
...
async function notify_redis({ error, success, changed_files }) {
	...
	if (error) {
		...
		let stack = error.stack.replace(new RegExp(forge_path, "g"), "");
		payload = {
			error,
			formatted,
			stack,
